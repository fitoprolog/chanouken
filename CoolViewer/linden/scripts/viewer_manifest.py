...
class WindowsManifest(ViewerManifest):
	def construct(self):
		...
		# Get llcommon and dependencies, if it was built as a DLL.
		llcommon_shared = 0
		if self.prefix(src=os.path.join(build_dir, "..", "llcommon", config), dst=""):
			if (self.path("llcommon.dll")):
				llcommon_shared = 1
			else:
				print("Skipping llcommon and dependencies (assuming statically linked)")
			self.end_prefix()
		if llcommon_shared == 1:
			if self.prefix(src=libreleasedir, dst=""):
		...
		if self.prefix(src=os.path.join(build_dir, "..", "llplugin", "slplugin", config), dst=""):
		...
		runtime_libs = os.path.join(rootdir, 'lib', 'vc141_x64')
	...
	def construct(self):
		...
		if self.prefix(src="", dst="Contents"):  # everything goes in Contents
			...
			if self.prefix(src="", dst="Resources"):
				...
				self.path(os.path.join(rootdir, 'doc', 'licenses-mac.txt'), dst='licenses.txt')
				...
				if (self.path(self.find_existing_file(os.path.join(os.pardir, lib,
																   self.args['configuration'],
																   libfile),
													  os.path.join(libreleasedir, libfile)),
							  dst=libfile)):
					dylibs[lib] = True
				else:
...
class LinuxManifest(ViewerManifest):
	def construct(self):
		...
		# Common library, if built shared
		if (not self.path("../llcommon/libllcommon.so", "lib/libllcommon.so")):
			print("Skipping libllcommon.so - assumed statically linked.")

		# Other libraries
		if self.prefix(libreleasedir, dst="lib"):
			self.path("libapr-1.so.0.4.5", "libapr-1.so.0")
			self.path("libaprutil-1.so.0.4.1", "libaprutil-1.so.0")
			...
			self.path("libexpat.so.1.8.1", "libexpat.so.1")
			...
			# Either of SDL1 or SDL2...
			self.path("libSDL-1.2.so.0.11.4", "libSDL-1.2.so.0")
			# NOTE: kept the old 0.16.0 version for Linux ARM64: remove after
			# the SDL2 library for it gets updated.
			if (not self.path("libSDL2-2.0.so.0.18.2", "libSDL2-2.0.so.0")):
				self.path("libSDL2-2.0.so.0.16.0", "libSDL2-2.0.so.0")
		...
		# Shared jemalloc library if any (it is now always statically linked
		# when used: removed this ?). Note that only the Release build is using
		# the release jemalloc (without stats code), while the Debug and
		# RelWithDebInfo builds use the debug version of jemalloc.
		if (release and self.prefix(libreleasedir, dst="lib")):
			if (not self.path("libjemalloc.so.2")):
				print("Skipping libjemalloc.so - not found (normal if built without it or statically linked).")
			self.end_prefix("lib")
		if (release and self.prefix(libdebugdir, dst="lib")):
			if (not self.path("libjemalloc.so.2")):
				print("Skipping libjemalloc.so - not found (normal if built without it or statically linked).")
			self.end_prefix("lib")
		...
		# SLPlugin
		self.path("../llplugin/slplugin/SLPlugin", "bin/llplugin/SLPlugin")
