...
LLControlVariable* LLControlGroup::declareControl(const char* name,
												  ...
												  bool hide_from_user)
{
	LLControlVariable* cur_control = getControl(name);
	if (cur_control)
 	{
		if (persist && cur_control->isType(type))
		{
			if (!cur_control->llsd_compare(cur_control->getDefault(),
										   initial_val))
			{
				// Sometimes we need to declare a control *after* it has been
				// loaded from a settings file.
				// Get the current value:
				LLSD cur_value = cur_control->getValue();
				// Set the default to the declared value:
				cur_control->setDefaultValue(initial_val);
				// Now set to the loaded value
				cur_control->setValue(cur_value);
			}
		}
		else
		{
	...
	LLControlVariable* control = new LLControlVariable(name, type, initial_val,
													   comment, persist,
													   hide_from_user);
...
LLColor4 LLControlGroup::getColor(const char* name)
{
	...
	if (i != mNameTable.end())
	{
		...
		switch (control->mType)
		{
			case TYPE_COL4:
			{
				return LLColor4(control->getValue());
			}
			case TYPE_COL4U:
			{
				return LLColor4(LLColor4U(control->getValue()));
			}
			default:
			{
									   ...
									   eControlType declare_as)
{
	...
	while (child_nodep)
	{
		...
		LLControlVariable* control = getControl(name.c_str());
		...
		switch (control->mType)
		{
			...
			case TYPE_U32:
			{
				...
				control->setValue((LLSD::Integer) initial);
				++validitems;
				break;
			}
			...
			case TYPE_VEC3:
			{
				...
				control->setValue(vector.getValue());
				++validitems;
				break;
			}
			...
			case TYPE_RECT:
			{
				...
				control->setValue(rect.getValue());
				++validitems;
				break;
			}
			...
			case TYPE_COL4U:
			{
				...
				control->setValue(color.getValue());
				++validitems;
				break;
			}
			...
			case TYPE_COL3:
			{
				...
				control->setValue(LLColor3(color.mV).getValue());
				++validitems;
				break;
			}
							   ...
							   bool save_default)
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (control && (save_default || control->isPersisted()))
		{
			if (!nondefault_only || !control->isSaveValueDefault())
			{
				...
				if (save_default)
				{
					settings[iter->first]["Persist"] = LLSD(control->isPersisted());
				...
				LLSD value = control->getSaveValue(!save_default);
			...
			{
				LL_DEBUGS("SaveSettings") << "Skipping " << control->getName()
										  << LL_ENDL;
			}
		}
	}
								 ...
								 bool set_default_values, bool save_values)
{
		 ...
		 itr != end; ++itr)
	{
		...
		LLControlVariable* cur_control = getControl(name.c_str());
		if (cur_control)
		{
			if (set_default_values)
			{
				...
				if (cur_control->isType(new_type))
				{
					cur_control->setDefaultValue(control_map["Value"]);
					cur_control->setPersist(persist);
					cur_control->setHiddenFromUser(hide_from_editor);
					cur_control->setComment(control_map["Comment"].asString());
				}
				else
				{
			...
			else if (cur_control->isPersisted())
			{
				cur_control->setValue(control_map["Value"], save_values);
			}
