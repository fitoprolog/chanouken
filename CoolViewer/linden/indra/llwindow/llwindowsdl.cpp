...
void maybe_lock_display()
{
	...
	{
		SDL_Surface* surface =
			SDL_GetWindowSurface(sWindowImplementation->mWindow);
		if (surface && SDL_MUSTLOCK(surface))
		{
			LL_DEBUGS("Window") << "Locking display..." << LL_ENDL;
			SDL_LockSurface(surface);
		}
	}
#else
	if (sWindowImplementation && sWindowImplementation->lockDisplay)
	{
		LL_DEBUGS("Window") << "Locking display..." << LL_ENDL;
		sWindowImplementation->lockDisplay();
	}
#endif
}
...
void maybe_unlock_display()
{
	...
	if (sWindowImplementation)
	{
		...
		if (surface && SDL_MUSTLOCK(surface))
		{
			...
			LL_DEBUGS("Window") << "Display unlocked." << LL_ENDL;
		}
	}
#else
	if (sWindowImplementation && sWindowImplementation->lockDisplay)
	{
		sWindowImplementation->unlockDisplay();
		LL_DEBUGS("Window") << "Display unlocked." << LL_ENDL;
	}
#endif
}
...
// When using SDL2, this method must be called at the end of createContext() so
// that mSDL_Display and mSDL_XWindowID got initialized...
void LLWindowSDL::setWindowIcon()
{
#if LL_X11 && 0	// Does not seem to make any difference...
								...
								bool fullscreen, bool disable_vsync)
{
	...
	if (mFullscreen &&
		SDL_SetWindowFullscreen(mWindow, SDL_WINDOW_FULLSCREEN) != 0)
	{
		llwarns << "Failure to set up full screen " << width << "x" << height
				<< llendl;
		mFullscreen = false;
	}
	if (mFullscreen)
...
# if 0	// This causes crashes in LLGLManager::initExtensions(): we would need
		// to migrate to a different method for checking existing OpenGL
		// extensions, such as with libepoxy. Sadly, the latter causes serious
		// rendering issues on old OpenGL (v2.1) systems...
		// The viewer is faster in compatibility profile mode anyway, and some
		// features used in the latter (such as glLineWidth()) are not usable
		// in the core GL profiles, causing rendering glitches. HB
...
# if LL_X11
	...
	{
		llinfos << "X11 log-parser detected " << gGLManager.mVRAM << "MB VRAM."
				<< llendl;
	}
	else
# endif // LL_X11
...
# endif // LL_X11
...
#else	// LL_SDL2
	const SDL_version* r_sdl_version = SDL_Linked_Version();
	llinfos << "Running against SDL " << S32(r_sdl_version->major) << "."
			<< S32(r_sdl_version->minor) << "." << S32(r_sdl_version->patch)
			<< llendl;
	...
	if (!video_info)
	{
		...
		setupFailure("SDL_GetVideoInfo() failed, Window creation error");
		return false;
	}

	if (video_info->current_h > 0)
	{
		mOriginalAspectRatio = (F32)video_info->current_w /
							   (F32)video_info->current_h;
		llinfos << "Original aspect ratio was " << video_info->current_w
	...
	// Note: these SetAttributes make Tom's 9600-on-AMD64 fail to get a visual,
	// but it is broken anyway when it does, and without these SetAttributes we
	// might easily get an avoidable substandard visual to work with on most
	// other machines.
	...
	S32 sdlflags = SDL_OPENGL | SDL_RESIZABLE | SDL_ANYFORMAT;
	...
	if (mFullscreen)
	{
		...
		mWindow = SDL_SetVideoMode(width, height, bits,
		...
		if (mWindow)
		{
			...
			llinfos << "Running at " << mFullscreenWidth << "x"
					<< mFullscreenHeight << "x" << mFullscreenBits << llendl;
		}
		else
		{
			llwarns << "Fullscreen creation failure. SDL error: "
	...
	if (!mFullscreen && !mWindow)
	{
		...
		llinfos << "Creating window " << width << "x" << height << "x" << bits
				<< llendl;
		...
		{
			llwarns << "Window creating is failing, disabling FSAA and trying again"
					<< llendl;
		...
		if (!mWindow)
		{
			llwarns << "Window creation failure. SDL error: " << SDL_GetError()
					<< llendl;
			setupFailure("Window creation error");
			return false;
		}
	}
	else if (!mFullscreen && mWindow)
	{
		llwarns << "Incoherency: !mFullscreen && mWindow. Skipping. Size: "
				<< width << "x" << height << "x" << bits << llendl;
	}
...
# endif // LL_X11
	{
		// Fall back to letting SDL detect VRAM. Note: I have not seen SDL's
		// detection ever actually find VRAM != 0, but if SDL *does* detect it
		// then this is a bonus.
		gGLManager.mVRAM = video_info->video_mem / 1024;
		if (gGLManager.mVRAM != 0)
		{
			llinfos << "SDL detected " << gGLManager.mVRAM << "MB VRAM."
					<< llendl;
		}
	}
	// If VRAM is not detected, that is handled later

	GLint depth_bits, stencil_bits, red_bits, green_bits, blue_bits,
		  alpha_bits;
	...
	GLint color_bits = red_bits + green_bits + blue_bits + alpha_bits;
	// fixme: actually, it's REALLY important for picking that we get at least
	// 8 bits each of red,green,blue. Alpha we can be a bit more relaxed about
	// if we have to.
	if (color_bits < 32)
	{
...
# endif // LL_X11
	...
	// We need to do this here, once video is initialized
	if (SDL_EnableKeyRepeat(SDL_DEFAULT_REPEAT_DELAY,
							SDL_DEFAULT_REPEAT_INTERVAL) == -1)
	{
		llwarns << "Could not enable key-repeat: " << SDL_GetError() << llendl;
	}
#endif	// LL_SDL2
...
#else
	// *FIXME: this is not really right... But what is ?
	return mWindow != NULL;
#endif
}
...
bool LLWindowSDL::setSize(const LLCoordScreen size)
{
	...
	SDL_PushEvent(&event);
#else
	// Push a resize event onto SDL's queue: we will handle it when it comes
	// out again.
	SDL_Event event;
...
#else
	SDL_SetGamma(mCurrentGamma, mCurrentGamma, mCurrentGamma);
	return true;
#endif
}
...
void LLWindowSDL::flashIcon(F32 seconds)
{
	...
	mFlashing = true;
#else
	llinfos << "Stub for flash icons during " << seconds << " seconds"
			<< llendl;
#endif // LL_X11
}
...
#else
static int x11_clipboard_filter(const SDL_Event* evt)
#endif
{
...
bool LLWindowSDL::getSelectionText(Atom selection, LLWString& text)
{
					   ...
					   XA_UTF8_STRING))
	{
						  ...
						  XA_COMPOUND_TEXT))
		{
			...
			if (!grab_property(mSDL_Display, mSDL_XWindowID, selection,
							   XA_STRING))
			{
				llwarns << "Failure to grab the selection property" << llendl;
			}
		}
	}
	...
	int res = XGetWindowProperty(mSDL_Display, mSDL_XWindowID,
								 PVT_PASTE_BUFFER, 0, 16384, False,
...
// Emulated secondary (copy/paste) clipboard: will only work within the viewer
// (i.e. no inter-program copy/paste operation available).

//virtual
bool LLWindowSDL::isClipboardTextAvailable()
{
	return !mSecondaryClipboard.empty();
}

//virtual
bool LLWindowSDL::pasteTextFromClipboard(LLWString& text)
{
	if (mSecondaryClipboard.empty())
	{
		return false;
	}

	text = mSecondaryClipboard;
	return true;
}
...
LLWindow::LLWindowResolution* LLWindowSDL::getSupportedResolutions(S32& num_resolutions)
{
	if (!mSupportedResolutions)
	{
		...
		{
			int count = 0;
			// Modes are sorted from biggest to smallest, so find the end...
			while (*modes && count < MAX_NUM_RESOLUTIONS)
			{
				++modes;
				++count;
			}
			...
			while (count--)
			{
				...
				if (w >= 800 && h >= 600)
				{
					// Make sure we do not add the same resolution multiple
					// times !
					bool resolution_exists = false;
					for (S32 i = 0; i < mNumSupportedResolutions; ++i)
					{
...
# endif // LL_X11
	...
	SDL_GrabMode wantmode = mCaptured ? SDL_GRAB_ON : SDL_GRAB_OFF;

	SDL_GrabMode newmode;
	if (!mFullscreen)	// Only bother if we are windowed
	{
# if LL_X11
		if (mSDL_Display)
		{
			// We dirtily mix raw X11 with SDL so that our pointer is not (as
			// often) constrained to the limits of the window while grabbed,
			// which feels nicer and hopefully eliminates some reported 'sticky
			// pointer' problems. We use raw X11 instead of SDL_WM_GrabInput()
			// because the latter constrains the pointer to the window and also
			// steals all *keyboard* input from the window manager, which was
			// frustrating users.
			int result;
			if (wantmode == SDL_GRAB_ON)
			{
				maybe_lock_display();
				result = XGrabPointer(mSDL_Display, mSDL_XWindowID, True, 0,
									  GrabModeAsync, GrabModeAsync, None, None,
									  CurrentTime);
				maybe_unlock_display();
				newmode = (GrabSuccess == result ? SDL_GRAB_ON : SDL_GRAB_OFF);
			}
...
# endif // LL_X11
	...
	{
		// Pretend we got what we wanted, when really we do not care.
		newmode = wantmode;
	}
	...
	while (SDL_PollEvent(&event))
	{
		switch (event.type)
		{
			...
			case SDL_KEYDOWN:
			{
				...
				if (gKeyboardp)
				{
					gKeyboardp->handleKeyDown(keysym, mKeyModifiers);
				}
				// Part of the fix for SL-13243
				if (SDLCheckGrabbyKeys(keysym, true) != 0)
				{
					SDLReallyCaptureInput(true);
				}
				if (event.key.keysym.unicode)
				{
					MASK mask = gKeyboardp ? gKeyboardp->currentMask(false)
										   : 0;
					handleUnicodeUTF16(event.key.keysym.unicode, mask);
				}
#endif
				break;
			}
			...
			case SDL_KEYUP:
			{
				...
				mSDLKeySym = keysym;
				// Part of the fix for SL-13243
				if (SDLCheckGrabbyKeys(keysym, false) == 0)
				{
					SDLReallyCaptureInput(false);
				}
				if (gKeyboardp)
				{
					gKeyboardp->handleKeyUp(keysym, mKeyModifiers);
				}
#endif
				break;
			}
			...
			{
				// VIDEOEXPOSE does not specify the damage, but hey, it is
				// OpenGL... repaint the whole thing !
			...
			{
				S32 width = event.resize.w;
				S32 height = event.resize.h;
				LL_DEBUGS("Window") << "Handling a resize event: " << width
									<< "x" << height << LL_ENDL;

				// *FIXME: I am not sure this is necessary !
			...
			case SDL_ACTIVEEVENT:
			{
				...
				{
					// Note that for SDL (particularly on X11), keyboard and
					// mouse focus are independent things. Here we are tracking
					// keyboard focus state changes.

					// We have to do our own state massaging because SDL can
					// send us two unfocus events in a row for example, which
					// confuses the focus code [SL-24071].
					if ((S32)event.active.gain != mHaveInputFocus)
					{
				...
				if (event.active.state & SDL_APPACTIVE)
				{
					...
					if ((S32)!event.active.gain != mIsMinimized)
					{
						mIsMinimized = !event.active.gain;

						mCallbacks->handleActivate(this, !mIsMinimized);
						llinfos << "SDL deiconification state switched to "
								<< S32(event.active.gain) << llendl;
					}
					else
					{
						llinfos << "Ignored bogus redundant SDL deiconification state switch to "
								<< S32(event.active.gain) << llendl;
					}
				}
				break;
...
static SDL_Cursor* sdl_cursor_from_bmp(const char* fname, int hotx, int hoty)
{
...
void LLWindowSDL::initCursors()
{
	...
	mSDLCursors[UI_CURSOR_ARROW] = sdl_cursor_from_bmp("llarrow.bmp", 0, 0);
	...
	mSDLCursors[UI_CURSOR_IBEAM] = sdl_cursor_from_bmp("ibeam.bmp", 15, 16);
	mSDLCursors[UI_CURSOR_CROSS] = sdl_cursor_from_bmp("cross.bmp", 16, 14);
	...
	mSDLCursors[UI_CURSOR_SIZENESW] = sdl_cursor_from_bmp("sizenesw.bmp",
														  17, 17);
	mSDLCursors[UI_CURSOR_SIZEWE] = sdl_cursor_from_bmp("sizewe.bmp", 16, 14);
	mSDLCursors[UI_CURSOR_SIZENS] = sdl_cursor_from_bmp("sizens.bmp", 17, 16);
	...
	mSDLCursors[UI_CURSOR_ARROWDRAGMULTI] =
		sdl_cursor_from_bmp("llarrowdragmulti.bmp", 0, 0);
	...
	mSDLCursors[UI_CURSOR_ARROWLOCKED] =
		sdl_cursor_from_bmp("llarrowlocked.bmp", 0, 0);
	mSDLCursors[UI_CURSOR_GRABLOCKED] = sdl_cursor_from_bmp("llgrablocked.bmp",
															2, 13);
	mSDLCursors[UI_CURSOR_TOOLTRANSLATE] =
		sdl_cursor_from_bmp("lltooltranslate.bmp", 0, 0);
	mSDLCursors[UI_CURSOR_TOOLROTATE] = sdl_cursor_from_bmp("lltoolrotate.bmp",
															0, 0);
	...
	mSDLCursors[UI_CURSOR_TOOLPICKOBJECT3] =
		sdl_cursor_from_bmp("toolpickobject3.bmp", 0, 0);
	mSDLCursors[UI_CURSOR_TOOLSIT] = sdl_cursor_from_bmp("toolsit.bmp", 0, 0);
	mSDLCursors[UI_CURSOR_TOOLBUY] = sdl_cursor_from_bmp("toolbuy.bmp", 0, 0);
	mSDLCursors[UI_CURSOR_TOOLPAY] = sdl_cursor_from_bmp("toolpay.bmp", 0, 0);
	...
	mSDLCursors[UI_CURSOR_TOOLMEDIAOPEN] =
		sdl_cursor_from_bmp("toolmediaopen.bmp", 0, 0);
...
LLSD LLWindowSDL::getNativeKeyData()
{
	...
	result["sdl_key_sym"] = (S32)mSDLKeySym;
#endif
...
S32 OSMessageBoxSDL(const std::string& text, const std::string& caption,
					U32 type)
{
#if LL_SDL2 && 0 // Sadly, this does not work ("No message system available").
