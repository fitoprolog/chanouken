								  ...
								  const LLCoordScreen* const posp)
{
	...
	mWindowHandle = CreateWindowEx(dw_ex_style, mWindowClassName, mWindowTitle,
	...
	if (wglChoosePixelFormatARB)
	{
		...
		{
			wglMakeCurrent(mhDC, 0);		// Set the current active rendering context to 0
	...
	if (wglCreateContextAttribsARB)
	{
		...
		while (!done)
		{
			...
			if (!mhRC)
			{
				...
				{
					// Decrement major version and start minor version over at
					// 3
					--attribs[1];
					attribs[3] = 3;
				}
...
void LLWindowWin32::gatherInput()
{
		...
		msg_count < MAX_MESSAGE_PER_UPDATE)
	{
		...
		// Turn watchdog off in here to not fail if windows is doing something
		// wacky
		mCallbacks->handlePauseWatchdog(this);
		DispatchMessage(&msg);
		mCallbacks->handleResumeWatchdog(this);
...
#if 0	// Attempted workaround for problem where typing fast and hitting
		// return would result in only part of the text being sent. JC

		bool key_posted = (bool)TranslateMessage(&msg);
		DispatchMessage(&msg);
		++msg_count;
	...
	// clear this once we've processed all mouse messages that might have
...
{
	LLWindowWin32* window_imp = (LLWindowWin32*)GetWindowLongPtr(h_wnd,
		GWLP_USERDATA);
	if (window_imp)
	{
		window_imp->mCallbacks->handleResumeWatchdog(window_imp);
		window_imp->mCallbacks->handlePingWatchdog(window_imp,
			"Main:StartWndProc");
		// Has user provided their own window callback?
		if (window_imp->mWndProc &&
			!window_imp->mWndProc(h_wnd, u_msg, w_param, l_param))
		{
...
#if 0	// This does not work, as LOWORD returns unsigned short.
		...
		case WM_PAINT:
		{
			...
			window_imp->mCallbacks->handlePaint(window_imp,
				update_rect.left,
				update_rect.top,
				update_width,
				update_height);
		...
		{
			window_imp->mCallbacks->handlePingWatchdog(window_imp,
				"Main:WM_PARENTNOTIFY");
			break;
		}

		case WM_SETCURSOR:
		{
			// This message is sent whenever the cursor is moved in a
			// window. You need to set the appropriate cursor appearance.
		...
		case WM_ACTIVATE:
		{
			...
			// JC - I'm not sure why, but if we do not report that we
		...
		case WM_COMMAND:
		{
			...
			{
				window_imp->mCallbacks->handleMenuSelect(window_imp,
					LOWORD(w_param));
			}
			break;
		}
		...
		case WM_LBUTTONDOWN:
		{
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleMouseDown(window_imp,
				gl_coord, mask))
			{
				return 0;
			}
...
#if 0		// RN: ignore right button double clicks for now
		case WM_RBUTTONDBLCLK:
#endif
		case WM_LBUTTONDBLCLK:
		{
			window_imp->mCallbacks->handlePingWatchdog(window_imp,
				"Main:WM_LBUTTONDBLCLK");
			// Because we move the cursor position in the app, we need to
			// query to find out where the cursor at the time the event is
			// handled. If we do not do this, many clicks could get
			// buffered up, and if the first click changes the cursor
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleDoubleClick(window_imp,
				gl_coord, mask))
			{
				return 0;
			}
		...
		case WM_LBUTTONUP:
		{
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleMouseUp(window_imp,
				gl_coord, mask))
			{
				return 0;
			}
		...
		case WM_RBUTTONDOWN:
		{
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleRightMouseDown(window_imp,
				gl_coord,
				mask))
			{
				return 0;
		...
		case WM_RBUTTONUP:
		{
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleRightMouseUp(window_imp,
				gl_coord, mask))
			{
				return 0;
			}
		...
		case WM_MBUTTONDOWN:
		{
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleMiddleMouseDown(window_imp,
				gl_coord,
				mask))
			{
				return 0;
		...
		case WM_MBUTTONUP:
		{
			...
			window_imp->mCallbacks->handleMouseMove(window_imp,
				gl_coord, mask);
			if (window_imp->mCallbacks->handleMiddleMouseUp(window_imp,
				gl_coord, mask))
			{
				return 0;
			}
		...
		case WM_MOUSEWHEEL:
		{
			...
			{
				window_imp->mCallbacks->handleScrollWheel(window_imp,
					-z_delta /
					WHEEL_DELTA);
				z_delta = 0;
			}
		...
		case WM_MOUSEMOVE:
		{
			...
			window_imp->convertCoords(window_coord, &gl_coord);
			MASK mask = gKeyboardp ? gKeyboardp->currentMask(true) : 0;
			window_imp->mCallbacks->handleMouseMove(window_imp, gl_coord,
				mask);
			return 0;
		}
		...
		case WM_SIZE:
		{
			...
			// There is an odd behavior with WM_SIZE that I would call a
			// bug. If the window is maximized, and you call MoveWindow()
			// with a size smaller than a maximized window, it ends up
			// sending WM_SIZE with w_param set to SIZE_MAXIMIZED, which
			// is not true. So the logic below does not work. Fixed it by
			// calling ShowWindow(SW_RESTORE) first (see moveWindow() in
			// this file). SL-44655

			// If we are now restored, but we were not before, this means
			// that the window was un-minimized.
			...
			// handle case of window being maximized from fully minimized
			// state
		...
		case WM_DPICHANGED:
		{
			...
			if (window_imp->mCallbacks->handleDPIChanged(window_imp, scale,
				width, height))
			{
		...
		case WM_SETFOCUS:
		{
			...
			// Stop flashing the task bar button when our window gains
			// focus
		...
		case WM_COPYDATA:
		{
			...
			window_imp->mCallbacks->handleDataCopy(window_imp,
				myCDS->dwData,
				myCDS->lpData);
			return 0;
		}
...
void LLWindowWin32::handleCompositionMessage(U32 indexes)
{
	...
	if ((indexes & GCS_COMPCLAUSE) && preedit_string.length() > 0)
	{
		...
		if (size > 0)
		{
				...
				data[size / sizeof(DWORD) - 1] == preedit_string_utf16_length)
			{
				...
				{
					const S32 length =
						wstring_wstring_length_from_utf16_length(preedit_string,
							offset,
							data[i + 1] - data[i]);
	...
	if (indexes & GCS_CURSORPOS)
	{
		...
		{
			caret_position =
				wstring_wstring_length_from_utf16_length(preedit_string, 0,
	...
	if (needs_update)
	{
		...
		// Some IME does not query char position after WM_IME_COMPOSITION,
							  ...
							  S32 focus_length, S32 *offset)
{
	...
	const S32 e = llmin((S32)wtext.length(),
		focus + focus_length + CONTEXT_EXCESS);
	...
	LRESULT* result)
{
	if (mPreeditor)
	{
		...
		case IMR_QUERYCHARPOSITION:
		{
			...
			const S32 position =
				wstring_wstring_length_from_utf16_length(wtext, preedit,
		...
		case IMR_RECONVERTSTRING:
		{
			...
			RECONVERTSTRING* const reconvert_string =
				(RECONVERTSTRING*)param;
		...
		case IMR_DOCUMENTFEED:
		{
			...
			S32 context_offset;
			LLWString context = find_context(wtext, preedit,
				preedit_length,
				&context_offset);
...
void LLWindowWin32::setDPIAwareness()
{
	...
	if (shcorep)
	{
		...
		if (spdap)
		{
			...
			if (hr != S_OK)
			{
				llwarns << "SetProcessDpiAwareness() function returned an error. Will use legacy DPI awareness API of Win XP/7"
						<< llendl;
			}
		}
		FreeLibrary(shcorep);
	...
	{
		llwarns << "Could not load shcore.dll library (included by <ShellScalingAPI.h> from Win 8.1 SDK. Will use legacy DPI awareness API of Win XP/7"
				<< llendl;
	}
}
...
F32 LLWindowWin32::getSystemUISize()
{
	...
	if (shcorep)
	{
		...
		if (gpdap && gdfmp)
		{
			...
			if (dpi_awareness == PROCESS_PER_MONITOR_DPI_AWARE)
			{
				...
				{
					llwarns << "Could not determine DPI for monitor. Setting scale to default 100%"
							<< llendl;
				}
			}
			else
			{
				llwarns << "Process is not per-monitor DPI-aware. Setting scale to default 100%"
						<< llendl;
			}
		}
		FreeLibrary(shcorep);
	...
	{
		llwarns << "Could not load shcore.dll library (included by <ShellScalingAPI.h> from Win 8.1 SDK). Using legacy DPI awareness API of Win XP/7"
				<< llendl;
