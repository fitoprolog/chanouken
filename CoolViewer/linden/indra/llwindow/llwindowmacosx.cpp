...
#if 0	// Not used (never enabled either in LL's viewer)
...
bool LLWindowMacOSX::createContext(bool fullscreen, bool disable_vsync)
{
	...
	// This sets up our view to recieve text from our non-inline text input
	// window.
...
void LLSplashScreenMacOSX::updateImpl(const std::string& mesg)
{
	if (mWindow)
	{
		...
		string = CFStringCreateWithCString(NULL, mesg.c_str(),
				   ...
				   attributedStringInfo segments)
{
	...
	if (replace_range[0] < replace_range[1])
	{
		...
		S32 location =
			wstring_wstring_length_from_utf16_length(text, 0,
													 replace_range[0]);
		S32 length =
			wstring_wstring_length_from_utf16_length(text, location,
													 replace_range[1]);
