...
public:
	...
	// *HACK: to compute window borders offsets (needed for SDL2)
	...
	// These two functions create a way to make a busy cursor instead of an
	// arrow when someone's busy doing something. Draw an arrow/hour if
	// busycount > 0.
	virtual void incBusyCount();
	virtual void decBusyCount();
	LL_INLINE virtual void resetBusyCount()				{ mBusyCount = 0; }
	LL_INLINE virtual S32 getBusyCount() const			{ return mBusyCount; }

	// Sets cursor, may set to arrow+hourglass
	virtual void setCursor(ECursorType cursor) = 0;
	LL_INLINE virtual ECursorType getCursor() const		{ return mCurrentCursor; }
