...
static opj_event_mgr_t sEventMgr;		// Event manager

// Helper function
LL_INLINE int ceildivpow2(int a, int b)
{
							...
							S32 max_channel_count)
{
	...
	{
		llwarns << "ERROR in decodeImpl: failed to decode image !" << llendl;
		if (image)
		{
		 ...
		 ++comp, ++dest)
	{
		...
		{
			llwarns << "ERROR in decodeImpl: failed to decode image ! (NULL comp data - OpenJPEG bug)"
					<< llendl;
			opj_image_destroy(image);
...
{
	constexpr int J2K_HEADER_LENGTH = 42;
...
{
	return raw_imagep ? encode(raw_imagep, NULL) : false;
}
