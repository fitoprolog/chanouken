...
#if defined(HB_DULLAHAN_EXTENDED) || CHROME_VERSION_MAJOR >= 75
...
void MediaPluginCEF::receiveMessage(const char* msgstr)
{
	...
	else if (message_class == LLPLUGIN_MESSAGE_CLASS_MEDIA)
	{
		if (message_name == "init")
		{
			...
			// Note: Linux arm64's Dullahan version needs updating before it
			// can do this...
#if CHROME_VERSION_MAJOR >= 91 && !SSE2NEON
...
#if defined(HB_DULLAHAN_EXTENDED)
...
# if CHROME_VERSION_MAJOR < 75
			settings.cookie_store_path = mUserDataDir + "cef_cookies";
# endif
			settings.cache_path = mUserDataDir + "cef_cache";
# if CHROME_VERSION_MAJOR >= 83
			settings.root_cache_path = mUserDataDir + "cef_cache";
...
#else
			S32 x = 40 * message_in.getValueS32("clicks_x");
			S32 y = -40 * message_in.getValueS32("clicks_y");
...
#  else
	mCEFLib->nativeKeyboardEvent(key_event, native_scan_code,
								 native_virtual_key, native_modifiers);
#  endif
# else
...
void MediaPluginCEF::unicodeInput(std::string event, LLSD native_key_data)
{
	...
	U64 lparam = ll_U32_from_sd(native_key_data["l_param"]);
	mCEFLib->nativeKeyboardEventWin(msg, wparam, lparam);
#elif LL_LINUX && defined(HB_DULLAHAN_EXTENDED)
	U32 native_virtual_key = native_key_data["virtual_key"].asInteger();
# if LL_SDL
	U32 native_scan_code = native_key_data["sdl_key_sym"].asInteger();
	if (native_scan_code == (U32)'\n')
