...
public:
	...
	virtual void visit(const LLOctreeNode<LLVolumeTriangle>* branch)
	{
		...
		LLVolumeOctreeListener* node =
			(LLVolumeOctreeListener*)branch->getListener(0);
		...
		else if (!branch->isLeaf())
		{
			...
			LLVolumeOctreeListener* child =
				(LLVolumeOctreeListener*)branch->getChild(0)->getListener(0);
		...
		for (S32 i = 0, count = branch->getChildCount(); i < count; ++i)
		{
			...
			LLVolumeOctreeListener* child =
				(LLVolumeOctreeListener*)branch->getChild(i)->getListener(0);
...
S32 LLPath::getNumPoints(const LLPathParams& params, F32 detail)
{
	...
	switch (params.getCurveType() & 0xf0)
	{
		case LL_PCODE_PATH_CIRCLE:
		{
			...
			F32 twist_mag = fabsf(params.getTwistBegin() - params.getTwist());
		...
		default:
		{
			...
			np = llfloor(fabsf(params.getTwistBegin() - params.getTwist()) *
						 3.5f * (detail - 0.5f)) + 2;
		}
	}
		...
		case LL_PCODE_PATH_TEST:
		{
			...
			for (S32 i = 0; i < np; ++i)
			{
				...
				mPath[i].mPos.set(0.f,
								  lerp(0.f,
									   -sinf(F_PI * params.getTwist() * t) *
									   0.5f, t),
								  lerp(-0.5,
									   cosf(F_PI * params.getTwist() * t) * 0.5f,
									   t));
				mPath[i].mScale.set(lerp(1.f, params.getScale().mV[0], t),
									lerp(1.f, params.getScale().mV[1], t),
									0.f, 1.f);
				mPath[i].mTexT = t;
				quat.setAngleAxis(F_PI * params.getTwist() * t, 1.f, 0.f, 0.f);
				LLMatrix3 tmp(quat);
...
bool LLVolume::generate()
{
	...
	if (regen_path || regen_prof)
	{
		...
		for (S32 s = 0; s < s_size; ++s)
		{
			...
			if (!offset.isFinite3())
			{
				llwarns_once << "Path with non-finite points. Resetting offset to 0."
							 << llendl;
				offset.clear();
			}
...
bool LLVolume::unpackVolumeFaces(const LLSD& mdl)
{
	...
	if (sOptimizeCache && !cacheOptimize())
	{
...
{
#if LL_OPENMP
	// NOTE: cannot use OpenMP when called from the mesh repository which is
	// itself a thread...
	if (is_main_thread())
	{
...
#		pragma omp parallel for
		...
		{
			if (success && !mVolumeFaces[i].cacheOptimize())
			{
				success = false;
			}
		}
...
void LLVolumeFace::optimize(F32 angle_cutoff)
{
	...
	{
		llinfos << "Optimization reached for this mesh = "
				<< new_face.mNumVertices <<  "/" << mNumVertices
				<< " new/old vertices." << llendl;
		llassert(new_face.mNumIndices == mNumIndices);
		swapData(new_face);
	}
	else
	{
		llinfos << "No optimization possible for this mesh" << llendl;
	}
}
...
bool LLVolumeFace::cacheOptimize()
{
	...
	size_t total_verts = meshopt_generateVertexRemapMulti(remap.data(),
														  ...
														  streams.data(),
														  streams.size());
	meshopt_remapIndexBuffer(indices.data(), mIndices, mNumIndices,
							 remap.data());
	bool failed = false;
	...
	meshopt_optimizeVertexCache(mIndices, indices.data(), mNumIndices,
								total_verts);
#if 0	// Do not do that: it causes rendering glitches with some meshes. HB
	meshopt_optimizeOverdraw(indices.data(), mIndices, mNumIndices,
							 (float*)buffers[0].scratch, total_verts,
							 buffers[0].stride, 1.05f);
#endif
	meshopt_optimizeVertexFetchRemap(remap.data(), indices.data(), mNumIndices,
									 total_verts);
	meshopt_remapIndexBuffer(mIndices, indices.data(), mNumIndices,
							 remap.data());
...
{
protected:
	LOG_CLASS(LLVCacheTriangleData);
...
public:
	...
	void complete()
	{
		...
		for (S32 i = 0; i < 3; ++i)
		{
			if (mVertex[i])
			{
				if (mVertex[i]->mActiveTriangles > 0)
				{
					--mVertex[i]->mActiveTriangles;
				}
				else
				{
					llwarns_once << "Bad active triangles count !" << llendl;
					llassert(false);
				}
			}
		}
	...
	LL_INLINE bool operator<(const LLVCacheTriangleData& rhs) const
	{
		// Highest score first
...
constexpr S32 MaxSizeVertexCache = 32;
constexpr F64 FindVertexScore_Scaler = 1.0 / (MaxSizeVertexCache - 3);

static F32 find_vertex_score(LLVCacheVertexData& data)
{
	F64 score = -1.0;
	...
	if (data.mActiveTriangles > 0)
	{
		...
		F64 valence_boost = powf((F64)data.mActiveTriangles,
...
{
protected:
	LOG_CLASS(LLVCacheLRU);

public:
	LLVCacheLRU()
	{
		for (S32 i = 0; i < MaxSizeVertexCache + 3; ++i)
		{
			mCache[i] = NULL;
		}
	...
	void addVertex(LLVCacheVertexData* data)
	{
		if (!data)
		{
			llwarns << "Called with NULL data !  Ignored." << llendl;
			llassert(false);
			return;
		}
	...
	void updateScores()
	{
		...
		while (data_iter != end_data)
		{
			...
			// Trailing 3 vertices are not actually in the cache for scoring
			// purposes
			if (data)
			{
				data->mCacheTag = -1;
		...
		while (data_iter != end_data)
		{
			...
			if (data)
			{
				for (std::vector<LLVCacheTriangleData*>::iterator
						iter = data->mTriangles.begin(),
						end = data->mTriangles.end();
					 iter != end; ++iter)
				{
						...
						tri->mVertex[0] && tri->mVertex[1] && tri->mVertex[2])
					{
						...
						if (!mBestTriangle ||
							mBestTriangle->mScore < tri->mScore)
						{
							mBestTriangle = tri;
						}
					}
...
// Define to 1 to reinstate LL's bogus mesh vertex sorting method that only
// works for Windows thanks to an idiosyncrasy in VS2017 STL implementation...
#define LL_BAD_VERTEX_SORTING 0
...
bool LLVolumeFace::cacheOptimize()
{
	...
	// Mapping of triangles to vertices
	...
	try
	{
		...
		LL_DEBUGS("MeshVolume") << "Populating vertex and triangle data."
								<< LL_ENDL;
		for (S32 i = 0; i < mNumIndices; ++i)
		{
			U16 idx = mIndices[i];
			if (idx >= mNumVertices)
			{
				llwarns << "Bad vertex indice (" << idx << "): maximum is: "
						<< mNumVertices << ". Aborting." << llendl;
				// Bad mesh data: report a failure.
				return false;
			}
			...
			vertex_data[idx].mTriangles.emplace_back(&(triangle_data[tri_idx]));
		}
	}
	catch (std::bad_alloc&)
	{
		LLMemory::allocationFailed();
		llwarns << "Out of memory trying to optimize vertices" << llendl;
		// Report a success so that the unoptimized mesh will be used
		return true;
	}

	LL_DEBUGS("MeshVolume") << "Calculating score value." << LL_ENDL;
	for (S32 i = 0; i < mNumVertices; ++i)
	{
		// Initialize score values (no cache -- might try a fifo cache here)
		LLVCacheVertexData& data = vertex_data[i];
		data.mScore = find_vertex_score(data);
		S32 count = data.mActiveTriangles = data.mTriangles.size();
		for (S32 j = 0; j < count; ++j)
		{
			if (data.mTriangles[j])
			{
				data.mTriangles[j]->mScore += data.mScore;
			}
			else
			{
				llwarns << "NULL triangle found, aborting" << llendl;
				// Bad mesh data: report a failure.
				return false;
			}
		}
	...
	S32 count = triangle_data.size();

	LL_DEBUGS("MeshVolume") << "Sorting triangle data by score." << LL_ENDL;
# if LL_BAD_VERTEX_SORTING
...
# else
	...
	std::sort(v.begin(), v.end(),
			  [&triangle_data](U16 lhs, U16 rhs)
			  {
				// Highest score first
				return triangle_data[lhs].mScore > triangle_data[rhs].mScore;
			  });
# endif
	std::vector<U16> new_indices;
	try
	{
		new_indices.reserve(mNumIndices * 3);
	}
	catch (std::bad_alloc&)
	{
		LLMemory::allocationFailed();
	...
	LL_DEBUGS("MeshVolume") << "Prime pumping by adding first triangle to cache."
							<< LL_ENDL;
	// Prime pump by adding first triangle to cache
	LLVCacheLRU cache;
# if LL_BAD_VERTEX_SORTING
	LLVCacheTriangleData* tri = &(triangle_data[0]);
# else
	LLVCacheTriangleData* tri = &(triangle_data[v[0]]);
# endif
	if (tri && tri->mVertex[0] && tri->mVertex[1] && tri->mVertex[2])
	{
	...
	{
		llwarns << "Could not get a valid first triangle" << llendl;
		return false;
	}

	LL_DEBUGS("MeshVolume") << "Searching for best triangle." << LL_ENDL;
...
# else
				U16 index = v[j];
				if (triangle_data[index].mActive)
				{
					tri = &triangle_data[index];
					break;
				}
# endif
			}
		...
		{
			LL_DEBUGS("MeshVolume") << "Could not get best triangle"
									<< LL_ENDL;
			// Report a success so that the unoptimized mesh will be used
			return true;
		}
	...
	// Starting from this point, we cannot any more ignore an optimization
	// failure (because the vertex data gets changed, starting with mIndices
	// just below)...

	LL_DEBUGS("MeshVolume") << "Updating indices." << LL_ENDL;
	for (S32 i = 0; i < mNumIndices; ++i)
	{
	...
	LL_DEBUGS("MeshVolume") << "Allocating new buffer space." << LL_ENDL;
	// Allocate space for new buffer
	S32 num_verts = mNumVertices;
	S32 size = ((num_verts * sizeof(LLVector2)) + 0xF) & ~0xF;
	S32 bytes = sizeof(LLVector4a) * 2 * num_verts + size;
	LLVector4a* pos = (LLVector4a*)allocate_volume_mem_64(bytes);
	if (!pos)
	{
	...
	bytes = sizeof(LLVector4a) * num_verts;

	LLVector4a* wght = NULL;
	if (mWeights)
	{
		wght = (LLVector4a*)allocate_volume_mem(bytes);
		if (!wght)
		{
			// NOTE: norm and tc are part of pos: do not free them !
			LLMemory::allocationFailed(bytes);
			llwarns << "Out of memory trying to optimize vertices" << llendl;
			free_volume_mem_64(pos);
			return false;
		}
	}

	LLVector4a* tangents = NULL;
	if (mTangents)
	{
		tangents = (LLVector4a*)allocate_volume_mem(bytes);
		if (!tangents)
		{
			LLMemory::allocationFailed(bytes);
			llwarns << "Out of memory trying to optimize vertices" << llendl;
	...
	catch (std::bad_alloc&)
	{
		...
		if (tangents)
		{
			free_volume_mem(tangents);
		}
		return false;
	}

	LL_DEBUGS("MeshVolume") << "Copying data to new buffer." << LL_ENDL;
	S32 cur_idx = 0;
	for (S32 i = 0; i < mNumIndices; ++i)
	{
		U16 idx = mIndices[i];
		if (new_idx[idx] == -1)
		{
			// This vertex has not been added yet
			new_idx[idx] = cur_idx;
			...
			if (mTangents)
			{
				tangents[cur_idx] = mTangents[idx];
			}
	...
	LL_DEBUGS("MeshVolume") << "Updating indices again." << LL_ENDL;
	for (S32 i = 0; i < mNumIndices; ++i)
	{
								...
								const LLVector4a& size0)
{
	...
	mOctree = new LLOctreeRoot<LLVolumeTriangle>(center0, size0, NULL);
...
void LLVolumeFace::createTangents()
{
	if (!mTangents)
	{
		...
		LLVector2* tc = (LLVector2*)mTexCoords;
#endif
		LLVector4a* tangents = (LLVector4a*)mTangents;
		...
		default_norm.set(0.f, 1.f, 0.f, 1.f);
		for (S32 i = 0; i < mNumVertices; ++i)
		{
			//tangents[i].normalize3fast();
