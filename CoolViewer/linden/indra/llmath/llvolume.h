...
public:
	...
	LL_INLINE const F32& getBegin() const				{ return mBegin; }
	LL_INLINE const F32& getEnd() const					{ return mEnd; }
	LL_INLINE const F32& getHollow() const				{ return mHollow; }
	LL_INLINE const U8& getCurveType() const			{ return mCurveType; }
...
public:
	...
	LL_INLINE const F32& getScaleX() const				{ return mScale.mV[0]; }
	LL_INLINE const F32& getScaleY() const				{ return mScale.mV[1]; }
	...
	LL_INLINE const F32& getShearX() const				{ return mShear.mV[0]; }
	LL_INLINE const F32& getShearY() const				{ return mShear.mV[1]; }
	LL_INLINE const U8& getCurveType () const			{ return mCurveType; }

	LL_INLINE const F32& getTwistBegin() const			{ return mTwistBegin; }
	LL_INLINE const F32& getTwistEnd() const			{ return mTwistEnd; }
	LL_INLINE const F32& getTwist() const				{ return mTwistEnd; } // deprecated
	LL_INLINE const F32& getRadiusOffset() const		{ return mRadiusOffset; }
	LL_INLINE const LLVector2& getTaper() const			{ return mTaper; }
	LL_INLINE const F32& getTaperX() const				{ return mTaper.mV[0]; }
	LL_INLINE const F32& getTaperY() const				{ return mTaper.mV[1]; }
	LL_INLINE const F32& getRevolutions() const			{ return mRevolutions; }
	LL_INLINE const F32& getSkew() const				{ return mSkew; }
	...
	LL_INLINE void setTwist(F32 twist)					{ setTwistEnd(twist); }	// deprecated
...
public:
	...
	// Deprecated
	LL_INLINE bool setTwist(F32 twist)					{ return setTwistEnd(twist); }
	...
	LL_INLINE const F32& getBeginS() const				{ return mProfileParams.getBegin(); }
 	LL_INLINE const F32& getBeginT() const				{ return mPathParams.getBegin(); }
 	LL_INLINE const F32& getEndS() const				{ return mProfileParams.getEnd(); }
 	LL_INLINE const F32& getEndT() const				{ return mPathParams.getEnd(); }

 	LL_INLINE const F32& getHollow() const				{ return mProfileParams.getHollow(); }
 	LL_INLINE const F32& getTwist() const				{ return mPathParams.getTwist(); }
 	LL_INLINE const F32& getRatio() const				{ return mPathParams.getScaleX(); }
 	LL_INLINE const F32& getRatioX() const				{ return mPathParams.getScaleX(); }
 	LL_INLINE const F32& getRatioY() const				{ return mPathParams.getScaleY(); }
 	LL_INLINE const F32& getShearX() const				{ return mPathParams.getShearX(); }
 	LL_INLINE const F32& getShearY() const				{ return mPathParams.getShearY(); }

	LL_INLINE const F32& getTwistBegin()const			{ return mPathParams.getTwistBegin(); }
	LL_INLINE const F32& getRadiusOffset() const		{ return mPathParams.getRadiusOffset(); }
	LL_INLINE const F32& getTaper() const 				{ return mPathParams.getTaperX(); }
	LL_INLINE const F32& getTaperX() const				{ return mPathParams.getTaperX(); }
	LL_INLINE const F32& getTaperY() const				{ return mPathParams.getTaperY(); }
	LL_INLINE const F32& getRevolutions() const			{ return mPathParams.getRevolutions(); }
	LL_INLINE const F32& getSkew() const				{ return mPathParams.getSkew(); }
	LL_INLINE const LLUUID& getSculptID() const			{ return mSculptID; }
	LL_INLINE const U8& getSculptType() const			{ return mSculptType; }
	LL_INLINE bool isSculpt() const						{ return mSculptID.notNull(); }

	LL_INLINE bool isMeshSculpt() const
	{
		return isSculpt() &&
			   ((mSculptType & LL_SCULPT_TYPE_MASK) == LL_SCULPT_TYPE_MESH);
	}
...
public:
	...
	void createOctree(F32 scaler = 0.25f,
					  const LLVector4a& center = LLVector4a(0, 0, 0),
...
public:
	...
	// Vertex buffer filled in by LLFace to cache this volume face geometry in
	// vram (declared as a LLPointer to LLRefCount to avoid dependency on
	// LLVertexBuffer)
