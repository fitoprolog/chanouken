...
public:
	...
	enum EType
	{
		...
		FT_OUTFIT = 47,
#endif
		// We actually use this value as a way to differenciate between the
		// folder used for creating clothing items and the one (configurable)
		// to store outfits. It is used exclusively via
		// LLInventoryModel::findChoosenCategoryUUIDForType() for 
		// LLAgentWearables::makeNewOutfit().
		FT_MY_OUTFITS = 48,
		...
		// Deprecated in SL, never implemented in OpenSim: constant used to
		// remove that folder in LLInventoryModel::createMissingSystemFolders()
		FT_OUTBOX = 51,
