...
public:
	...
	LL_INLINE void overrideParcelFlags(U32 flags)			{ mParcelFlags = flags; }
	...
	LL_INLINE void overrideSaleTimerExpires(F32 secs_left)	{ mSaleTimerExpires.setTimerExpirySec(secs_left); }

	// MANIPULATORS
	LL_INLINE void generateNewID()							{ mID.generate(); }
	...
	LL_INLINE void setMediaPreventCameraZoom(U8 enable)		{ mMediaPreventCameraZoom = enable; }

	void setMediaURLResetTimer(F32 time);
	LL_INLINE void setLocalID(S32 local_id)					{ mLocalID = local_id; }

	// Blow away all the extra stuff lurking in parcels, including urls, access
	// lists, etc
	void clearParcel();

	// This value is not persisted out to the parcel file, it is only a
	// per-process blocker for attempts to purchase.
	LL_INLINE void setInEscrow(bool in_escrow)				{ mInEscrow = in_escrow; }
	...
	LL_INLINE void setAllParcelFlags(U32 flags)				{ mParcelFlags = flags; }
	void setParcelFlag(U32 flag, bool b);

	LL_INLINE void setAllowModify(bool b)					{ setParcelFlag(PF_CREATE_OBJECTS, b); }
	LL_INLINE void setAllowGroupModify(bool b)				{ setParcelFlag(PF_CREATE_GROUP_OBJECTS, b); }
	LL_INLINE void setAllowAllObjectEntry(bool b)			{ setParcelFlag(PF_ALLOW_ALL_OBJECT_ENTRY, b); }
	LL_INLINE void setAllowGroupObjectEntry(bool b)			{ setParcelFlag(PF_ALLOW_GROUP_OBJECT_ENTRY, b); }
	LL_INLINE void setAllowTerraform(bool b)				{ setParcelFlag(PF_ALLOW_TERRAFORM, b); }
	LL_INLINE void setAllowDamage(bool b)					{ setParcelFlag(PF_ALLOW_DAMAGE, b); }
	LL_INLINE void setAllowFly(bool b)						{ setParcelFlag(PF_ALLOW_FLY, b); }
	LL_INLINE void setAllowLandmark(bool b)					{ setParcelFlag(PF_ALLOW_LANDMARK, b); }
	LL_INLINE void setAllowGroupScripts(bool b)				{ setParcelFlag(PF_ALLOW_GROUP_SCRIPTS, b); }
	LL_INLINE void setAllowOtherScripts(bool b)				{ setParcelFlag(PF_ALLOW_OTHER_SCRIPTS, b); }
	LL_INLINE void setAllowDeedToGroup(bool b)				{ setParcelFlag(PF_ALLOW_DEED_TO_GROUP, b); }
	...
	LL_INLINE void setDenyAnonymous(bool b)					{ setParcelFlag(PF_DENY_ANONYMOUS, b); }
	LL_INLINE void setDenyAgeUnverified(bool b)				{ setParcelFlag(PF_DENY_AGEUNVERIFIED, b); }
	LL_INLINE void setRestrictPushObject(bool b)			{ setParcelFlag(PF_RESTRICT_PUSHOBJECT, b); }
	...
	LL_INLINE void setDrawDistance(F32 dist)				{ mDrawDistance = dist; }
	...
	void packAccessEntries(LLMessageSystem* msg, const access_map_t& list);
	...
	// Scans the pass list and removes any items with an expiration time
	// earlier than "now".
	...
	LL_INLINE bool isInEscrow() const						{ return mInEscrow; }
	...
	// Dealing with sales and parcel conversion. Will trivially return false if
	// there is no sale going on. Pass in the current time in usec which will
	// be used for comparison.
	bool isSaleTimerExpired(const U64& time);

	LL_INLINE F32 getSaleTimerExpires()						{ return mSaleTimerExpires.getRemainingTimeF32(); }

	// Need to record a few things with the parcel when a sale starts.
	...
	LL_INLINE F32 getDrawDistance() const					{ return mDrawDistance; }
	...
	LL_INLINE S32 getMaxPrimCapacity() const				{ return mMaxPrimCapacity; }	// Does not include prim bonus
	LL_INLINE S32 getPrimCount() const						{ return mOwnerPrimCount + mGroupPrimCount + mOtherPrimCount + mSelectedPrimCount; }
	...
	LL_INLINE void setMaxPrimCapacity(S32 max)				{ mMaxPrimCapacity = max; }	// Does not include prim bonus
	// Sim-wide
	LL_INLINE void setSimWideMaxPrimCapacity(S32 current)	{ mSimWideMaxPrimCapacity = current; }
	LL_INLINE void setSimWidePrimCount(S32 current)			{ mSimWidePrimCount = current; }

	// This parcel only (not sim-wide)
	LL_INLINE void setOwnerPrimCount(S32 current)			{ mOwnerPrimCount = current; }
	LL_INLINE void setGroupPrimCount(S32 current)			{ mGroupPrimCount = current; }
	LL_INLINE void setOtherPrimCount(S32 current)			{ mOtherPrimCount = current; }
	LL_INLINE void setSelectedPrimCount(S32 current)		{ mSelectedPrimCount = current; }
	LL_INLINE void setTempPrimCount(S32 current)			{ mTempPrimCount = current; }
	LL_INLINE void setParcelPrimBonus(F32 bonus)			{ mParcelPrimBonus = bonus; }
	...
	LL_INLINE void setPreviousOwnerID(LLUUID prev_owner)	{ mPreviousOwnerID = prev_owner; }
