...
enum ExportPolicy {
  ...
  ep_full_perm,		// Used on OpenSIM grids that do not support the PERM_EXPORT bit: allow exporting of full perm objects.
...
public:
	...
	// Fixex hierarchy of permissions, applies appropriate permissions at each
	...
	// These functions return true on success. They return false if the given
	// agent isn't allowed to make the change. You can pass LLUUID::null as the
	// agent id if the change is being made by the simulator itself, not on
...
// Inlines
LL_INLINE bool LLPermissions::allowModifyBy(const LLUUID& agent,
											const LLUUID& group) const
{
	return allowOperationBy(PERM_MODIFY, agent, group);
}

LL_INLINE bool LLPermissions::allowCopyBy(const LLUUID& agent,
										  const LLUUID& group) const
{
	return allowOperationBy(PERM_COPY, agent, group);
}

LL_INLINE bool LLPermissions::allowMoveBy(const LLUUID& agent,
										  const LLUUID& group) const
{
	return allowOperationBy(PERM_MOVE, agent, group);
}

LL_INLINE bool LLPermissions::allowModifyBy(const LLUUID& agent) const
{
	return allowOperationBy(PERM_MODIFY, agent, LLUUID::null);
}

LL_INLINE bool LLPermissions::allowCopyBy(const LLUUID& agent) const
{
	return allowOperationBy(PERM_COPY, agent, LLUUID::null);
}

LL_INLINE bool LLPermissions::allowMoveBy(const LLUUID& agent) const
{
	return allowOperationBy(PERM_MOVE, agent, LLUUID::null);
}

LL_INLINE bool LLPermissions::allowTransferTo(const LLUUID& agent_id) const
{
	if (mIsGroupOwned)
	{
