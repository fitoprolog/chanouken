...
void LLSettingsSky::blend(const LLSettingsBase::ptr_t& end, F64 blendf)
{
	...
	LLSettingsSky::ptr_t other =
		boost::dynamic_pointer_cast<LLSettingsSky>(end);
	if (other)
	{
	...
	if (legacy_haze.empty())
	{
		legacy_haze.emplace_back(SETTING_AMBIENT, false, LLSD::TypeArray,
								 boost::bind(&Validator::verifyVectorMinMax,
											 _1, _2,
											 LLSD(LLSDArray(0.0)(0.0)(0.0)("*")),
											 LLSD(LLSDArray(3.0)(3.0)(3.0)("*"))));
...
const LLSettingsSky::validation_list_t& LLSettingsSky::validationList()
{
	...
	if (validation.empty())
	{
		// Note the use of LLSD(LLSDArray()()()...). This is due to an issue
		// with the copy constructor for LLSDArray.  Directly binding the
		// LLSDArray as a parameter without first wrapping it in a pure LLSD
		// object would result in deeply nested arrays like this
		// [[[[[[[[[[v1,v2,v3]]]]]]]]]]
...
LLSD LLSettingsSky::defaults(F32 position)
{
	...
	if (dfltsetting.size() == 0)
	{
		...
		dfltsetting[SETTING_CLOUD_SCROLL_RATE] = LLSDArray(0.2)(0.01);
