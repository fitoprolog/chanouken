...
// LLDAELoader
//-----------------------------------------------------------------------------
LLDAELoader::LLDAELoader(std::string filename, S32 lod,
...
bool LLDAELoader::openFile(std::string filename)
{
...
//static
LLModel* LLDAELoader::loadModelFromDomMesh(domMesh* meshp)
{
	LLVolumeParams volume_params;
	volume_params.setType(LL_PCODE_PROFILE_SQUARE, LL_PCODE_PATH_LINE);
	LLModel* ret = new LLModel(volume_params, 0.f);
	createVolumeFacesFromDomMesh(ret, meshp);
    if (ret->mLabel.empty())
    {
	    ret->mLabel = getElementLabel(meshp);
    }
    return ret;
}
										...
										U32 submodel_limit)
{
	...
	// Remove all faces that definitely won't fit into one model and submodel
	// limit
...
bool LLDAELoader::createVolumeFacesFromDomMesh(LLModel* modelp, domMesh* meshp)
{
	if (meshp)
	{
		modelp->clearFacesAndMaterials();
		...
		LLSD placeholder;
		addVolumeFacesFromDomMesh(modelp, meshp, placeholder);

		if (modelp->getNumVolumeFaces() > 0)
		{
			modelp->normalizeVolumeFaces();
			modelp->optimizeVolumeFaces();
	...
	{
		llwarns << "NULL mesh pointer passed" << llendl;
	}
