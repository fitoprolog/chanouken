...
// Supposed to fix a vertex normal scaling bug at mesh importing, but seems
// to be doing the fix in the wrong place (it would be a renderer issue).
// Seen implemented in FS only...
#define LL_MESH_IMPORT_SCALE_FIX 0
						 ...
						 bool nowrite, bool as_slm, S32 submodel_id)
{
	...
	for (S32 idx = 0; idx < MODEL_NAMES_LENGTH; ++idx)
	{
		...
		if (modelp && modelp->getNumVolumeFaces() > 0 &&
			modelp->getVolumeFace(0).mPositions != NULL)
		{
