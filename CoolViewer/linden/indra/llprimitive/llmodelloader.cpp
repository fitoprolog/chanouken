...
void stretch_extents(LLModel* model, LLMatrix4a& mat, LLVector4a& min,
...
bool LLModelLoader::doLoadModel()
{
	if (mTrySLM)
	{
		...
		if (getSLMFilename(mFilename, slm_filename))
		{
			...
			{
				time_t dae_time = LLFile::lastModidied(mFilename);
				if (!dae_time || dae_time < slm_time)
				{
					// If this fails, fall through and try loading from DAE
