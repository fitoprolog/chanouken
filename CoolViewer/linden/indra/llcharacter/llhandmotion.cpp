...
bool LLHandMotion::onUpdate(F32 time, U8* joint_mask)
{
	...
	{
		time_delta = 0.f;
		llwarns_once << "Negative time passed; zeroed." << llendl;
	}
	mLastTime = time;

	eHandPose* requestedHandPose = (eHandPose*)mCharacter->getAnimationData("Hand Pose");
	...
	if (mCurrentPose != mNewPose)
	{
		...
		if (mNewPose != HAND_POSE_SPREAD)
		{
			incoming_wght = mCharacter->getVisualParamWeight(gHandPoseNames[mNewPose]);
		...
		if (mCurrentPose != HAND_POSE_SPREAD)
		{
			outgoing_wght = mCharacter->getVisualParamWeight(gHandPoseNames[mCurrentPose]);
