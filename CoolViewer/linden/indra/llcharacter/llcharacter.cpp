...
#if 0		// *HACK: not deleting the old param causes a memory leak each time
			// the user triggers the "Rebuild Character" feature, but deleting
			// it would cause a crash when editing the Appearance, because the
			// old param is still referenced in the wearables that were loaded
			// while the character rebuild happened...
			delete old_param;
#endif
		}
		else
