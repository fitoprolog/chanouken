...
void LLMotionController::updateLoadingMotions()
{
		 ...
		 iter != mLoadingMotions.end(); )
	{
		...
		if (!motionp)
		{
			continue; // Maybe it should not happen but I have seen it - MG
		}
		LLMotion::LLMotionInitStatus status =
			motionp->onInitialize(mCharacter);
...
{
	return mPaused && LLFrameTimer::getFrameCount() - mPausedFrame > 2;
}
