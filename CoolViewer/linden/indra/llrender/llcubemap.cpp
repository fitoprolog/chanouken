...
bool LLCubeMap::project(F32& v_val, F32& h_val, bool& outside, U8 side,
		...
		for (U16 v = v_minu; v < v_maxu; ++v)
		{
			for (U16 h = h_minu; h < h_maxu; ++h)
			{
				...
				if (ray * center > 0.999)
				{
					...
					for (U8 cc = 0; cc < 3; ++cc)
					{
						td[offset + cc] = U8((td[offset + cc] +
											  col.mV[cc]) * 0.5f);
					}
				}
			}
		}
		mImages[side]->setSubImage(mRawImages[side], 0, 0, RESOLUTION,
								   RESOLUTION);
	}
}
