...
bool init_from_xml(LLFontRegistry* registry, LLXMLNodePtr node)
{
		 ...
		 child = child->getNextSibling())
	{
		...
		if (child->hasName("font"))
		{
			...
			if (font_succ)
			{
				// If this is the first time we've seen this font name, create
				// a new template map entry for it.
...
LLFontGL* LLFontRegistry::createFont(const LLFontDescriptor& desc)
{
	...
	if (!match_desc)
	{
		llwarns << "Failure: no template font for " << norm_desc.getName()
				<< " - style = " << (S32)norm_desc.getStyle() << llendl;
		return NULL;
	}

	// See whether this best-match font has already been instantiated in the
	// requested size.
	...
	// Build list of font names to look for. Files specified for this font come
	// first, followed by those from the default descriptor.
	...
	if (file_names.empty())
	{
		llwarns << "Failure: no file names specified." << llendl;
		return NULL;
	}
	...
	// and become the "head" font, set to non-fallback. Rest will consitute the
	// fallback list.
...
#elif LL_LINUX
	font_paths.emplace_back("");	// Needed to load system fonts.
#elif LL_WINDOWS
