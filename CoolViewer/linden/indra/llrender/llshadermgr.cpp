...
bool LLShaderMgr::attachShaderFeatures(LLGLSLShader* shader)
{
	...
	if (features->calculatesLighting ||
		(gUseNewShaders && features->calculatesAtmospherics))
	{
	...
	{
		if (gUseNewShaders &&
			!shader->attachVertexObject("windlight/atmosphericsFuncs.glsl"))
		{
			return false;
		}
	...
	if (features->hasObjectSkinning &&
		!shader->attachVertexObject("avatar/objectSkinV.glsl"))
	{
		return false;
	}
	...
	if (gUseNewShaders)
	{
		...
		// We want this BEFORE shadows and AO because those facilities use
		// pos/norm access
	...
	{
		if (gUseNewShaders &&
			!shader->attachFragmentObject("windlight/atmosphericsFuncs.glsl"))
		{
			return false;
		}
...
void LLShaderMgr::dumpShaderSource(U32 shader_count, GLcharARB** shader_text)
{
	llinfos << "";
...
GLhandleARB LLShaderMgr::loadShaderFile(const std::string& filename,
										...
										S32 texture_index_channels)
{
	...
	{
		llwarns << "GL error entering loadShaderFile(): " << error
	...
	for (gpu_class = try_gpu_class; gpu_class > 0; --gpu_class)
	{
		...
		if (file)
		{
			llinfos << "Loading file: " << fname << llendl;
			break; // Done
		}
	}
	...
	else
	{
		...
		else if (gUseNewShaders && major_version == 3)
		{
			if (minor_version < 10)
			{
		...
		{
			// "varying" state is "out" in a vertex program, "in" in a fragment
			// program ("varying" is deprecated after version 1.20)
	...
	if (texture_index_channels > 0 && type == GL_FRAGMENT_SHADER_ARB)
	{
		...
		*/
		...
		if (texture_index_channels == 1)
		{
			...
			header.emplace_back("return texture2D(tex0, texcoord);\n");
			header.emplace_back("}\n");
		}
		else if (major_version > 1 || minor_version >= 30)
		{
			static const char* if_line =
				"\t%sif (vary_texture_index == %d) { return texture2D(tex%d, texcoord); }\n";
			static const char* case_line =
				"\t\tcase %d: return texture2D(tex%d, texcoord);\n";
			...
			if (gGLManager.mIsNVIDIA)
			{
				...
				for (S32 i = 0; i < texture_index_channels; ++i)
				{
					header.emplace_back(llformat(if_line, i > 0 ? "else " : "",
												 i, i));
				}
			...
			else
			{
				...
				for (S32 i = 0; i < texture_index_channels; ++i)
				{
					header.emplace_back(llformat(case_line, i, i));
				}
	...
	if (error != GL_NO_ERROR)
	{
		llwarns << "GL error in glCreateShaderObjectARB: " << error
				<< " - Shader file: " << filename << llendl;
	}
	...
	if (error != GL_NO_ERROR)
	{
		llwarns << "GL error in glShaderSourceARB: " << error
				<< " - Shader file: " << filename << llendl;
	}
	...
	{
		llwarns << "GL error in glCompileShaderARB: " << error
				<< " - Shader file: " << filename << llendl;
		ret = 0;
	}
	stop_glerror();
...
void LLShaderMgr::initAttribsAndUniforms()
{
	...
	{
		sReservedUniforms.emplace_back("sunlight_color_copy");
		sReservedUniforms.emplace_back("ambient");
	}
	...
	llassert(sReservedUniforms.size() == (size_t)(GLOW_DELTA + 1));
	...
	llassert(sReservedUniforms.size() ==
				(size_t)(DEFERRED_SHADOW_TARGET_WIDTH + 1));
	...
	sReservedUniforms.emplace_back("bloomMap");
	...
	sReservedUniforms.emplace_back("translationPalette");
	...
	{
		sReservedUniforms.emplace_back("d1");
		sReservedUniforms.emplace_back("d2");
	}
	...
	if (gUseNewShaders)
	{
		...
		sReservedUniforms.emplace_back("no_atmo");
		...
		sReservedUniforms.emplace_back("water_edge");
		sReservedUniforms.emplace_back("sun_up_factor");
		sReservedUniforms.emplace_back("moonlight_color");
	}

	llassert(sReservedUniforms.size() ==
				(gUseNewShaders ? (size_t)END_RESERVED_UNIFORMS
								: (size_t)(DISPLAY_GAMMA + 1)));
