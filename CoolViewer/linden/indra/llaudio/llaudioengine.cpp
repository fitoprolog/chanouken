...
//virtual
bool LLAudioEngine::init(S32 num_channels, void* userdata)
{
	setDefaults();
	...
	llinfos << "Audio engine successfully created with " << num_channels
			<< " channels." << llendl;
...
constexpr F32 default_max_decode_time = .002f; // 2 ms
...
bool LLAudioEngine::preloadSound(const LLUUID& uuid)
{
	...
	if (gAudioDecodeMgrp && gAudioDecodeMgrp->addDecodeRequest(uuid))
	{
...
bool LLAudioEngine::getUISoundFile(const LLUUID& uuid, std::string& sound_file,
...
#endif
{
	...
	std::string filename = uuid.asString() + ".dsf";
...
bool LLAudioEngine::hasDecodedFile(const LLUUID& uuid)
{
	...
	if (isUISound(uuid) && getUISoundFile(uuid, sound_file))
	{
		return true;
	}
...
//static
void LLAudioEngine::assetCallback(const LLUUID& uuid, LLAssetType::EType,
								  void*, S32 result_code, LLExtStat)
{
	...
	else
	{
		...
		if (!adp)
        {
			...
			llwarns << "Got asset callback without audio data for " << uuid
					<< llendl;
			// Make sure that corrupted sound will never be requested again
			LLAudioData::blockSound(uuid);
        }
		else
		{
...
bool LLAudioSource::play(const LLUUID& audio_id)
{
	...
	{
		llwarns << "Cannot get the channel !" << llendl;
		return false;
	}
}
...
bool LLAudioSource::hasPendingPreloads() const
{
		 ...
		 iter != end; ++iter)
	{
		...
		// !hasValidData(), hence the check for hasValidData()
...
bool LLAudioData::load()
{
	...
	if (!mBufferp)
	{
		...
		llinfos_once << "Not able to allocate a new audio buffer, aborting."
					 << llendl;
		return false;
	}
	...
	if (!mBufferp->loadWAV(sound_file))
	{
