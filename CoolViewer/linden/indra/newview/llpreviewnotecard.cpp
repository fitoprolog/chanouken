									   ...
									   S32 status, LLExtStat)
{
	...
	S32 pos = 0;
	if (preview->mEditor->getLength() > 0)
	{
		pos = preview->mEditor->getCursorPos();
	}
	...
	else
	{
		...
		preview->mEditor->setText(std::string(buffer));
	}

	preview->mEditor->makePristine();
	if (pos > 0)
	{
		preview->mEditor->setCursorPos(pos);
		preview->mEditor->scrollToPos(pos);
	}

	bool modifiable = canModify(preview->mObjectUUID, preview->getItem());
...
bool LLPreviewNotecard::saveIfNeeded(LLInventoryItem* copyitem)
{
	...
	if (!agent_url.empty() && !task_url.empty())
	{
		...
		if (mObjectUUID.isNull() && !agent_url.empty())
		{
			...
			info = boost::make_shared<LLBufferedAssetUploadInfo>(mItemUUID,
		...
		else if (mObjectUUID.notNull() && !task_url.empty())
		{
			...
			info = boost::make_shared<LLBufferedAssetUploadInfo>(mObjectUUID,
																 mItemUUID,
																 LLAssetType::AT_NOTECARD,
																 buffer,
