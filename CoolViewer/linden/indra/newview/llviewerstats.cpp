...
U32 LLViewerStats::sComplexityReports = 0;
U32 LLViewerStats::sTooComplexReports = 0;
...
const StatAttributes STAT_INFO[LLViewerStats::ST_COUNT] =
{
	...
	StatAttributes("Logitech LCD", false, false) 		// Unused
};
...
void LLViewerStats::updateStatistics(U32 frame_count)
{
	...
	static LLCachedControl<bool> render_vbo_enable(gSavedSettings,
												   "RenderVBOEnable");
	setStat(ST_ENABLE_VBO, (F64)render_vbo_enable);

	static LLCachedControl<S32> render_lighting_detail(gSavedSettings,
													   "RenderLightingDetail");
	setStat(ST_LIGHTING_DETAIL, (F64)render_lighting_detail);
	...
	// Deprecated/never actually used
	setStat(ST_LOGITECH_LCD, false);

	F32 max_virtual_mem_kb = (F32)LLMemory::getMaxVirtualMemKB();
	F32 avail_virtual_mem_kb = (F32)LLMemory::getAvailableVirtualMemKB();
	U32 memory_usage = 100.f * (max_virtual_mem_kb - avail_virtual_mem_kb) /
					   max_virtual_mem_kb;
	mMemoryUsageStat.addValue(memory_usage);
}
...
void LLViewerStats::sendStats()
{
	...
	agent["fps"] = (F32)gForegroundFrameCount /
				   gForegroundTime.getElapsedTimeF32();
	...
	// Let's cache this non-varying data...
	static LLSD system;
	if (system.isUndefined())
	{
		system["ram"] = (S32)LLMemoryInfo::getPhysicalMemoryKB();
		system["os"] = LLOSInfo::getInstance()->getOSStringSimple();
		system["cpu"] = LLCPUInfo::getInstance()->getCPUString();
...
{
	mNextAvStatsSendingTime = gFrameTimeSeconds + SEND_AVATAR_STATS_PERIOD;
	sComplexityReports = sTooComplexReports = 0;
}
...
void LLViewerStats::avatarRenderingStats()
{
	...
	for (S32 i = 0, count = LLCharacter::sInstances.size(); i < count; ++i)
	{
			...
			!avatarp->isOrphaned() && avatarp->isFullyLoaded(true))
		{
			...
			if (complexity)
			{
				...
				data[region_id.asString()][KEY_AGENTS][av_id.asString()] = info;
			}
		}
	}
	...
	// concerned regions
	LLCoros* coro = LLCoros::getInstance();
		 ...
		 it != end; ++it)
	{
		...
		coro->launch("LLViewerStats::getAvatarRenderStatsCoro",
		...
		coro->launch("LLViewerStats::sendAvatarRenderStatsCoro",
											 ...
											 std::string region_name)
{
		...
		result.has(KEY_OVER_COMPLEXITY_LIMIT))
	{
		...
		sComplexityReports += reporting;
		U32 overlimit = result[KEY_OVER_COMPLEXITY_LIMIT].asInteger();
		sTooComplexReports += overlimit;
		if (reporting != 0 || overlimit != 0)
		{
			llinfos << "Complexity reports: " << sComplexityReports
					<< " - Too complex reports: " << sTooComplexReports
					<< llendl;
		}
	}
	else
...
void output_statistics(void*)
{
	...
	llinfos << "Texture usage: " << LLImageGL::sGlobalTextureMemoryInBytes
			<< llendl;
	llinfos << "Bound textures usage: "
			<< LLImageGL::sBoundTextureMemoryInBytes << llendl;
	llinfos << "Raw usage: " << LLImageRaw::sGlobalRawMemory << llendl;
	llinfos << "Formatted usage: " << LLImageFormatted::sGlobalFormattedMemory
			<< llendl;
	LLImage::dumpStats();
