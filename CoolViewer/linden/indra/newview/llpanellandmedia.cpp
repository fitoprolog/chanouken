...
void LLPanelLandMedia::refresh()
{
	...
	else
	{
		...
		can_change_media = LLViewerParcelMgr::isParcelModifiableByAgent(parcel,
		...
		{
			floater_url_entry->addURLToCombobox(getMediaURL());
		}
...
void LLPanelLandMedia::onCommitType(LLUICtrl* ctrl, void* userdata)
{
	...
	current_type = LLMIMETypes::widgetType(self->childGetText("mime_type"));
