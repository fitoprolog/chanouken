...
void LLSky::init()
{
	...
	mVOGroundp =
		(LLVOGround*)gObjectList.createObjectViewer(LLViewerObject::LL_VO_GROUND,
													NULL);
	gPipeline.createObject((LLViewerObject*)mVOGroundp.get());
...
void LLSky::setFogRatio(F32 fog_ratio)
{
	if (mVOSkyp.notNull())
	{
...
{
	return mVOSkyp.notNull() ? mVOSkyp->getFogRatio() : 0.f;
}
