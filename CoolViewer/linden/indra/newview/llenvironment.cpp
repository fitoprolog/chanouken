...
static const std::string PUMP_EXPERIENCE = "experience_permission";
...
bool LLEnvironmentRequest::doRequest(LLEnvironment::env_apply_fn cb)
{
	...
	coros->launch("LLEnvironmentRequest::environmentRequestCoro",
...
{
public:
	typedef boost::shared_ptr<LLSettingsInjected<SETTINGT> >  ptr_t;
					   ...
					   const LLUUID& experience_id, F32 transition)
	{
		if (transition > 0.1f)
		{
			typename Injection::ptr_t injection =
				boost::make_shared<Injection>(transition, keyname, value, true,
											  experience_id);
						 ...
						 F64 transition)
	{
			 ...
			 ++it)
		{
				...
				(experience.isNull() || experience == (*it)->mExperience))
			{
				...
				{
					typename Injection::ptr_t injection =
						boost::make_shared<Injection>(transition, keyname,
													  (*it)->mLastValue, false,
													  LLUUID::null);
			 ...
			 it != mOverrideExps.end(); )
		{
			if (experience.isNull() || it->second == experience)
			{
				...
				{
					typename Injection::ptr_t injection =
						boost::make_shared<Injection>(transition, it->first,
...
protected:
	struct Injection
	{
		typedef boost::shared_ptr<Injection> ptr_t;
...
public:
	typedef boost::shared_ptr<LLDayInjection> ptr_t;
	typedef boost::weak_ptr<LLDayInjection> wptr_t;
...
{
	mSky = mInjectedSky =
		boost::make_shared<LLSettingsInjectedSky>(gEnvironment.getCurrentSky());
	mWater = mInjectedWater =
		boost::make_shared<LLSettingsInjectedWater>(gEnvironment.getCurrentWater());
...
{
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLDayInjection::testExperiencesOnParcel",
				  [this, parcel_id]()
				  {
					testExperiencesOnParcelCoro(boost::static_pointer_cast<LLDayInjection>(this->shared_from_this()),
												parcel_id);
				  });
}
									  ...
									  F64 transition)
{
	...
	mBlenderSky = boost::make_shared<LLSettingsBlenderTimeDelta>(target_sky,
																 start_sky,
																 skyp,	
																 transition);
										...
										F64 transition)
{
	...
	mBlenderWater =
		boost::make_shared<LLSettingsBlenderTimeDelta>(scratch_water,
													   start_water, waterp,
													   transition);
...
void LLDayInjection::onEnvironmentChanged(LLEnvironment::EEnvSelection env)
{
	...
	LLEnvironment::DayInstance::ptr_t trans =
		boost::make_shared<InjectedTransition>(boost::static_pointer_cast<LLDayInjection>(shared_from_this()),
...
void LLDayInjection::InjectedTransition::animate()
{
	...
	if (!mInjection->isOverriddenSky())
	{
		...
		mBlenderSky =
			boost::make_shared<LLSettingsBlenderTimeDelta>(mSky, mStartSky,
	...
	mBlenderWater =
		boost::make_shared<LLSettingsBlenderTimeDelta>(mWater, mStartWater,
...
void LLEnvironment::initClass()
{
	...
	mCurrentEnvironment = boost::make_shared<DayInstance>(ENV_DEFAULT);
	...
	LLEventPump& pump = LLEventPumps::getInstance()->obtain(LISTENER_NAME);
...
void LLEnvironment::cleanupClass()
{
	...
	mInstanceValid = false;
	LLEventPump& pump = LLEventPumps::getInstance()->obtain(PUMP_EXPERIENCE);
...
//static
bool LLEnvironment::overrideWindlight()
{
	...
	static LLCachedControl<bool>local_env(gSavedSettings,
										  "UseLocalEnvironment");
	return (parcel_env || local_env) &&
		   gAgent.hasRegionCapability("ExtEnvironment") &&
...
void LLEnvironment::setFixedTimeOfDay(F32 position)
{
	...
	LLTrackBlenderLoopingManual::ptr_t sky_blender =
		boost::make_shared<LLTrackBlenderLoopingManual>(skyp, dayp, sky_track);
	LLSettingsWater::ptr_t waterp = envp->getWater();
	LLTrackBlenderLoopingManual::ptr_t water_blender =
		boost::make_shared<LLTrackBlenderLoopingManual>(waterp, dayp,
...
// Helper function
static void set_wl_fixed_time(F64 time_of_day)
{
	...
	gWLSkyParamMgr.mAnimator.setDayTime(time_of_day);
...
{
	if (gUseNewShaders && !isExtendedEnvironmentEnabled())
	{
	...
	if (!gUseNewShaders)
	{
		// Resets UseParcelEnvironment and UseLocalEnvironment
										  ...
										  bool create)
{
	...
	if (create)
	{
		...
		{
			environment = boost::make_shared<LLDayInjection>(env);
		}
		else
		{
			environment = boost::make_shared<DayInstance>(env);
		}
								   ...
								   S32 env_version)
{
	...
	if (!gUseNewShaders && overrideWindlight())
	{
		gWLSkyParamMgr.mAnimator.setDayRateAndOffset(daylength, dayoffset);
		gWLSkyParamMgr.mAnimator.update(gWLSkyParamMgr.mCurParams);
	}
								   ...
								   S32 env_version)
{
	...
	{
		fixed_env_t fixedenv(boost::static_pointer_cast<LLSettingsSky>(settings),
	...
	else if (settings_type == "water")
	{
		fixed_env_t fixedenv(LLSettingsSky::ptr_t(),
							 boost::static_pointer_cast<LLSettingsWater>(settings));
		setEnvironment(env, fixedenv);
	}
}
...
void LLEnvironment::updateEnvironment(F64 transition, bool forced)
{
	...
	DayInstance::ptr_t trans =
		boost::make_shared<DayTransition>(mCurrentEnvironment->getSky(),
...
void LLEnvironment::requestParcel(S32 parcel_id, env_apply_fn cb)
{
	...
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLEnvironment::coroRequestEnvironment",
...
void LLEnvironment::updateParcel(S32 parcel_id, const LLUUID& asset_id,
								 ...
								 env_apply_fn cb)
{
	UpdateInfo::ptr_t updates = boost::make_shared<UpdateInfo>(asset_id,
															   display_name,
															   day_length,
															   day_offset,
															   altitudes,
															   flags);
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLEnvironment::coroUpdateEnvironment",
											  ...
											  altitudes_vect_t altitudes)
{
	...
	{
		dayp = boost::static_pointer_cast<LLSettingsDay>(settings);
	}
	else
	{
...
void LLEnvironment::updateParcel(S32 parcel_id,
								 ...
								 altitudes_vect_t altitudes, env_apply_fn cb)
{
	UpdateInfo::ptr_t updates = boost::make_shared<UpdateInfo>(dayp,
															   day_length,
															   day_offset,
															   altitudes);
	LLCoros* coros = LLCoros::getInstance();
...
{
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLEnvironment::coroResetEnvironment",
												 ...
												 LLSettingsBase::ptr_t settings)
{
	...
	if (type == "daycycle")
	{
		return boost::static_pointer_cast<LLSettingsDay>(settings);
	}
...
void LLEnvironment::onAgentPositionHasChanged(const LLVector3& localpos)
{
	...
	// Mark currrent Windlight sky parameters as dirty, to allow for an update
											   ...
											   F64 transition_time)
{
	LLDayInjection::ptr_t injection =
		boost::dynamic_pointer_cast<LLDayInjection>(getEnvironmentInstance(ENV_PUSH));
	if (injection)
	{
												  ...
												  S32 status)
{
	...
	if (type == "daycycle")
	{
		environment->setInjectedDay(boost::static_pointer_cast<LLSettingsDay>(settings),
									experience_id, transition_time);
	}
	else if (type == "sky")
	{
		environment->setInjectedSky(boost::static_pointer_cast<LLSettingsSky>(settings),
									experience_id, transition_time);
	}
	else if (type == "water")
	{
		environment->setInjectedWater(boost::static_pointer_cast<LLSettingsWater>(settings),
									  experience_id, transition_time);
	}
...
{
	ptr_t environment = boost::make_shared<DayInstance>(mEnv);
...
void LLEnvironment::DayInstance::animate()
{
	...
	if (!(mAnimateFlags & NO_ANIMATE_WATER))
	{
		...
		else
		{
			mWater = LLEnvSettingsWater::buildDefaultWater();
			mBlenderWater =
				boost::make_shared<LLTrackBlenderLoopingTime>(mWater,
															  mDayCycle, 0,
															  mDayLength,
															  mDayOffset,
	...
	if (!(mAnimateFlags & NO_ANIMATE_SKY))
	{
		...
		else
		{
			mSky = LLEnvSettingsSky::buildDefaultSky();
			mBlenderSky =
				boost::make_shared<LLTrackBlenderLoopingTime>(mSky, mDayCycle,
															  1, mDayLength,
															  mDayOffset,
															  DEFAULT_UPDATE_THRESHOLD);
