...
#if 0
	msg->setHandlerFuncFast(_PREHASH_GrantedProxies, processGrantedProxies);
#endif
...
void LLAvatarTracker::processNotify(LLMessageSystem* msg, bool online)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		if (info)
		{
			...
			if (chat_notify)
			{
				std::string name;
				if (gCacheNamep &&
					gCacheNamep->getFullName(agent_id, name))
				{
					if (!LLAvatarName::sLegacyNamesForFriends &&
						LLAvatarNameCache::useDisplayNames())
					{
	...
	if (notify)
	{
		...
		LLNotificationPtr n =
			gNotifications.add(online ? "FriendOnline" : "FriendOffline",
							   args);
