...
void LLVoiceClient::requestVoiceAccountProvision(S32 retries)
{
	...
	coro->launch("LLVivoxVoiceClient::voiceAccountProvisionCoro",
	...
	switch (mState)
	{
		...
		case stateDaemonLaunched:
			if (mUpdateTimer.hasExpired())
			{
				...
				{
					// If the connect failed, the socket may have been put into
					// a bad state. Delete it.
					LL_DEBUGS("Voice") << "Failure to connect to socket"
									   << LL_ENDL;
					closeSocket();
					if (++mRetries > 10)
					{
		...
		case stateConnecting:
			...
			if (mPump)
			{
				...
				mPump->addChain(read_chain, NEVER_CHAIN_EXPIRY_SECS);
...
void LLVoiceClient::reapSession(sessionState* session)
{
	...
	{
		LL_DEBUGS("Voice") << "NOT deleting session " << session->mSIPURI
						   << " (it's the current session)" << LL_ENDL;
	}
	else if (session == mNextAudioSession)
	{
		LL_DEBUGS("Voice") << "NOT deleting session " << session->mSIPURI
						   << " (it's the next session)" << LL_ENDL;
	}
	else
	{
...
void LLVoiceClient::parcelChanged()
{
	...
	LLCoros* coro = LLCoros::getInstance();
	coro->launch("LLVivoxVoiceClient::parcelVoiceInfoRequestCoro",
				 boost::bind(&LLVoiceClient::parcelVoiceInfoRequestCoro, url));
}
