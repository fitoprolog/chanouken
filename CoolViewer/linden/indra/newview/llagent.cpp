...
void LLAgent::handleServerFeaturesTransition()
{
	...
	if (mRegionp)
	{
		...
		// Make sure the name tags will be refreshed, using the (possibly
		// new) avatar name cache capability.
	...
	// SSB transition: not be needed any more in SL, but I kept it in case
	// OpenSim would implement SSB, sometime in the future. HB
...
//mk
		...
		gWorld.updateAgentOffset(agent_offset_global);
	}
...
void LLAgent::setPositionAgent(const LLVector3& pos_agent)
{
	...
	if (isAgentAvatarValid())
	{
		parent = (LLViewerObject*)gAgentAvatarp->getParent();
	}
	if (parent)
	{
		...
		pos_agent_sitting = gAgentAvatarp->getPosition() *
							parent->getRotation() + parent->getPositionAgent();
...
LLVector3 LLAgent::getReferenceUpVector()
{
	...
	if (parent)
	{
		...
		if (camera_mode == CAMERA_MODE_THIRD_PERSON)
		{
			...
			up_vector = up_vector * ~(parent->getRenderRotation());
		}
...
void LLAgent::updateLookAt(S32 mouse_x, S32 mouse_y)
{
	...
	if (gViewerWindowp->getMouseVelocityStat()->getCurrent() < 0.01f &&
	...
	else
	{
		...
		LLCoordFrame frameCamera = (LLCoordFrame)gViewerCamera;
		...
		else if (cameraThirdPerson())
		{
			...
			{
				frameCamera.yaw(-x_from_center * yaw_from_mouse_position *
								DEG_TO_RAD);
				frameCamera.pitch(-y_from_center * pitch_from_mouse_position *
								  DEG_TO_RAD);
			}
	...
	if (mCameraMode == CAMERA_MODE_MOUSELOOK)
	{
		...
		if (isAgentAvatarValid())
		{
			...
			if (parent)
			{
				...
				{
					resetAxes(at_axis * ~(parent->getRenderRotation()));
				}
			}
		}
	}
...
void LLAgent::setupSitCamera()
{
	...
	if (parent)
	{
		LLQuaternion parent_rot = parent->getRenderRotation();
...
LLVector3d LLAgent::calcCameraPositionTargetGlobal(bool* hit_limit)
{
	...
	else if (mCameraMode == CAMERA_MODE_MOUSELOOK)
	{
		...
		head_offset.clear();
		LLViewerObject* parent = (LLViewerObject*)gAgentAvatarp->getParent();
		if (gAgentAvatarp->mIsSitting && parent)
		{
			...
			const LLMatrix4& mat = parent->getRenderMatrix();
			camera_position_global =
...
//mk
	...
	if (parent)
	{
		...
		at_axis.mV[VZ] = 0.f;
		at_axis.normalize();
		LLQuaternion obj_rot = parent->getRenderRotation();
...
//mk
	...
	if (teleportCore(pos_global))
	{
		...
		msg->addUUIDFast(_PREHASH_AgentID, gAgentID);
		msg->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
		msg->addUUIDFast(_PREHASH_LandmarkID, lm_asset_id);
		sendReliableMessage();
	}
}
...
void LLAgent::teleportViaLure(const LLUUID& lure_id, bool godlike)
{
	...
	if (teleportCore())
	{
		...
		msg->addU32(_PREHASH_TeleportFlags, teleport_flags);
		sendReliableMessage();
	}
}
							  ...
							  const LLVector3& look_at)
{
	...
	if (teleportCore(pos_global))
	{
		...
		msg->addUUIDFast(_PREHASH_SessionID, gAgentSessionID);
		msg->nextBlockFast(_PREHASH_Info);
		msg->addU64(_PREHASH_RegionHandle, region_handle);
		msg->addVector3(_PREHASH_Position, pos_local);
		msg->addVector3(_PREHASH_LookAt,
						mTeleportKeepsLookAt ? gViewerCamera.getAtAxis()
											 : look_at);
		sendReliableMessage();
	}
}
...
void LLAgent::setTeleportState(ETeleportState state, const std::string& reason)
{
	...
	switch (state)
	{
		...
		case TELEPORT_START:
			LL_DEBUGS("Teleport") << "Switched to state TELEPORT_START"
								  << LL_ENDL;
			// Remember we started the TP process at this position
			...
			mDepartureHandle = mRegionp ? mRegionp->getHandle() : 0;
	...
	LLViewerStats::getInstance()->resetAvatarStats();
						...
						LLVector3 position)
{
	...
	{
		mLookAt = (LLHUDEffectLookAt*)LLHUDManager::createEffect(LLHUDObject::LL_HUD_EFFECT_LOOKAT);
