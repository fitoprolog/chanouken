...
void LLFloaterColorPicker::onClickPipette(void* data)
{
	...
	if (self)
	{
		...
		{
			LLToolPipette* toolp = LLToolPipette::getInstance();
			toolp->setSelectCallback(onColorSelect, self);
			LLToolMgr::getInstance()->setTransientTool(toolp);
		}
	}
}
...
void LLFloaterColorPicker::draw()
{
	...
	mPipetteBtn->setToggleState(LLToolMgr::getInstance()->getCurrentTool() ==
