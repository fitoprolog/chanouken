	...
	if (!handled)
	{
		...
		if (allowsEmbeddedItems())
		{
			...
			if (item_at_pos)
			{
				...
				LLToolDragAndDrop::getInstance()->setDragStart(screen_x,
															   screen_y);
...
bool LLViewerTextEditor::handleHover(S32 x, S32 y, MASK mask)
{
	...
	if (hasMouseCapture())
	{
		...
		else if (mDragItem)
		{
			...
			LLToolDragAndDrop* dadtool = LLToolDragAndDrop::getInstance();
			if (dadtool->isOverThreshold(screen_x, screen_y))
			{
				dadtool->beginDrag(LLAssetType::lookupDragAndDropType(mDragItem->getType()),
								   mDragItem->getUUID(),
								   LLToolDragAndDrop::SOURCE_NOTECARD,
								   getSourceID(), mObjectID);

				return dadtool->handleHover(x, y, mask);
			}
										   ...
										   std::string& tooltip_msg)
{
	...
	switch (cargo_type)
	{
		...
		case DAD_GESTURE:
			...
			break;
		case DAD_SETTINGS:
			supported = LLEnvironment::isExtendedEnvironmentEnabled();
			break;
		default:
			supported = false;
	}
											  ...
											  llwchar wc)
{
	std::string title = LLTrans::getString("Landmark") + ": " +
						item->getName();
	open_landmark((LLViewerInventoryItem*)item, title, false, item->getUUID(),
				  true);
}
										  ...
										  const LLSD& response)
{
	...
	if (option == 0)
	{
		...
		itemptr = new LLPointer<LLInventoryItem>(gInventory.getItem(notification["payload"]["item_id"].asUUID()));
