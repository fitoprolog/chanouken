...
void LLFloaterURLEntry::buildURLHistory()
{
	...
	for (LLSD::array_iterator iter = parcel_history.beginArray(),
		 ...
		 iter != end; ++iter)
	{
		url_list->addSimpleElement((*iter).asString());
	}
}
...
{
	LLPanelLandMedia* panel_media = dynamic_cast<LLPanelLandMedia*>(mParentPanelHandle.get());
	if (panel_media)
	{
	...
	{
		LLPanelFace* panel_face = dynamic_cast<LLPanelFace*>(mParentPanelHandle.get());
		if (panel_face)
		{
...
void LLFloaterURLEntry::onBtnOK(void* userdata)
{
	...
	{
		LLCoros::getInstance()->launch("LLFloaterURLEntry::getMediaTypeCoro",
