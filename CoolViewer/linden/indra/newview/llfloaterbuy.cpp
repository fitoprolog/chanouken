...
{
	LLScrollListCtrl* list = getChild<LLScrollListCtrl>("object_list");
	list->deleteAllItems();

	list = getChild<LLScrollListCtrl>("item_list");
	list->deleteAllItems();
}
...
void LLFloaterBuy::show(const LLSaleInfo& sale_info)
{
	...
	LLCtrlListInterface* object_list;
	object_list = self->childGetListInterface("object_list");
	...
	icon_name = LLInventoryIcon::getIconName(LLAssetType::AT_OBJECT,
