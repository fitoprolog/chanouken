...
void LLToolBrushLand::modifyLandInSelectionGlobal()
{
	if (gViewerParcelMgr.selectionEmpty() ||
		...
		LLToolMgr::getInstance()->getCurrentTool() == LLToolSelectLand::getInstance())
	{
		return;
	}
		...
		iter != end; ++iter)
	{
		...
		bool parcel_selected = gViewerParcelMgr.getParcelSelection()->getWholeParcelSelected();
		LLParcel* selected_parcel = gViewerParcelMgr.getParcelSelection()->getParcel();
