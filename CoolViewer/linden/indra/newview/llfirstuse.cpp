...
{
	sConfigVariables.reserve(19);	// lessens memory fragmentation
	sConfigVariables.emplace_back("FirstAppearance");
	sConfigVariables.emplace_back("FirstAttachNewJoint");
	...
	sConfigVariables.emplace_back("FirstFlexible");
	...
	sConfigVariables.emplace_back("FirstTeleport");
...
void LLFirstUse::resetFirstUse()
{
	...
	gSavedSettings.setString("FirstAttachNewJointList", "");
}
...
void LLFirstUse::useTeleport()
{
	...
	{
		LLVector3d teleportDestination = gTracker.getTrackedPositionGlobal();
		if (teleportDestination != LLVector3d::zero)
		{
			gSavedSettings.setWarning("FirstTeleport", false);
			gNotifications.add("FirstTeleport");
		}
	}
}
...
//static
void LLFirstUse::useAttach(const std::string& joint)
{
	if (gSavedSettings.getWarning("FirstAttachNewJoint"))
	{
		std::string joints = gSavedSettings.getString("FirstAttachNewJointList");
		if (!joints.empty())
		{
			joints += ",";
		}
		...
		if (joints.empty() ||
			temp.find("," + joint + ",") == std::string::npos)
		{
			joints += joint;
			gSavedSettings.setString("FirstAttachNewJointList", joints);

			LLSD args;
			args["JOINTNAME"] = LLTrans::getString(joint);
			gNotifications.add("FirstAttachNewJoint", args);
		}
	}
}
...
{
	simpleNotification("FirstFlexible");
}
