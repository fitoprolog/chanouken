...
LLUUID LLResourceUploadInfo::finishUpload(const LLSD& result)
{
	...
	{
		inventory_item_flags = (U32)result["inventory_flags"].asInteger();
		if (inventory_item_flags)
		{
			llinfos << "inventory_item_flags " << inventory_item_flags
					<< llendl;
		}
	}
	...
	item = new LLViewerInventoryItem(new_inv_item_id, getFolderId(),
									 new_perms, result["new_asset"].asUUID(),
									 getAssetType(), getInventoryType(),
									 getName(), getDescription(),
									 LLSaleInfo::DEFAULT, inventory_item_flags,
									 creation_date_now);
...
LLSD LLNewFileResourceUploadInfo::exportTempFile()
{
	...
	LLSD args;
	std::string error_message, error_label;
	bool error = false;
		...
		if (encode_result != LLVORBISENC_NOERR)
		{
			switch (encode_result)
			{
				case LLVORBISENC_DEST_OPEN_ERR:
					error_message =
						llformat("Could not open temporary compressed sound file for writing: %s\n",
								 filename.c_str());
				...
				default:
					error_message =
						llformat("Unknown vorbis encode failure on: %s\n",
								 orig_filename.c_str());
	...
	else if (exten == "anim")
	{
		...
		if (!infile)
		{
			error_message = llformat("Failed to open animation file %s\n",
									 filename.c_str());
		}
		else if (size <= 0)
		{
			error_message = "Animation file " + orig_filename +
							" is empty !\n";
		}
		else
		{
			...
			else
			{
				...
				LLKeyframeMotion* motionp =
					new LLKeyframeMotion(getAssetId());
				motionp->setCharacter(gAgentAvatarp);
				if (motionp->deserialize(dp, false))
				{
					...
					{
						error_message =
							"Failed saving temporary animation file\n";
					}
				}
				else
				{
					error_message =
						"Failed reading animation file: " + orig_filename +
						"\n";
				}
				delete motionp;
			}
	...
	else if (exten == "tmp")
	{
		...
		if (in)
		{
			...
			if (fscanf(in, "LindenResource\nversion %d\n", &version))
			{
				if (version == 2)
				{
					...
					while (fgets(buf, 1024, in))
					{
						...
						{
							error_message =
								llformat("corrupt resource file: %s",
										 orig_filename.c_str());
				...
				{
					error_message =
						llformat("Unknown linden resource file version in file: %s",
								 orig_filename.c_str());
			...
			if (!error)
			{
				...
				{
					error_message =
						llformat("Unable to create temporary file: %s",
								 filename.c_str());
	...
	else
	{
		// Unknown extension
		error_message =
			llformat("Unknown file extension .%s\nExpected .wav, .tga, .bmp, .jpg, .jpeg, .bvh or .anim",
					 exten.c_str());
		error = true;
	}
	...
	else
	{
		...
		{
			result["error"] = LLSD::Binary(true);
			result["message"] = llformat("Unable to access temporary file: %s",
										 filename.c_str());
		}
	}
...
LLSD LLScriptAssetUpload::generatePostBody()
{
	...
	body["item_id"] = getItemId();
	// NOTE: old code had the running flag as a BOOL (it's now a real bool) and
	// a BOOL is actually an S32 (which translates into an LLSD::Integer
	// instead of an LLSD::Boolean)...
	// OpenSim expects the LLSD for is_script_running to be an Integer, while
	// SL's servers accept either an Integer or a Boolean. For compatibility
	// with OpenSim, let's pass is_script_running as an LLSD integer.
												   ...
												   const LLResourceUploadInfo::ptr_t& info)
{
	...
	LLUUID id = cpmgr->enqueueCoprocedure("Upload", name,
											...
											LLResourceUploadInfo::ptr_t& info)
{
	...
	if (info->getAssetType() == LLAssetType::AT_LSL_TEXT)
	{
		reason = LLTrans::getString("AssetUploadFailed") + reason;
		report_script_upload_error(info->getItemId(), reason);
	}
...
// unless a user-callback was specified.
void next_in_bulk_upload_queue(const LLSD& result = LLSD(),
							   void* userdata = NULL)
{
						 ...
						 void* userdata, bool temp_upload)
{
	...
	if (!url.empty() && !temp_upload)
	{
		...
		{
			info->setCapCallback(next_in_bulk_upload_queue, NULL);
		}
											 ...
											 S32 upload_price)
{
	...
	if (item_folder_id.notNull())
	{
		...
		S32 creation_date_now = time_corrected();
		LLPointer<LLViewerInventoryItem> item;
		item = new LLViewerInventoryItem(server_response["new_inventory_item"].asUUID(),
										 item_folder_id, new_perms,
										 server_response["new_asset"].asUUID(),
										 asset_type, inventory_type, item_name,
		...
		if (inv)
		{
			...
			inv->getPanel()->setSelection(server_response["new_inventory_item"].asUUID(),
										  TAKE_FOCUS_NO);
