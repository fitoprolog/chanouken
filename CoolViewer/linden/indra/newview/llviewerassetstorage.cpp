...
void LLViewerAssetStorage::checkForTimeouts()
{
	...
	while (mCoroWaitList.size() &&
		   cpmgr->count("AssetStorage") < 2 * COPROC_DEFAULT_QUEUE_SIZE / 5)
	{
											...
											bool is_priority)
{
	...
	// Coroutine buffer has fixed size, so buffer any request above limit
	if (cpmgr->count("AssetStorage") >= 2 * COPROC_DEFAULT_QUEUE_SIZE / 5)
	{
...
{
	LLEventPumps::getInstance()->obtain(pump_name).post(LLSD());
}
