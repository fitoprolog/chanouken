...
// Helper function for llviewerassetupload.cpp upload errors
void report_script_upload_error(const LLUUID& item_id,
								const std::string& error)
{
	LLPreview* previewp = LLPreview::find(item_id);
	if (!previewp)
	{
		// No open preview floater for this asset Id
		return;
	}
	...
	// Is it a script asset in inventory ?
	LLPreviewScript* invscriptp = dynamic_cast<LLPreviewScript*>(previewp);
	if (invscriptp)
	{
		invscriptp->callbackLSLCompileFailed(response);
		return;
	}

	// Is it a script asset in an in-world object ?
	LLLiveLSLEditor* objscriptp = dynamic_cast<LLLiveLSLEditor*>(previewp);
	if (objscriptp)
	{
		objscriptp->callbackLSLCompileFailed(response);
	}
}
...
// Wrapper function, to avoid having to expose LLScriptEditor class definition
// in llpreviewscript.h just for a couple of calls in llstartup.cpp...
void load_script_functions(const std::string& filename)
{
...
void LLScriptEditor::onBtnDynamicHelp(void* userdata)
{
	...
	floater = new LLFloater("lsl_help");
			  ...
			  SCRIPT_MIN_WIDTH, SCRIPT_MIN_HEIGHT)
{
	...
	mono_check->setEnabled(gIsInSecondLife &&
						   have_script_upload_cap(LLUUID::null));
	mono_check->set(gSavedSettings.getBool("SaveInventoryScriptsAsMono"));
...
void LLPreviewScript::saveIfNeeded()
{
	...
	LLBufferedAssetUploadInfo::invnUploadFinish_f proc =
