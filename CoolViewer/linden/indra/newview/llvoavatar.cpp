...
#include "llphysicsshapebuilderutil.h"	// For COLLISION_TOLERANCE
...
LLVOAvatar::LLVOAvatar(const LLUUID& id, LLPCode pcode,
										 ...
										 LLVector4a& new_max)
{
	...
	// Load position
		...
		// position is not updated in real time, unlike getRenderPosition().
	...
	for (S32 part_index = 0; part_index < num_parts; )
	{
		...
		if (!buff)
		{
			buff = new LLVertexBuffer(LLDrawPoolAvatar::VERTEX_DATA_MASK,
									  GL_STREAM_DRAW_ARB);
			if (!buff->allocateBuffer(num_verts, num_indices, true))
			{
				...
				memset((U8*)buff->getMappedIndices(), 0,
					   buff->getIndicesSize());
			}
			facep->setVertexBuffer(buff);
		}
		else if (buff->getNumIndices() == (S32)num_indices &&
				 buff->getNumVerts() == (S32)num_verts)
		{
			terse_update = true;
		}
		else if (!buff->resizeBuffer(num_verts, num_indices))
		{
			llwarns << "Failure to resize a vertex buffer with " << num_verts
					<< " vertices and " << num_indices << " indices" << llendl;
			// Attempt to create a dummy triangle
			facep->setSize(1, 3);
		...
		if (buff->getNumIndices() == (S32)num_indices &&
			buff->getNumVerts() == (S32)num_verts)
		{
														 ...
														 const LLUUID& id)
{
	...
	if (id == IMG_DEFAULT_AVATAR || id == IMG_DEFAULT || id == IMG_INVISIBLE)
	{
		...
		// host.
		LLViewerFetchedTexture* result = gTextureList.findImage(id);
		if (result)
		{
			return result;
		}
...
LLViewerTexture* LLVOAvatar::getBakedTexture(U8 te)
{
	...
	if (!isEditingAppearance())
	{
		...
		return LLViewerTextureManager::staticCastToFetchedTexture(baked_img,
																  true);
	}
...
S32 LLVOAvatar::setTETexture(U8 te, const LLUUID& id)
{
	...
	LLViewerFetchedTexture* image = getBakedTextureImage(te, id);
	llassert(image);
	return setTETextureCore(te, image);
}

void LLVOAvatar::idleUpdate(const F64& time)
{
	...
	// Update should be happening max once every 4 frames.
	mNeedsExtentUpdate = (LLDrawable::getCurrentFrame() +
						  mID.mData[0]) % 4 == 0 ||
...
void LLVOAvatar::idleUpdateMisc(bool detailed_update)
{
	...
	if (detailed_update || !useImpostors())
	{
		LL_FAST_TIMER(FTM_ATTACHMENT_UPDATE);
		LLDrawable* drawable;
		for (S32 i = 0, count = mAttachedObjectsVector.size(); i < count; ++i)
		{
			LLViewerObject* attached_object = mAttachedObjectsVector[i].first;
			LLViewerJointAttachment* attachment =
				mAttachedObjectsVector[i].second;
			if (!attachment || !attachment->getValid() ||
				!attached_object || attached_object->isDead() ||
				!(drawable = attached_object->mDrawable))
			{
				continue;
			}

			LLSpatialBridge* bridge = drawable->getSpatialBridge();
			if (visible || !bridge || bridge->getRadius() >= 2.f)
			{
				LLObjectSelectionHandle selection = gSelectMgr.getSelection();
				// if selecting any attachments, update all of them as non
				// damped
				if (selection->getObjectCount() && selection->isAttachment())
				{
					gPipeline.updateMoveNormalAsync(drawable);
				}
				else
				{
					gPipeline.updateMoveDampedAsync(drawable);
				}
...
bool LLVOAvatar::isVisuallyMuted()
{
	...
	if (gFrameTimeSeconds > mCachedVisualMuteUpdateTime)
	{
		...
		muted = muted && useImpostors();

		mCachedVisualMute = muted;
//MK
		if (gRLenabled && gRLInterface.mShowavsDistMax < EXTREMUM &&
			isFullyLoaded() && mHeadp && isAgentAvatarValid() &&
			gAgentAvatarp->mHeadp)
		{
			LLVector3d ours =
				gAgent.getPosGlobalFromAgent(gAgentAvatarp->mHeadp->getWorldPosition());
			LLVector3d theirs =
				gAgent.getPosGlobalFromAgent(mHeadp->getWorldPosition());
			F32 distance = (F32)(theirs - ours).length();
			// If the avatar is farther than the "camavdist" distance, render
			// as silhouette.
			mCachedRLVMute = distance > gRLInterface.mShowavsDistMax &&
			// But if the outer sphere is opaque, no need to render a
			// silhouette or even the avatar at all.
							 (gRLInterface.mCamDistDrawAlphaMax <= 0.999f ||
							  distance > gRLInterface.mCamDistDrawMax);
			if (mCachedRLVMute)
			{
...
//mk
		...
		visible = (LLDrawable::getCurrentFrame() + mID.mData[0]) %
				   mUpdatePeriod == 0;
	}
	else
	{
...
bool LLVOAvatar::updateCharacter()
{
	...
	if (!isPuppetAvatar())
	{
		...
		// for puppet avatars since it breaks their animations !
		updateTimeStep();
	}
...
U32 LLVOAvatar::renderSkinned()
{
	...
	if (sDebugInvisible)
	{
		...
		{
			llcont << "Doesn't need avatar render !";
		}
		else
		{
			llcont << "Rendering !";
	...
	{
		U32 enabled = isUIAvatar() ? GL_FALSE : GL_TRUE;
		LLGLState blend(GL_BLEND, enabled);
		LLGLState test(GL_ALPHA_TEST, enabled);
...
#if LL_DARWIN
    // NORSPEC-59: blatant hack to avoid driver crash on rendering mMeshLODs
	// for eyelashes and baked hair below.
...
U32 LLVOAvatar::renderRigid()
{
	...
	if (isTextureVisible(TEX_EYES_BAKED) || isUIAvatar())
	{
		...
		if (eyeball_left)
		{
			num_indices += eyeball_left->render(mAdjustedPixelArea, true,
												mIsDummy);
		}
		LLViewerJoint* eyeball_right = getViewerJoint(MESH_ID_EYEBALL_RIGHT);
		if (eyeball_right)
		{
			num_indices += eyeball_right->render(mAdjustedPixelArea, true,
												 mIsDummy);
		}
	}
	...
	for (U32 i = 0, count = getNumTEs(); i < count; ++i)
	{
		...
		for (U32 j = 0; j < num_wearables; ++j)
		{
			LLViewerFetchedTexture* imagep =
				LLViewerTextureManager::staticCastToFetchedTexture(getImage(i,
																			j),
																   true);
			if (imagep)
			{
					...
					id != IMG_INVISIBLE)
				{
					...
					if (t_dict && t_dict->mIsLocalTexture)
					{
						ids.emplace(id);
					}
				}
...
void LLVOAvatar::updateTextures()
{
		 ...
		 ++texture_index)
	{
		...
		if (isSelf())	// There's no texture stats for non-self avatars
		{
			for (U32 wearable_index = 0; wearable_index < num_wearables;
				 ++wearable_index)
			{
				imagep =
					LLViewerTextureManager::staticCastToFetchedTexture(getImage(texture_index,
																				wearable_index),
																	   true);
				if (!imagep) continue;
...
bool LLVOAvatar::startMotion(const LLUUID& id, F32 time_offset)
{
	...
	if (getSex() == SEX_FEMALE)
	{
		...
		{
			return LLCharacter::startMotion(ANIM_AGENT_FEMALE_WALK,
											time_offset);
		}
		else if (id == ANIM_AGENT_SIT)
		{
			return LLCharacter::startMotion(ANIM_AGENT_SIT_FEMALE,
											time_offset);
		}
	}
	...
	if (vobj && vobj->mDrawable)
	{
		...
		if (vvo)
		{
			...
			if (skindatap &&
				...
				skindatap->mAlternateBindMatrix.size() > 0)
			{
...
bool LLVOAvatar::processFullyLoadedChange(bool loading)
{
	...
	bool changed = mFullyLoaded != mPreviousFullyLoaded ||
...
void LLVOAvatar::updateMeshTextures()
{
	...
	for (U32 i = 0; i < count; ++i)
	{
		...
		{
			baked_img =
				LLViewerTextureManager::staticCastToFetchedTexture(getImage(mBakedTextureDatas[i].mTextureIndex,
																			0),
																   true);
		...
		for (U32 i = 0, count = mBakedTextureDatas.size(); i < count; ++i)
		{
			...
			if (layer_baked)
			{
				LLViewerFetchedTexture* image =
					LLViewerTextureManager::staticCastToFetchedTexture(getImage(mBakedTextureDatas[i].mTextureIndex, 0),
																				true);
		 ...
		 ++type)
	{
		...
		for (U8 te = 0; te < TEX_NUM_INDICES; ++te)
		{
			if (LLAvatarAppearanceDictionary::getTEWearableType((ETextureIndex)te) == type)
			{
				...
				if (is_god || LLObjectBackup::validateTexturePerms(te_id))
				{
...
void LLVOAvatar::updateSettings()
{
	...
	sRenderName = llclamp((S32)render_name, (S32)RENDER_NAME_NEVER,
						  (S32)RENDER_NAME_ALWAYS);
...
LLVOAvatarUI::LLVOAvatarUI(const LLUUID& id, LLPCode pcode,
						   LLViewerRegion* regionp)
