...
//mk
	...
	{
		// *TODO: Pop up a one-time warning with object details
		llwarns << "PATHFINDING BUG: editing a Permanent object that is also Physical !"
				<< llendl;
	}
	mCheckPhysics->setEnabled(roots_selected > 0 && !is_flexible &&
							  !is_permanent &&
...
void LLPanelObject::sendPosition(bool btn_down)
{
	...
	bool is_attachmnet = mObject->isAttachment();
	if (!is_attachmnet)
	{
