...
bool LLPrefsNetwork::postBuild()
{
	...
	U32 max_requests;
	max_requests = llclamp(gSavedSettings.getU32("TextureFetchConcurrency"),
						   2U, 32U);
	childSetValue("max_texture_http_concurrency", (F32)max_requests);
	childSetEnabled("max_texture_http_concurrency", enabled);

	max_requests = llclamp(gSavedSettings.getU32("MeshMaxConcurrentRequests"),
						   2U, 128U);
	childSetValue("max_mesh_http_concurrency", (F32)max_requests);

	max_requests = llclamp(gSavedSettings.getU32("Mesh2MaxConcurrentRequests"),
						   2U, 32U);
...
{
	mSetCacheButton->setEnabled(!HBFileSelector::isInUse());
	LLPanel::draw();
}
...
void LLPrefsNetwork::apply()
{
	...
	gSavedSettings.setF32("ThrottleBandwidthKBPS",
						  childGetValue("max_bandwidth").asReal());
...
void LLPrefsNetwork::onClickClearDiskCache(void*)
{
	...
	gNotifications.add("CacheWillClear");
}
