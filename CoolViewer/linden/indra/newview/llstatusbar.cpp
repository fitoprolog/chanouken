	...
	mNetworkDown(false)
{
	...
	mAbsoluteMaxBandwidth = gSavedSettings.getF32("ThrottleBandwidthKBPS");
	...
	mSGBandwidth->setStat(&viewerstats->mKBitStat);
	...
	mSGPacketLoss->setStat(&viewerstats->mPacketsLostPercentStat);
	...
	mSGMemoryUsage->setStat(&viewerstats->mMemoryUsageStat);
	...
	mSGMemoryUsage->setPerSec(false);
	mSGMemoryUsage->setClickedCallback(onClickStatGraph);
...
void LLStatusBar::draw()
{
	...
	if (mDirty ||
		...
		!mHealthTimer.hasExpired() || !mMemAllocFailureTimer.hasExpired() ||
		!mNotificationsTimer.hasExpired())
	{
		mDirty = false;
		refresh();
		mUpdateTimer.reset();
...
void LLStatusBar::refresh()
{
	...
	if (update_bw_scale)
	{
		...
		// Refresh the indicator tool tip suffix with the max encountered
		// bandwidth so far.
	...
	static U32 saved_margin = 0;
	static F32 saved_ratio = 0;
	static LLCachedControl<U32> safety_margin(gSavedSettings,
											  "MainMemorySafetyMargin");
	static LLCachedControl<F32> first_step_ratio(gSavedSettings,
												 "SafetyMargin1stStepRatio");
	static LLCachedControl<bool> safety_check(gSavedSettings,
											  "MemorySafetyChecks");
	showing = safety_check;
	if (showing && !mMemAllocFailureTimer.hasExpired())
	{
		// Set visibility based on flashing
		showing = ((S32)(mMemAllocFailureTimer.getElapsedTimeF32() *
						 FLASH_FREQUENCY) & 1) != 0;
	}
	if (showing &&
		(saved_margin != safety_margin || saved_ratio != first_step_ratio))
	{
		saved_margin = safety_margin;
		saved_ratio = first_step_ratio;
		U32 max_mem, max_phys;
		LLMemoryInfo::getMaxMemoryKB(max_phys, max_mem);
		// Threshold at which the texture bias gets increased
		U32 bias_mem = max_mem - (U32)(first_step_ratio * 1024.f) * safety_margin;
		// Threshold at which the draw distance gets decreased
		U32 dd_mem = max_mem - 1024 * safety_margin;
		mSGMemoryUsage->setThreshold(0, 100.f * (F32)bias_mem / (F32)max_mem);
		mSGMemoryUsage->setThreshold(1, 100.f * (F32)dd_mem / (F32)max_mem);

	}
	mSGMemoryUsage->setVisible(mVisibility && showing);
...
//mk
	...
	U32 too_complex = LLViewerStats::getTooComplexReports();
...
{
	std::string money_str = LLLocale::getMonetaryString(balance);
...
void LLStatusBar::setMemAllocFailure()
{
	if (mMemAllocFailureTimer.hasExpired() ||
		mMemAllocFailureTimer.getElapsedTimeF32() > FLASH_TIMER_EXPIRY * 0.5f)
	{
		mMemAllocFailureTimer.reset();
		mMemAllocFailureTimer.setTimerExpirySec(FLASH_TIMER_EXPIRY);
	}
	mDirty = true;
}
...
void LLStatusBar::onClickTooComplex(void*)
{
	...
	LLSD args;
	args["REPORTERS"] = llformat("%d", LLViewerStats::getComplexityReports());
	args["JELLYDOLLS"] = llformat("%d", LLViewerStats::getTooComplexReports());
