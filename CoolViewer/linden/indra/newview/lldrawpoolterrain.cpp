...
void LLDrawPoolTerrain::beginDeferredPass(S32 pass)
{
	...
	if (gUseNewShaders && LLPipeline::sUnderWaterRender)
	{
...
void LLDrawPoolTerrain::beginShadowPass(S32 pass)
{
	...
	if (gUseNewShaders)
	{
		bool sun_up = gEnvironment.getIsSunUp();
...
//mk
	...
	if (gUseNewShaders)
	{
		renderFullShaderEE(detail_texture0p, detail_texture1p,
						   detail_texture2p, detail_texture3p, tp0, tp1);
	}
	else
	{
		renderFullShaderWL(detail_texture0p, detail_texture1p,
						   detail_texture2p, detail_texture3p, tp0, tp1);
	}
}

void LLDrawPoolTerrain::renderFullShaderWL(LLViewerTexture* detail_texture0p,
										   ...
										   const LLVector4& tp1)
{
	...
	LLTexUnit* unit4 = gGL.getTexUnit(4);
	unit4->disable();
	unit4->activate();
...
void LLDrawPoolTerrain::renderFullShaderEE(LLViewerTexture* detail_texture0p,
										   LLViewerTexture* detail_texture1p,
										   LLViewerTexture* detail_texture2p,
										   LLViewerTexture* detail_texture3p,
										   const LLVector4& tp0,
...
void LLDrawPoolTerrain::dirtyTextures(const LLViewerTextureList::dirty_list_t& textures)
{
	LLViewerFetchedTexture* tex =
		LLViewerTextureManager::staticCastToFetchedTexture(mTexturep);
