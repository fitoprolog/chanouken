												 ...
												 const std::string& notify)
{
	...
	mCanTransfer = perms.allowOperationBy(PERM_TRANSFER, gAgentID);
...
void HBFloaterLocalEnv::captureCurrentEnvironment()
{
	...
	if (!gEnvironment.hasEnvironment(LLEnvironment::ENV_LOCAL))
	{
		mLiveSky =
			gEnvironment.getEnvironmentFixedSky(LLEnvironment::ENV_PARCEL,
												true)->buildClone();
		mLiveWater =
			gEnvironment.getEnvironmentFixedWater(LLEnvironment::ENV_PARCEL,
												  true)->buildClone();
	}
	else if (gEnvironment.getEnvironmentDay(LLEnvironment::ENV_LOCAL))
	{
		...
		mLiveWater =
			gEnvironment.getEnvironmentFixedWater(LLEnvironment::ENV_LOCAL)->buildClone();
	}
	else
	{
		// Otherwise we can just use the sky.
		mLiveSky =
			gEnvironment.getEnvironmentFixedSky(LLEnvironment::ENV_LOCAL);
		mLiveWater =
			gEnvironment.getEnvironmentFixedWater(LLEnvironment::ENV_LOCAL);
		update_local = false;
	}
	...
	if (update_local)
	{
		gEnvironment.setEnvironment(LLEnvironment::ENV_LOCAL, mLiveSky,
									FLOATER_LOCAL_ENV_UPDATE);
		gEnvironment.setEnvironment(LLEnvironment::ENV_LOCAL, mLiveWater,
									FLOATER_LOCAL_ENV_UPDATE);
	}
	gEnvironment.setSelectedEnvironment(LLEnvironment::ENV_LOCAL);
...
void HBFloaterLocalEnv::onCommitFixedTime(LLUICtrl*, void* userdata)
{
	...
	if (self)
	{
		F32 value = self->mFixedTimeSlider->getValueF32();
		gEnvironment.setFixedTimeOfDay(value);
	}
}
