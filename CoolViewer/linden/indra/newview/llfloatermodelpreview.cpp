...
constexpr F32 PREVIEW_EDGE_WIDTH = 1.f;
constexpr F32 PREVIEW_PSYH_EDGE_WIDTH = 1.f;
...
void LLFloaterModelUploadBase::requestAgentUploadPermissions()
{
	...
	llinfos << "Requesting upload model permissions from: " << url
			<< llendl;
	LLCoros::getInstance()->launch("LLFloaterModelUploadBase::requestUploadPermCoro",
								   boost::bind(&LLFloaterModelUploadBase::requestUploadPermCoro,
											   this, url,
											   getPermObserverHandle()));
}
...
void LLFloaterModelPreview::onViewOptionChecked(LLUICtrl* ctrl, void* userdata)
{
	...
	if (mp)
	{
		...
		if (new_value)
		{
			// Cannot display both physics and skin weights...
...
void LLModelPreview::genGlodLODs(S32 which_lod, U32 decimation,
								 bool enforce_tri_limit)
{
	...
	// *HACK: needed by libGLOD !
	gNoFixedFunction = false;
	...
	if (object_dirty)
	{
			 ...
			 iter != mBaseModel.end(); ++iter)
		{
			...
			for (U32 i = 0; i < mVertexBuffer[5][mdl].size(); ++i)
			{
				...
				if (num_indices > 2)
				{
					glodInsertElements(mObject[mdl], i, GL_TRIANGLES,
									   num_indices, GL_UNSIGNED_SHORT,
									   (GLvoid*)buff->getIndicesPointer(), 0,
									   0.f);
				}
				stop_gloderror();
			}
	...
	for (S32 lod = start; lod >= end; --lod)
	{
		...
		U32 req_count = (F32)triangle_count / triangle_ratio;
		mRequestedTriangleCount[lod] = llmax(req_count, mMinTriangleLimit);
		...
		for (U32 mdl_idx = 0; mdl_idx < mBaseModel.size(); ++mdl_idx)
		{
			...
			for (GLint i = 0; i < patch_count; ++i)
			{
				...
				else
				{
					...
					memset((U8*)buff->getIndicesPointer(), 0,
						   buff->getIndicesSize());
				}
...
F32 LLModelPreview::genMeshOptimizerPerModel(LLModel* base_model,
											 ...
											 F32 indices_decim,
											 F32 error_threshold, bool sloppy)
{
	...
	U32* output_indices =
		(U32*)allocate_volume_mem_64(size_indices * sizeof(U32));
	U32* combined_indices =
		(U32*)allocate_volume_mem_64(size_indices * sizeof(U32));
	...
	S32 target_indices = llmax(3, llfloor(size_indices / indices_decim));

	S32 type_size = LLVertexBuffer::sTypeSize[LLVertexBuffer::TYPE_VERTEX];
	S32 new_indices = LLMeshOptimizer::simplifyU32(output_indices,
												   combined_indices,
												   size_indices,
												   combined_positions,
	...
	free_volume_mem_64(output_indices);
	free_volume_mem_64(combined_indices);
											...
											F32 err_threshold, bool sloppy)
{
	...
	F32 result_code = 0.f;
	// Leave at least one triangle
	S32 target_indices = llmax(3, llfloor(size_indices / indices_ratio));
	S32 type_size = LLVertexBuffer::sTypeSize[LLVertexBuffer::TYPE_VERTEX];
	S32 new_indices = LLMeshOptimizer::simplify(output, face.mIndices,
												size_indices, face.mPositions,
												face.mNumVertices, type_size,
												target_indices, err_threshold,
												sloppy, &result_code);
	if (result_code < 0)
	{
										  ...
										  U32 decimation, bool with_tri_limit)
{
	...
	if (which_lod > -1 && which_lod < NUM_LOD)
	{
		...
		if (lod_mode == LIMIT_TRIANGLES)
		{
			...
			// convert it to an aproximate ratio.
			indices_decim = (F32)base_triangle_count / tri_limit;
		}
		else
		{
			cname = "lod_error_threshold_" + lod_name[which_lod];
	...
	for (S32 lod = start; lod >= end; --lod)
	{
		...
		mRequestedTriangleCount[lod] = llmax((U32)tri_limit,
											 mMinTriangleLimit);
		mRequestedErrorThreshold[lod] = lod_err_thres;
		...
		for (U32 mdl_idx = 0; mdl_idx < mBaseModel.size(); ++mdl_idx)
		{
				...
				model_meshopt_mode == MESH_OPTIMIZER_AUTO)
			{
				...
				if (ratio < 0.f)
				{
					// U16 vertices overflow; this should not happen, but just
					// in case... Fallback to per-face optimization.
					model_meshopt_mode = MESH_OPTIMIZER_SLOPPY;
					out << " per model method overflow, defaulting to per face.";
				}
				else if (model_meshopt_mode != MESH_OPTIMIZER_AUTO)
				{
			...
			{
				constexpr F32 allowed_ratio_drift = 2.f;
				if (ratio * allowed_ratio_drift < indices_decim)
				{
					...
					if (ratio < 0.f)
					{
						...
						// Fallback to normal method. Note: this already
						// succeeded in the MESH_OPTIMIZER_COMBINE step above,
						// so we do not need to check for ratio. HB
					...
					{
						out << " sloppily simplified using per model method.";
					}
				}
				else
				{
			...
			if (model_meshopt_mode == MESH_OPTIMIZER_SLOPPY)
			{
				...
				out << " sloppily simplified using per face method.";
			}
...
void LLModelPreview::genBuffers(S32 lod, bool include_skin_weights)
{
		 ...
		 iter != model->end(); ++iter)
	{
		...
		for (S32 i = 0, count = mdl->getNumVolumeFaces(); i < count; ++i)
		{
			...
			LLPointer<LLVertexBuffer> vb = new LLVertexBuffer(mask, 0);
			...
			if (skinned)
			{
				if (!vb->getWeight4Strider(weights_strider))
				{
					return;
				}
			}
...
bool LLModelPreview::render()
{
	...
	if (!mModel[mPreviewLOD].empty())
	{
		...
		{
			llwarns << "Vertex Buffer[" << mPreviewLOD << "]"
		...
		if (!skin_weight)
		{
				 ...
				 iter != end; ++iter)
			{
					 ...
					 i < count; ++i)
				{
					...
					if (edges)
					{
						gGL.lineWidth(PREVIEW_EDGE_WIDTH);
						glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
						buffer->drawRange(LLRender::TRIANGLES, 0,
										  buffer->getNumVerts() - 1,
										  buffer->getNumIndices(), 0);
						glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
#if 0					// Useless since PREVIEW_EDGE_WIDTH == 1.f
						gGL.lineWidth(1.f);
#endif
					}
				}
						...
						if (render_mesh)
						{
							...
							if (pass > 0)
							{
									 ...
									 i < count; ++i)
								{
									...
									gGL.lineWidth(PREVIEW_PSYH_EDGE_WIDTH);
									glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
									buffer->drawRange(LLRender::TRIANGLES, 0,
													  buffer->getNumVerts() - 1,
													  buffer->getNumIndices(), 0);
									...
									glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
#if 0								// Useless since PREVIEW_PSYH_EDGE_WIDTH == 1.f
									gGL.lineWidth(1.f);
#endif
								}
							}
							 ...
							 iter != end; ++iter)
						{
							...
							if (physics.mHull.empty())
							{
									 ...
									 i < count; ++i)
								{
									...
									LLStrider<U16> idx;
									if (!buffer->getIndexStrider(idx, 0))
									{
										decomp->mMutex.unlock();
										gObjectPreviewProgram.bind();
										gGL.popMatrix();
				 ...
				 iter != end; ++iter)
			{
					 ...
					 model_iter != model_end; ++model_iter)
				{
						 ...
						 i < count; ++i)
					{
						...
						if (!buffer->getWeight4Strider(weight))
						{
							gObjectPreviewProgram.bind();
							gGL.popMatrix();
							return false;
...
{
	LLVector3 root_pos = mPreviewAvatar->mRoot->getPosition();
	const LLVector4a* ext = mPreviewAvatar->mDrawable->getSpatialExtents();
	const LLVector4a& min = ext[0];
	const LLVector4a& max = ext[1];
	F32 center = (max[2] - min[2]) * 0.5f;
	F32 ground = root_pos[2] - center - z_offset;
...
void LLModelPreview::textureLoadedCallback(bool success,
										   ...
										   void* userdata)
{
	// *BUG: we should check that the preview still exists !!!
	LLModelPreview* preview = (LLModelPreview*)userdata;
	if (!preview) return;

	preview->refresh();

	if (is_final && preview->mModelLoader)
	{
