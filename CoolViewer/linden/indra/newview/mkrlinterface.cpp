...
#if 0	// Would be used by disabled code
	...
	mTplocalMax(EXTREMUM)
{
	...
	mAllowedGetDebug.emplace_back("WindLightUseAtmosShaders");
...
void RLInterface::usePerAccountSettings()
{
	if (gRLenabled)
	{
		sRecvimMessage = gSavedPerAccountSettings.getString("RestrainedLoveRecvimMessage");
		sSendimMessage = gSavedPerAccountSettings.getString("RestrainedLoveSendimMessage");
	}
}
								...
								const std::string& option, bool force)
{
	...
	// Lua command line relayed from an object), even black-listed ones.
	...
	if (parseCommand(command, behav, option, param))
	{
		...
		else if (behav == "findfolders")
		{
			...
			if (opt_count)
			{
				...
				std::string separator = opt_count > 1 ? options.at(1) : ",";
								...
								const std::string& cmd_line)
{
	...
	if (cmd_line.find(',') != std::string::npos)
	{
		...
		{
			const std::string& command = list_of_commands.at(i);
						...
						std::string option)
{
	...
	else if (command == "attachthis" || command == "addoutfitthis")
	{
		...
		if (!pathes_str.empty())
		{
			...
			for (U32 i = 0; i < pathes.size(); ++i)
			{
				...
				forceAttach(pathes.at(i), false, AttachOverOrReplace);
			}
		}
	}
	else if (command == "attachthisover" || command == "addoutfitthisover")
	{
		...
		if (!pathes_str.empty())
		{
			...
			for (U32 i = 0; i < pathes.size(); ++i)
			{
				forceAttach(pathes.at(i), false, AttachOver);
			}
		}
	}
	...
	else if (command == "attachallthis" || command == "addoutfitallthis")
	{
		...
		if (!pathes_str.empty())
		{
			...
			for (U32 i = 0; i < pathes.size(); ++i)
			{
				// Will have to be changed back to AttachReplace eventually,
				// but not before a clear and early communication
				forceAttach(pathes.at(i), true, AttachOverOrReplace);
			}
		}
	}
			 ...
			 command == "addoutfitallthisover")
	{
		...
		if (!pathes_str.empty())
		{
			...
			for (U32 i = 0; i < pathes.size(); ++i)
			{
				forceAttach(pathes.at(i), true, AttachOver);
			}
		}
	}
...
bool RLInterface::hasLockedHuds()
{
	...
	for (S32 i = 0, count = gAgentAvatarp->mAttachedObjectsVector.size();
		 i < count; ++i)
	{
		LLViewerObject* object = gAgentAvatarp->mAttachedObjectsVector[i].first;
		if (object && object->isHUDAttachment() && !canDetach(object))
		{
			return true;
		}
	}
		...
		if (cats && !no_mod)
		{
			for (S32 i = 0, count = cats->size(); i < count; ++i)
			{
				LLViewerInventoryCategory* childp =
					(LLViewerInventoryCategory*)cats->at(i);
				if (childp)
				{
														   ...
														   LLInventoryCategory* root)
{
	...
	// it
	LLInventoryCategory* cat = cats->at(max_size_index);
...
bool RLInterface::shouldMoveToSharedSubFolder(LLViewerInventoryCategory* catp)
{
	...
	// to this method.
	return catp->getName().find(RL_RLV_REDIR_FOLDER_PREFIX) == 0;
}
...
void RLInterface::moveToSharedSubFolder(LLViewerInventoryCategory* catp)
{
	...
	for (S32 i = 0; i < sub_folders - 1; ++i)
	{
		...
		{
			LLInventoryCategory* old_catp = cats->at(j);
		...
		{
			LLUUID id = gInventory.createNewCategory(target_catp->getUUID(),
...
void RLInterface::addAttachmentPointName(LLViewerObject* vobj)
{
	...
	{
		LLUUID cat_id = gInventory.createNewCategory(parent_id,
													 LLFolderType::FT_NONE,
													 new_name);
		...
		for (S32 i = 0, count = items->size(); i < count; ++i)
		{
			...
			else if (item->getType() == LLAssetType::AT_SETTINGS)
			{
				if (!mContainsSetenv && !sRLNoSetEnv &&
					(gUseNewShaders || LLEnvironment::overrideWindlight()))
				{
		...
		if (!is_rlv_root && attachpt)
		{
			...
			if (subcatitems && subcatitems->size() == 1)
			{
				LLViewerInventoryItem* subcatitem =
					(LLViewerInventoryItem*)subcatitems->at(0);
				if (subcatitem &&
									  ...
									  EChatAudible audible)
{
	...
	unsigned char hash = name.at(0) + name.at(len - 1) + len +
						 (mLaunchTimestamp >> 16);
...
void updateAndSave(WLColorControl* color)
{
	...
	color->update(gWLSkyParamMgr.mCurParams);
}
...
void updateAndSave(WLFloatControl* floatControl)
{
	if (floatControl)
	{
		floatControl->update(gWLSkyParamMgr.mCurParams);
	}
}
...
bool RLInterface::forceEnvironment(std::string command, std::string option)
{
	...
	bool override_wl = gUseNewShaders || LLEnvironment::overrideWindlight();
	if (override_wl)
	{
	...
	if (command == "daytime")
	{
		...
		if (val >= 0.f)
		{
			...
			{
				gWLSkyParamMgr.mAnimator.setDayTime(val);
	...
	else if (command == "bluehorizonr")
	{
		...
		{
			gWLSkyParamMgr.mBlueHorizon.r = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mBlueHorizon);
		}
	}
	else if (command == "bluehorizong")
	{
		...
		{
			gWLSkyParamMgr.mBlueHorizon.g = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mBlueHorizon);
		}
	}
	else if (command == "bluehorizonb")
	{
		...
		{
			gWLSkyParamMgr.mBlueHorizon.b = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mBlueHorizon);
		}
	}
	else if (command == "bluehorizoni")
	{
		...
		{
			F32 old_intensity = llmax(gWLSkyParamMgr.mBlueHorizon.r,
									  gWLSkyParamMgr.mBlueHorizon.g,
									  gWLSkyParamMgr.mBlueHorizon.b);
			if (val == 0.f || old_intensity == 0.f)
			{
				gWLSkyParamMgr.mBlueHorizon.r = gWLSkyParamMgr.mBlueHorizon.g =
					gWLSkyParamMgr.mBlueHorizon.b = val * 2.f;
			}
			else
			{
				F32 factor = val * 2.f / old_intensity;
				gWLSkyParamMgr.mBlueHorizon.r *= factor;
				gWLSkyParamMgr.mBlueHorizon.g *= factor;
				gWLSkyParamMgr.mBlueHorizon.b *= factor;
			}
			updateAndSave(&gWLSkyParamMgr.mBlueHorizon);
		}
	}
	else if (command == "bluehorizon")
	{
		...
		{
			gWLSkyParamMgr.mBlueHorizon.r = r;
			gWLSkyParamMgr.mBlueHorizon.g = g;
			gWLSkyParamMgr.mBlueHorizon.b = b;
			updateAndSave(&gWLSkyParamMgr.mBlueHorizon);
		}
	}
	...
	else if (command == "bluedensityr")
	{
		...
		{
			gWLSkyParamMgr.mBlueDensity.r = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mBlueDensity);
		}
	}
	else if (command == "bluedensityg")
	{
		...
		{
			gWLSkyParamMgr.mBlueDensity.g = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mBlueDensity);
		}
	}
	else if (command == "bluedensityb")
	{
		...
		{
			gWLSkyParamMgr.mBlueDensity.b = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mBlueDensity);
		}
	}
	else if (command == "bluedensityi")
	{
		...
		{
			F32 old_intensity = llmax(gWLSkyParamMgr.mBlueDensity.r,
									  gWLSkyParamMgr.mBlueDensity.g,
									  gWLSkyParamMgr.mBlueDensity.b);
			if (val == 0.f || old_intensity == 0.f)
			{
				gWLSkyParamMgr.mBlueDensity.r = gWLSkyParamMgr.mBlueDensity.g =
					gWLSkyParamMgr.mBlueDensity.b = val * 2.f;
			}
			else
			{
				F32 factor = val * 2.f / old_intensity;
				gWLSkyParamMgr.mBlueDensity.r *= factor;
				gWLSkyParamMgr.mBlueDensity.g *= factor;
				gWLSkyParamMgr.mBlueDensity.b *= factor;
			}
			updateAndSave(&gWLSkyParamMgr.mBlueDensity);
		}
	}
	else if (command == "bluedensity")
	{
		...
		{
			gWLSkyParamMgr.mBlueDensity.r = r;
			gWLSkyParamMgr.mBlueDensity.g = g;
			gWLSkyParamMgr.mBlueDensity.b = b;
			updateAndSave(&gWLSkyParamMgr.mBlueDensity);
		}
	}
	...
	else if (command == "hazehorizon")
	{
		...
		{
			gWLSkyParamMgr.mHazeHorizon.r = val * 2.f;
			gWLSkyParamMgr.mHazeHorizon.g = val * 2.f;
			gWLSkyParamMgr.mHazeHorizon.b = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mHazeHorizon);
		}
	}
	else if (command == "hazedensity")
	{
		...
		{
			gWLSkyParamMgr.mHazeDensity.r = val * 2.f;
			gWLSkyParamMgr.mHazeDensity.g = val * 2.f;
			gWLSkyParamMgr.mHazeDensity.b = val * 2.f;
			updateAndSave(&gWLSkyParamMgr.mHazeDensity);
		}
	}
	...
	else if (command == "densitymultiplier")
	{
		...
		{
			gWLSkyParamMgr.mDensityMult.x = val * 0.001f;
			updateAndSave(&gWLSkyParamMgr.mDensityMult);
#if 0
			gWLWaterParamMgr.mFogDensity.mExp = 5.f;
			gWLWaterParamMgr.mFogDensity.update(gWLWaterParamMgr.mCurParams);
#endif
		}
	}
	...
	else if (command == "distancemultiplier")
	{
		...
		{
			gWLSkyParamMgr.mDistanceMult.x = val;
			updateAndSave(&gWLSkyParamMgr.mDistanceMult);
#if 0
			gWLWaterParamMgr.mUnderWaterFogMod.mX = 1.f;
			gWLWaterParamMgr.mUnderWaterFogMod.update(gWLWaterParamMgr.mCurParams);
#endif
		}
	}
	...
	else if (command == "maxaltitude")
	{
		...
		{
			gWLSkyParamMgr.mMaxAlt.x = val;
			updateAndSave(&gWLSkyParamMgr.mMaxAlt);
		}
	}
	...
	else if (command == "sunmooncolorr" || command == "sunlightcolorr")
	{
		...
		{
			gWLSkyParamMgr.mSunlight.r = val * 3.f;
			updateAndSave(&gWLSkyParamMgr.mSunlight);
		}
	}
	else if (command == "sunmooncolorg" || command == "sunlightcolorg")
	{
		...
		{
			gWLSkyParamMgr.mSunlight.g = val * 3.f;
			updateAndSave(&gWLSkyParamMgr.mSunlight);
		}
	}
	else if (command == "sunmooncolorb" || command == "sunlightcolorb")
	{
		...
		{
			gWLSkyParamMgr.mSunlight.b = val * 3.f;
			updateAndSave(&gWLSkyParamMgr.mSunlight);
		}
	}
	else if (command == "sunmooncolori" || command == "sunlightcolori")
	{
		if (override_wl)
		{
			...
			if (val == 0.f || old_intensity == 0.f)
			{
				suncolour.mV[0] = suncolour.mV[1] = suncolour.mV[2] =
								  val * 3.f;
			}
			else
			{
		...
		{
			F32 old_intensity = llmax(gWLSkyParamMgr.mSunlight.r,
									  gWLSkyParamMgr.mSunlight.g,
									  gWLSkyParamMgr.mSunlight.b);
			if (val == 0.f || old_intensity == 0.f)
			{
				gWLSkyParamMgr.mSunlight.r = gWLSkyParamMgr.mSunlight.g =
											 gWLSkyParamMgr.mSunlight.b =
											 val * 3.f;
			}
			else
			{
				F32 factor = val * 3.f / old_intensity;
				gWLSkyParamMgr.mSunlight.r *= factor;
				gWLSkyParamMgr.mSunlight.g *= factor;
				gWLSkyParamMgr.mSunlight.b *= factor;
			}
			updateAndSave(&gWLSkyParamMgr.mSunlight);
		}
	}
	else if (command == "sunmooncolor" || command == "sunlightcolor")
	{
		...
		{
			gWLSkyParamMgr.mSunlight.r = r;
			gWLSkyParamMgr.mSunlight.g = g;
			gWLSkyParamMgr.mSunlight.b = b;
			updateAndSave(&gWLSkyParamMgr.mSunlight);
		}
	}
	...
	else if (command == "ambientr")
	{
		...
		{
			gWLSkyParamMgr.mAmbient.r = val * 3.f;
			updateAndSave(&gWLSkyParamMgr.mAmbient);
		}
	}
	else if (command == "ambientg")
	{
		...
		{
			gWLSkyParamMgr.mAmbient.g = val * 3.f;
			updateAndSave(&gWLSkyParamMgr.mAmbient);
		}
	}
	else if (command == "ambientb")
	{
		...
		{
			gWLSkyParamMgr.mAmbient.b = val * 3.f;
			updateAndSave(&gWLSkyParamMgr.mAmbient);
		}
	}
	else if (command == "ambienti")
	{
		...
		{
			F32 old_intensity = llmax(gWLSkyParamMgr.mAmbient.r,
									  gWLSkyParamMgr.mAmbient.g,
									  gWLSkyParamMgr.mAmbient.b);
			if (val == 0.f || old_intensity == 0.f)
			{
				gWLSkyParamMgr.mAmbient.r = gWLSkyParamMgr.mAmbient.g =
											gWLSkyParamMgr.mAmbient.b =
											val * 3.f;
			}
			else
			{
				F32 factor = val * 3.f / old_intensity;
				gWLSkyParamMgr.mAmbient.r *= factor;
				gWLSkyParamMgr.mAmbient.g *= factor;
				gWLSkyParamMgr.mAmbient.b *= factor;
			}
			updateAndSave(&gWLSkyParamMgr.mAmbient);
		}
	}
	else if (command == "ambient")
	{
		...
		{
			gWLSkyParamMgr.mAmbient.r = r;
			gWLSkyParamMgr.mAmbient.g = g;
			gWLSkyParamMgr.mAmbient.b = b;
			updateAndSave(&gWLSkyParamMgr.mAmbient);
		}
	}
	...
	else if (command == "sunglowfocus")
	{
		...
		{
			gWLSkyParamMgr.mGlow.b = -val * 5.f;
			updateAndSave(&gWLSkyParamMgr.mGlow);
		}
	}
	else if (command == "sunglowsize")
	{
		...
		{
			gWLSkyParamMgr.mGlow.r = (2.f - val) * 20.f;
			updateAndSave(&gWLSkyParamMgr.mGlow);
		}
	}
	...
	else if (command == "scenegamma")
	{
		...
		{
			gWLSkyParamMgr.mWLGamma.x = val;
			updateAndSave(&gWLSkyParamMgr.mWLGamma);
		}
	}

	// No Windlight equivalents for the following settings
	...
	{
		// *TODO: implement the EE approximation
		if (!override_wl)
		{
			gWLSkyParamMgr.mCurParams.setSunAngle(F_TWO_PI * val);
		}
	}
	...
	else if (command == "eastangle")
	{
		...
		if (!override_wl)
		{
			gWLSkyParamMgr.mCurParams.setEastAngle(F_TWO_PI * val);
		}
	}
	...
	else if (command == "starbrightness")
	{
		...
		{
			gWLSkyParamMgr.mCurParams.setStarBrightness(val);
		}
	}
	...
	else if (command == "cloudcolorr")
	{
		...
		{
			gWLSkyParamMgr.mCloudColor.r = val;
			updateAndSave(&gWLSkyParamMgr.mCloudColor);
		}
	}
	else if (command == "cloudcolorg")
	{
		...
		{
			gWLSkyParamMgr.mCloudColor.g = val;
			updateAndSave(&gWLSkyParamMgr.mCloudColor);
		}
	}
	else if (command == "cloudcolorb")
	{
		...
		{
			gWLSkyParamMgr.mCloudColor.b = val;
			updateAndSave(&gWLSkyParamMgr.mCloudColor);
		}
	}
	else if (command == "cloudcolori")
	{
		...
		{
			F32 old_intensity = llmax(gWLSkyParamMgr.mCloudColor.r,
									  gWLSkyParamMgr.mCloudColor.g,
									  gWLSkyParamMgr.mCloudColor.b);
			if (val == 0.f || old_intensity == 0.f)
			{
				gWLSkyParamMgr.mCloudColor.r = gWLSkyParamMgr.mCloudColor.g =
											   gWLSkyParamMgr.mCloudColor.b =
											   val;
			}
			else
			{
				gWLSkyParamMgr.mCloudColor.r *= val / old_intensity;
				gWLSkyParamMgr.mCloudColor.g *= val / old_intensity;
				gWLSkyParamMgr.mCloudColor.b *= val / old_intensity;
			}
			updateAndSave(&gWLSkyParamMgr.mCloudColor);
		}
	}
	else if (command == "cloudcolor")
	{
		...
		{
			gWLSkyParamMgr.mCloudColor.r = r;
			gWLSkyParamMgr.mCloudColor.g = g;
			gWLSkyParamMgr.mCloudColor.b = b;
			updateAndSave(&gWLSkyParamMgr.mCloudColor);
		}
	}
	...
	else if (command == "cloudx" || command == "clouddensityx")
	{
		...
		{
			gWLSkyParamMgr.mCloudMain.r = val;
			updateAndSave(&gWLSkyParamMgr.mCloudMain);
		}
	}
	else if (command == "cloudy" || command == "clouddensityy")
	{
		...
		{
			gWLSkyParamMgr.mCloudMain.g = val;
			updateAndSave(&gWLSkyParamMgr.mCloudMain);
		}
	}
	else if (command == "cloudd" || command == "clouddensityd")
	{
		...
		{
			gWLSkyParamMgr.mCloudMain.b = val;
			updateAndSave(&gWLSkyParamMgr.mCloudMain);
		}
	}
	else if (command == "cloud" || command == "clouddensity")
	{
		...
		{
			gWLSkyParamMgr.mCloudMain.r = r;
			gWLSkyParamMgr.mCloudMain.g = g;
			gWLSkyParamMgr.mCloudMain.b = b;
			updateAndSave(&gWLSkyParamMgr.mCloudMain);
		}
	}
	...
	else if (command == "clouddetailx")
	{
		...
		{
			gWLSkyParamMgr.mCloudDetail.r = val;
			updateAndSave(&gWLSkyParamMgr.mCloudDetail);
		}
	}
	else if (command == "clouddetaily")
	{
		...
		{
			gWLSkyParamMgr.mCloudDetail.g = val;
			updateAndSave(&gWLSkyParamMgr.mCloudDetail);
		}
	}
	else if (command == "clouddetaild")
	{
		...
		{
			gWLSkyParamMgr.mCloudDetail.b = val;
			updateAndSave(&gWLSkyParamMgr.mCloudDetail);
		}
	}
	else if (command == "clouddetail")
	{
		...
		{
			gWLSkyParamMgr.mCloudDetail.r = r;
			gWLSkyParamMgr.mCloudDetail.g = g;
			gWLSkyParamMgr.mCloudDetail.b = b;
			updateAndSave(&gWLSkyParamMgr.mCloudDetail);
		}
	}
	...
	else if (command == "cloudcoverage")
	{
		...
		{
			gWLSkyParamMgr.mCloudCoverage.x = val;
			updateAndSave(&gWLSkyParamMgr.mCloudCoverage);
		}
	}
	...
	else if (command == "cloudscale")
	{
		...
		{
			gWLSkyParamMgr.mCloudScale.x = val;
			updateAndSave(&gWLSkyParamMgr.mCloudScale);
		}
	}

	// No Windlight equivalent for this one
	...
	else if (command == "cloudscrollx")
	{
		...
		{
			gWLSkyParamMgr.mCurParams.setCloudScrollX(val + 10.f);
		}
	}
	else if (command == "cloudscrolly")
	{
		...
		{
			gWLSkyParamMgr.mCurParams.setCloudScrollY(val + 10.f);
		}
	}
	else if (command == "cloudscroll")
	{
		...
		{
			gWLSkyParamMgr.mCurParams.setCloudScrollX(x + 10.f);
			gWLSkyParamMgr.mCurParams.setCloudScrollY(y + 10.f);
			updateAndSave(&gWLSkyParamMgr.mCloudDetail);
		}
	}
	...
	// These could possibly be made Windlight-compatible by implementing
	// the corresponding image changes support in the Windlight renderer
	...
	if (!override_wl)
	{
		...
		// Update the Windlight floater, when it exists
		LLFloaterWindlight* fwlp = LLFloaterWindlight::findInstance();
		if (fwlp)
		{
			fwlp->refresh();
		}
...
std::string RLInterface::getEnvironment(std::string command)
{
	...
	if (command == "daytime")
	{
		...
		else if (gWLSkyParamMgr.mAnimator.mIsRunning && estate)
		{
			res = -1.f;
		}
		else
		{
			res = gWLSkyParamMgr.mAnimator.getDayTime();
		}
	}
	...
	else if (command == "bluehorizonr")
	{
		...
		{
			res = gWLSkyParamMgr.mBlueHorizon.r * 0.5f;
		}
	}
	else if (command == "bluehorizong")
	{
		...
		{
			res = gWLSkyParamMgr.mBlueHorizon.g * 0.5f;
		}
	}
	else if (command == "bluehorizonb")
	{
		...
		{
			res = gWLSkyParamMgr.mBlueHorizon.b * 0.5f;
		}
	}
	else if (command == "bluehorizoni")
	{
		...
		{
			LLColor3 bluehorizon = skyp->getBlueHorizon();
			res = llmax(bluehorizon.mV[0], bluehorizon.mV[1],
						bluehorizon.mV[2]) * 0.5f;
		}
		else
		{
			res = llmax(gWLSkyParamMgr.mBlueHorizon.r,
						gWLSkyParamMgr.mBlueHorizon.g,
						gWLSkyParamMgr.mBlueHorizon.b) * 0.5f;
		}
	}
	else if (command == "bluehorizon")
	{
		...
		{
			str << gWLSkyParamMgr.mBlueHorizon.r * 0.5f << ";";
			str << gWLSkyParamMgr.mBlueHorizon.g * 0.5f << ";";
			str << gWLSkyParamMgr.mBlueHorizon.b * 0.5f;
		}
		return str.str();
	}
	...
	else if (command == "bluedensityr")
	{
		...
		{
			res = gWLSkyParamMgr.mBlueDensity.r * 0.5f;
		}
	}
	else if (command == "bluedensityg")
	{
		...
		{
			res = gWLSkyParamMgr.mBlueDensity.g * 0.5f;
		}
	}
	else if (command == "bluedensityb")
	{
		...
		{
			res = gWLSkyParamMgr.mBlueDensity.b * 0.5f;
		}
	}
	else if (command == "bluedensityi")
	{
		...
		{
			LLColor3 bluedensity = skyp->getBlueDensity();
			res = llmax(bluedensity.mV[0], bluedensity.mV[1],
						bluedensity.mV[2]) * 0.5f;
		}
		else
		{
			res = llmax(gWLSkyParamMgr.mBlueDensity.r,
						gWLSkyParamMgr.mBlueDensity.g,
						gWLSkyParamMgr.mBlueDensity.b) * 0.5f;
		}
	}
	else if (command == "bluedensity")
	{
		...
		{
			str << gWLSkyParamMgr.mBlueDensity.r * 0.5f << ";";
			str << gWLSkyParamMgr.mBlueDensity.g * 0.5f << ";";
			str << gWLSkyParamMgr.mBlueDensity.b * 0.5f;
		}
		return str.str();
	}
	...
	else if (command == "hazehorizon")
	{
		...
		{
			res = llmax(gWLSkyParamMgr.mHazeHorizon.r,
						gWLSkyParamMgr.mHazeHorizon.g,
						gWLSkyParamMgr.mHazeHorizon.b) * 0.5f;
		}
	}
	else if (command == "hazedensity")
	{
		...
		{
			res = llmax(gWLSkyParamMgr.mHazeDensity.r,
						gWLSkyParamMgr.mHazeDensity.g,
						gWLSkyParamMgr.mHazeDensity.b) * 0.5f;
		}
	}
	...
	else if (command == "densitymultiplier")
	{
		...
		{
			res = gWLSkyParamMgr.mDensityMult.x * 1000.f;
		}
	}
	else if (command == "distancemultiplier")
	{
		...
		{
			res = gWLSkyParamMgr.mDistanceMult.x;
		}
	}
	else if (command == "maxaltitude")
	{
	...
	else if (command == "sunmooncolorr")
	{
		...
		{
			res = gWLSkyParamMgr.mSunlight.r / 3.f;
		}
	}
	else if (command == "sunmooncolorg")
	{
		...
		{
			res = gWLSkyParamMgr.mSunlight.g / 3.f;
		}
	}
	else if (command == "sunmooncolorb")
	{
		...
		{
			res = gWLSkyParamMgr.mSunlight.b / 3.f;
		}
	}
	else if (command == "sunmooncolori")
	{
		...
		{
			res = llmax(gWLSkyParamMgr.mSunlight.r,
						gWLSkyParamMgr.mSunlight.g,
						gWLSkyParamMgr.mSunlight.b) / 3.f;
		}
	}
	else if (command == "sunmooncolor")
	{
		...
		{
			str << gWLSkyParamMgr.mSunlight.r / 3.f << ";";
			str << gWLSkyParamMgr.mSunlight.g / 3.f << ";";
			str << gWLSkyParamMgr.mSunlight.b / 3.f;
		}
		return str.str();
	}
	...
	else if (command == "ambientr")
	{
		...
		{
			res = gWLSkyParamMgr.mAmbient.r / 3.f;
		}
	}
	else if (command == "ambientg")
	{
		...
		{
			res = gWLSkyParamMgr.mAmbient.g / 3.f;
		}
	}
	else if (command == "ambientb")
	{
		...
		{
			res = gWLSkyParamMgr.mAmbient.b / 3.f;
		}
	}
	else if (command == "ambienti")
	{
		...
		{
			res = llmax(gWLSkyParamMgr.mAmbient.r, gWLSkyParamMgr.mAmbient.g,
						gWLSkyParamMgr.mAmbient.b) / 3.f;
		}
	}
	else if (command == "ambient")
	{
		...
		{
			str << gWLSkyParamMgr.mAmbient.r / 3.f << ";";
			str << gWLSkyParamMgr.mAmbient.g / 3.f << ";";
			str << gWLSkyParamMgr.mAmbient.b / 3.f;
		}
		return str.str();
	}
	...
	else if (command == "sunglowfocus")
	{
		...
		{
			res = -gWLSkyParamMgr.mGlow.b / 5.f;
		}
	}
	else if (command == "sunglowsize")
	{
		...
		{
			res = 2.f - gWLSkyParamMgr.mGlow.r / 20.f;
		}
	}
	else if (command == "scenegamma")
	{
	...
	else if (command == "sunmoonposition")
	{
		...
		if (!override_wl)
		{
			res = gWLSkyParamMgr.mCurParams.getSunAngle() / F_TWO_PI;
		}
	}
	...
	else if (command == "eastangle")
	{
		...
		if (!override_wl)
		{
			res = gWLSkyParamMgr.mCurParams.getEastAngle() / F_TWO_PI;
		}
	}
	...
	else if (command == "starbrightness")
	{
		...
		{
			res = gWLSkyParamMgr.mCurParams.getStarBrightness();
		}
	}
	...
	else if (command == "cloudcolorr")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudColor.r;
		}
	}
	else if (command == "cloudcolorg")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudColor.g;
		}
	}
	else if (command == "cloudcolorb")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudColor.b;
		}
	}
	else if (command == "cloudcolori")
	{
		...
		{
			res = llmax(gWLSkyParamMgr.mCloudColor.r,
						gWLSkyParamMgr.mCloudColor.g,
						gWLSkyParamMgr.mCloudColor.b);
		}
	}
	else if (command == "cloudcolor")
	{
		...
		{
			str << gWLSkyParamMgr.mCloudColor.r << ";";
			str << gWLSkyParamMgr.mCloudColor.g << ";";
			str << gWLSkyParamMgr.mCloudColor.b;
		}
		return str.str();
	}
	...
	else if (command == "cloudx")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudMain.r;
		}
	}
	else if (command == "cloudy")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudMain.g;
		}
	}
	else if (command == "cloudd")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudMain.b;
		}
	}
	else if (command == "cloud")
	{
		...
		{
			str << gWLSkyParamMgr.mCloudMain.r << ";";
			str << gWLSkyParamMgr.mCloudMain.g << ";";
			str << gWLSkyParamMgr.mCloudMain.b;
		}
		return str.str();
	}
	...
	else if (command == "clouddetailx")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudDetail.r;
		}
	}
	else if (command == "clouddetaily")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudDetail.g;
		}
	}
	else if (command == "clouddetaild")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudDetail.b;
		}
	}
	else if (command == "clouddetail")
	{
		...
		{
			str << gWLSkyParamMgr.mCloudDetail.r << ";";
			str << gWLSkyParamMgr.mCloudDetail.g << ";";
			str << gWLSkyParamMgr.mCloudDetail.b;
		}
		return str.str();
	}
	...
	else if (command == "cloudcoverage")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudCoverage.x;
		}
	}
	else if (command == "cloudscale")
	{
		...
		{
			res = gWLSkyParamMgr.mCloudScale.x;
		}
	}

	// No Windlight equivalents for the following setting
	else if (command == "cloudvariance")
	{
		if (override_wl)
		{
	...
	else if (command == "cloudscrollx")
	{
		...
		{
			res = gWLSkyParamMgr.mCurParams.getCloudScrollX() - 10.f;
		}
	}
	else if (command == "cloudscrolly")
	{
		...
		{
			res = gWLSkyParamMgr.mCurParams.getCloudScrollY() - 10.f;
		}
	}
	else if (command == "cloudscroll")
	{
		...
		{
			str << gWLSkyParamMgr.mCurParams.getCloudScrollX() - 10.f << ";";
			str << gWLSkyParamMgr.mCurParams.getCloudScrollY() - 10.f;
		}
		return str.str();
	}
	...
	else if (command == "sunimage" || command == "suntexture")
	{
		...
		if (voskyp)
		{
			LLViewerTexture* imagep = voskyp->getSunTex();
			if (imagep)
			{
				return imagep->getID().asString();
			}
		}
		return "";
	}
	else if (command == "moonimage" || command == "moontexture")
	{
		...
		if (voskyp)
		{
			LLViewerTexture* imagep = voskyp->getMoonTex();
			if (imagep)
			{
				return imagep->getID().asString();
			}
	...
	else if (command == "cloudimage" || command == "cloudtexture")
	{
		...
		if (voskyp)
		{
			LLViewerTexture* imagep = voskyp->getCloudNoiseTex();
			if (imagep)
			{
				return imagep->getID().asString();
			}
...
bool RLInterface::forceDebugSetting(std::string command, std::string option)
{
	...
	// Enable Windlight shaders
	if (!sRLNoSetEnv && command == "renderresolutiondivisor")
	{
		gSavedSettings.setBool("WindLightUseAtmosShaders", true);
	}
...
bool RLInterface::canDetach(LLViewerObject* attached_object)
{
		 ...
		 it != end; ++it)
	{
		if (it->second == "detach")
		{
			...
			this_prim = gObjectList.findObject(LLUUID(it->first));
...
void RLInterface::drawRenderLimit(bool force_opaque)
{
	...
	else
	{
		...
		// blend mode
		if (force_opaque || mCamDistDrawAlphaMax >= UPPER_ALPHA_LIMIT ||
			LLDrawPoolAlpha::sShowDebugAlpha ||
			(gSelectMgr.mRenderHighlightSelections &&
			 (!gSelectMgr.getSelection()->isEmpty() ||
			  LLToolDragAndDrop::getInstance()->getCargoCount())))
		{
