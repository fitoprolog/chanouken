...
void LLViewerRegion::loadObjectCache()
{
	if (!mCacheLoaded)
	{
		LL_DEBUGS("ObjectCache") << "Loading object cache for region handle: "
								 << mHandle << LL_ENDL;
		...
		{
			LLVOCache::getInstance()->readFromCache(mHandle, mCacheID,
													mCacheMap);
			if (mCacheMap.empty())
			{
				mCacheDirty = true;
...
void LLViewerRegion::saveObjectCache()
{
	if (!mCacheLoaded)
	{
		LL_DEBUGS("ObjectCache") << "Cache map not loaded for region handle: "
								 << mHandle << ". Skiping." << LL_ENDL;
		return;
	}
	if (mCacheMap.empty())
	{
		LL_DEBUGS("ObjectCache") << "Cache map empty for region handle: "
								 << mHandle << ". Skiping." << LL_ENDL;
		return;
	}
	...
	if (LLVOCache::instanceExists())
	{
		LL_DEBUGS("ObjectCache") << "Saving object cache for region handle: "
								 << mHandle << LL_ENDL;

		constexpr F32 THRESHOLD = 600.f;	// Seconds
		bool removal_enabled = sVOCacheCullingEnabled &&
							   (LLApp::isExiting() ||
								mRegionTimer.getElapsedTimeF32() > THRESHOLD);
		LLVOCache::getInstance()->writeToCache(mHandle, mName, mCacheID,
											   mCacheMap, mCacheDirty,
											   removal_enabled);
	}

	LL_DEBUGS("ObjectCache") << "Clearing cache map for region handle: "
							 << mHandle << LL_ENDL;
	mCacheDirty = false;
	mCacheMap.clear();
}
...
//static
std::string LLViewerRegion::getMaturityIconName(U8 sim_access)
{
	if (sim_access <= SIM_ACCESS_PG)
	{
		return "access_pg.tga";
	}
	else if (sim_access <= SIM_ACCESS_MATURE)
	{
		return "access_mature.tga";
	}
	return "access_adult.tga";
}
...
void LLViewerRegion::processRegionInfo(LLMessageSystem* msg, void**)
{
	...
	{
		msg->getString(_PREHASH_RegionInfo2, "ProductName",
	...
	{
		msg->getU64("RegionInfo3", "RegionFlagsExtended",
					LLRegionInfoModel::sRegionFlags);
	}
	else
	{
		U32 flags = 0;
		msg->getU32(_PREHASH_RegionInfo, "RegionFlags", flags);
...
void LLViewerRegion::updateVisibleEntries(F32 max_time)
{
	...
	for (LLVOCacheEntry::vocache_entry_set_t::iterator
			iter = mVisibleEntries.begin(),
			end = mVisibleEntries.end(); iter != end; )
	{
		LLVOCacheEntry::vocache_entry_set_t::iterator curiter = iter++;
		LLVOCacheEntry* vo_entry = *curiter;
		if (vo_entry && vo_entry->isValid() &&
			vo_entry->getState() < LLVOCacheEntry::WAITING)
		{
			// Set a large number to force to load this object.
			vo_entry->setSceneContribution(LARGE_SCENE_CONTRIBUTION);

			mWaitingList.insert(vo_entry);
		}
		else
		{
	...
	for (U32 i = 0, count = mVisibleGroups.size(); i < count; ++i)
	{
		...
		for (LLViewerOctreeGroup::element_iter i = group->getDataBegin(),
											   end2 = group->getDataEnd();
			 i != end2; ++i)
		{
			if (*i && (*i)->hasVOCacheEntry())
			{
				LLVOCacheEntry* vo_entry =
					(LLVOCacheEntry*)(*i)->getVOCacheEntry();
				if (vo_entry->getParentID() == 0 && vo_entry->isValid())
				{
					vo_entry->calcSceneContribution(local_origin,
													needs_update,
													last_update,
													dist_threshold);
...
void LLViewerRegion::createVisibleObjects(F32 max_time)
{
	...
	bool do_throttle = sNewObjectCreationThrottle > 0;
	bool has_new_obj = false;
	LLTimer update_timer;
	for (LLVOCacheEntry::vocache_entry_priority_list_t::iterator
			iter = mWaitingList.begin(), end = mWaitingList.end();
		 iter != end; ++iter)
	{
...
void LLViewerRegion::clearCachedVisibleObjects()
{
	...
	for (LLVOCacheEntry::vocache_entry_set_t::iterator iter = mVisibleEntries.begin();
	...
	for (LLVOCacheEntry::vocache_entry_set_t::iterator
			iter = mActiveSet.begin(), end = mActiveSet.end();
		 iter != end; ++iter)
	{
		LLVOCacheEntry* vo_entry = *iter;
		if (!vo_entry)
		...
		LLViewerOctreeEntry* octree_entry = vo_entry->getEntry();
		if (!octree_entry)
		{
			continue;
		}
		LLDrawable* drawablep = (LLDrawable*)octree_entry->getDrawable();
...
void LLViewerRegion::killInvisibleObjects(F32 max_time)
{
	...
	LLVOCacheEntry::vocache_entry_set_t::iterator iter;
		 ...
		 update_counter > 0; --update_counter, ++iter)
	{
		...
		LLVOCacheEntry* vo_entry = *iter;
		if (!vo_entry->isAnyVisible(camera_origin, local_origin,
									back_threshold) &&
			vo_entry->mLastCameraUpdated < sLastCameraUpdated)
		{
...
LLViewerObject* LLViewerRegion::addNewObject(LLVOCacheEntry* entry)
{
	...
	if (!entry->getEntry()->hasDrawable())
	{
		...
		if (obj && !entry->isState(LLVOCacheEntry::ACTIVE))
		{
...
U32 LLViewerRegion::getPacketsLost() const
{
	...
	if (!cdp)
	{
		llinfos << "Couldn't find circuit for " << mHost << llendl;
		return 0;
	}
	else
	{
...
void LLViewerRegion::setSimulatorFeatures(const LLSD& sim_features)
{
	...
	// We will not earase URLs passed on login (see below)...
...
void LLViewerRegion::decodeBoundingInfo(LLVOCacheEntry* entry)
{
	...
	if (!entry || !entry->isValid() || !entry->getDP())
	{
		return;
	}
	...
	if (parent_id > 0) // Got a new parent
	{
		...
		else // Not in cache.
		{
			...
			else
			{
				...
				if (parent->getGroup())
				{
					// Re-insert parent to vo-cache tree because its bounding
					// info changed.
																   ...
																   U32 flags)
{
	...
	if (flags != 0xffffffff)
	{
		...
		if (fullid.notNull())
		{
			LL_DEBUGS("ObjectCache") << "Set cache entry flags for object "
									 << fullid << " to: " << flags << LL_ENDL;
...
{
	LLVOCacheEntry::vocache_entry_map_t::iterator iter;
	iter = mCacheMap.find(local_id);
								...
								U8& cache_miss_type)
{
	...
	if (flags != 0xffffffff)
	{
		LL_DEBUGS("ObjectCache") << "Setting cache entry flags for object ";
		LLUUID fullid;
		LLViewerObjectList::getUUIDFromLocal(fullid, local_id,
											 gMessageSystemp->getSenderIP(),
											 gMessageSystemp->getSenderPort());
...
void LLViewerRegion::dumpCache()
{
	...
	for (LLVOCacheEntry::vocache_entry_map_t::iterator
			iter = mCacheMap.begin(), end = mCacheMap.end();
		 iter != end; ++iter)
	{
...
void LLViewerRegion::unpackRegionHandshake()
{
	...
	{
		msg->getS32("RegionInfo3", "CPUClassID", mClassID);
		msg->getS32("RegionInfo3", "CPURatio", mCPURatio);
		msg->getString("RegionInfo3", "ColoName", mColoName);
		msg->getString("RegionInfo3", "ProductSKU", mProductSKU);
		msg->getString("RegionInfo3", "ProductName", mProductName);
	}
	...
	LL_DEBUGS("RegionTexture") << "Region: " << sim_name << " - Compostion did "
							   << (changed ? "" : "not ")
	...
	// After loading cache, signal that simulator can start sending data.
...
#if 1	// Deprecated in SL (now using "RegionObjects"). Kept for OpenSim, but is
		// it at all used there ?  HB
...
{
	LLCoros* corosp = LLCoros::getInstance();

	if (getCapability("Seed") == url)
	{
		std::string coro =
			corosp->launch("LLViewerRegion::requestBaseCapabilitiesCompleteCoro",
	...
	std::string coro =
		corosp->launch("LLViewerRegion::requestBaseCapabilitiesCoro",
								   ...
								   const std::string& url)
{
	if (name == "EventQueueGet")
	{
		delete mEventPoll;
		mEventPoll = NULL;
		mEventPoll = new LLEventPoll(getName(), url, getHost());
	}
	...
	else if (name == "SimulatorFeatures")
	{
		...
		std::string coro_name =
			coros->launch("LLViewerRegion::requestSimulatorFeatureCoro",
