	...
	mIsDirty(true)
{
	...
	childSetCommitCallback("threaded_localfs_check",
						   onCommitCheckBoxAfterRestart, this);
	childSetCommitCallback("use_io_uring_check",
						   onCommitCheckBoxAfterRestart, this);
...
# define EXT_EDIT "\"C:\\Program Files\\Notepad++\\notepad++.exe\" %s"
	...
	if (!control || control->isHiddenFromUser())
	{
		childSetEnabled("use_io_uring_check", false);
#if LL_LINUX
		childSetToolTip("use_io_uring_check", getString("no_io_uring_linux"));
#else
		childSetToolTip("use_io_uring_check", getString("no_io_uring"));
#endif
	}
...
void HBPrefsCoolImpl::refreshValues()
{
	...
	mThreadedFilesystem					= gSavedSettings.getBool("ThreadedFilesystem");
	mUseIOUring							= gSavedSettings.getBool("UseIOUring");
...
void HBPrefsCoolImpl::cancel()
{
	...
	gSavedSettings.setBool("ThreadedFilesystem",				mThreadedFilesystem);
	gSavedSettings.setBool("UseIOUring",						mUseIOUring);
	...
	if (self)
	{
		...
		if (combo)
		{
			...
			if (!label.empty())
			{
				...
				LLCoros::getInstance()->launch("HBPrefsCoolImpl::fetchDictionaryCoro(aff)",
										       boost::bind(&HBPrefsCoolImpl::fetchDictionaryCoro,
														   dict_name + ".aff"));
				LLCoros::getInstance()->launch("HBPrefsCoolImpl::fetchDictionaryCoro(dic)",
										       boost::bind(&HBPrefsCoolImpl::fetchDictionaryCoro,
														   dict_name + ".dic"));
			}
		}
													   ...
													   void* user_data)
{
	...
	else
	{
		...
		else
		{
			...
			gWLSkyParamMgr.mAnimator.setDayTime(value);
