...
static LLSD ensure_array_4(LLSD in, F32 fill)
{
	if (in.size() >= 4)
	{
		return in;
	...
	for (S32 idx = 0, count = in.size(); idx < count; ++idx)
	{
		out.append(in[idx]);
	}
											...
											bool update_name)
{
	...
	LLResourceUploadInfo::ptr_t info =
		boost::make_shared<LLBufferedAssetUploadInfo>(inv_item_id,
													  LLAssetType::AT_SETTINGS,
													  buffer.str(),
											...
											inv_result_fn callback)
{
	...
	LLResourceUploadInfo::ptr_t info =
		boost::make_shared<LLBufferedAssetUploadInfo>(object_id, inv_item_id,
													  LLAssetType::AT_SETTINGS,
													  buffer.str(),
...
LLSettingsSky::ptr_t LLEnvSettingsSky::buildClone() const
{
	...
	U32 flags = getFlags();
	ptr_t skyp = boost::make_shared<LLEnvSettingsSky>(settings);
	skyp->setFlags(flags);
	return skyp;
}
...
void LLEnvSettingsSky::updateSettings()
{
	...
	bool override_wl = LLEnvironment::overrideWindlight();

	if (override_wl && !gUseNewShaders)
	{
		// *HACK: override the Windlight Sun position with ours
		gSky.setOverrideSun(true);
		gSky.setSunDirection(sun_direction, LLVector3::zero);
		// *HACK: convert EEP settings into legacy Windlight settings
		LLSettingsSky::ptr_t skyp =
			boost::static_pointer_cast<LLSettingsSky>(shared_from_this());
		gWLSkyParamMgr.applyToParcel(convertToLegacy(skyp));
	}
	...
	{
		// Reset to the default Windlight textures
...
LLSettingsSky::ptr_t LLEnvSettingsSky::buildSky(LLSD settings)
{
	...
	return boost::make_shared<LLEnvSettingsSky>(settings, true);
}
...
LLSettingsSky::ptr_t LLEnvSettingsSky::buildDefaultSky()
{
	...
	return boost::make_shared<LLEnvSettingsSky>(default_settings);
}
															 ...
															 LLSD& messages)
{
	...
	return boost::make_shared<LLEnvSettingsSky>(new_settings);
}
...
//static
LLSD LLEnvSettingsSky::convertToLegacy(const LLSettingsSky::ptr_t& skyp)
{
	LLSD legacy(LLSD::emptyMap());
	LLSD settings = skyp->getSettings();

	convertAtmosphericsToLegacy(legacy, settings);

	legacy[SETTING_CLOUD_COLOR] = ensure_array_4(settings[SETTING_CLOUD_COLOR],
												 1.f);
	legacy[SETTING_CLOUD_POS_DENSITY1] =
		ensure_array_4(settings[SETTING_CLOUD_POS_DENSITY1], 1.f);
	legacy[SETTING_CLOUD_POS_DENSITY2] =
		ensure_array_4(settings[SETTING_CLOUD_POS_DENSITY2], 1.f);
	legacy[SETTING_CLOUD_SCALE] =
		LLSDArray(settings[SETTING_CLOUD_SCALE])(LLSD::Real(0.0))(LLSD::Real(0.0))(LLSD::Real(1.0));
	// BEWARE: there is a 10.f offset for Windlight cloud scroll rates !
	legacy[SETTING_CLOUD_SCROLL_RATE] =
		LLSDArray(LLSD::Real(settings[SETTING_CLOUD_SCROLL_RATE][0].asReal() + 10.f))
				 (LLSD::Real(settings[SETTING_CLOUD_SCROLL_RATE][1].asReal() + 10.f));
	legacy[SETTING_LEGACY_ENABLE_CLOUD_SCROLL] =
		LLSDArray(LLSD::Boolean(!is_approx_zero(settings[SETTING_CLOUD_SCROLL_RATE][0].asReal())))
				 (LLSD::Boolean(!is_approx_zero(settings[SETTING_CLOUD_SCROLL_RATE][1].asReal())));
	legacy[SETTING_CLOUD_SHADOW] =
		LLSDArray(settings[SETTING_CLOUD_SHADOW].asReal())(0.f)(0.f)(1.f);
	legacy[SETTING_GAMMA] = LLSDArray(settings[SETTING_GAMMA])(0.f)(0.f)(1.f);
	legacy[SETTING_GLOW] = ensure_array_4(settings[SETTING_GLOW], 1.f);
	legacy[SETTING_LIGHT_NORMAL] =
		ensure_array_4(skyp->getLightDirection().getValue(), 0.f);
	legacy[SETTING_MAX_Y] = LLSDArray(settings[SETTING_MAX_Y])(0.f)(0.f)(1.f);
	// Convert from 0-500 to 0-2
	legacy[SETTING_STAR_BRIGHTNESS] =
		settings[SETTING_STAR_BRIGHTNESS].asReal() / 250.f;
	legacy[SETTING_SUNLIGHT_COLOR] =
		ensure_array_4(settings[SETTING_SUNLIGHT_COLOR], 1.f);

	LLVector3 dir = skyp->getSunDirection();

	F32 phi = asinf(dir.mV[2]);
	F32 cos_phi = cosf(phi);
	F32 theta = cos_phi != 0.f ? -asinf(dir.mV[1] / cos_phi) : 0.f;

	// Get Sun angle back into a valid range for Windlight; it varies as
	// follow:
	// - 06:00 to 12:00 -> from 0 to PI/2 for both EE and WL
	// - 12:00 to 18:00 -> from PI/2 to 0 for EE, from PI/2 to PI for WL
	// - 18:00 to 24:00 -> from 0 to -PI/2 for EE, from PI to 3*PI/2 for WL
	// - 00:00 to 06:00 -> from -PI/2 to 0 for EE, from 3*PI/2 to 2*PI for WL
	if (dir.mV[0] >= 0.f)	// True from 00:00 to 12:00
	{
		if (phi < 0.f)		// True from 00:00 to 06:00
		{
			phi += F_TWO_PI;
			// *HACK: to avoid "loosing" the Moon...
			if (phi < 4.7124f)	// Adjusted phi must stay slightly above 3*PI/2
			{
				phi = 4.7124f;
			}
		}
	...
	{
		phi = F_PI - phi;	// EE phi is negative after 18:00
	}
	legacy[SETTING_LEGACY_SUN_ANGLE] = phi;

	// *TODO: verify this is actually valid (theta is very rarely different
	// from zero)... HB
	if (theta < 0.f)
	{
		theta += F_TWO_PI;
...
//static
void LLEnvSettingsSky::convertAtmosphericsToLegacy(LLSD& legacy,
												   LLSD& settings)
{
	// These may need to be inferred from new settings' density profiles if the
	// legacy settings values are not available.
	if (settings.has(SETTING_LEGACY_HAZE))
	{
		...
		{
			legacy[SETTING_AMBIENT] =
				ensure_array_4(legacyhaze[SETTING_AMBIENT], 1.f);
		}
		else if (settings.has(SETTING_AMBIENT))
		{
			legacy[SETTING_AMBIENT] = ensure_array_4(settings[SETTING_AMBIENT],
													 1.f);
		}

		legacy[SETTING_BLUE_DENSITY] =
			ensure_array_4(legacyhaze[SETTING_BLUE_DENSITY], 1.f);
		legacy[SETTING_BLUE_HORIZON] =
			ensure_array_4(legacyhaze[SETTING_BLUE_HORIZON], 1.f);

		legacy[SETTING_DENSITY_MULTIPLIER] =
			LLSDArray(legacyhaze[SETTING_DENSITY_MULTIPLIER].asReal())(0.f)(0.f)(1.f);
		legacy[SETTING_DISTANCE_MULTIPLIER] =
			LLSDArray(legacyhaze[SETTING_DISTANCE_MULTIPLIER].asReal())(0.f)(0.f)(1.f);

		legacy[SETTING_HAZE_DENSITY] =
			LLSDArray(legacyhaze[SETTING_HAZE_DENSITY])(0.f)(0.f)(1.f);
		legacy[SETTING_HAZE_HORIZON] =
			LLSDArray(legacyhaze[SETTING_HAZE_HORIZON])(0.f)(0.f)(1.f);
	}
}
		...
		if (folder_id.notNull())
		{
				 ...
				 iter != end; ++iter)
			{
				...
				if (preset == name)
				{
					...
					if (!gUseNewShaders)
					{
						// *HACK: make sure handleUseLocalEnvironmentChanged() is
						// triggered in llviewercontrol.cpp
...
LLSettingsWater::ptr_t LLEnvSettingsWater::buildClone() const
{
	...
	U32 flags = getFlags();
	ptr_t waterp = boost::make_shared<LLEnvSettingsWater>(settings);
...
void LLEnvSettingsWater::updateSettings()
{
	...
	if (override_wl && !gUseNewShaders)
	{
		LLSettingsWater::ptr_t waterp =
			boost::static_pointer_cast<LLSettingsWater>(shared_from_this());
		gWLWaterParamMgr.applyToParcel(convertToLegacy(waterp));
	}
	...
	{
		poolp->setTransparentTextures(DEFAULT_WATER_TEXTURE);
		poolp->setNormalMaps(DEFAULT_WATER_NORMAL);
	}
}
...
LLSettingsWater::ptr_t LLEnvSettingsWater::buildWater(LLSD settings)
{
	...
	return boost::make_shared<LLEnvSettingsWater>(settings);
}
...
LLSettingsWater::ptr_t LLEnvSettingsWater::buildDefaultWater()
{
	...
	return boost::make_shared<LLEnvSettingsWater>(default_settings);
}
																 ...
																 LLSD& messages)
{
	...
	return boost::make_shared<LLEnvSettingsWater>(new_settings);
}
...
//static
LLSD LLEnvSettingsWater::convertToLegacy(const LLSettingsWater::ptr_t& waterp)
{
	LLSD legacy(LLSD::emptyMap());
	LLSD settings = waterp->getSettings();

	legacy[SETTING_LEGACY_BLUR_MULTIPLIER] = settings[SETTING_BLUR_MULTIPLIER];
	legacy[SETTING_LEGACY_FOG_COLOR] =
		ensure_array_4(settings[SETTING_FOG_COLOR], 1.f);
...
LLSettingsDay::ptr_t LLEnvSettingsDay::buildClone() const
{
	...
	ptr_t dayp = boost::make_shared<LLEnvSettingsDay>(settings);
...
LLSettingsDay::ptr_t LLEnvSettingsDay::buildDeepCloneAndUncompress() const
{
	...
	ptr_t day_clone = boost::make_shared<LLEnvSettingsDay>(settings);
...
{
	LLSettingsSky::ptr_t skyp = boost::make_shared<LLEnvSettingsSky>(settings);
...
{
	LLSettingsWater::ptr_t waterp =
		boost::make_shared<LLEnvSettingsWater>(settings);
...
LLSettingsDay::ptr_t LLEnvSettingsDay::buildDefaultDayCycle()
{
	...
	default_dayp = boost::make_shared<LLEnvSettingsDay>(default_settings);
															 ...
															 LLSD& messages)
{
	...
	LLSettingsDay::ptr_t dayp =
		boost::make_shared<LLEnvSettingsDay>(new_settings);
	dayp->initialize();
											 ...
											 asset_built_fn cb)
{
	if (settings->getSettingsType() == "daycycle")
	{
		if (cb)
		{
			cb(boost::static_pointer_cast<LLSettingsDay>(settings));
		}
		return;
	}
	...
	LLEnvSettingsBase::getSettingsAsset(getDefaultAssetId(),
										[settings, cb](LLUUID,
													   ...
													   S32, LLExtStat)
										{
											combineIntoDayCycle(boost::static_pointer_cast<LLSettingsDay>(dayp),
																settings, cb);
										});
}
...
//static
LLSD LLEnvSettingsDay::convertToLegacy(const LLEnvSettingsDay::ptr_t& dayp)
{
	cycle_track_t& trackwater = dayp->getCycleTrack(TRACK_WATER);

	LLSettingsWater::ptr_t waterp;
	if (!trackwater.empty())
	{
		waterp =
			boost::static_pointer_cast<LLSettingsWater>((*trackwater.begin()).second);
	}

	if (!waterp)
	{
		waterp = LLEnvSettingsWater::buildDefaultWater();
	}

	LLSD llsdwater = LLEnvSettingsWater::convertToLegacy(waterp);

	cycle_track_t &tracksky = dayp->getCycleTrack(1);   // First sky track
	std::map<std::string, LLSettingsSky::ptr_t> skys;

	LLSD llsdcycle(LLSD::emptyArray());

	for (cycle_track_it_t it = tracksky.begin(), end = tracksky.end();
		 it != end; ++it)
	{
		...
		skys[name.str()] =
			boost::static_pointer_cast<LLSettingsSky>(it->second);

		F32 frame = tracksky.size() == 1 &&
					it == tracksky.begin() ? -1.f : it->first;
		llsdcycle.append(LLSDArray(LLSD::Real(frame))(name.str()));
	}

	LLSD llsdskylist(LLSD::emptyMap());

	for (std::map<std::string, LLSettingsSky::ptr_t>::iterator
			it = skys.begin(), end = skys.end();
		 it != end; ++it)
	{
		LLSD llsdsky = LLEnvSettingsSky::convertToLegacy(it->second);
		llsdsky[SETTING_NAME] = it->first;
		llsdskylist[it->first] = llsdsky;
	}
