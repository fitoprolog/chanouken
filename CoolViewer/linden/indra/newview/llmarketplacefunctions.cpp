...
void LLMarketplaceInventoryObserver::changed(U32 mask)
{
	...
	if (mask & (LLInventoryObserver::INTERNAL | LLInventoryObserver::STRUCTURE))
	{
			 ...
			 it != end; ++it)
		{
			...
			{
				// If it's a folder known to the marketplace, let's check it's
				// in proper shape
			...
			else
			{
				...
				if (item &&
					!item->getPermissions().allowOperationBy(PERM_COPY,
															 gAgentID,
															 group_id))
				{
...
void LLMarketplaceData::initializeSLM(const status_updated_signal_t::slot_type& cb)
{
		...
		mMarketPlaceStatus == MarketplaceStatusCodes::MARKET_PLACE_CONNECTION_FAILURE)
	{
		...
		else
		{
			...
			LLCoros::getInstance()->launch("getMerchantStatus",
...
void LLMarketplaceData::getSLMListings()
{
	...
	if (market_id.notNull())
	{
		...
		LLCoros::getInstance()->launch("getSLMListings",
									   boost::bind(&LLMarketplaceData::getSLMListingsCoro,
												   this, url, market_id));
	}
}
...
void LLMarketplaceData::getSLMListing(S32 listing_id)
{
	...
	LLCoros::getInstance()->launch("getSLMListings",
								   boost::bind(&LLMarketplaceData::getSLMListingCoro,
											   this, url, folder_id));
}
										 ...
										 const LLUUID& version_id, S32 count)
{
	...
	LL_DEBUGS("Marketplace") << "Sending resquest: " << url << " - Body:"
							 << data << LL_ENDL;
	setUpdating(folder_id, true);
	LLCoros::getInstance()->launch("createSLMListingCoro",
								   boost::bind(&LLMarketplaceData::createSLMListingCoro,
											   this, url, folder_id, data));
}
										 ...
										 bool is_listed, S32 count)
{
	...
	LL_DEBUGS("Marketplace") << "Sending resquest: " << url << " - Body:"
							 << data << LL_ENDL;
	setUpdating(folder_id, true);
	LLCoros::getInstance()->launch("updateSLMListingCoro",
								   boost::bind(&LLMarketplaceData::updateSLMListingCoro,
											   this, url, folder_id,
											   version_id, is_listed, data));
}
...
void LLMarketplaceData::deleteSLMListing(S32 listing_id)
{
	...
	setUpdating(folder_id, true);
	LL_DEBUGS("Marketplace") << "Sending resquest: " << url << LL_ENDL;
	LLCoros::getInstance()->launch("deleteSLMListingCoro",
								   boost::bind(&LLMarketplaceData::deleteSLMListingCoro,
											   this, url, folder_id));
}
							 ...
							 bool resolve_links = false)
{
	...
	if (!vitem->getPermissions().allowOperationBy(PERM_TRANSFER, gAgentID))
	{
	...
	if (!allow_no_copy &&
		!vitem->getPermissions().allowOperationBy(PERM_COPY, gAgentID))
	{
		tooltip_msg = LLTrans::getString("TooltipOutboxNoCopy");
		return false;
	}
									 ...
									 bool fix_hierarchy, S32 depth)
{
		 ...
		 iter != item_array_copy.end(); ++iter)
	{
													 ...
													 group_id))
		{
			...
			type = item->getInventoryType();
			perms = item->getPermissions().getMaskNextOwner();
		}
		U32 key = (((U32)(type) & 0xFF) << 24) | (perms & 0xFFFFFF);
		items_vector[key].emplace_back(item->getUUID());
	}
		...
		if (fix_hierarchy && !has_bad_items)
		{
				 ...
				 unique_key != default_key))
			{
					 ...
					 it != end; ++it)
				{
					...
					LLUUID folder_uuid = gInventory.createNewCategory(parent_uuid,
																	  new_folder_type,
																	  folder_name);
					gInventory.notifyObservers();
										  ...
										  std::string& error_msg)
{
	...
	{
		LLViewerInventoryItem* item = *iter;
		if (!item || !can_move_to_marketplace(item, error_msg, true, false))
		{
			return false;
		}
	}
...
// This function also works for the outbox, but then allow_no_copy must be
// false (while it is true, which is the default value, for the Marketplace
// Listings folder).
//static
bool LLMarketplace::canMoveItemInto(const LLViewerInventoryCategory* root_folder,
									...
									std::string& tooltip_msg,
									S32 bundle_size, bool allow_no_copy,
									bool from_paste)
{
	...
	if (accept)
	{
		accept = can_move_to_marketplace(inv_item, tooltip_msg, allow_no_copy,
										 true);
	}
	...
	if (accept)
	{
		...
		if (version_folder)
		{
			...
			if (!move_in_stock &&
				!inv_item->getPermissions().allowOperationBy(PERM_COPY,
															 gAgentID,
															 gAgent.getGroupID()))
			{
	...
	if (accept)
	{
		...
		if (accept)
		{
			for (S32 i = 0, count = descendent_items.size(); i < count; ++i)
			{
				...
				if (!can_move_to_marketplace(item, tooltip_msg, allow_no_copy))
				{
					accept = false;
					break;
				}
								 ...
								 const LLUUID& dest_folder, bool copy)
{
	...
	else
	{
		...
		// Check that the agent has transfer permission on the item: this is
		// required as a resident cannot put on sale items they cannot
		// transfer. Proceed with move if we have permission.
		std::string error_msg;
		if (can_move_to_marketplace(inv_item, error_msg, true, true))
		{
			...
			if (depth == 0)
			{
				...
				dest_id = gInventory.createNewCategory(dest_id,
													   LLFolderType::FT_NONE,
													   vitem->getName());
				gInventory.notifyObservers();
				++depth;
								   ...
								   bool move_no_copy_items)
{
	...
	if (hasPermissionsForSale(inv_cat, error_msg))
	{
		...
		if (copy)
		{
			...
			// Copy the folder
			copy_inventory_category(&gInventory, inv_cat, dest_id,
									LLUUID::null, move_no_copy_items);
		}
		else
		{
			// Reparent the folder
									...
									const LLUUID& cat_id)
{
	...
	else
	{
		...
		// when moving a stock folder out of a version folder...
		LLViewerInventoryCategory* cat = gInventory.getCategory(cat_id);
		...
		if (version_id.notNull())
		{
			...
			{
				// If we move from an active (listed) listing, check that it's
				// still valid, if not, unlist
				marketdata->activateListing(version_id, false);
			}
		}
#if 0	// Nope, does npt work for forcing an inventory folder label update
		// when moving a stock folder out of a version folder...
