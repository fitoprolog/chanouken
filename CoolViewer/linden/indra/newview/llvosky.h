...
public:
	...
	LLHaze(F32 g, const LLColor3& sca, F32 fo = 2.f)
	:	mG(g),
		mSigSca(0.25f / F_PI * sca),
		mFalloff(fo),
		mAbsCoef(0.f)
	{
	...
	LLHaze(F32 g, F32 sca, F32 fo = 2.f)
	:	mG(g),
		mSigSca(0.25f / F_PI * LLColor3(sca, sca, sca)),
		mFalloff(fo)
	{
		mAbsCoef = 0.01f * sca / sAirScaAvg;
	}

	LL_INLINE F32 getG() const								{ return mG; }

	LL_INLINE void setG(F32 g)								{ mG = g; }

	// sea level
	LL_INLINE const LLColor3& getSigSca() const				{ return mSigSca;}

	LL_INLINE void setSigSca(const LLColor3& s)
	{
		mSigSca = s;
		mAbsCoef = 0.01f * color_intens(mSigSca) / sAirScaIntense;
	}

	LL_INLINE void setSigSca(F32 s0, F32 s1, F32 s2)
	{
		mSigSca = sAirScaAvg * LLColor3(s0, s1, s2);
		constexpr F32 FACTOR = 0.01f / 3.f;
		mAbsCoef = FACTOR * (s0 + s1 + s2);
	}

	LL_INLINE F32 getFalloff() const						{ return mFalloff; }

	LL_INLINE void setFalloff(F32 fo)						{ mFalloff = fo; }

	LL_INLINE F32 getAbsCoef() const						{ return mAbsCoef; }

	LL_INLINE LLColor3 calcSigSca(F32 h) const
	{
		return calcFalloff(h * mFalloff) * mSigSca;
	}

	LL_INLINE void calcSigSca(F32 h, LLColor3& result) const
	{
		result = mSigSca;
		result *= calcFalloff(h * mFalloff);
	}

	LL_INLINE LLColor3 calcSigExt(F32 h) const
	{
		return calcFalloff(h * mFalloff) * (1.f + mAbsCoef) * mSigSca;
	}

	F32 calcPhase(F32 cos_theta) const;

	// Proportion of light that is scattered into 'path' from 'in' over
	// distance dt. Assumes that vectors 'path' and 'in' are normalized.
	// Scattering coef / 2pi

	static LL_INLINE F32 calcFalloff(F32 h)
	{
		return h <= 0.f ? 1.f : (F32)LL_FAST_EXP(-ATM_EXP_FALLOFF * h);
	}

	static LL_INLINE LLColor3 calcAirSca(F32 h)
	{
		return calcFalloff(h) * sAirScaSeaLevel;
	}

	static LL_INLINE void calcAirSca(F32 h, LLColor3& result)
	{
...
private:
	static const LLColor3	sAirScaSeaLevel;
	static const F32		sAirScaIntense;
	static const F32		sAirScaAvg;
...
public:
	...
	LLVOSky(const LLUUID& id, LLPCode pcode, LLViewerRegion* regionp);
	...
	LL_INLINE const LLHaze& getHaze() const					{ return mHaze; }
	LL_INLINE LLHaze& getHaze()								{ return mHaze; }
	LL_INLINE F32 getHazeConcentration() const				{ return mHazeConcentration; }
	LL_INLINE void setHaze(const LLHaze& h)					{ mHaze = h; }
	...
	LL_INLINE void setFogRatio(F32 fog_ratio)				{ mFogRatio = fog_ratio; }
	LL_INLINE LLColor4U getFadeColor() const				{ return mFadeColor; }
	LL_INLINE F32 getFogRatio() const						{ return mFogRatio; }
...
private:
	void initCubeMap();
	void initAtmosphericsWL();
	void initAtmosphericsEE(const LLSettingsSky::ptr_t& skyp);
	void calcAtmosphericsWL();
	void calcAtmosphericsEE();	// AKA calc() in LL's viewer

	void updateDirections(const LLSettingsSky::ptr_t& skyp);

	void initSkyTextureDirs(S32 side, S32 tile);
	void createSkyTextureWL(S32 side, S32 tile);
	void createSkyTextureEE(const LLSettingsSky::ptr_t& skyp,
							S32 side, S32 tile);

	LLColor4 calcSkyColorInDirWL(const LLVector3& dir, bool is_shiny = false);
	LLColor4 calcSkyColorInDirEE(const LLSettingsSky::ptr_t& skyp,
								 const LLVector3& dir, bool is_shiny = false);

	void calcSkyColorWLVert(LLVector3& pn, LLColor3& vary_haze_col);
	void calcSkyColorEEVert(LLVector3& pn);
