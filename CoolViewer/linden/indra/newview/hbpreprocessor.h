...
private:
	...
	// Method used to evaluate a logical expression using Lua (when enabled in
	// the viewer at compile time). Note that the "!=", "||", "&&", "!" and "^"
	// operators are automatically translated into their Lua equivalent.
	// This method returns a bool, which is true if the result is not zero or
	// false otherwise (including in case of and error).
	// In case of error, the mErrorMessage variables is set.
	// If Lua is disabled at compile time, then this method simply converts
	// 'expression' to a float with atof() and returns true if the result is
	// not exactly 0.f (so it only works for simple numbers).
