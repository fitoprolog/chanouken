...
void LLPreviewTexture::draw()
{
	...
	if (!isMinimized())
	{
		...
		if (mImage.notNull())
		{
			...
			// Don not bother decoding more than we can display, unless
...
{
	// We don't allow negative width or height. Also, if height is positive but
	// too small, we reset to default. A default 0.f value for mAspectRatio
...
{
	mImage = LLViewerTextureManager::getFetchedTexture(mImageID, FTT_DEFAULT,
													   MIPMAP_YES,
