...
#include "llinventoryactions.h"		// init_object_inventory_panel_actions()
...
void LLTaskInvFVBridge::showProperties()
{
	if (!LLFloaterProperties::show(mUUID, mPanel->getTaskUUID()))
	{
		...
		LLFloaterProperties* floater =
			new LLFloaterProperties("obj item properties", rect,
...
{
public:
	LLTaskCategoryBridge(LLPanelInventory* panel,
						 const LLUUID& uuid,
						 const std::string& name);
									  ...
									  std::string& tooltip_msg)
{
	...
	if (object)
	{
		LLToolDragAndDrop* draganddrop = LLToolDragAndDrop::getInstance();
		switch (cargo_type)
		{
		case DAD_CATEGORY:
			accept = draganddrop->dadUpdateInventoryCategory(object, drop);
			break;
		case DAD_TEXTURE:
		case DAD_SOUND:
		case DAD_LANDMARK:
		...
		case DAD_SETTINGS:
			...
			if (accept && drop)
			{
				LLToolDragAndDrop::dropInventory(object,
												 (LLViewerInventoryItem*)cargo_data,
												 draganddrop->getSource(),
												 draganddrop->getSourceID());
			}
			break;
		...
		case DAD_SCRIPT:
			...
			if (LLToolDragAndDrop::isInventoryDropAcceptable(object,
															 (LLViewerInventoryItem*)cargo_data)
				&& (LLToolDragAndDrop::SOURCE_WORLD != draganddrop->getSource())
				&& (LLToolDragAndDrop::SOURCE_NOTECARD != draganddrop->getSource()))
			{
				accept = true;
			}
#else
...
{
public:
	LLTaskTextureBridge(LLPanelInventory* panel,
						const LLUUID& uuid,
						const std::string& name,
...
//mk
	...
	if (!LLPreview::show(mUUID))
	{
		...
		LLPreviewTexture* preview =
			new LLPreviewTexture("preview task texture", rect, getName(),
								 mUUID, mPanel->getTaskUUID());
...
void LLTaskSoundBridge::openSoundPreview(void* data)
{
	...
	if (self && !LLPreview::show(self->mUUID))
	{
		...
		LLPreviewSound* floaterp =
			new LLPreviewSound("preview task sound", rect, self->getName(),
							   self->mUUID, self->mPanel->getTaskUUID());
		floaterp->open();
	}
}
...
{
public:
	LLTaskLandmarkBridge(LLPanelInventory* panel,
						 const LLUUID& uuid,
						 const std::string& name);
...
{
public:
	LLTaskCallingCardBridge(LLPanelInventory* panel,
							const LLUUID& uuid,
							const std::string& name);
...
{
public:
	LLTaskScriptBridge(LLPanelInventory* panel,
					   const LLUUID& uuid,
					   const std::string& name);
...
{
public:
	LLTaskLSLBridge(LLPanelInventory* panel,
					const LLUUID& uuid,
					const std::string& name);
...
{
public:
	LLTaskObjectBridge(LLPanelInventory* panel,
					   const LLUUID& uuid,
					   const std::string& name);
...
{
public:
	LLTaskNotecardBridge(LLPanelInventory* panel,
						 const LLUUID& uuid,
						 const std::string& name);
...
{
public:
	LLTaskGestureBridge(LLPanelInventory* panel,
						const LLUUID& uuid,
						const std::string& name);
...
void LLTaskGestureBridge::openItem()
{
	...
	LLPreviewGesture* preview = LLPreviewGesture::show(title, item_id,
													   object_id);
...
{
	// Don't need to deactivate gesture because gestures inside objects can
	// never be active.
	LLPreview::hide(mUUID);
	return LLTaskInvFVBridge::removeItem();
}
...
{
public:
	LLTaskAnimationBridge(LLPanelInventory* panel,
						  const LLUUID& uuid,
						  const std::string& name);
...
void LLTaskAnimationBridge::openItem()
{
	...
	if (object->permModify() || gAgent.isGodlike())
	{
		...
		if (!LLPreview::show(mUUID))
		{
			...
			LLPreviewAnim* preview = new LLPreviewAnim("preview anim", rect,
													   getName(), mUUID, 0,
													   mPanel->getTaskUUID());
			// Take focus if you are looking at one of these
			preview->setFocus(true);
...
void LLPanelInventory::clearContents()
{
	...
	LLToolDragAndDrop* draganddrop = LLToolDragAndDrop::getInstance();
	if (draganddrop->getSource() == LLToolDragAndDrop::SOURCE_WORLD)
	{
		draganddrop->endDrag();
	}
										...
										S32 serial_num, void* data)
{
	...
	if (inventory)
	{
		// We need to copy the ones that need refreshing onto a temporary
		// object because we cannot iterate through the object inventory
		// twice... A pox on stateful iteration !
		...
		for (S32 i = 0, count = refresh.size(); i < count; ++i)
		{
			refresh[i]->refresh();
		}
	}
