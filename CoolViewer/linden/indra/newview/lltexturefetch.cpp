...
private:
	...
	public:
		// Threads: Ttf
		CacheReadResponder(LLTextureFetch* fetcher, const LLUUID& id,
		...
		void completed(bool success) override
		{
			...
			if (worker)
			{
 				worker->callbackCacheRead(success, mFormattedImage, mImageSize,
										  mImageLocal);
			}
		}
...
public:
	...
	LL_INLINE LLTextureFetch& getFetcher()			{ return *mFetcher; }
...
protected:
	LLTextureFetchWorker(LLTextureFetch* fetcher, FTType f_type,
						 const std::string& url, const LLUUID& id,
						 const LLHost& host, F32 priority, S32 discard,
						 S32 size);
...
private:
	...
	bool						mInLocalCache;
	bool						mCanUseHTTP;
	bool						mCanUseNET;	// Can get via UDP from sim server
	bool						mSkipCache;	// true to ignored cached texture
	S32							mRetryAttempt;
	S32							mActiveCount;
	...
	U32							mHttpReplyOffset;	// Actual received data offset
...
LLTextureFetchWorker::~LLTextureFetchWorker()
{
	...
	if (mHttpActive)
	{
		...
		mFetcher->getHttpRequest().requestCancel(mHttpHandle,
												 LLCore::HttpHandler::ptr_t());
	}
	if (mCacheReadHandle != LLTextureCache::nullHandle() &&
		mFetcher->mTextureCache)
	{
...
{
	static const F32 PRIORITY_SCALE = (F32)LLWorkerThread::PRIORITY_LOWBITS /
									  LLViewerFetchedTexture::maxDecodePriority();
	mWorkPriority = llmin((U32)LLWorkerThread::PRIORITY_LOWBITS,
...
{
	//llassert_always(priority >= 0 &&
	//				priority <= LLViewerTexture::maxDecodePriority());
	...
	if (delta > mImagePriority * .05f || mState == DONE)
	{
		...
		U32 work_priority = mWorkPriority |
							(getPriority() &
							 LLWorkerThread::PRIORITY_HIGHBITS);
		setPriority(work_priority);
	}
}
...
bool LLTextureFetchWorker::doWork(S32 param)
{
	...
	if (mFetcher->mDebugPause)
	{
		return false;	// Debug: do not do any work
	}
	if (mState != DONE)
	{
		mFetchTimer.reset();
	...
	if (mState == INIT)
	{
		...
		mCacheReadHandle = LLTextureCache::nullHandle();
		mCacheWriteHandle = LLTextureCache::nullHandle();
		mState = mSkipCache ? CACHE_POST : LOAD_FROM_TEXTURE_CACHE;
	...
	if (mState == LOAD_FROM_TEXTURE_CACHE)
	{
		if (mCacheReadHandle == LLTextureCache::nullHandle())
		{
			...
			if (size > 0)
			{
				...
				CacheReadResponder* responder =
					new CacheReadResponder(mFetcher, mID, mFormattedImage);
				if (mUrl.compare(0, 7, "file://") == 0)
				{
					// Read file from local disk
					std::string filename = mUrl.substr(7);
					mCacheReadHandle =
						mFetcher->mTextureCache->readFromFile(filename, mID,
															  cache_priority,
															  offset, size,
															  responder);
				}
				...
				{
					mCacheReadHandle =
						mFetcher->mTextureCache->readFromCache(mID,
															   cache_priority,
															   offset, size,
															   responder);
				}
		...
		if (mLoaded)
		{
			...
			else if (mCacheReadTimer.getElapsedTimeF32() > 30.f)
			{
				llwarns << "Texture " << mID << ": cache read timeout." << llendl;
				removeFromCache();
	...
	if (mState == CACHE_POST)
	{
		...
		else if (mUrl.compare(0, 7, "file://") == 0)
		{
			...
			llwarns << "Texture " << mID << ": failed to load local file "
					<< mUrl << llendl;
			return true;
		}
		else
	...
	if (mState == SEND_HTTP_REQ)
	{
		...
		if (!mUrl.empty())
		{
			...
			options = with_headers ? mFetcher->mHttpOptionsWithHeaders
			...
			else
			{
				...
				mHttpHandle =
					mFetcher->mHttpRequest->requestGetByteRange(mHttpPolicyClass,
																mWorkPriority,
																mUrl,
																mRequestedOffset,
																req_size,
...
decode_image:
	if (mState == DECODE_IMAGE)
	{
		// Set priority first since Responder may change it
		setPriority(LLWorkerThread::PRIORITY_LOW | mWorkPriority);
		if (mDesiredDiscard < 0 || mFormattedImage->getDataSize() <= 0 ||
			mLoadedDiscard < 0)
		{
		...
		mDecodeHandle =
			mFetcher->mImageDecodeThread->decodeImage(mFormattedImage,
													  image_priority, discard,
													  mNeedsAux,
													  new DecodeResponder(mFetcher,
																		  mID));
		// Fall through
	}
	...
	if (mState == WRITE_TO_CACHE)
	{
		...
		{
			// If we are in a local cache or we did not actually receive any
			// new data, or we failed to load anything, skip
			mState = DONE;
			return false;
		}
		...
		if (datasize <= 0)
		{
			...
			// to be there... Use proper fallback code (skip) instead.
			mState = DONE;
			return false;
		}
		...
		CacheWriteResponder* responder = new CacheWriteResponder(mFetcher,
																 mID);
		mCacheWriteHandle =
			mFetcher->mTextureCache->writeToCache(mID, cache_priority,
												  mFormattedImage->getData(),
												  datasize, mFileSize,
												  mRawImage, mDecodedDiscard,
												  responder);
		// Fall through
	}
	...
	if (mState == WAIT_ON_WRITE)
	{
		...
		else
		{
			...
			{
				// We are waiting for this write to complete before we can
				// receive more data (we cannot touch mFormattedImage until the
				// write completes). Prioritize the write.
									   ...
									   LLCore::HttpResponse* response)
{
	...
	{
		success = false;
		// Missing map tiles are normal, do not complain about them
		if (mFTType != FTT_MAP_TILE)
		{
...
bool LLTextureFetchWorker::deleteOK()
{
	...
	if (mState == WAIT_HTTP_RESOURCE2 && mFetcher->isHttpWaiter(mID))
	{
										  ...
										  bool partial, bool success)
{
	...
	if (mLoaded)
	{
		llwarns << "Duplicate callback for " << mID << llendl;
...
LLTextureFetch::LLTextureFetch(LLTextureCache* cache,
							   LLImageDecodeThread* decodethread)
:	LLWorkerThread("Texture fetch", true, true),
	mTextureCache(cache),
	mImageDecodeThread(decodethread),
	mDebugPause(false),
...
bool LLTextureFetch::createRequest(FTType f_type, const std::string& url,
								   ...
								   bool can_use_http, bool skip_cache)
{
	if (mDebugPause)
	{
		return false;
	...
	if (worker)
	{
		...
		{
			// Heed to wait for previous aborted request to complete
			return false;
		}
	...
	{
		worker = new LLTextureFetchWorker(this, f_type, url, id, host,
										  priority, desired_discard,
										  desired_size);
		lockQueue();
...
uuid_list_t LLTextureFetch::deleteAllRequests()
{
	...
	for (U32 i = 0, count = fetching_ids.size(); i < count; ++i)
	{
		...
		if (texp && texp->getDontDiscard())
		{
			// Do not interrupt the fetching of "do not discard" images !
			continue;
		}
	...
	if (!worker->haveWork())
	{
		...
		if (!mDebugPause)
		{
			LL_DEBUGS("TextureFetch") << "Adding work for inactive worker: "
									  << id << LL_ENDL;
			worker->addWork(0, LLWorkerThread::PRIORITY_HIGH |
							   worker->mWorkPriority);
		}
		return false;
	}
...
// Replicates and expands upon the base class's getPending() implementation.
// getPending() and runCondition() replicate one another's logic to an extent
// and are sometimes used for the same function (deciding whether or not to
// sleep/pause a thread). So the implementations need to stay in step, at least
// until this can be refactored and the redundancy eliminated.
...
{
	static LLCachedControl<F32> band_width(gSavedSettings,
...
void LLTextureFetch::shutDownTextureCacheThread()
{
	if (mTextureCache)
	{
		llassert_always(mTextureCache->isQuitting() ||
						mTextureCache->isStopped());
		mTextureCache = NULL;
	}
}
...
void LLTextureFetch::shutDownImageDecodeThread()
{
	...
	{
		llassert_always(mImageDecodeThread->isQuitting() ||
						mImageDecodeThread->isStopped());
...
void LLTextureFetch::sendRequestListToSimulators()
{
		 ...
		 iter != mNetworkQueue.end(); )
	{
		...
		{
			F32 delta_priority = fabsf(req->mRequestedPriority - req->mImagePriority);
		...
		if (worker->mFileSize > 0)
		{
			if (state == LLTextureFetchWorker::LOAD_FROM_SIMULATOR)
			{
				S32 data_size = FIRST_PACKET_SIZE +
								(worker->mLastPacket - 1) * MAX_IMG_PACKET_SIZE;
...
void LLTextureFetch::dump()
{
	...
	LLQueuedThread::QueuedRequest* qreq;
	LLWorkerThread::WorkRequest* wreq;
	for (request_queue_t::iterator iter = mRequestQueue.begin(),
								   end = mRequestQueue.end();
		 iter != end; ++iter)
	{
		qreq = *iter;
		if (!qreq) continue;
		wreq = (LLWorkerThread::WorkRequest*)qreq;
		worker = (LLTextureFetchWorker*)wreq->getWorkerClass();
		if (worker)
		{
			llinfos << " ID: " << worker->mID << " - PRI: "
					<< llformat("0x%08x", wreq->getPriority()) << " - STATE: "
					<< e_state_name[worker->mState] << llendl;
		}
	}
}
...
// operations. Even done this way, I'm not fully trusting we are truly safe.
// This module is due for a major refactoring and we'll deal with it then.
//
// Threads: Ttf
...
void LLTextureFetch::releaseHttpWaiters()
{
	...
	if (needed <= 0)
	{
		// Would only happen if High/LowWater were changed behind our back. In
		// that case, defer fill until usage falls within limits.
		return;
	}

	// Quickly make a copy of all the LLUIDs. Get off the mutex as early as
	// possible.
	uuid_vec_t tids;
	{
		mNetworkQueueMutex.lock();
	...
	// Now lookup the UUUIDs to find valid requests and sort them in priority
	// order, highest to lowest. We are going to modify priority later as a
	// side-effect of releasing these objects. That, in turn, would violate the
	// partial ordering assumption of std::set, std::map, etc, so we do not use
	// those containers. We use a vector and an explicit sort to keep the
	// containers valid later.
	typedef std::vector<LLTextureFetchWorker*> worker_list_t;
	worker_list_t tids2;
	tids2.reserve(tids.size());
	for (uuid_vec_t::iterator iter = tids.begin(), end = tids.end();
		 iter != end; ++iter)
	{
		...
		{
			// If worker is not found, this should be due to a request for
			// deletion. We signal our recognition that this uuid should not be
			// used for resource waiting anymore by erasing it from the
			// resource waiter list. That allows deleteOK to do final deletion
			// on the worker.
	...
	if ((S32)tids2.size() > needed)
	{
		worker_list_t::iterator begin = tids2.begin();
		LLTextureFetchWorker::Compare compare;
		std::partial_sort(begin, begin + needed, tids2.end(), compare);
	}

	// Release workers up to the high water mark. Since we are not holding any
	// locks at this point, we can be in competition with other callers. Do
	// defensive things like getting refreshed counts of requests and checking
	// if someone else has moved any worker state around....
	for (worker_list_t::iterator iter2 = tids2.begin(), end = tids2.end();
		 iter2 != end; ++iter2)
	{
		...
		if (LLTextureFetchWorker::WAIT_HTTP_RESOURCE2 != worker->mState)
		{
			...
			worker->unlockWorkMutex();
			if (!gTeleportDisplay)	// Normal when force-removing fetches
			{
				llwarns << "Resource-waited texture " << worker->mID
						<< " in unexpected state: " << worker->mState
						<< ". Removing from wait list." << llendl;
			}
...
// Threads: T*
S32 LLTextureFetch::getHttpWaitersCount()
{
	mNetworkQueueMutex.lock();
	S32 ret = mHttpWaitResource.size();
	mNetworkQueueMutex.unlock();
	return ret;
}
