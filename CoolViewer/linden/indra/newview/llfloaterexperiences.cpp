...
bool LLFloaterExperiences::postBuild()
{
	...
	LLEventPump& pump = LLEventPumps::getInstance()->obtain("experience_permission");
...
void LLFloaterExperiences::onOpen()
{
	...
	if (region)
	{
		...
		region->setCapsReceivedCB(boost::bind(&LLFloaterExperiences::refreshContents,
											  this));
	}
}
												  ...
												  Callback_t cb)
{
	invokationFn_t getfn =
		boost::bind(COROCAST2(&LLCoreHttpUtil::HttpCoroutineAdapter::getAndSuspend),
					...
					// _4 -> headers
					_1, _2, _3, _4);
	LLCoros::getInstance()->launch("LLFloaterExperiences::retrieveExperienceList",
								   boost::bind(&LLFloaterExperiences::retrieveExperienceListCoro,
											   url, handle, tab_map,
											   error_notify, cb, getfn));
}
												...
												Callback_t cb)
{
	invokationFn_t postfn =
		boost::bind(COROCAST(&LLCoreHttpUtil::HttpCoroutineAdapter::postAndSuspend),
					...
					// _4 -> headers
					_1, _2, LLSD(), _3, _4);
	LLCoros::getInstance()->launch("LLFloaterExperiences::requestNewExperience",
								   boost::bind(&LLFloaterExperiences::retrieveExperienceListCoro,
											   url, handle, tab_map,
											   error_notify, cb, postfn));
}
