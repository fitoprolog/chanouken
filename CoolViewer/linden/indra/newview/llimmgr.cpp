...
bool friendship_offer_callback(const LLSD& notification, const LLSD& response)
{
	...
	if (option == 0)		// Accept
	{
		...
		LLCoros::getInstance()->launch("acceptFriendshipOffer",
									   boost::bind(&accept_friendship_coro,
												   url, payload));
	}
	else if (option == 1)	// Decline
	{
		...
		LLCoros::getInstance()->launch("declineFriendshipOffer",
									   boost::bind(&decline_friendship_coro,
												   url, payload));
	}
			...
			else
			{
				...
				if (!url.empty())
				{
					gIMMgrp->addSession(payload["session_name"].asString(), type,
									   session_id);
					LLCoros::getInstance()->launch("chatterBoxInvitationCoro",
									   ...
									   EInstantMessage dialog)
{
	...
	if (!url.empty())
	{
		LLCoros::getInstance()->launch("startConferenceCoro",
									   boost::bind(&LLIMMgr::startConferenceCoro,
												   url, temp_session_id,
												   gAgentID,
...
void LLIMMgr::setFloaterOpen(bool set_open)
{
	if (set_open)
	{
		...
		bool active_is_im = floater_current &&
							(floater_current->getName() == "im_floater" ||
		...
		for (S32 i = 0; i < floater_chatterbox->getFloaterCount(); i++)
		{
			...
			{
				// Only LLFloaterIMSessions are called "im_floater"
...
bool LLIMMgr::requestOfflineMessages()
{
	...
	LL_DEBUGS("InstantMessaging") << "Using capability for offline instant messages request"
								  << LL_ENDL;
	LLCoros::getInstance()->launch("requestOfflineMessagesCoro",
								   boost::bind(&LLIMMgr::requestOfflineMessagesCoro,
											   cap_url));
	return true;
}
...
void LLIMMgr::requestOfflineMessagesCoro(const std::string& url)
{
	...
	if (!contents.size())
	{
		llwarns << "No contents received for offline messages via capability"
				<< llendl;
		return;
	}
	...
	// Agent region could have changed...
		 ...
		 it != end; ++it)
	{
		...
		if (message_data.has("offline"))
		{
			...
			LL_DEBUGS("InstantMessaging") << "Found 'offline': " << im_type
										  << LL_ENDL;
		}
