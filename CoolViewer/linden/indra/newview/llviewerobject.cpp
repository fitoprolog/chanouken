											 ...
											 S32 flags)
{
	...
	switch (pcode)
	{
		case LL_PCODE_VOLUME:
			res = new LLVOVolume(id, pcode, regionp);
			break;
		...
		case LL_PCODE_LEGACY_AVATAR:
		{
			...
			{
				LLVOAvatarUI* avatar = new LLVOAvatarUI(id, pcode, regionp);
				avatar->initInstance();
				res = avatar;
			}
			else if (flags & CO_FLAG_PUPPET_AVATAR)
			{
				LLVOAvatarPuppet* avatar = new LLVOAvatarPuppet(id, pcode,
																regionp);
				avatar->initInstance();
				res = avatar;
			}
		...
		case LL_PCODE_LEGACY_GRASS:
			res = new LLVOGrass(id, pcode, regionp);
			break;

		case LL_PCODE_LEGACY_PART_SYS:
			llwarns_once << "Old particle system requested: ignoring."
						 << llendl;
			//res = new LLVOPart(id, pcode, regionp);
			break;

		case LL_PCODE_LEGACY_TREE:
			res = new LLVOTree(id, pcode, regionp);
			break;

		case LL_VO_CLOUDS:
			res = new LLVOClouds(id, pcode, regionp);
			break;

		case LL_VO_SURFACE_PATCH:
			res = new LLVOSurfacePatch(id, pcode, regionp);
			break;

		case LL_VO_SKY:
			res = new LLVOSky(id, pcode, regionp);
			break;

		case LL_VO_VOID_WATER:
			res = new LLVOVoidWater(id, pcode, regionp);
			break;

		case LL_VO_WATER:
			res = new LLVOWater(id, pcode, regionp);
			break;

		case LL_VO_GROUND:
			res = new LLVOGround(id, pcode, regionp);
			break;

		case LL_VO_PART_GROUP:
			res = new LLVOPartGroup(id, pcode, regionp);
			break;

		case LL_VO_HUD_PART_GROUP:
			res = new LLVOHUDPartGroup(id, pcode, regionp);
			break;

		case LL_VO_WL_SKY:
			res = new LLVOWLSky(id, pcode, regionp);
			break;

		default:
			llwarns_once << "Unknown object pcode: " << (S32)pcode << llendl;
	}
...
//mk
				...
				for (S32 i = 0; i < LL_EPARAMS_COUNT; ++i)
				{
					...
					{
						// Send an update message in case it was formerly in
						// use
...
//mk
	...
	// because it's not set for velocity interpolation, causing much jumping
	// and hopping around...
#if 0
...
void LLViewerObject::loadFlags(U32 flags)
{
	...
	LL_DEBUGS("ObjectCache") << "Flags for object " << mID << " set to: "
							 << flags << LL_ENDL;
...
void LLViewerObject::idleUpdate(const F64& time)
{
	if (!mDead)
	{
...
void LLViewerObject::doUpdateInventory(LLPointer<LLViewerInventoryItem>& item,
									   U8 key, bool is_new)
{
	...
	if (!is_new && mInventory)
	{
		...
		// permissions, we have perfect visibility, so we want the serial
		// number to match. Otherwise, take our best guess and make sure that
		// the serial number does not match.
		deleteInventoryItem(item_id);
		LLPermissions perm(item->getPermissions());
		LLPermissions* obj_perm = gSelectMgr.findObjectPermissions(this);
		bool is_atomic = item->getType() != (S32)LLAssetType::AT_OBJECT;
		if (obj_perm)
		{
			perm.setOwnerAndGroup(LLUUID::null, obj_perm->getOwner(),
								  obj_perm->getGroup(), is_atomic);
		}
		else
		{
			if (group_owned)
			{
				perm.setOwnerAndGroup(LLUUID::null, new_owner, new_group,
									  is_atomic);
			}
			else if (new_owner.notNull())
			{
				// The object used to be in inventory, so we can assume the
				// owner and group will match what they are there.
				perm.setOwnerAndGroup(LLUUID::null, new_owner, new_group,
									  is_atomic);
			}
			// *FIXME: could make an even better guess by using the mPermGroup
			// flags
			else if (permYouOwner())
			{
				// Best guess.
				perm.setOwnerAndGroup(LLUUID::null, gAgentID,
									  item->getPermissions().getGroup(),
									  is_atomic);
		...
		LLViewerInventoryItem* old_item = item;
		LLViewerInventoryItem* new_item = new LLViewerInventoryItem(old_item);
		new_item->setPermissions(perm);
		mInventory->emplace_front(new_item);
		doInventoryCallback();
		++mExpectedInventorySerialNum;
	}
	else if (is_new)
	...
	if (puppet)
	{
		...
		if (!puppet->mPlaying)
		{
...
#endif
			{
				...
				puppet->mRootVolp->recursiveMarkForUpdate(true);
			}
		}
	}
	else
...
struct LLFilenameAndTask
{
	...
	{
		++sCount;
		LL_DEBUGS("ViewerObject") << "Constructing LLFilenameAndTask: "
								  << sCount << LL_ENDL;
	}
	~LLFilenameAndTask()
	{
		--sCount;
		LL_DEBUGS("ViewerObject") << "Destroying LLFilenameAndTask: "
								  << sCount << LL_ENDL;
	}
...
void LLViewerObject::processTaskInv(LLMessageSystem* msg, void**)
{
	...
	msg->getStringFast(_PREHASH_InventoryData, _PREHASH_Filename,
					   unclean_filename);
										...
										LLExtStat ext_status)
{
		...
		ft->mSerial >= object->mInventorySerialNum)
	{
		...
		LL_DEBUGS("ViewerObject") << "Receiving inventory task file for serial: "
								  << object->mInventorySerialNum
								  << " - Expected serial: "
								  << object->mExpectedInventorySerialNum
...
{
	std::string filename_and_local_path =
		gDirUtilp->getExpandedFilename(LL_PATH_CACHE, filename);
	...
	if (ifs.is_open())
	{
		...
		while (ifs.good())
		{
			...
			if (strcmp("inv_item", keyword) == 0)
			{
				...
				mInventory->emplace_front(std::move(inv));
			}
			...
			{
				llwarns << "Too many unknown token in inventory file: "
						<< filename << ". Aborting." << llendl;
			}
			else
			{
				llwarns << "Unknown token '" << keyword
						<< "' in inventory file: " << filename << llendl;
			}
		}
		ifs.close();
...
void LLViewerObject::updateInventory(LLViewerInventoryItem* item, U8 key,
									 bool is_new)
{
...
LLViewerInventoryItem* LLViewerObject::getInventoryItemByAsset(const LLUUID& asset_id)
{
	...
	if (mInventory && asset_id.notNull())
	{
		for (LLInventoryObject::object_list_t::const_iterator
				it = mInventory->begin(), end = mInventory->end();
			 it != end; ++it)
		{
			...
			{
				LLViewerInventoryItem* item = obj->asViewerInventoryItem();
...
void LLViewerObject::setPixelAreaAndAngle()
{
	...
	// detail sometimes, but that's better than not enough. I do not think
	...
	if (num_tes != old_num_tes)
	{
		if (num_tes)
		{
			...
			for (U32 i = 0; i < num_tes; ++i)
			{
				...
				{
					new_images[i] = mTEImages[old_num_tes - 1];
					new_normmaps[i] = mTENormalMaps[old_num_tes - 1];
					new_specmaps[i] = mTESpecularMaps[old_num_tes - 1];
				}
				else
				{
					new_images[i] = NULL;
...
void LLViewerObject::setTEImage(U8 te, LLViewerTexture* imagep)
{
	if (te != 255 && te < getNumTEs() && imagep && mTEImages[te] != imagep)
	{
		LLUUID old_image_id;
		LLTextureEntry* tep = getTE(te);
		if (tep)
		...
		const LLUUID& image_id = imagep->getID();
		LLPrimitive::setTETexture(te, image_id);

		LLViewerTexture* baked_tex = getBakedTextureForMagicId(image_id);
		mTEImages[te] = baked_tex ? baked_tex : imagep;
		updateAvatarMeshVisibility(image_id, old_image_id);
		setChanged(TEXTURE);
...
S32 LLViewerObject::setTETextureCore(U8 te, LLViewerTexture* image)
{
	const LLTextureEntry* tep = getTE(te);
	if (!tep || !image) return 0;

	const LLUUID& image_id = image->getID();
	LLUUID old_image_id = tep->getID();
	if (image_id.notNull() && old_image_id == image_id)
	{
		return 0;
	}

	S32 retval = LLPrimitive::setTETexture(te, image_id);

	LLViewerTexture* baked_tex = getBakedTextureForMagicId(image_id);
	mTEImages[te] = baked_tex ? baked_tex : image;
	updateAvatarMeshVisibility(image_id, old_image_id);
	setChanged(TEXTURE);
...
S32 LLViewerObject::setTENormalMapCore(U8 te, LLViewerTexture* image)
{
	const LLTextureEntry* tep = getTE(te);
	if (!tep) return 0;

	const LLUUID& uuid = image ? image->getID() : LLUUID::null;
	if (tep->getID() != uuid || uuid.isNull())
	{
		LLMaterial* mat = tep->getMaterialParams();
		if (mat)
		{
			LL_DEBUGS("Materials") << "te = "<< (S32)te
								   << ", setting normal map id = " << uuid
								   << LL_ENDL;
...
S32 LLViewerObject::setTESpecularMapCore(U8 te, LLViewerTexture* image)
{
	const LLTextureEntry* tep = getTE(te);
	if (!tep) return 0;
...
//virtual
S32 LLViewerObject::setTETexture(U8 te, const LLUUID& uuid)
{
	// Invalid host == get from the agent's sim
	LLViewerFetchedTexture* image =
		LLViewerTextureManager::getFetchedTexture(uuid, FTT_DEFAULT, true,
												  LLGLTexture::BOOST_NONE,
												  LLViewerTexture::LOD_TEXTURE,
												  0, 0, LLHost());
...
//virtual
S32 LLViewerObject::setTENormalMap(U8 te, const LLUUID& uuid)
{
...
//virtual
S32 LLViewerObject::setTESpecularMap(U8 te, const LLUUID& uuid)
{
	LLViewerFetchedTexture* image = NULL;
	if (uuid.notNull())
	{
...
bool LLViewerObject::isImageAlphaBlended(U8 te) const
{
	...
	GLenum format = image->getPrimaryFormat();
	switch (format)
	{
		case GL_RGB:
			break;
...
LLHUDIcon* LLViewerObject::setIcon(LLViewerTexture* icon_image, F32 scale)
{
	if (!mIcon)
	{
									   ...
									   bool local_origin)
{
	...
	if (*in_use && new_value == *param)
	{
		return false;
	}
	*in_use = true;
	param->copy(new_value);
	parameterChanged(param_type, param, true, local_origin);
	return true;
}
											...
											bool local_origin)
{
	...
	{
		*is_in_use = in_use;
		parameterChanged(param_type, param, in_use, local_origin);
		return true;
	}
	return false;
}
...
public:
			  ...
			  const LLSD& input) const override
	{
		...
		for (S32 i = 0; i < num_entries; ++i)
		{
			...
			if (node)
			{
				// The LLSD message builder does not know how to handle U8, so
				// we need to send as S8 and cast
