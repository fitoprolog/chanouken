...
bool HBFloaterSoundsList::postBuild()
{
	...
	mBlockSoundBtn = getChild<LLButton>("block");
	mBlockSoundBtn->setClickedCallback(onBlockSoundBtn, this);
...
//static
void HBFloaterSoundsList::onBlockSoundBtn(void* userdata)
{
	HBFloaterSoundsList* self = (HBFloaterSoundsList*)userdata;
 	if (!self) return;
	...
	LLAudioData::blockSound(sound_id, !LLAudioData::isBlockedSound(sound_id));
	self->mIsDirty = true;
	self->mLastUpdate = 0.f;		// Force an immediate update
	self->setButtonsStatus();
}
