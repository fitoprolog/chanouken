...
#include "llrender.h"
#include "llrenderutils.h"					// gBox, gSphere
...
#include "lltoolfocus.h"					// LLToolCamera::getInstance()
#include "lltoolmgr.h"
...
class LLDebugText
{
      ...
      mMinX(U32_MAX)
  {
    ...
    void update()
    {
      ...
      if (debug_show_fps)
      {
        LLViewerStats* vstats = LLViewerStats::getInstance();
        addText(window_width - 60, ypos,
            llformat("%d fps",
              (S32)(vstats->mFPSStat.getMeanPerSec() + 0.5f)));
        ypos += mIncY;
      }
...
#undef SETENUM
      ...
      {
        if (!gPipeline.getUseVertexShaders())
        {
          ...
          if (selected_objects->getObjectCount() == 0)
          {
            ...
            if (region)
            {
                  ...
                  i < count; ++i)
              {
                ...
                if (object && object->getRegion() == region &&
                    object->getVolume())
                {
                  ++object_count;
                  S32 bytes = 0;
        ...
        addText(xpos, ypos,
            llformat("%d MB vertex data",
              LLVertexBuffer::sAllocatedBytes / (1024 * 1024)));
        ypos += mIncY;

        addText(xpos, ypos,
            llformat("%d vertex buffers", LLVertexBuffer::sGLCount));
        ypos += mIncY;

        addText(xpos, ypos,
            llformat("%d mapped buffers",
              LLVertexBuffer::sMappedCount));
        ypos += mIncY;

        addText(xpos, ypos,
            llformat("%d vertex buffer binds",
              LLVertexBuffer::sBindCount));
        ypos += mIncY;
...
void LLViewerWindow::adjustRectanglesForFirstUse()
{
  ...
  LLRect view_rect = gFloaterViewp->getRect();
...
void LLViewerWindow::reshape(S32 width, S32 height)
{
  ...
  if (!LLApp::isExiting())
  {
    ...
    LLViewerStats* vstats = LLViewerStats::getInstance();
    vstats->setStat(LLViewerStats::ST_WINDOW_WIDTH, (F64)width);
    vstats->setStat(LLViewerStats::ST_WINDOW_HEIGHT, (F64)height);
...
bool LLViewerWindow::handleKey(KEY key, MASK mask)
{
  ...
  // Give main menus a chance to handle keys
  ...
  // Give login menus a chance to handle keys
...
bool LLViewerWindow::handleUnicodeChar(llwchar uni_char, MASK mask)
{
  ...
  // *HACK: for some reason, some keys cause both handleKey() and
  // handleUnicodeChar() to be called, so we must sort out this issue here...
  if (mask & (MASK_CONTROL | MASK_ALT))
  {
    KEY key = uni_char & 0xFFFF;
    if (sLastAcceleratorKey == key
        // *HACK: for AZERTY PC keyboards and ² -> ` key translation (see
        // LLKeyboard::translateKey()).
#if LL_LINUX
        || (sLastAcceleratorKey == 0x60 && key == 0xb2)
#elif LL_WINDOWS
        || (sLastAcceleratorKey == 0x60 && key == 0xde)
#endif
       )
    {
      // When the key corresponds to an accelerator, we must not continue
      // processing it here, else it might get wrongly stuffed in a text
      // or line input field...
      if (LLView::sDebugKeys)
      {
    ...
    if (gMenuBarViewp &&
        gMenuBarViewp->handleAcceleratorKey(key, mask))
    {
      if (LLView::sDebugKeys)
      {
        llinfos << "Key handled by the menu accelerators" << llendl;
  ...
  if (gMenuBarViewp && gMenuBarViewp->handleUnicodeChar(uni_char, true))
  {
    return true;
  }
  ...
  // See if the chat bar needs to be auto-focused
    ...
    bool down)
{
  ...
  if (down)
  {
    ...
    if (LLToolMgr::getInstance()->getCurrentTool()->handleAnyMouseClick(x,
          y,
          mask,
          clicktype,
          down))
...
void LLViewerWindow::handleFocus(LLWindow* window)
{
  ...
  // Resume foreground running timer since we artifically limit framerate
  // when not frontmost
...
bool LLViewerWindow::handleTranslatedKeyDown(KEY key, MASK mask, bool repeated)
{
  ...
  // really low, hitting return sends your chat text before it's all
  // entered/processed.
...
void LLViewerWindow::handlePingWatchdog(LLWindow* window, const char* msg)
{
  ...
  message.assign(msg);
  gAppViewerp->pingMainloopTimeout(&message);
}
...
bool LLViewerWindow::handlePerFrameHover()
{
  ...
  mMouseVelocityStat.addValue(mouse_vel.length());
  ...
  if (gHoverViewp)
  {
    ...
    mSuppressToolbox = toolmgr->getBaseTool() == toolpie &&
      toolmgr->getCurrentTool() != toolpie;
      ...
      tool != LLToolDragAndDrop::getInstance() && !LLPipeline::sFreezeTime)
  {
    ...
    if (gFloaterToolsp->isMinimized() ||
        (tool != toolpie &&	// not default tool
         ...
         toolmgr->getCurrentToolset() != gFaceEditToolset &&
  ...
  if (LLEditMenuHandler::gEditMenuHandler == NULL &&
      gSelectMgr.getSelection()->getObjectCount())
  {
    ...
    bool for_hud)
{
  ...
  bool in_edit = LLToolMgr::getInstance()->inEdit();
  ...
  else if (!gSelectMgr.getSelection()->isEmpty())
  {
    ...
    if (!sel_is_mod_ok && tool == LLToolCompScale::getInstance())
    {
      draw_handles = false;
    }
...
void LLViewerWindow::movieSize(S32 new_width, S32 new_height)
{
      ...
      size.mY != new_height + border_height)
  {
    ...
    border_width = size.mX - gViewerWindowp->getWindowDisplayWidth();
    border_height = size.mY- gViewerWindowp->getWindowDisplayHeight();
    ...
    if (gWindowp->getFullscreen())
    {
      gViewerWindowp->changeDisplaySettings(false, new_size,
          disable_sync, true);
    }
    else
    {
...
bool LLViewerWindow::thumbnailSnapshot(LLImageRaw* raw, S32 preview_width,
    ...
    bool do_rebuild, ESnapshotType type)
{
...
bool LLViewerWindow::rawSnapshot(LLImageRaw* raw,
    ...
    bool show_ui, bool do_rebuild,
    ESnapshotType type, S32 max_size)
{
  if (!raw)
  {
    return false;
  ...
  {
    // Snapshot taking is disabled due to memory restriction.
    llwarns << "Not enough free memory to take a snapshot !" << llendl;
    return false;
  }
  ...
  gObjectList.generatePickList(gViewerCamera);
    ...
    for (S32 subimage_x = 0; subimage_x < scale_factor; ++subimage_x)
    {
      ...
      if (read_width && read_height)
      {
        ...
        for (U32 out_y = 0; out_y < read_height; ++out_y)
        {
          ...
          // Ping the watchdog thread every 100 lines to keep us
          // alive (arbitrary number, feel free to change)
          if (out_y % 100 == 0)
          {
            static const std::string LLViewerWindowRawSnapshot =
              "LLViewerWindow:rawSnapshot";
            gAppViewerp->pingMainloopTimeout(&LLViewerWindowRawSnapshot);
          }
...
void LLViewerWindow::stopGL(bool save_state)
{
  if (!gGLManager.mIsDisabled)
  {
    ...
    llinfos << "Remaining allocated texture memory: "
      << LLImageGL::sGlobalTextureMemoryInBytes << " bytes."
      << llendl;
  }
}
...
void LLViewerWindow::restoreGL(const std::string& progress_message)
{
  if (gGLManager.mIsDisabled)
  {
    ...
    // HACK: now that GL is restarted, we can reset that flag
...
void LLViewerWindow::toggleFullscreen(bool show_progress)
{
  if (gWindowp)
  {
    mWantFullscreen = mIsFullscreenChecked = !gWindowp->getFullscreen();
    mShowFullscreenProgress = show_progress;
  }
}

void LLViewerWindow::getTargetWindow(bool& fullscreen, S32& width,
    S32& height) const
{
  fullscreen = mWantFullscreen;

  if (gWindowp && (bool)gWindowp->getFullscreen() == mWantFullscreen)
  {
...
{
  mResDirty = true;
  mWantFullscreen = mIsFullscreenChecked = fullscreen_checked;
}
...
bool LLViewerWindow::checkSettings()
{
  ...
  bool is_fullscreen = gWindowp->getFullscreen();

  // *HACK: to avoid crashes in OpenGL, it is necessary to disable VBOs
  // before changing to/from full screen mode after login. LL implemented
  // another hack (an arbitrary 30 frames delay with VBO off, via
  // sDelayedVBOEnable in LLPipeline), but it is not sufficient to avoid a
  // crash once logged in. For the latter case, we now make sure that VBOs
  // are cleanly disabled before we attempt to change the full screen mode,
  // then we let the mode change happen, and finally we reenable VBOs (sadly,
  // this method does not work at login time and LL's hack is still needed
  // for it). HB
  if (LLStartUp::isLoggedIn() && mWantFullscreen != is_fullscreen &&
      LLVertexBuffer::sEnableVBOs)
  {
    // Note: LLVertexBuffer::sEnableVBOs will sync with RenderVBOEnable
    // via LLPipeline::doResetVertexBuffers(), called from display(), on
    // next frame.
    if (gSavedSettings.getBool("RenderVBOEnable"))
    {
      gSavedSettings.setBool("RenderVBOEnable", false);
      mRestoreVBO = true;
    }
    // Wait until VBOs got actually turned off...
    return false;
  }
  if (mRestoreVBO && mWantFullscreen == is_fullscreen)
  {
    // The full screen mode change happened: we can now restore VBOs...
  ...
  if (mWantFullscreen)
  {
    ...
    LLCoordScreen desired_size(gSavedSettings.getS32("FullScreenWidth"),
        gSavedSettings.getS32("FullScreenHeight"));
    gWindowp->getSize(&screen_size);
    if (!is_fullscreen || screen_size.mX != desired_size.mX ||
        screen_size.mY != desired_size.mY)
    {
      ...
      changeDisplaySettings(true, desired_size,
          gSavedSettings.getBool("DisableVerticalSync"),
...
bool LLViewerWindow::changeDisplaySettings(bool fullscreen, LLCoordScreen size,
    bool disable_vsync,
    bool show_progress_bar)
{
  ...
  bool old_fullscreen = gWindowp->getFullscreen();
  ...
  // Going from windowed to non-windowed
  ...
  bool got_position = gWindowp->getPosition(&old_pos);
  ...
  if (!result_first_try)
  {
    ...
    result_second_try = gWindowp->switchContext(old_fullscreen, old_size,
        disable_vsync);
  ...
  if (!gWindowp->getFullscreen() && success)
  {
  ...
  mWantFullscreen = gWindowp->getFullscreen();
  mShowFullscreenProgress = false;
...
void LLPickInfo::getSurfaceInfo()
{
  ...
  if (gViewerWindowp->cursorIntersect(ll_round((F32)mMousePt.mX),
        ...
        &mObjectFace, &intersection,
        &mSTCoords, &normal, &tangent))
  {
