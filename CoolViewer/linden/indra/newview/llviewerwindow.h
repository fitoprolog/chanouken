...
public:
	LLViewerWindow(const std::string& title, S32 x, S32 y, U32 width,
				   U32 height, bool fullscreen, bool ignore_pixel_depth);
	...
	void handlePingWatchdog(LLWindow* window, const char* msg) override;
	void handlePauseWatchdog(LLWindow* window) override;
	void handleResumeWatchdog(LLWindow* window) override;
	...
	LL_INLINE LLStat* getMouseVelocityStat()			{ return &mMouseVelocityStat; }
	...
	bool thumbnailSnapshot(LLImageRaw* raw, S32 width, S32 height,
						   bool show_ui, bool do_rebuild, ESnapshotType type);
	...
	void toggleFullscreen(bool show_progress = false);

	// Handle shutting down GL and bringing it back up
	void requestResolutionUpdate(bool fullscreen_checked);
	bool checkSettings();
	void restartDisplay(bool show_progress_bar);
	bool changeDisplaySettings(bool fullscreen, LLCoordScreen size,
							   bool disable_vsync, bool show_progress_bar);
