...
void LLFloaterBuyLand::refreshUI()
{
	...
	if (showing_error)
	{
		mChildren.setBadge("step_error",
						   mCannotBuyIsError ? LLViewChildren::BADGE_ERROR
	...
	if (!showing_error)
	{
		mChildren.setBadge("step_1",
						   mSiteMembershipUpgrade ? LLViewChildren::BADGE_NOTE
	...
	if (!showing_error)
	{
		mChildren.setBadge("step_2",
						   mSiteLandUseUpgrade ? LLViewChildren::BADGE_NOTE
											   : LLViewChildren::BADGE_OK);
	...
	if (!showing_error)
	{
		mChildren.setBadge("step_3",
						   !enough_money ? LLViewChildren::BADGE_WARN
										   : mCurrency.getAmount() > 0 ? LLViewChildren::BADGE_NOTE
																	   : LLViewChildren::BADGE_OK);
