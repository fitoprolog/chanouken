...
private:
	// Not using fast_hset here because some fast containers hashes (e.g.
	// hopscotch_hash) do not manage to deal with it ("incomplete type struct
	// std::hash<LLMeshRepoThread::UUIDBasedRequest>" complaints even though a
	// valid std::hash template is provided for it)...
	typedef boost::unordered_set<UUIDBasedRequest> base_requests_set_t;
...
public:
	...
	typedef std::set<LLCore::HttpHandler::ptr_t> http_request_t;
	http_request_t					mHttpRequestSet;

	// Map of pending header requests and currently desired LODs
	typedef std::map<LLVolumeParams, std::vector<S32> > pending_lod_map_t;
...
public:
	...
	typedef std::map<LLVolumeParams, uuid_list_t> mesh_load_map_t;
	mesh_load_map_t								mLoadingMeshes[4];

	typedef fast_hmap<LLUUID, LLMeshSkinInfo> skin_map_t;
	skin_map_t									mSkinMap;
	...
	typedef fast_hmap<LLUUID, uuid_list_t> skin_load_map_t;
