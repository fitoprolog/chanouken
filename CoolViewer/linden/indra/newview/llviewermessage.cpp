...
public:
	...
	void done() override
	{
		LL_DEBUGS("Messaging") << "Discard done, Scheduling removal of item "
							 << mObjectID << LL_ENDL;
...
private:
	...
	{
		LL_DEBUGS("Messaging") << "Removing item " << mObjectID << LL_ENDL;
...
bool check_offer_throttle(const std::string& from_name, bool check_only)
{
	...
	{
		LL_DEBUGS("Messaging") << "Throttle Not Expired, Count: "
						  ...
						  const std::string& from_name)
{
	...
	// because we must call the proper virtual method in the test for:
	// item->getInventoryType() == LLInventoryType::IT_ATTACHMENT
	LLViewerInventoryItem* item;
	for (uuid_vec_t::const_iterator it = items.begin(), end = items.end();
		 it != end; ++it)
	{
		item = gInventory.getItem(*it);
		if (!item)
		{
			llinfos << "Received item " << *it
			...
			if (check_offer_throttle(from_name, throttled))
			{
				switch (asset_type)
				{
					case LLAssetType::AT_NOTECARD:
						open_notecard((LLViewerInventoryItem*)item,
									  "Note: " + item->getName(), LLUUID::null,
									  true, LLUUID::null, false);
						break;
					case LLAssetType::AT_LANDMARK:
						open_landmark((LLViewerInventoryItem*)item,
									  "Landmark: " + item->getName(), true,
									  LLUUID::null, false);
						break;
					case LLAssetType::AT_TEXTURE:
						open_texture(*it, "Texture: " + item->getName(), true,
									 LLUUID::null, false);
						break;
					default: ;
				}
			}
			else	// Ff we are throttled, do not display them
			{
		...
		// or when the item is a calling card, or when the item is a newly
		// attached object (existing objects are reported as "new" items when
		// they get attached).
		...
		if (!user_is_away && gInventory.isObjectDescendentOf(*it, laf_id))
		{
			continue;
		}
	...
	if (inv)
	{
		...
		inv->getPanel()->setSelection(show_item, TAKE_FOCUS_NO);
...
//mk
	...
	switch (button)
	{
		case IOR_ACCEPT:
			...
			// inventory_offered, task_inventory_offer or
			// group_notice_inventory is 1 greater than the offer integer
			// value.
			...
			LL_DEBUGS("Messaging") << "Initializing an opener for tid: "
			...
			switch (mIM)
			{
				...
				case IM_GROUP_NOTICE_REQUESTED:
					...
					// observer gOpenTaskOffer. Since it already exists, we do
					// not need to actually do anything
					break;

				default:
					llwarns << "Unknown offer type" << llendl;
			}
//MK
			if (gRLenabled)
			{
...
//mk
		...
		case IOR_DECLINE:
			// DECLINE. The math for the dialog works, because the decline for
			// inventory_offered, task_inventory_offer or
			// group_notice_inventory is 2 greater than the offer integer
			// value.
			// Generates IM_INVENTORY_DECLINED, IM_TASK_INVENTORY_DECLINED or
...
void process_offer_callingcard(LLMessageSystem* msg, void**)
{
	...
	LL_DEBUGS("Messaging") << "Callingcard offer from source " << source_id
						   << LL_ENDL;
...
public:
	...
	void done() override
	{
		...
		for (folder_ref_t::iterator it = mCompleteFolders.begin(),
									end = mCompleteFolders.end();
			 it != end; ++it)
		{
			gInventory.collectDescendentsIf(*it, land_cats, land_items,
											LLInventoryModel::EXCLUDE_TRASH,
											is_landmark);
			gInventory.collectDescendentsIf(*it, card_cats, card_items,
											LLInventoryModel::EXCLUDE_TRASH,
											is_card);
		}
...
void process_crossed_region(LLMessageSystem* msg, void**)
{
	...
	{
		llwarns << "Incorrect agent or region Id. Ignoring message." << llendl;
		return;
	}
...
#if 0	// Not used...
	U64	space_time_usec;
	msg->getU64Fast(_PREHASH_TimeInfo, _PREHASH_UsecSinceStart,
					space_time_usec);
	gWorld.setSpaceTimeUSec(space_time_usec);

	U32 seconds_per_day;
	msg->getU32Fast(_PREHASH_TimeInfo, _PREHASH_SecPerDay, seconds_per_day);
	U32 seconds_per_year;
	msg->getU32Fast(_PREHASH_TimeInfo, _PREHASH_SecPerYear, seconds_per_year);
#endif
...
void process_kill_object(LLMessageSystem* msg, void**)
{
	...
	for (S32 i = 0; i < num_objects; ++i)
	{
		...
		if (id.isNull())
		{
			LL_DEBUGS("Messaging") << "Unknown kill for local " << local_id
								   << LL_ENDL;
			continue;
		}
		else
		{
			LL_DEBUGS("Messaging") << "Kill message for local " << local_id
								   << LL_ENDL;
		}
...
void process_sim_stats(LLMessageSystem* msg, void**)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		viewerstats->addSample(stat_id, stat_value);
	}
...
void process_object_animation(LLMessageSystem* msg, void**)
{
	...
	if (!volp)
	{
		llwarns << "Received animation state for non-volume object: " << uuid
				<< llendl;
		return;
	}
	...
	{
		llwarns << "Received animation state for non-animated object: " << uuid
				<< llendl;
		return;
	}
	...
	if (!avatarp)
	{
		llinfos << "No puppet avatar for object: " << uuid << ". Ignoring."
				<< llendl;
		return;
	}
...
void process_avatar_appearance(LLMessageSystem* msg, void**)
{
	...
	{
		llwarns << "avatar_appearance sent for unknown avatar " << uuid
				<< llendl;
	}
}
...
void block_experience(const LLUUID& experience, const LLSD& result)
{
	LLSD permission;
	...
	LLEventPumps::getInstance()->obtain("experience_permission").post(data);
}
...
bool script_question_cb(const LLSD& notification, const LLSD& response)
{
	...
	if (response["BlockExperience"])
	{
		...
		if (region && experience.notNull())
		{
			LLExperienceCache* exp = LLExperienceCache::getInstance();
			exp->setExperiencePermission(experience, "Block",
...
//mk
	...
	static LLStringThrottle question_throttle(LLREQUEST_PERMISSION_THROTTLE_LIMIT,
...
void container_inventory_arrived(LLViewerObject* object,
								 ...
								 void* data)
{
	LL_DEBUGS("Messaging") << "Got container inventory" << LL_ENDL;
	...
	if (inventory->size() > 2)
	{
			 ...
			 it != end; ++it)
		{
			if ((*it)->getType() != LLAssetType::AT_CATEGORY)
			{
				...
				item_id.generate();
				creation_date_utc = time_corrected();
				LLPointer<LLViewerInventoryItem> new_item =
					new LLViewerInventoryItem(item_id, cat_id,
											  item->getPermissions(),
											  item->getAssetUUID(),
											  item->getType(),
		...
		if (inv)
		{
			inv->getPanel()->setSelection(cat_id, TAKE_FOCUS_NO);
		}
	}
	else if (inventory->size() == 2)
	{
		// We are going to get one fake root category as well as the one actual
		// object
		...
		LLPointer<LLViewerInventoryItem> new_item =
			new LLViewerInventoryItem(item_id, cat_id, item->getPermissions(),
									  item->getAssetUUID(), item->getType(),
									  item->getInventoryType(),
									  item->getName(),
									  item->getDescription(),
									  LLSaleInfo::DEFAULT, item->getFlags(),
									  creation_date_utc);
		new_item->updateServer(true);
		gInventory.updateItem(new_item);
		gInventory.notifyObservers();
	...
	// We have got the inventory, now delete this object if this was a take
