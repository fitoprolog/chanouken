...
void LLToolPie::leftMouseCallback(const LLPickInfo& pick_info)
{
	...
	LLToolPie::getInstance()->handleLeftClickPick();
}
...
//mk
		 ...
		 (parent && !parent->isAvatar() && parent->flagUsePhysics())))
	{
		...
		LLToolGrab* toolgrab = LLToolGrab::getInstance();
		LLToolMgr::getInstance()->getCurrentToolset()->selectTool(toolgrab);
		return toolgrab->handleObjectHit(mPick);
	}
	...
	if (gSavedSettings.getBool("LeftClickSteersAvatar"))
	{
		...
		if (object && object == gAgentAvatarp)
		{
			...
			LLToolCamera* toolcamera = LLToolCamera::getInstance();
			LLToolMgr::getInstance()->setTransientTool(toolcamera);
			gViewerWindowp->hideCursor();
			toolcamera->setMouseCapture(true);
			toolcamera->pickCallback(mPick);
...
{
	LLToolPie::getInstance()->mPick = pick_info;
	LLToolPie::getInstance()->handleRightClickPick();
}
...
bool LLToolPie::handleRightClickPick()
{
	...
	else if (mPick.mPickType == LLPickInfo::PICK_LAND)
	{
		...
		LLHUDEffectSpiral* effectp =
			(LLHUDEffectSpiral*)LLHUDManager::createEffect(LLHUDObject::LL_HUD_EFFECT_SPHERE);
...
void LLToolPie::selectionPropertiesReceived()
{
	...
	LLObjectSelection* selection =
		LLToolPie::getInstance()->getLeftClickSelection();
	if (selection)
	{
...
//MK
				...
				if (gRLenabled &&
					!gRLInterface.canTouchFar(selected_object,
											  LLToolPie::getInstance()->getPick().mIntersection))
				{
					return;
				}
//mk
...
bool LLToolPie::handleMediaClick(const LLPickInfo& pick)
{
	...
	else if (gKeyboardp)
	{
		...
		LLEditMenuHandler::gEditMenuHandler = mfocus->getFocusedMediaImpl();
