...
{
	S32 row = S32(pos.mV[VY] * STEP_FACTOR);
	S32 column = S32(pos.mV[VX] * STEP_FACTOR);
	return mOwnership &&
		   (mOwnership[row * mParcelGridsPerEdge + column] &
			PARCEL_SOUND_LOCAL) != 0;
}
...
U8 LLViewerParcelOverlay::parcelLineFlags(S32 row, S32 col) const
{
	...
	{
		llwarns_once << "Attempted to get ownership out of regions overlay, row: "
					 << row << " - col: " << col << llendl;
		return mask;
	}
	return mOwnership[row * mParcelGridsPerEdge + col];
}
												...
												const F32* color) const
{
	...
	for (S32 i = 0; i <= mParcelGridsPerEdge; ++i)
	{
		...
		{
			U8 overlay = mOwnership[line_offset + j];
			...
			// 2D map.
			if (i != mParcelGridsPerEdge &&
				(j == mParcelGridsPerEdge || (overlay & PARCEL_WEST_LINE)))
			{
				// We have a left border: draw it
				gGL.vertex2f(left, bottom);
				gGL.vertex2f(left, bottom + map_parcel_width);
			}
			if (j != mParcelGridsPerEdge &&
				(i == mParcelGridsPerEdge || (overlay & PARCEL_SOUTH_LINE)))
			{
