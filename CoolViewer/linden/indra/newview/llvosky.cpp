...
bool clip_quad_to_horizon(F32& t_left, F32& t_right, LLVector3 v_clipped[4],
						  const LLVector3 v_corner[4], F32 cos_max_angle)
{
	...
	if (t_left >= 1.f || t_right >= 1.f)
	{
		return false;
	}

#if 0
	const bool left_clip = t_left > 0.f;
	const bool right_clip = t_right > 0.f;
	...
	else
	{
		...
		v_clipped[1] = left_clip ? ((1 - t_left) * v_corner[1] +
									 t_left * v_corner[0])
								 : v_corner[1];
		v_clipped[2] = v_corner[2];
		v_clipped[3] = right_clip ? ((1 - t_right) * v_corner[3] +
									 t_right * v_corner[2])
								  : v_corner[3];
	}
#endif
...
LL_INLINE LLColor3 refr_ind_calc(const LLColor3& wave_length)
{
	LLColor3 refr_ind;
	for (S32 i = 0; i < 3; ++i)
	{
		const F32 wl2 = wave_length.mV[i] * wave_length.mV[i] * 1e-6f;
		refr_ind.mV[i] = 6.43e3f + 2.95e6f / (146.f - 1.f / wl2) +
						 2.55e4f / (41.f - 1.f / wl2);
		refr_ind.mV[i] *= 1e-8f;
		refr_ind.mV[i] += 1.f;
	}
	return refr_ind;
...
static LLColor3 calc_air_sca_sea_level()
{
	...
	static LLColor3 n21 = refr_ind * refr_ind - LLColor3::white;
	static LLColor3 n4 = n21 * n21;
	static LLColor3 wl2 = WAVE_LEN * WAVE_LEN * 1e-6f;
	static LLColor3 wl4 = wl2 * wl2;
	static LLColor3 mult_const = fsigma * (2.f * F_PI * F_PI / 3.f * 1e24f) * n4;
	static F32 dens_div_N = F32(ATM_SEA_LEVEL_NDENS / Ndens2);
	return dens_div_N * componentDiv(mult_const, wl4);
}
...
LL_INLINE F32 LLHaze::calcPhase(F32 cos_theta) const
{
	const F32 g2 = mG * mG;
	const F32 den = 1.f + g2 - 2 * mG * cos_theta;
	return (1.f - g2) * gFastLn.pow(den, -1.5f);
}
...
LLVOSky::LLVOSky(const LLUUID& id, LLPCode pcode, LLViewerRegion* regionp)
...
void LLVOSky::init()
{
	...
	mHazeConcentration = haze_int / (color_intens(LLHaze::calcAirSca(0)) +
									 haze_int);
	...
	if (gUseNewShaders)
	{
		...
		for (S32 side = 0; side < NUM_CUBEMAP_FACES; ++side)
		{
			for (S32 tile = 0; tile < NUM_TILES; ++tile)
			{
				...
				if (has_sky)	// Paranoia
				{
					createSkyTextureEE(skyp, side, tile);
				}
			}
			mSkyTex[side].create();
...
void LLVOSky::initCubeMap()
{
	...
	else if (LLPipeline::sRenderWater && gGLManager.mHasCubeMap &&
...
void LLVOSky::restoreGL()
{
	...
	if (gUseNewShaders)
	{
		...
		if (skyp)	// Paranoia
		{
			setSunTextures(skyp->getSunTextureId(),
						   skyp->getNextSunTextureId());
	...
	else
	{
		...
		mMoonTexturep[0]->setAddressMode(LLTexUnit::TAM_CLAMP);
		mBloomTexturep[0] =
			LLViewerTextureManager::getFetchedTexture(IMG_BLOOM1);
	...
	if (LLPipeline::sRenderWater && gGLManager.mHasCubeMap &&
		LLCubeMap::sUseCubeMaps)
	{
		initCubeMap();
	}
...
void LLVOSky::createSkyTextureWL(S32 side, S32 tile)
{
	S32 tile_x = tile % NUM_TILES_X;
	S32 tile_y = tile / NUM_TILES_X;
	...
	for (S32 y = tile_y_pos; y < tile_y_pos + sTileResY; ++y)
	{
		for (S32 x = tile_x_pos; x < tile_x_pos + sTileResX; ++x)
		{
			...
			mSkyTex[side].setPixel(calcSkyColorInDirWL(sky_dir),  x, y);
#if 0		// This was the original code...
			mShinyTex[side].setPixel(calcSkyColorInDirWL(sky_dir, true),
									 x, y);
#else		// But EEP introduced this and it seems more logical !   HB
			const LLVector3& shiny_dir = mShinyTex[side].getDir(x, y);
			mShinyTex[side].setPixel(calcSkyColorInDirWL(shiny_dir, true),
									 x, y);
#endif
		}
	}
...
void LLVOSky::createSkyTextureEE(const LLSettingsSky::ptr_t& skyp,
								 S32 side, S32 tile)
{
	S32 tile_x = tile % NUM_TILES_X;
	S32 tile_y = tile / NUM_TILES_X;
	...
	for (S32 y = tile_y_pos; y < tile_y_pos + sTileResY; ++y)
	{
		...
		{
			const LLVector3& sky_dir = mSkyTex[side].getDir(x, y);
			mSkyTex[side].setPixel(calcSkyColorInDirEE(skyp, sky_dir), x, y);
			const LLVector3& shiny_dir = mShinyTex[side].getDir(x, y);
			mShinyTex[side].setPixel(calcSkyColorInDirEE(skyp, shiny_dir,
														 true),
									 x, y);
		}
	}
...
void LLVOSky::initAtmosphericsWL()
{
	bool err;
	// Uniform parameters for convenience
	mSunlight = LLColor3(gWLSkyParamMgr.mCurParams.getVector("sunlight_color",
						 err));
	mAmbient = LLColor3(gWLSkyParamMgr.mCurParams.getVector("ambient", err));
	mGamma = gWLSkyParamMgr.mCurParams.getVector("gamma", err)[0];
	mBlueDensity = LLColor3(gWLSkyParamMgr.mCurParams.getVector("blue_density",
							err));
	mBlueHorizon = LLColor3(gWLSkyParamMgr.mCurParams.getVector("blue_horizon",
																err));
	mHazeDensity = gWLSkyParamMgr.mCurParams.getVector("haze_density", err)[0];
	mHazeHorizon = LLColor3(gWLSkyParamMgr.mCurParams.getVector("haze_horizon",
																err)).mV[0];
	mDensityMultiplier =
			gWLSkyParamMgr.mCurParams.getVector("density_multiplier", err)[0];
	mMaxY = gWLSkyParamMgr.mCurParams.getVector("max_y", err)[0];
	mGlow = LLColor3(gWLSkyParamMgr.mCurParams.getVector("glow", err));
	mCloudShadow = gWLSkyParamMgr.mCurParams.getVector("cloud_shadow", err)[0];
	// Light norm is different. We need the Sun's direction, not the light
	// direction which could be from the moon. And we need to clamp it just
	// like for the GPU.
	LLVector3 sun_dir = gSky.getSunDirection();
	// CFR_TO_OGL
	mSunNorm = LLVector4(sun_dir.mV[1], sun_dir.mV[2], sun_dir.mV[0], 0.f);
	mUnClampedSunNorm = mSunNorm;
	if (mSunNorm.mV[1] < -0.1f)
	{
		mSunNorm.mV[1] = -0.1f;
	}
	...
	mDomeRadius = SKY_DOME_RADIUS;
	mDomeOffset = SKY_DOME_OFFSET;
	if (LLEnvironment::overrideWindlight())
	{
		const LLSettingsSky::ptr_t& skyp = gEnvironment.getCurrentSky();
		if (skyp)
		{
...
void LLVOSky::initAtmosphericsEE(const LLSettingsSky::ptr_t& skyp)
{
...
LLColor4 LLVOSky::calcSkyColorInDirWL(const LLVector3& dir, bool is_shiny)
{
	constexpr F32 saturation = 0.3f;
	if (dir.mV[VZ] < -0.02f)
	{
		LLColor4 col = LLColor4(llmax(mFogColor[0], 0.2f),
								llmax(mFogColor[1], 0.2f),
								llmax(mFogColor[2], 0.22f), 0.f);
		if (is_shiny)
		{
			...
			desat_fog = desat_fog * saturation + greyscale *
						(1.f - saturation);
		...
		x *= x;
		col.mV[0] *= x * x;
		col.mV[1] *= gFastLn.pow(x, 2.5f);
		col.mV[2] *= x * x * x;
		return col;
	}

	// Undo OGL_TO_CFR_ROTATION and negate vertical direction.
	LLVector3 pn(-dir[1] , -dir[2], -dir[0]);
	LLColor3 vary_haze_col;
	calcSkyColorWLVert(pn, vary_haze_col);

	LLColor3 sky_color;
	if (gPipeline.canUseWindLightShaders())
	{
		sky_color = vary_haze_col;
	}
	else
	{
		LLColor3 color1 = smear(1.f) - componentSaturate(vary_haze_col * 2.f);
		componentPowF(color1, mGamma);
		sky_color = smear(1.f) - color1;
	}
	...
	if (is_shiny)
	{
		...
		LLColor3 greyscale = smear(brightness);
		sky_color = sky_color * saturation + greyscale * (1.f - saturation);
...
void LLVOSky::calcSkyColorWLVert(LLVector3& pn, LLColor3& vary_haze_col)
{
	...
	// Sunlight attenuation effect (hue and brightness) due to atmosphere this
	// is used later for sunlight modulation at various altitudes
	LLColor3 light_atten = (mBlueDensity + smear(mHazeDensity * 0.25f)) *
						   (mDensityMultiplier * mMaxY);

	// Calculate relative weights
	LLColor3 temp1 = mBlueDensity + smear(mHazeDensity);
	LLColor3 blue_weight = componentDiv(mBlueDensity, temp1);
	LLColor3 haze_weight = componentDiv(smear(mHazeDensity), temp1);
	...
	componentMultBy(sunlight,
					componentExp((light_atten * -1.f) * temp2.mV[1]));
	...
	// function
	temp2.mV[0] = gFastLn.pow(temp2.mV[0], mGlow.mV[2]);

	// Add "minimum anti-solar illumination"
	temp2.mV[0] += 0.25f;

	// Haze color above cloud
	vary_haze_col = mBlueHorizon * blue_weight * (sunlight + mAmbient) +
					componentMult(mHazeHorizon * haze_weight,
								  sunlight * temp2.mV[0] + mAmbient);
	...
	LLColor3 additive_col_below_cloud =
		mBlueHorizon * blue_weight * (sunlight + ambient) +
		componentMult(mHazeHorizon * haze_weight,
					  sunlight * temp2.mV[0] + ambient);

	// Final atmosphere additive
	componentMultBy(vary_haze_col, LLColor3::white - temp1);

	sunlight = mSunlight;
	temp2.mV[1] = llmax(F_APPROXIMATELY_ZERO, mSunNorm.mV[1] * 2.f);
	temp2.mV[1] = 1.f / temp2.mV[1];
	componentMultBy(sunlight, componentExp(light_atten * -temp2.mV[1]));
	...
	// the clouds
	vary_haze_col += componentMult(additive_col_below_cloud - vary_haze_col,
								   LLColor3::white - componentSqrt(temp1));

	if (pn[1] < 0.f)
	{
		static const LLColor3 dark_brown(0.082f, 0.076f, 0.066f);
		static const LLColor3 brown(0.430f, 0.386f, 0.322f);
		F32 haze_brightness = vary_haze_col.brightness();

		if (pn[1] < -0.05f)
		{
			vary_haze_col = colorMix(dark_brown, brown, -pn[1] * 0.9f) *
							(sunlight + mAmbient) * haze_brightness;
		}

		if (pn[1] > -0.1f)
		{
			vary_haze_col = colorMix(LLColor3::white * haze_brightness,
									 vary_haze_col,
									 fabsf((pn[1] + 0.05f) * -20.f));
		}
	}
}

LLColor4 LLVOSky::calcSkyColorInDirEE(const LLSettingsSky::ptr_t& skyp,
...
void LLVOSky::calcSkyColorEEVert(LLVector3& pn)
{
	// Project the direction ray onto the sky dome.
	F32 phi = acosf(pn[1]);
	F32 sin_a = sinf(F_PI - phi);
...
void LLVOSky::calcAtmosphericsWL()
{
	initAtmosphericsWL();

	LLColor3 vary_haze_col, vary_sun_light_col, vary_ambient_col;
	// Initialize temp variables
	LLColor3 sunlight = mSunlight;
	...
	// Vary_sunlight will work properly with moon light
	F32 lighty = mUnClampedSunNorm.mV[1];
	...
	// Distance
	temp2.mV[2] = mDensityMultiplier;

	// Transparency (-> temp1)
	temp1 = componentExp((temp1 * -1.f) * temp2.mV[2]);

	// Increase ambient when there are more clouds
	LLColor3 ambient = mAmbient + (smear(1.f) - mAmbient) *
					   mCloudShadow * 0.5f;

	// Haze color
	vary_haze_col = mBlueHorizon * blue_weight *
					(sunlight * (1.f - mCloudShadow) + ambient) +
					componentMult(mHazeHorizon * haze_weight,
								  sunlight * (1.f - mCloudShadow) *
								  temp2.mV[0] + ambient);

	// Brightness of surface both sunlight and ambient
	vary_sun_light_col = componentMult(sunlight, temp1) * 1.f;
	vary_sun_light_col.clamp();
	vary_sun_light_col = smear(1.f) - vary_sun_light_col;
	vary_sun_light_col = componentPowF(vary_sun_light_col, mGamma);
	vary_sun_light_col = smear(1.f) - vary_sun_light_col;
	vary_ambient_col = componentMult(ambient, temp1) * 0.5;
	vary_ambient_col.clamp();
	vary_ambient_col = smear(1.f) - vary_ambient_col;
	vary_ambient_col = componentPowF(vary_ambient_col, mGamma);
	vary_ambient_col = smear(1.f) - vary_ambient_col;

	componentMultBy(vary_haze_col, LLColor3::white - temp1);

	mSun.setColor(vary_sun_light_col);
	...
	F32 dp = getToSunLast() * LLVector3::z_axis;
	if (dp < 0.f)
	{
		dp = 0.f;
	}

	// Since WL scales everything by 2, there should always be at least a
	// 2:1 brightness ratio between sunlight and point lights in windlight to
	// normalize point lights.
	static LLCachedControl<F32> sun_dynamic_range(gSavedSettings,
												  "RenderSunDynamicRange");
	F32 range = llmax((F32)sun_dynamic_range, 0.0001f);
	gWLSkyParamMgr.mSceneLightStrength = 2.f * (1.f + range * dp);

	mSunDiffuse = vary_sun_light_col;
	mSunAmbient = vary_ambient_col;
	mMoonDiffuse = vary_sun_light_col;
	mMoonAmbient = vary_ambient_col;

	mTotalAmbient = vary_ambient_col;
	mTotalAmbient.setAlpha(1.f);

	mFadeColor = mTotalAmbient + (mSunDiffuse + mMoonDiffuse) * 0.5f;
	mFadeColor.setAlpha(0.f);
}

// Extended environment version
void LLVOSky::calcAtmosphericsEE()
{
	const LLSettingsSky::ptr_t& skyp = gEnvironment.getCurrentSky();
	if (skyp)	// Paranoia
	{
...
bool LLVOSky::updateSkyWL()
{
	static S32 next_frame = 0;

	if (mUpdateTimer.getElapsedTimeF32() > UPDATE_EXPIRY)
	{
		mUpdateTimer.reset();
		...
		mInterpVal = mInitialized ? (F32)next_frame / (F32)MAX_TILES
								  :  1.f;
		...
		if (mForceUpdate || frame == TOTAL_TILES)
		{
			LLSkyTex::stepCurrent();

			static const F32 LIGHT_DIRECTION_THRESHOLD = cosf(DEG_TO_RAD);
			constexpr F32 COLOR_CHANGE_THRESHOLD = 0.01f;

			LLVector3 direction = mSun.getDirection();
			direction.normalize();
			const F32 dot_lighting = direction * mLastLightingDirection;

			LLColor3 delta_col(mLastTotalAmbient.mV[0] - mTotalAmbient.mV[0],
							   mLastTotalAmbient.mV[1] - mTotalAmbient.mV[1],
							   mLastTotalAmbient.mV[2] - mTotalAmbient.mV[2]);
			if (mForceUpdate ||
				(!direction.isExactlyZero() &&
				 (!mInitialized || dot_lighting < LIGHT_DIRECTION_THRESHOLD ||
				  delta_col.length() > COLOR_CHANGE_THRESHOLD)))
			{
				mLastLightingDirection = direction;
				mLastTotalAmbient = mTotalAmbient;
				mInitialized = true;

				if (mCubeMap && mForceUpdate)
				{
			...
			// *TODO really, sky texture and env map should be shared on a
			// single texture.
			...
			gPipeline.markRebuild(gSky.mVOGroundp->mDrawable,
...
bool LLVOSky::updateSkyEE()
{
	...
	else // mCubeMapUpdateStage >= 0 && mCubeMapUpdateStage < NUM_CUBEMAP_FACES
	{
		...
		{
			createSkyTextureEE(skyp, side, start_tile + tile);
		}
...
bool LLVOSky::updateGeometry(LLDrawable* drawable)
{
	...
	const F32 cos_max_angle = gUseNewShaders ? 1.f : cosHorizon();
	...
	if (gUseNewShaders)
	{
		sun_flag = !mMoon.isVisible() || look_at * mSun.getDirection() > 0.f;
	}
	else if (mSun.isVisible())
	{
		if (mMoon.isVisible())
		{
			sun_flag = look_at * mSun.getDirection() > 0.f;
		}
		else
		{
			sun_flag = true;
										 ...
										 const LLVector3& right)
{
	...
	else
	{
		...
		hb.setU(HEAVENLY_BODY_FACTOR * horiz_enlargement * hb.getDiskRadius() *
				hb_right);
		hb.setV(HEAVENLY_BODY_FACTOR * vert_enlargement * hb.getDiskRadius() *
				hb_up);
		...
		if (!clip_quad_to_horizon(t_left, t_right, v_clipped, hb.corners(),
								  cos_max_angle))
		{
			hb.setVisible(false);
			return false;
	...
	*indicesp++ = index_offset + 3;
	...
	if (is_sun && !gUseNewShaders)
	{
		if (t_left > 0.f && t_right > 0.f)
		{
			F32 t = (t_left + t_right) * 0.5f;
			mSun.setHorizonVisibility(0.5f + 0.5f * cosf(t * F_PI));
		}
		else
		{
									   ...
									   const LLHeavenBody& hb)
{
	...
	if (!gUseNewShaders)
	{
		dt = clip_side_to_horizon(v_corner[1], v_corner[0], cosHorizon());
		if (dt > 0.f && dt < 1.f)
		{
			TEX0t = LLVector2(0.f, dt);
			TEX1t = LLVector2(1.f, dt);
			lower_corner = (1.f - dt) * v_corner[1] + dt * v_corner[0];
		}
		else
		{
			dt = llmax(0.f, llmin(1.f, dt));
		}
	}
...
void LLVOSky::updateFog(F32 distance)
{
	...
	if (gUseNewShaders)
	{
		const LLSettingsSky::ptr_t& skyp = gEnvironment.getCurrentSky();
		initAtmosphericsEE(skyp);
		res_color[0] = calcSkyColorInDirEE(skyp, tosun);
		res_color[1] = calcSkyColorInDirEE(skyp, perp_tosun);
		res_color[2] = calcSkyColorInDirEE(skyp, tosun_45);
	}
	else
	{
		initAtmosphericsWL();
		res_color[0] = calcSkyColorInDirWL(tosun);
		res_color[1] = calcSkyColorInDirWL(perp_tosun);
		res_color[2] = calcSkyColorInDirWL(tosun_45);
	}
