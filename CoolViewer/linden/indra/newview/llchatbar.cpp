			 ...
			 gSavedSettings.getBool("TabAutoCompleteName"))
	{
		...
		if (text.length() > 0)
		{
			...
			if (pos >= (S32)text.length() ||
				(pos > 0 && !LLStringUtil::isPartOfWord(text.at(pos))))
			{
				--pos;
			}
		...
		if (!text.empty())
		{
			...
			if (channel == 0)
			{
				...
				if (gSavedSettings.getBool("AllowMUpose") &&
					utf8text.length() > 3 && utf8text.at(0) == ':')
				{
...
void LLChatBar::onCommitGesture(LLUICtrl* ctrl, void* data)
{
	...
	LLCtrlListInterface* gestures =
		self->mGestureCombo ? self->mGestureCombo->getListInterface() : NULL;
	if (gestures)
	{
