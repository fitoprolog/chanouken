...
void LLPanelWorldMap::draw()
{
		 ...
		 layer_idx < count; ++layer_idx)
	{
		...
		LLWorldMapLayer* layer = &gWorldMap.mMapLayers[mLayer][layer_idx];
		LLViewerTexture* current_image = layer->mLayerImage;
		if (!current_image || current_image->isMissingAsset())
		{
			continue; // Better to draw nothing than the missing asset image
		}

		LLVector3d origin_global((F64)layer->mLayerExtents.mLeft *
								 REGION_WIDTH_METERS,
								 (F64)layer->mLayerExtents.mBottom *
								 REGION_WIDTH_METERS,
								 0.f);
		...
		F32 pix_width = sMapScale * (layer->mLayerExtents.getWidth() + 1);
		F32 pix_height = sMapScale * (layer->mLayerExtents.getHeight() + 1);
		...
		current_image->setBoostLevel(LLGLTexture::BOOST_MAP_VISIBLE);
		current_image->setKnownDrawSize(ll_roundp(pix_width * ui_scale_x),
		 ...
		 it != end; ++it)
	{
		...
		LLViewerTexture* simimage = info->mCurrentImage[mLayer];
		...
		bool sim_visible = (sMapScale >= map_scale_cutoff &&
							simimage && simimage->hasGLTexture());
			  ...
			  gTextureFetchp->getApproxNumRequests() < MAX_SIMULTANEOUS_TEX)))
		{
			...
			simimage = info->mCurrentImage[mLayer];
		}
		if (!overlayimage && info->mMapImageID[2].notNull() &&
			(textures_requested_this_tick < MIN_REQUEST_PER_TICK ||
			 (textures_requested_this_tick < MAX_REQUEST_PER_TICK &&
			  gTextureFetchp->getApproxNumRequests() < MAX_SIMULTANEOUS_TEX)))
		{
		...
		if (simimage)
		{
			...
			simimage->setKnownDrawSize(ll_roundp(x_draw_size * ui_scale_x),
									   ll_roundp(y_draw_size * ui_scale_y));
		}

		if (overlayimage)
		{
			overlayimage->setBoostLevel(LLGLTexture::BOOST_MAP);
			overlayimage->setKnownDrawSize(ll_roundp(x_draw_size * ui_scale_x),
										   ll_roundp(y_draw_size *
													 ui_scale_y));
		}
		...
		if (sim_visible && info->mAlpha > 0.001f)
		{
			...
			if (simimage && simimage->hasGLTexture())
			{
				unit0->bind(simimage);
			...
			if (map_show_land_for_sale && overlayimage &&
				overlayimage->hasGLTexture())
			{
				unit0->bind(overlayimage);
				overlayimage->setAddressMode(LLTexUnit::TAM_CLAMP);
				gGL.color4f(1.f, 1.f, 1.f, alpha);
				gGL.begin(LLRender::TRIANGLES);
					gGL.texCoord2f(0.f, 1.f);
			...
			if ((info->mRegionFlags & REGION_FLAGS_NULL_LAYER) == 0)
			{
				// Draw an alpha of 1 where the sims are visible (except NULL
				// sims)
				gGL.setSceneBlendType(LLRender::BT_REPLACE);
				gGL.setColorMask(false, true);
				gGL.color4f(1.f, 1.f, 1.f, 1.f);
		...
		if (!mesg.empty())
		{
			...
			// text.
			if (simimage &&	simimage->getDiscardLevel() != 1 &&
									...
									LLRect* sticky_rect_screen)
{
	...
	if (info)
	{
		...
		if (info->mAccess != SIM_ACCESS_DOWN)
		{
			...
			// We may not have an agent count when the map is really zoomed
			// out, so do not display anything about the count. JC
			if (agent_count >= 0)
			{
		...
		std::string region_flags =
			LLViewerRegion::regionFlagsToString(info->mRegionFlags);
