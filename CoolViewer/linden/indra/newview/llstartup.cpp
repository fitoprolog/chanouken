...
# if LL_LINUX
		...
		// Sadly, FMOD Studio crashes when trying to initialize it with
		// PulseAudio while the latter is not available on the system, making
		// the viewer abort on first startup on such systems. We therefore
		// need a way to disable PulseAudio support before we launch the viewer
		// and can change the debug settings... HB
		if (getenv("LL_NO_PUSEAUDIO"))
		{
			gSavedSettings.setBool("FMODDisablePulseAudio", true);
		}
...
#endif
		bool init = gAudiop->init(AUDIO_NUM_SOURCES, window_handle);
		if (init)
		{
...
bool LLStartUp::idleStartup()
{
  ...
  if (getStartupState() == STATE_FIRST)
  {
    ...
    if (found_template)
    {
      ...
      if (NET_USE_OS_ASSIGNED_PORT == port &&
  ...
  if (getStartupState() == STATE_LOGIN_SHOW)
  {
    ...
    // NOTE: This also is where gToolMgr used to be instantiated before
    // being turned into a singleton.
  ...
  {
    LL_DEBUGS("AppInit") << "Need TPV agreement" << LL_ENDL;
  ...
  {
    // *HACK: (possibly) enable multi-threaded decoding after preloading
    // of UI textures (trying to work around incompletely loaded
    // rounded_square.tga texture). HB
    LLImageDecodeThread::sCanUseThreads = true;
  ...
  {
    gFrameSleepTime = 1;
    LL_DEBUGS("AppInit") << "STATE_XMLRPC_LOGIN" << LL_ENDL;
    progress += 0.02f;
    display_startup();
    ...
    if (gIsInSecondLife)
    {
      use_mfa = gSavedSettings.getBool("UseMFAinSL");
    ...
    // Adjust the prim parameters limits accordingly
    ...
    load_script_functions("lsl_functions_sl.xml");
    if (!gIsInSecondLife)
    {
      // Load script functions symbols specific to OpenSim/Aurora
      load_script_functions("lsl_functions_os.xml");
  ...
  if (getStartupState() == STATE_LOGIN_PROCESS_RESPONSE)
  {
      ...
      case LLUserAuth::E_OK:
        ...
        else
        {
          ...
          else if (reason_response == "mfa_challenge")
          {
            ...
            // MFA support is/became required on this grid !
            gSavedSettings.setBool(gIsInSecondLife ? "UseMFAinSL"
                : "UseMFAinOS",
                true);
          }
    ...
    if (successful_login)
    {
      ...
      {
        LLEconomy::getInstance()->setBenefits(benefits);
  ...
  if (getStartupState() == STATE_INVENTORY_SEND)
  {
    ...
    llinfos << "Setting Inventory changed mask and notifying observers"
      << llendl;
  ...
  if (getStartupState() == STATE_WEARABLES_WAIT)
  {
    ...
    {
      // No point in waiting for clothing, we don't even know what gender
      // we are. Pop a dialog to ask and proceed to draw the world. JC
      //
      // *NOTE: We might hit this case even if we have an initial outfit,
      // but if the load has not started already then something is wrong
      // so fall back to generic outfits. JC
  ...
  if (getStartupState() == STATE_CLEANUP)
  {
...
#if 0	// Let the user a choice: with Estate time disabled, the computer
    // clock UTC time would be used as a base...
    else
    {
    ...
    // Signal our login to the automation script, if any
    if (gAutomationp)
    {
    ...
    // debug message from the Advanced menu).
    LLError::Log::sDebugMessages = HBFloaterDebugTags::hasActiveDebugTags();
...
void LLStartUp::registerViewerCallbacks(LLMessageSystem* msg)
{
  ...
  msg->setHandlerFunc("ObjectUpdateCompressed",
  ...
  msg->setHandlerFunc("AgentCachedTextureResponse",
      LLAgent::processAgentCachedTextureResponse);

  msg->setHandlerFunc("RebakeAvatarTextures",
  ...
  msg->setHandlerFunc("SetFollowCamProperties",
      process_set_follow_cam_properties);

  msg->setHandlerFunc("ClearFollowCamProperties",
  ...
  msg->setHandlerFunc("ForceObjectSelect",
  ...
  msg->setHandlerFunc("RegionHandshake",
  ...
  msg->setHandlerFunc("TeleportFailed", process_teleport_failed);
  msg->setHandlerFunc("TeleportLocal", process_teleport_local);
  msg->setHandlerFunc("ScriptTeleportRequest",
      process_script_teleport_request);

  msg->setHandlerFunc("ImageNotInDatabase",
  ...
  msg->setHandlerFunc("ClassifiedInfoReply",
      LLPanelClassified::processClassifiedInfoReply);
  ...
  msg->setHandlerFunc("ParcelObjectOwnersReply",
      LLPanelLandObjects::processParcelObjectOwnersReply);

  msg->setHandlerFunc("InitiateDownload", process_initiate_download);
  msg->setHandlerFunc("LandStatReply", LLFloaterTopObjects::handleLandReply);
...
{
  return sStartupState >= STATE_WORLD_INIT;
}
