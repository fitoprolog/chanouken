...
LLVOWLSky::LLVOWLSky(const LLUUID& id, LLPCode pcode, LLViewerRegion* regionp)
...
{
	mFanVerts = NULL;		// Windlight
	mFsSkyVerts = NULL;		// Extended environment
	mStripsVerts.clear();
	mStarsVerts = NULL;
...
bool LLVOWLSky::primeFanVerts()
{
	mFanVerts = new LLVertexBuffer(LLDrawPoolWLSky::SKY_VERTEX_DATA_MASK,
								   GL_STATIC_DRAW_ARB);
	if (!mFanVerts->allocateBuffer(getFanNumVerts(), getFanNumIndices(), true))
	{
		llwarns << "Failure to allocate a vertex buffer on sky update with "
				<< getFanNumVerts() << " vertices and "
				<< getFanNumIndices() << " indices." << llendl;
		return false;
	}
	...
	if (!success)
	{
		llwarns << "Failed to extract striders from WindLight sky vertex buffer."
				<< llendl;
		return false;
	}
	...
	*texcoords++ = LLVector2(0.5f, 0.5f);
	...
	// Fan in a circle around the apex
	...
	{
		F32 theta = 2.f * F_PI * (F32)i / (F32)num_slices;

		// Standard transformation from spherical to rectangular coordinates
		F32 x0 = sin_phi0 * cosf(theta);
		F32 z0 = sin_phi0 * sinf(theta);

		*vertices++ = LLVector3(x0 * radius, scaled_y0, z0 * radius);
		// Generate planar uv coordinates. Note: x and z are transposed in
		// order for things to animate correctly in the global coordinate
		// system where +x is east and +y is north
		*texcoords++ = LLVector2(0.5f - z0 * 0.5f, 0.5f - x0 * 0.5f);
	...
	// The last vertex of the last triangle should wrap around to the beginning
	*indices++ = 0;
	*indices++ = num_slices;
	*indices++ = 1;
...
bool LLVOWLSky::primeFsSkyVerts()
{
	mFsSkyVerts =
		new LLVertexBuffer(LLDrawPoolWLSky::ADV_ATMO_SKY_VERTEX_DATA_MASK,
						   GL_STATIC_DRAW_ARB);
	if (!mFsSkyVerts->allocateBuffer(4, 6, true))
	{
		llwarns << "Failure to allocate a vertex buffer on full screen sky update."
				<< llendl;
		return false;
	}
	...
	if (!success)
	{
		llwarns << "Failed to extract striders from full screen sky vertex buffer."
				<< llendl;
		return false;
	}

	*vertices++ = LLVector3(-1.f, -1.f, 0.f);
	*vertices++ = LLVector3( 1.f, -1.f, 0.f);
	*vertices++ = LLVector3(-1.f,  1.f, 0.f);
	*vertices = LLVector3( 1.f,  1.f, 0.f);

	*texcoords++ = LLVector2(0.f, 0.f);
	*texcoords++ = LLVector2(1.f, 0.f);
	*texcoords++ = LLVector2(0.f, 1.f);
	*texcoords = LLVector2(1.f, 1.f);
...
bool LLVOWLSky::updateGeometry(LLDrawable* drawable)
{
	...
	{
		mFanVerts = NULL;
		if (mFsSkyVerts.isNull() && !primeFsSkyVerts())
		{
			return false;
		}
	}
	...
	{
		mFsSkyVerts = NULL;
		if (mFanVerts.isNull() && !primeFanVerts())
		{
			return false;
		}
	}
...
void LLVOWLSky::drawFsSky()
{
	...
	S32 indices = mFsSkyVerts->getNumIndices();
	mFsSkyVerts->drawRange(LLRender::TRIANGLES, 0,
						   mFsSkyVerts->getNumVerts() - 1, indices, 0);
	gPipeline.addTrianglesDrawn(indices, LLRender::TRIANGLES);
...
{
	const F32 distance_to_stars = gUseNewShaders ? getDomeRadius()
												 : HORIZON_DIST * 0.25f - 2.5f;
	...
	if (mStarsVerts.isNull() || !mStarsVerts->isWriteable())
	{
		mStarsVerts =
			new LLVertexBuffer(LLDrawPoolWLSky::STAR_VERTEX_DATA_MASK,
							   GL_DYNAMIC_DRAW);
		if (!mStarsVerts->allocateBuffer(STAR_NUM_VERTS * 6, 0, true))
		{
