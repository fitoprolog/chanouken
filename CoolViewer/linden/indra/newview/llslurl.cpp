...
#define APP_AGENT_REGEX "((x-grid-location-info://[-\\w\\.]+/app/agent/)|((secondlife|hop):///app/agent/))[\\da-f-]+/"
...
#define APP_GROUP_REGEX "((x-grid-location-info://[-\\w\\.]+/app/group/)|((secondlife|hop):///app/group/))[\\da-f-]+/about"
...
#define APP_EXP_REGEX "((x-grid-location-info://[-\\w\\.]+/app/experience/)|((secondlife|hop):///app/experience/))[\\da-f-]+/profile"
...
#define APP_OBJ_REGEX "((x-grid-location-info://[-\\w\\.]+/app/objectim/)|((secondlife|hop):///app/objectim/))[\\da-f-]+[/\?]"
...
LLSLURL::LLSLURL(const std::string& slurl)
{
	...
	else
	{
				 ...
				 scheme == LLSLURL::SLURL_X_GRID_LOCATION_INFO_SCHEME)
		{
			...
			{
				// Not a valid https/http/x-grid-location-info slurl, so it is
				// likely just an URL...
				return;
			}
		}
