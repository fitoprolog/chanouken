...
#endif
#if LL_LINUX
	text->replaceTextAll("liburing", getString("liburing"), false);
#else
	text->replaceTextAll("liburing\n", "", false);
#endif
#if LL_MESHOPT
	text->replaceTextAll("meshoptimizer", getString("meshoptimizer"), false);
#else
	text->replaceTextAll("meshoptimizer\n", "", false);
#endif
#if LL_PHMAP
...
void LLFloaterAbout::setSupportText()
{
	...
	text += " [DEVEL]";
#endif
	text += llformat(" v%d.%d.%d.%d, 64 bits, %s %s", LL_VERSION_MAJOR,
...
//mk
	...
	// CPU
	text = "CPU: ";
	text += LLCPUInfo::getInstance()->getCPUString() + "\n";

	// Moved hack adjustment to Windows memory size into llsys.cpp
	U32 memory = LLMemoryInfo::getPhysicalMemoryKB() / 1024;
	...
	ui_str = getString("graphics_vendor");
	ui_str.setArg("[VENDOR]",
	...
	if (gPacketsIn > 0)
	{
		...
		ui_str.setArg("[STATS]",
					  llformat("%d/%d (%.1f%%)",
							   (S32)stats->mPacketsLostStat.getCurrent(),
							   gPacketsIn,
							   100.f * stats->mPacketsLostStat.getCurrent() /
							   (F32)gPacketsIn));
		text.append(ui_str);
		text += '\n';
	}
