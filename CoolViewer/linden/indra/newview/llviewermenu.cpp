...
// Local constants. *TODO: translate
...
void init_menus()
{
	...
	LLRect menu_bar_rect = gLoginMenuBarViewp->getRect();
	gLoginMenuBarViewp->setRect(LLRect(menu_bar_rect.mLeft, menu_bar_rect.mTop,
								gViewerWindowp->getRootView()->getRect().getWidth() -
								menu_bar_rect.mLeft, menu_bar_rect.mBottom));
...
void menu_toggle_control(void* user_data)
{
	...
	bool checked = !gSavedSettings.getBool(setting.c_str());
	if (checked && setting == "HighResSnapshot")
	{
		// High Res Snapshot active, must uncheck RenderUIInSnapshot
		gSavedSettings.setBool("RenderUIInSnapshot", false);
	}
	gSavedSettings.setBool(setting.c_str(), checked);
}
...
bool safety_checks_enabled(void*)
{
	static LLCachedControl<bool> checks(gSavedSettings, "MemorySafetyChecks");
	return checks;
}

bool allow_swapping_check_control(void*)
{
	static LLCachedControl<bool> allow_swap(gSavedSettings, "AllowSwapping");
	return allow_swap || !safety_checks_enabled(NULL);
}

bool reset_vb_check_control(void*)
{
	static LLCachedControl<bool> reset_vb(gSavedSettings,
										  "ResetVertexBuffersOnLowMem");
	return reset_vb && safety_checks_enabled(NULL);
}

bool mem_alerts_as_notify_check_control(void*)
{
	static LLCachedControl<bool> notif(gSavedSettings,
									   "MemoryAlertsAsNotifications");
	return notif && safety_checks_enabled(NULL);
}
...
bool enable_region_owner(void*)
{
	return (gAgent.getRegion() &&
			gAgent.getRegion()->getOwner() == gAgentID) ||
...
{
	static LLCachedControl<bool> obj_cache(gSavedSettings,
										   "ObjectCacheEnabled");
	return obj_cache;
}

bool full_region_caching_checked(void*)
{
	static LLCachedControl<bool> full_region(gSavedSettings,
											 "RequestFullRegionCache");
	return full_region && object_cache_enabled(NULL);
}

bool object_cache_occlusion_checked(void*)
{
	static LLCachedControl<bool> occlusions(gSavedSettings,
											"UseObjectCacheOcclusion");
	return occlusions && object_cache_enabled(NULL);
}

bool bias_retention_checked(void*)
{
	static LLCachedControl<bool> biased(gSavedSettings,
										"BiasedObjectRetention");
	return biased && object_cache_enabled(NULL);
}
...
bool ais3_checked(void*)
{
	...
	return use_ais && http_inventory_checked(NULL);
}
...
bool fmod_pulse_enabled(void*)
{
	static bool pulse_ok = !getenv("LL_NO_PUSEAUDIO");
	static LLCachedControl<bool> no_fmod(gSavedSettings, "AudioDisableFMOD");
	return pulse_ok && !no_fmod;
}
#endif

void clear_assets_cache(void*)
{
	gSavedSettings.setBool("ClearAssetsCache", true);
	gNotifications.add("AssetsCacheWillClear");
}
...
void init_client_menu(LLMenuGL* menu)
{
...

	{
		...
		sub->append(new LLMenuItemCheckGL("Fast timers view",
		...
		sub->append(new LLMenuItemCallGL("Debug tags", handle_debug_tags,
										 NULL));
		{
			...
			sub2->append(new LLMenuItemCallGL("Registered command handlers",
											  dump_cmd_handlers));
			sub2->append(new LLMenuItemCallGL("Memory stats",
											  output_statistics));
			sub2->createJumpKeys();
		}
		sub->appendSeparator();
	...
	sub_menu = new LLMenuGL("HUD info");
	{
		...
		sub_menu->append(new LLMenuItemCheckGL("Show mesh queue",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowMeshQueue"));
		sub_menu->append(new LLMenuItemToggleGL("Show camera info",
												&gDisplayCameraPos));
		sub_menu->append(new LLMenuItemToggleGL("Show FOV Info",
												&gDisplayFOV));
		sub_menu->append(new LLMenuItemCheckGL("Show matrices",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowRenderMatrices"));
		sub_menu->append(new LLMenuItemCheckGL("Show avatars render info",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowAvatarRenderInfo"));
		sub_menu->append(new LLMenuItemCheckGL("Show render info",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowRenderInfo"));
		sub_menu->append(new LLMenuItemToggleGL("Show wind info",
												&gDisplayWindInfo));
		sub_menu->append(new LLMenuItemCheckGL("Show time",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowTime"));
		sub_menu->append(new LLMenuItemCheckGL("Show frame rate",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowFPS"));
		sub_menu->append(new LLMenuItemCheckGL("Show color under cursor",
											   menu_toggle_control, NULL,
											   menu_check_control,
											   (void*)"DebugShowColor"));
		sub_menu->appendSeparator();
		sub_menu->append(new LLMenuItemCheckGL("Show info HUD background",
											   menu_toggle_control,
											   hud_info_bg_enabled,
											   hud_info_bg_checked,
	...
	sub_menu = new LLMenuGL("User interface");
...
//mk
	{
		LLMenuGL* sub = new LLMenuGL("Lua scripting");
...
# endif
	...
	{
		LLMenuGL* sub = new LLMenuGL("Network");
...
#if 0	// Do not expose this for now: LL seems incapable to get it working !
		sub->append(new LLMenuItemCheckGL("Use capability for offline IMs retrieval",
										  menu_toggle_control,
										  NULL, menu_check_control,
		...
		sub->append(new LLMenuItemCheckGL("Use the HTTP/2 protocol",
										  menu_toggle_control,
										  pipelining_enabled, http2_checked,
										  (void*)"EnableHTTP2"));
#endif
...
#if !LL_DARWIN || LL_JEMALLOC	// See the comment in LLAppViewer::init()
	{
		LLMenuGL* sub = NULL;
		sub = new LLMenuGL("Memory management");
		sub->append(new LLMenuItemCheckGL("Memory usage safety checks",
										  menu_toggle_control, NULL,
										  menu_check_control,
										  (void*)"MemorySafetyChecks"));
		sub->append(new LLMenuItemCheckGL("Allow swapping",
										  menu_toggle_control,
										  safety_checks_enabled,
										  allow_swapping_check_control,
										  (void*)"AllowSwapping"));
		sub->append(new LLMenuItemCheckGL("Reset VB on critical low mem condition",
										  menu_toggle_control,
										  safety_checks_enabled,
										  reset_vb_check_control,
										  (void*)"ResetVertexBuffersOnLowMem"));
		sub->append(new LLMenuItemCheckGL("Issue low mem alerts as notifications",
										  menu_toggle_control,
										  safety_checks_enabled,
										  mem_alerts_as_notify_check_control,
										  (void*)"MemoryAlertsAsNotifications"));
		sub->createJumpKeys();
		menu->appendMenu(sub);
	}
#endif
	{
		LLMenuGL* sub = new LLMenuGL("Caches");

		sub->append(new LLMenuItemCallGL("Clear group cache",
										 LLGroupMgr::debugClearAllGroups));

		sub->append(new LLMenuItemCallGL("Clear assets cache (after restart)",
										 clear_assets_cache));
	...
	{
		LLMenuGL* sub = new LLMenuGL("Media");

		sub->append(new LLMenuItemCallGL("Restart audio engine",
										 &restart_audio_engine));
#if LL_OPENAL && LL_FMOD
		sub->append(new LLMenuItemCheckGL("Disable OpenAL",
										  menu_toggle_control, NULL,
										  menu_check_control,
										  (void*)"AudioDisableOpenAL"));
		sub->append(new LLMenuItemCheckGL("Disable FMOD",
										  menu_toggle_control, NULL,
										  menu_check_control,
...
#if !LL_DEBUG
	menu->append(new LLMenuItemCheckGL("Watchdog timeout (after restart)",
									   menu_toggle_control, NULL,
									   menu_check_control,
# if LL_WINDOWS
									   (void*)"WatchdogEnabledWindows"
# else
...
void init_debug_world_menu(LLMenuGL* menu)
{
	...
	menu->append(new LLMenuItemCheckGL("Sparse classic clouds updates",
									   menu_toggle_control,
									   NULL, menu_check_control,
									   (void*)"SparseClassicClouds"));
	menu->append(new LLMenuItemCheckGL("Show wind vectors",
...
#if 0	// Does not works particularly well at the moment
void reload_ui(void*)
{
...
{
	LLFloater* floater = new LLFloater("floater_font_test");
...
{
	LLFloater* floater = new LLFloater("skin_preview_floater");
	LLUICtrlFactory::getInstance()->buildFloater(floater,
...
void init_debug_ui_menu(LLMenuGL* menu)
{
	...
	LLMenuGL* sub_menu = new LLMenuGL("Debug");
	sub_menu->append(new LLMenuItemCheckGL("Selection manager",
										   menu_toggle_control, NULL,
										   menu_check_control,
										   (void*)"DebugSelectMgr"));
	sub_menu->append(new LLMenuItemToggleGL("Clicks", &gDebugClicks));
	sub_menu->append(new LLMenuItemToggleGL("Views", &LLView::sDebugRects));
	sub_menu->append(new LLMenuItemToggleGL("Mouse events",
											&LLView::sDebugMouseHandling));
	sub_menu->append(new LLMenuItemToggleGL("Keys", &LLView::sDebugKeys));
	sub_menu->append(new LLMenuItemToggleGL("WindowProc", &gDebugWindowProc));
	sub_menu->createJumpKeys();
	menu->appendMenu(sub_menu);

	sub_menu = new LLMenuGL("XUI");
	sub_menu->append(new LLMenuItemCallGL("Font test...",
										  handle_font_test_floater));
	sub_menu->append(new LLMenuItemCallGL("Skin preview...",
										  handle_skin_preview_floater));
	sub_menu->append(new LLMenuItemCallGL("Export menus to XML...",
										  handle_export_menus_to_xml,
										  enable_picker_actions, NULL));
	sub_menu->append(new LLMenuItemCallGL("Edit UI...",
										  LLFloaterEditUI::show));
	sub_menu->append(new LLMenuItemCallGL("Load floater/panel from XML...",
										  handle_load_from_xml,
										  enable_picker_actions, NULL));
	sub_menu->append(new LLMenuItemCallGL("Save frontmost floater to XML...",
										  handle_save_to_xml,
										  enable_picker_actions, NULL));
	sub_menu->append(new LLMenuItemCheckGL("Show XUI names",
										   toggle_show_xui_names, NULL,
										   check_show_xui_names, NULL));
	sub_menu->createJumpKeys();
	menu->appendMenu(sub_menu);
...
bool can_do_deferred_rendering(void*)
{
	static LLCachedControl<bool> bumps(gSavedSettings, "RenderObjectBump");
	static LLCachedControl<bool> windlight(gSavedSettings,
										   "WindLightUseAtmosShaders");
	static LLCachedControl<bool> avatarvp(gSavedSettings, "RenderAvatarVP");
	return bumps && windlight && avatarvp;
}

bool deferred_rendering_enabled(void*)
{
	static LLCachedControl<bool> deferred(gSavedSettings, "RenderDeferred");
	return deferred;
}

bool clear_arb_enabled(void*)
{
	return !gGLManager.mIsNVIDIA;
}

bool clear_arb_check(void*)
{
	return gClearARBbuffer;
}
...
void handle_selected_texture_info(void*)
{
		 ...
		 iter != end; ++iter)
	{
		...
		std::string msg = "Texture info for primitive \"" + node->mName;
		msg += "\" (UUID: " + node->getObject()->getID().asString() + "):";
		LLChat chat(msg);
		...
		bool can_see_id = gAgent.isGodlikeWithoutAdminMenuFakery() ||
						  (node->mPermissions->getOwner() == gAgentID &&
						   ((gIsInSecondLife &&
							 node->mPermissions->getCreator() == gAgentID) ||
							(!gIsInSecondLife && (node->mPermissions->getMaskOwner() &
...
void handle_selected_material_info(void*)
{
	...
	std::string msg, material_id_string;
	for (LLObjectSelection::valid_iterator
			iter = gSelectMgr.getSelection()->valid_begin(),
			end = gSelectMgr.getSelection()->valid_end();
		 iter != end; ++iter)
	{
		...
		msg = "Material info for primitive: " + node->mName;
		LLChat chat(msg);
		LLFloaterChat::addChat(chat);
		...
		for (U8 i = 0; i < te_count; ++i)
		{
			...
			const LLMaterialID& material_id = tep->getMaterialID();
		...
		bool can_see_id = gAgent.isGodlikeWithoutAdminMenuFakery() ||
						  (node->mPermissions->getOwner() == gAgentID &&
						   ((gIsInSecondLife &&
							 node->mPermissions->getCreator() == gAgentID) ||
							(!gIsInSecondLife &&
							 (node->mPermissions->getMaskOwner() &
							  PERM_ITEM_UNRESTRICTED) == PERM_ITEM_UNRESTRICTED)));
		for (map_t::iterator it = faces_per_material.begin(),
							 end = faces_per_material.end(); it != end; ++it)
		{
			const LLMaterialID& material_id = it->first;
			if (can_see_id)
			{
				material_id_string = material_id.asString();
			}
			else
			{
				material_id_string = material_id.isNull() ? "no material"
														  : "material present";
			}
			msg = material_id_string + " on face ";
...
void reload_selected_texture(void*)
{
		 ...
		 iter != gSelectMgr.getSelection()->valid_end(); ++iter)
	{
		...
		if (sculpt &&
			(sculpt->getSculptType() &
			 LL_SCULPT_TYPE_MASK) != LL_SCULPT_TYPE_MESH)
		{
			const LLUUID& uuid = sculpt->getSculptTexture();
			if (uuid.notNull())
			{
				if (!reloaded.count(uuid))
				{
					LLViewerFetchedTexture* tex =
						LLViewerTextureManager::getFetchedTexture(uuid,
																  FTT_DEFAULT,
																  true,
					...
					if (tex)
					{
						S32 count = tex->getNumVolumes(LLRender::SCULPT_TEX);
						const LLViewerTexture::ll_volume_list_t* volumes =
							tex->getVolumeList(LLRender::SCULPT_TEX);
						...
						{
							LLVOVolume* volume = volumes->at(i);
							if (volume)
							{
					...
					reloaded.emplace(uuid);		// Mark as reloaded
				}
				// Force an object geometry rebuild
				object->markForUpdate(true);
			}
		}
		LLViewerTexture* deflt =
		...
		for (U8 i = 0, count = object->getNumTEs(); i < count; ++i)
		{
			...
			if (img)
			{
				const LLUUID& uuid = img->getID();
				if (uuid.notNull())
				{
					// To flag as texture changed:
					object->setTETexture(i, IMG_DEFAULT);
					if (!reloaded.count(uuid))
					{
						LLViewerFetchedTexture* tex =
							LLViewerTextureManager::staticCastToFetchedTexture(img);
						if (tex)
						{
							// Force a reload of the raw image
							tex->forceRefetch();
					...
					object->setTETexture(i, uuid);
				}
			}
			img = object->getTENormalMap(i);
			if (img && img != deflt)
			{
				const LLUUID& uuid = img->getID();
				if (!reloaded.count(uuid))
		...
		// Also allow to reload linden trees' texture
		LLVOTree* treevobj = dynamic_cast<LLVOTree*>(object);
		if (treevobj)
		{
			LLViewerFetchedTexture* tex = treevobj->getTreeTexture();
			if (tex)
			{
				const LLUUID& uuid = tex->getID();
				if (!reloaded.count(uuid))
				{
					// Force a reload of the raw image
					tex->forceRefetch();
...
{
	S32 details = (S32)(intptr_t)userdata;
	gSavedSettings.setS32("RenderShadowDetail", details);
}

bool shadows_check_control(void* userdata)
{
	static LLCachedControl<S32> shadows(gSavedSettings, "RenderShadowDetail");
	S32 details = (S32)(intptr_t)userdata;
	return shadows == details;
}

bool shadows_enabled(void*)
{
	static LLCachedControl<S32> shadows(gSavedSettings, "RenderShadowDetail");
	return deferred_rendering_enabled(NULL) && shadows > 0;
}

bool mesh_shadows_check_control(void*)
{
	static LLCachedControl<bool> shadows(gSavedSettings, "RenderRiggedShadow");
	return shadows_enabled(NULL) && shadows;
}

bool deferred_wl_rendering_enabled(void*)
{
	static LLCachedControl<bool> deferred(gSavedSettings, "RenderDeferred");
	return deferred && !gUseNewShaders;
}

bool deferred_invisible_check_control(void*)
{
	static LLCachedControl<bool> invisi(gSavedSettings,
										"RenderDeferredInvisible");
	return gUseNewShaders || invisi;
}
...
void handle_objects_visibility(void*)
{
	...
	static LLCachedControl<U32> delay(gSavedSettings,
									  "OcclusionPauseDuration");
	if (delay)
	{
		// *HACK: pause object cache occlusion to force a re-calculation of the
		// occlusions.
		LLVOCachePartition::sPauseOcclusionEndTime = gFrameTimeSeconds + delay;
	}
#if 0	// Does not seem to make any difference, with or without object cache
		// occlusion pausing...
	// Reset existing occlusions
		 ...
		 iter != end; ++iter)
	{
		...
		{
			LLVOCachePartition* partp = regionp->getVOCachePartition();
			if (partp)
			{
				partp->resetOccluders();
			}
		}
	}
#endif
	for (S32 i = 0, count = gObjectList.getNumObjects(); i < count; ++i)
	{
		...
		{
#if 1		// Also resets rotation of target-omega objects
...
void init_debug_rendering_menu(LLMenuGL* menu)
{
	...
	LLMenuGL* sub_menu = new LLMenuGL("Types");
	menu->appendMenu(sub_menu);
	...
	sub_menu->append(new LLMenuItemCheckGL("Avatar",
											&LLPipeline::toggleRenderTypeControl, NULL,
											&LLPipeline::hasRenderTypeControl,
											(void*)LLPipeline::RENDER_TYPE_AVATAR,
											'4', MASK_CONTROL|MASK_ALT|MASK_SHIFT));
	sub_menu->append(new LLMenuItemCheckGL("Animesh",
											&LLPipeline::toggleRenderTypeControl, NULL,
											&LLPipeline::hasRenderTypeControl,
	...
	sub_menu->append(new LLMenuItemCheckGL("Ground",
											&LLPipeline::toggleRenderTypeControl, NULL,
											&LLPipeline::hasRenderTypeControl,
											(void*)LLPipeline::RENDER_TYPE_GROUND,
											'8', MASK_CONTROL|MASK_ALT|MASK_SHIFT));
	...
	sub_menu->append(new LLMenuItemCheckGL("Highlighted",
											LLPipeline::toggleRenderDebugFeature, NULL,
											LLPipeline::toggleRenderDebugFeatureControl,
											(void*)LLPipeline::RENDER_DEBUG_FEATURE_HIGHLIGHTED,
	...
	sub_menu->append(new LLMenuItemCheckGL("World axes", menu_toggle_control,
										   NULL, menu_check_control,
										   (void*)"ShowAxes"));
	sub_menu->append(new LLMenuItemCheckGL("Hit boxes", menu_toggle_control,
										   NULL, menu_check_control,
										   (void*)"RenderDebugHitBox"));
	sub_menu->append(new LLMenuItemCheckGL("Bounding boxes",
										   LLPipeline::toggleRenderDebug, NULL,
										   LLPipeline::toggleRenderDebugControl,
	...
	sub_menu->append(new LLMenuItemCheckGL("Avatar complexity/visibility rank",
										   LLPipeline::toggleRenderDebug, NULL,
										   LLPipeline::toggleRenderDebugControl,
										   (void*)LLPipeline::RENDER_DEBUG_AVATAR_DRAW_INFO,
										   'C', MASK_CONTROL|MASK_ALT));
	sub_menu->append(new LLMenuItemCheckGL("Attachments memory/area",
										   LLPipeline::toggleRenderDebug, NULL,
										   LLPipeline::toggleRenderDebugControl,
	...
	sub_menu->append(new LLMenuItemCheckGL("Texture size",
										   LLPipeline::toggleRenderDebug, NULL,
										   LLPipeline::toggleRenderDebugControl,
	...
	sub_menu->append(new LLMenuItemToggleGL("Frame test",
											&LLPipeline::sRenderFrameTest));

	sub_menu->append(new LLMenuItemCallGL("Frame profile",
										  frame_render_profile, NULL, NULL));

	sub_menu->append(new LLMenuItemCheckGL("Debug GL", menu_toggle_control,
										   NULL, menu_check_control,
										   (void*)"RenderDebugGL"));
	...
	sub_menu->append(new LLMenuItemCheckGL("Deferred rendering",
										   menu_toggle_control,
										   can_do_deferred_rendering,
										   menu_check_control,
										   (void*)"RenderDeferred",
										   'D', MASK_CONTROL|MASK_ALT));

	sub_menu->append(new LLMenuItemCheckGL("No shadow",
										   shadows_toggle,
										   deferred_rendering_enabled,
										   shadows_check_control,
										   NULL));

	sub_menu->append(new LLMenuItemCheckGL("Sun and Moon shadows",
										   shadows_toggle,
										   deferred_rendering_enabled,
										   shadows_check_control,
										   (void*)1));

	sub_menu->append(new LLMenuItemCheckGL("All lights shadows",
										   shadows_toggle,
										   deferred_rendering_enabled,
										   shadows_check_control,
										   (void*)2));

	sub_menu->append(new LLMenuItemCheckGL("Rigged mesh shadows",
										   menu_toggle_control,
										   shadows_enabled,
										   mesh_shadows_check_control,
										   (void*)"RenderRiggedShadow"));
	...
	sub_menu->append(new LLMenuItemCheckGL("Screen space ambient occlusion",
										   menu_toggle_control,
										   deferred_rendering_enabled,
										   menu_check_control,
										   (void*)"RenderDeferredSSAO"));

	sub_menu->append(new LLMenuItemCheckGL("Render invisiprims",
										   menu_toggle_control,
										   deferred_wl_rendering_enabled,
										   deferred_invisible_check_control,
	...
	sub_menu = new LLMenuGL("Textures");

	sub_menu->append(new LLMenuItemToggleGL("Animate textures",
											&LLVOVolume::sAnimateTextures));

	sub_menu->append(new LLMenuItemToggleGL("Disable textures",
											&LLViewerTexture::sDontLoadVolumeTextures));

	sub_menu->append(new LLMenuItemCheckGL("Load textures progressively",
										   menu_toggle_control, NULL,
										   menu_check_control,
										  (void*)"TextureProgressiveLoad"));
	sub_menu->append(new LLMenuItemCheckGL("Scale down fetched textures",
										   menu_toggle_control, NULL,
										   menu_check_control,
										   (void*)"TextureRescaleFetched"));
	sub_menu->append(new LLMenuItemCheckGL("Boost attachments textures LOD",
										   menu_toggle_control, NULL,
										   menu_check_control,
										  (void*)"TextureBoostAttachments"));
	sub_menu->append(new LLMenuItemCheckGL("Boost fetches with speed",
										   menu_toggle_control, NULL,
										   menu_check_control,
										   (void*)"TextureFetchBoostWithSpeed",
										   'B', MASK_CONTROL|MASK_SHIFT));
	sub_menu->append(new LLMenuItemCheckGL("Boost proportional to active fetches",
										   menu_toggle_control, NULL,
										   menu_check_control,
										   (void*)"TextureFetchBoostWithFetches"));
	sub_menu->append(new LLMenuItemCallGL("Boost textures fetches now",
										  boost_texture_fetches_now, NULL,
										  NULL, 'B', MASK_CONTROL));
	...
	menu->append(new LLMenuItemCallGL("Selected material info",
	...
	menu->append(new LLMenuItemCheckGL("Clear ARB buffers when reused",
									   menu_toggle_control, clear_arb_enabled,
									   clear_arb_check,
									   (void*)"RenderClearARBBuffer"));
...
bool enable_avatar_textures(void*)
{
	...
	// by the agent. We reuse LLObjectBackup's logic which implements both the
		 ...
		 t  < (S32)LLWearableType::WT_COUNT; ++t)
	{
		...
		for (U32 index = 0; index < count; ++index)
		{
			...
			if (itemp &&
				!LLObjectBackup::validatePerms(&itemp->getPermissions()))
			{
				return false;
			}
		}
...
void init_debug_avatar_menu(LLMenuGL* menu)
{
	LLMenuGL* sub_menu = new LLMenuGL("Auto-pilot recorder");

	sub_menu->append(new LLMenuItemCallGL("Begin record",
										  LLAgentPilot::beginRecord,
										  enable_autopilot_begin_record));
	sub_menu->append(new LLMenuItemCallGL("End record",
										  LLAgentPilot::endRecord,
										  enable_autopilot_end_record));
	sub_menu->append(new LLMenuItemCallGL("Forget record",
										  LLAgentPilot::forgetRecord,
										  enable_autopilot_start_playback));
	sub_menu->appendSeparator();
	sub_menu->append(new LLMenuItemCallGL("Start playback",
										  LLAgentPilot::startPlayback,
										  enable_autopilot_start_playback));
	sub_menu->append(new LLMenuItemCallGL("Stop playback",
										  LLAgentPilot::stopPlayback,
										  enable_autopilot_stop_playback));
	sub_menu->append(new LLMenuItemToggleGL("Loop playback",
											&LLAgentPilot::sLoop));
	sub_menu->append(new LLMenuItemToggleGL("Allow flying",
	...
	sub_menu->append(new LLMenuItemCallGL("Force visual params to default",
	...
	sub_menu->append(new LLMenuItemCheckGL("Show avatar joints",
										   LLPipeline::toggleRenderDebug,
										   NULL,
										   LLPipeline::toggleRenderDebugControl,
										   (void*)LLPipeline::RENDER_DEBUG_AVATAR_JOINTS));
	sub_menu->append(new LLMenuItemCheckGL("Show agent target",
										   LLPipeline::toggleRenderDebug,
										   NULL,
										   LLPipeline::toggleRenderDebugControl,
										   (void*)LLPipeline::RENDER_DEBUG_AGENT_TARGET));
	sub_menu->append(new LLMenuItemCheckGL("Show above for self only",
										   menu_toggle_control, NULL,
										   menu_check_control,
	...
	menu->append(new LLMenuItemCheckGL("Local baking/appearance",
									   ...
									   MASK_CONTROL | MASK_ALT| MASK_SHIFT));
	sub_menu = new LLMenuGL("Grab baked texture");
	...
	sub_menu = new LLMenuGL("All avatars' animations speed");
	sub_menu->append(new LLMenuItemCallGL("10% slower",
										  handle_slower_animations));
	sub_menu->append(new LLMenuItemCallGL("10% faster",
										  handle_faster_animations));
	sub_menu->append(new LLMenuItemCallGL("Reset to normal speed",
...
void derez_objects(EDeRezDestination dest, const LLUUID& dest_id)
{
	...
	if (error.empty() && objects_list.size() > 0)
	{
		...
		for (U8 packet_number = 0; packet_number < packet_count; ++packet_number)
		{
			...
			{
				LLViewerObject* object = objects_list[object_index++];
				msg->nextBlockFast(_PREHASH_ObjectData);
				msg->addU32Fast(_PREHASH_ObjectLocalID, object->getLocalID());
				// VEFFECT: DerezObject
				effectp = (LLHUDEffectSpiral*)LLHUDManager::createEffect(LLHUDObject::LL_HUD_EFFECT_POINT);
...
//MK
	if (gRLenabled)
	{
		...
		LLToolPie* toolp = LLToolPie::getInstance();
		if (!gRLInterface.canTouchFar(objp, toolp->getPick().mIntersection))
		{
			return true;
		}
	}
...
//mk
		...
		if (gAgent.getFocusOnAvatar() && !toolmgr->inEdit() &&
		...
		gBasicToolset->selectTool(LLToolCompCreate::getInstance());
...
//MK
		if (gRLenabled)
		{
			...
			LLToolPie* tp = LLToolPie::getInstance();
			if (!gRLInterface.canTouchFar(objp, tp->getPick().mIntersection))
			{
				return false;
			}
		}
//mk
		...
		toolmgr->setCurrentToolset(gBasicToolset);
		gFloaterToolsp->setEditTool(LLToolCompTranslate::getInstance());
...
class LLMuteParticle final : public view_listener_t
{
	...
	{
		std::string data = userdata.asString();
		LLToolPie* toolpie = LLToolPie::getInstance();
		LLUUID id = toolpie->getPick().mParticleOwnerID;
		if (!data.empty() && id.notNull() && id != gAgentID)
		{
			...
			const LLUUID& source_id = toolpie->getPick().mParticleSourceID;
...
class LLReportParticleAbuse final : public view_listener_t
{
	...
	{
		LLToolPie* toolpie = LLToolPie::getInstance();
		const LLUUID& owner_id = toolpie->getPick().mParticleOwnerID;
		const LLUUID& source_id = toolpie->getPick().mParticleSourceID;
...
bool callback_freeze(const LLSD& notification, const LLSD& response)
{
	...
	if (option == 0 || option == 1)
	{
		...
		if (avatarp && avatarp->getRegion())
		{
			...
			msg->addU32("Flags", option == 1 ? 0x0 : 0x1);
...
class LLAvatarFreeze final : public view_listener_t
{
	...
	{
		LLVOAvatar* avatar;
		avatar = find_avatar_from_object(gSelectMgr.getSelection()->getPrimaryObject());
		if (avatar)
		{
...
//mk
			...
			{
				gNotifications.add("FreezeAvatar", LLSD(), payload,
								   callback_freeze);
			}
		}
		return true;
...
//mk
			...
			LLParcel* parcel;
			parcel = gViewerParcelMgr.selectParcelAt(pos)->getParcel();
			if (parcel &&
				gViewerParcelMgr.isParcelOwnedByAgent(parcel,
													  GP_LAND_MANAGE_BANNED))
			{
				...
				{
    				gNotifications.add("EjectAvatarFullname", LLSD(), payload,
    								   callback_eject);
				}
			}
			else
			{
				...
				if (!fullname.empty())
				{
    				LLSD args;
    				args["AVATAR_NAME"] = fullname;
    				gNotifications.add("EjectAvatarFullnameNoBan", args,
									   payload, callback_eject);
				}
				else
				{
    				gNotifications.add("EjectAvatarNoBan", LLSD(), payload,
    								   callback_eject);
				}
			}
		}
	...
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		if (new_value)
		{
			...
			if (new_value)
			{
				...
				LLParcel* parcel;
				parcel = gViewerParcelMgr.selectParcelAt(pos_global)->getParcel();
				if (parcel && (!new_value || region->isOwnedGroup(pos)))
				{
					new_value = gViewerParcelMgr.isParcelOwnedByAgent(parcel,
																	  GP_LAND_ADMIN);
				}
			}
		}
...
void handle_buy_object(LLSaleInfo sale_info)
{
	...
	if (price > 0 && gStatusBarp && price > gStatusBarp->getBalance())
	{
...
class LLViewFullscreen final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gViewerWindowp->toggleFullscreen(true);
		return true;
	}
};
...
class LLEditEnableDuplicate final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canDuplicate();
//MK
		if (gRLenabled && gRLInterface.mContainsRez)
		{
			new_value = false;
...
class LLToolsBuyOrTake final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		if (is_selection_buy_not_take())
		{
			...
			if (total_price == 0 ||
				(gStatusBarp && total_price <= gStatusBarp->getBalance()))
			{
				handle_buy(NULL);
			}
			else
...
class LLToolsEnableSelectNextPart final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool new_value = (!gSelectMgr.getSelection()->isEmpty() &&
						  ...
						  LLToolMgr::getInstance()->getCurrentTool() ==
							LLToolFace::getInstance();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
{
	bool cycle_faces = LLToolMgr::getInstance()->getCurrentTool() ==
						LLToolFace::getInstance();
...
class LLEditEnableCut final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canCut();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLEditEnableCopy final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canCopy();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLEditEnablePaste final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canPaste();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLEditEnableDelete final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canDoDelete();
//MK
		if (gRLenabled && gRLInterface.mContainsRez &&
...
class LLEditEnableDeselect final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canDeselect();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLEditEnableSelectAll final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canSelectAll();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLEditEnableUndo final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canUndo();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLEditEnableRedo final : public view_listener_t
{
	...
	{
		bool new_value = LLEditMenuHandler::gEditMenuHandler &&
						 LLEditMenuHandler::gEditMenuHandler->canRedo();
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
...
class LLPromptShowOneOfURLs final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		LLControlVariable* ctrlp = gSavedSettings.getControl(setting.c_str());
		if (!ctrlp)
		{
...
//mk
			...
			gSavedSettings.getBool("EditCameraMovement"))
		{
			// Zoom in if we are looking at the avatar
			gAgent.setFocusOnAvatar(false);
			gAgent.setFocusGlobal(tp->getPick());
		...
		gViewerParcelMgr.selectParcelAt(tp->getPick().mPosGlobal);
...
class LLWorldEnableWindlightRegionTime final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		bool ok = (!gUseNewShaders && !LLEnvironment::overrideWindlight()) ||
				  // Case of OpenSim (no EE) when in EE rendering mode...
				  (gUseNewShaders &&
				   !LLEnvironment::isExtendedEnvironmentEnabled());
...
class LLToolsSelectOnlyMyObjects final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gSavedSettings.setBool("SelectOwnedOnly",
							   !gSavedSettings.getBool("SelectOwnedOnly"));
		return true;
	}
};
...
class LLToolsSelectOnlyMovableObjects final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gSavedSettings.setBool("SelectMovableOnly",
...
class LLToolsSelectBySurrounding final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gSavedSettings.setBool("RectangleSelectInclusive",
							   !gSavedSettings.getBool("RectangleSelectInclusive"));
		return true;
	}
};
...
class LLToolsShowSelectionHighlights final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gSavedSettings.setBool("RenderHighlightSelections",
...
class LLToolsShowHiddenSelection final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gSavedSettings.setBool("RenderHiddenSelections",
							   !gSavedSettings.getBool("RenderHiddenSelections"));
		return true;
	}
};
...
class LLToolsShowSelectionLightRadius final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		gSavedSettings.setBool("RenderLightRadius",
							   !gSavedSettings.getBool("RenderLightRadius"));
		return true;
	}
};
...
//mk
		...
		// *HACK: force LLVolumeGeometryManager::genDrawInfo() on all faces
		// of all objects, so that transparent faces are rendered again.
		for (S32 i = 0, count = gObjectList.getNumObjects(); i < count; ++i)
		{
			LLViewerObject* object = gObjectList.getObject(i);
			if (object && !object->isDead())
...
//mk
		...
		if (tool_name == "focus")
		{
			toolmgr->getCurrentToolset()->selectToolByIndex(1);
		}
		else if (tool_name == "move")
		{
			toolmgr->getCurrentToolset()->selectToolByIndex(2);
		}
		else if (tool_name == "edit")
		{
			toolmgr->getCurrentToolset()->selectToolByIndex(3);
		}
		else if (tool_name == "create")
		{
			toolmgr->getCurrentToolset()->selectToolByIndex(4);
		}
		else if (tool_name == "land")
		{
			toolmgr->getCurrentToolset()->selectToolByIndex(5);
		}
		else
		{
...
class LLFileEnableUpload final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		...
		bool new_value = !HBFileSelector::isInUse() && gStatusBarp &&
						 gStatusBarp->getBalance() >= cost;
		gMenuHolderp->findControl(userdata["control"].asString())->setValue(new_value);
		return true;
	}
};
						  ...
						  std::deque<std::string>& files, void*)
{
	...
	if (no_upload)
	{
		...
		LLResourceUploadInfo::ptr_t
			info(new LLNewFileResourceUploadInfo(filename,
						  ...
						  std::string& filename, void*)
{
	...
	else if (type == HBFileSelector::FFLOAD_SOUND)
	{
		...
		S32 cost = LLEconomy::getInstance()->getSoundUploadCost();
		LLFloaterNameDesc* floaterp = new LLFloaterNameDesc(filename, cost);
		LLUICtrlFactory::getInstance()->buildFloater(floaterp,
													 "floater_sound_preview.xml");
		floaterp->childSetLabelArg("ok_btn", "[AMOUNT]", llformat("%d", cost));
	}
	else if (type == HBFileSelector::FFLOAD_ANIM)
	{
		if (ext == "bvh")
		{
			LLFloaterAnimPreview* floaterp =
		...
		if (new_value)
		{
			struct ff final : public LLSelectedNodeFunctor
			{
				...
				bool apply(LLSelectNode* node) override
				{
					...
					// texture. This is done later in llviewerobjectbackup.cpp.
...
class LLFileImportObject final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		LLObjectBackup::importObject(false);
		return true;
	}
};
...
class LLFileUpLoadImportObject final : public view_listener_t
{
	bool handleEvent(LLPointer<LLEvent>, const LLSD& userdata) override
	{
		LLObjectBackup::importObject(true);
		return true;
	}
};
...
//mk
	...
	addMenu(new LLToolsSelectOnlyMovableObjects(),
			"Tools.SelectOnlyMovableObjects");
	addMenu(new LLToolsSelectBySurrounding(), "Tools.SelectBySurrounding");
	addMenu(new LLToolsShowSelectionHighlights(),
			"Tools.ShowSelectionHighlights");
	addMenu(new LLToolsShowHiddenSelection(), "Tools.ShowHiddenSelection");
	addMenu(new LLToolsShowSelectionLightRadius(),
			"Tools.ShowSelectionLightRadius");
