...
struct LLPanelFaceSetAlignedTEFunctor final : public LLSelectedTEFunctor
{
	...
	struct LLMaterialEditFunctor
	{
		...
		void apply(LLMaterialPtr& material)
		{
			(material->*(MaterialEditFunc))(_data);
		}
					 ...
					 const LLUUID& only_for_obj_id = LLUUID::null)
	{
		...
		struct LLSelectedTEEditMaterial : public LLSelectedTEMaterialFunctor
		{
			...
			:	_panel(panel),
				_edit(editp),
				_only_for_obj_id(only_for_obj_id)
			{
			}

			LLMaterialPtr apply(LLViewerObject* object, S32 face,
								LLTextureEntry* tep,
								LLMaterialPtr& cur_mat) override
			{
				if (!_edit)
				{
					return NULL;
				...
				if (_only_for_obj_id.notNull() &&
					_only_for_obj_id != object->getID())
				{
					return NULL;
				}

				LLMaterialPtr new_mat = _panel->createDefaultMaterial(cur_mat);
				if (!new_mat)
				{
					return NULL;
				}
				...
				// useful)
				//
				// _panel->isAlpha() "lies" when one face has alpha and the
				// rest do not (NORSPEC-329) need to get per-face answer to
				...
				U8 dflt_alpha_mode =
					cur_mat.isNull() ? orig_deflt_alpha_mode
									 : cur_mat->getDiffuseAlphaMode();
				...
				new_mat->setDiffuseAlphaMode(dflt_alpha_mode);

				// Apply change
				_edit->apply(new_mat);

				U32 new_alpha_mode = new_mat->getDiffuseAlphaMode();
				if (!is_alpha_face &&
					new_alpha_mode == LLMaterial::DIFFUSE_ALPHA_MODE_BLEND)
				{
					new_alpha_mode = LLMaterial::DIFFUSE_ALPHA_MODE_NONE;
					new_mat->setDiffuseAlphaMode(new_alpha_mode);
				}

				const LLUUID& obj_id = object->getID();
				if (new_alpha_mode != orig_deflt_alpha_mode ||
					new_mat->getNormalID().notNull() ||
					new_mat->getSpecularID().notNull())
				{
					LL_DEBUGS("Materials") << "Putting material on object "
										   << obj_id << " - Face " << face
										   << " - Material: "
										   << new_mat->asLLSD() << LL_ENDL;
					LLMaterialMgr::getInstance()->put(obj_id, face, *new_mat);
				}
				else
				{
				...
				object->setTEMaterialParams(face, new_mat);

				return new_mat;
			}

			LLMaterialEditFunctor<MaterialEditFunc>*	_edit;
			LLPanelFace*								_panel;
			LLUUID										_only_for_obj_id;
	...
	bool apply(LLViewerObject* object, S32 te) override
	{
		...
		if (set_aligned)
		{
			...
			if (set_aligned)
			{
				...
				{
					object->setTEOffset(te, offset_x, offset_y);
...
LLFace* LLPanelFace::getLastSelectedFace()
{
	struct get_last_face_func final : public LLSelectedTEGetFunctor<LLFace*>
	{
		LLFace* get(LLViewerObject* object, S32 te) override
		{
			...
			return drawable ? drawable->getFace(te) : NULL;
		}
	} func;
	LLFace* last_face = NULL;
	gSelectMgr.getSelection()->getSelectedTEValue(&func, last_face);
	return last_face;
}
...
void LLPanelFace::getState()
{
		...
		(objectp->permModify() || gAgent.isGodlikeWithoutAdminMenuFakery()))
	{
		...
		LLUUID id;
		{
			struct tex_get final : public LLSelectedTEGetFunctor<LLUUID>
			{
				LLUUID get(LLViewerObject* object, S32 te_index) override
				{
					...
					const LLUUID& te_id = te ? te->getID() : LLUUID::null;
					if (te && LLAvatarAppearanceDictionary::isBakedImageId(te_id))
					{
						return te_id;
					}
					LLUUID id;
					...
					{
						LLViewerTexture* tex = gTextureList.findImage(te_id);
						if (!tex)
						{
		...
		mLabelAlignMedia->setEnabled(editable && has_media);
		...
		LLColor4 color = LLColor4::white;
		{
			...
			{
				LLColor4 get(LLViewerObject* object, S32 face) override
				{
					LLTextureEntry* te = object->getTE(face);
					return te ? te->getColor() : LLColor4::white;
				}
			} func;
		...
		GLenum image_format = 0;
		{
			...
			{
				GLenum get(LLViewerObject* object, S32 te_index) override
				{
					GLenum image_format = GL_RGB;
					LLViewerTexture* image = object->getTEImage(te_index);
					if (image)
					{
						image_format = image->getPrimaryFormat();
					}
					return image_format;
				}
			} func;
		...
		U8 alpha_mode;
		{
			...
			{
				U8 get(LLViewerObject* object, S32 te_index) override
				{
					U8 ret = 1;
					LLTextureEntry* te = object->getTE(te_index);
					LLMaterial* mat = te ? te->getMaterialParams().get()
										 : NULL;
					if (mat)
					{
		...
		bool enabled;
		{
			struct planar_get final : public LLSelectedTEGetFunctor<bool>
			{
				bool get(LLViewerObject* object, S32 face) override
				{
					LLTextureEntry* te = object->getTE(face);
					return te &&
						   te->getTexGen() == LLTextureEntry::TEX_GEN_PLANAR;
				}
			} func1;
		...
		bool identical_planar_texgen;
		{
			...
			{
				LLTextureEntry::e_texgen get(LLViewerObject* object,
											 S32 face) override
				{
					LLTextureEntry* te = object->getTE(face);
			...
			case MATTYPE_SPECULAR:
			{
				...
				:	public LLSelectedTEGetFunctor<F32>
				{
					F32 get(LLViewerObject* object, S32 face) override
					{
						...
						repeats_s /= object->getScale().mV[VX];
						repeats_t /= object->getScale().mV[VY];
...
#if 1
						F32 repeats_s = te->mScaleS /
										object->getScale().mV[s_axis];
						F32 repeats_t = te->mScaleT /
										object->getScale().mV[t_axis];
						return llmax(repeats_s, repeats_t);
#else
						return te->mScaleS / object->getScale().mV[s_axis];
#endif
					}
				} func;
				identical = selection->getSelectedTEValue(&func, repeats,
		...
		if (mIsAlpha || normmap_id.notNull() || specmap_id.notNull())
		{
		...
		// Materials
		{
			...
			if (material && editable)
			{
				...
				// There's duplicate code below, with what we already dealt
...
void LLPanelFace::updateMaterial()
{
		...
		shininess == SHINY_TEXTURE)
	{
		...
		LLMaterialPtr cur_mat;
		selection->getSelectedTEValue(&func, cur_mat);
		bool new_mat = cur_mat.isNull();
		LLMaterialPtr material;
		if (new_mat)
		{
			material = new LLMaterial();
		}
		else
		{
			material = new LLMaterial(cur_mat->asLLSD());
		}
		if (material.isNull())
		{
		...
		{
			material->setDiffuseAlphaMode(mComboAlphaMode->getCurrentIndex());
			material->setAlphaMaskCutoff((U8)mMaskCutoff->getValue().asInteger());
		}
			...
			!mNormalCtrl->getTentative())
		{
			...
			material->setNormalOffset(mBumpyOffsetU->getValue().asReal(),
			...
			material->setNormalRotation(mBumpyRot->getValue().asReal() * DEG_TO_RAD);
		}
		else if (!mNormalCtrl->getTentative())
		{
			LL_DEBUGS("Materials") << "Removing normal map texture, bumpiness = "
								   << bumpiness << LL_ENDL;
			material->setNormalID(LLUUID::null);
			material->setNormalOffset(0.f, 0.f);
			material->setNormalRepeat(1.f, 1.f);
			material->setNormalRotation(0.f);
		}
...

		{
			...
			material->setSpecularOffset(mShinyOffsetU->getValue().asReal(),
			...
			material->setSpecularRotation(mShinyRot->getValue().asReal() * DEG_TO_RAD);

			// Override shininess to 0.2f if this is a new material
			if (!new_mat)
			{
				material->setSpecularLightColor(mShinyColorSwatch->get());
				material->setSpecularLightExponent(mGlossiness->getValue().asInteger());
				material->setEnvironmentIntensity(mEnvironment->getValue().asInteger());
			}
		}
		else if (!mSpecularCtrl->getTentative())
		{
			LL_DEBUGS("Materials") << "Removing specular map texture, shininess = "
								   << shininess << LL_ENDL;
			material->setSpecularID(LLUUID::null);
			material->setSpecularOffset(0.f, 0.f);
			material->setSpecularRepeat(1.f, 1.f);
			material->setSpecularRotation(0.f);
		...
		LL_DEBUGS("Materials") << "Updating material:\n" << material->asLLSD()
							   << LL_ENDL;
		gSelectMgr.selectionSetMaterials(material);
	}
	else
	{
...
LLMaterialPtr LLPanelFace::createDefaultMaterial(LLMaterialPtr curmat)
{
	...
	if (curmat.isNull())
	{
		...
		newmat = new LLMaterial();
		if (newmat)
		{
			newmat->setDiffuseAlphaMode(alpha_mode);
		}
	}
	else
	{
		newmat = new LLMaterial(curmat->asLLSD());
	}
	if (!newmat)
	{
...
// diffuse, normal and specular maps. The elements enabling/disabling is done
// in getState(), based on the primitive parameters.
//static
