...
# Cool VL Viewer wrapper script v3.31 (c)2007-2021 Henri Beauchamp.
...
if [ "$1" == "--check-libs" ] ; then
	...
	if [ -x ./bin/tracy ] ; then
		echo
		echo "Checking for missing system libraries for the Tracy server:"
		...
		echo "==> The bundled Tracy server $cando be used on this system."
	fi
	cando="cannot"
