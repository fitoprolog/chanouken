...
void LLWLWaterParamMgr::applyToParcel(const LLSD& water_params)
{
	static const std::string name = "current parcel environment";

	LL_DEBUGS("Windlight") << "Setting from parcel environment:\n"
						   <<  ll_pretty_print_sd(water_params)
						   << LL_ENDL;
...
{
	// Bind the variables only if we are using shaders
	if (gPipeline.canUseVertexShaders())
	{
		for (LLViewerShaderMgr::shader_iter
				it = gViewerShaderMgrp->beginShaders(),
				end = gViewerShaderMgrp->endShaders();
			 it != end; ++it)
		{
			if (it->mProgramObject &&
				it->mShaderGroup == LLGLSLShader::SG_WATER)
			{
...
void LLWLWaterParamMgr::updateShaderUniforms(LLGLSLShader* shader)
{
	if (shader->mShaderGroup == LLGLSLShader::SG_WATER)
	{
		shader->uniform4fv(LLShaderMgr::LIGHTNORM, 1,
						   gWLSkyParamMgr.getRotatedLightDir().mV);
		shader->uniform3fv(LLShaderMgr::WL_CAMPOSLOCAL, 1,
						   gViewerCamera.getOrigin().mV);
		shader->uniform4fv(LLShaderMgr::WATER_FOGCOLOR, 1,
						   LLDrawPoolWater::sWaterFogColor.mV);
		...
		shader->uniform1f(LLShaderMgr::DISTANCE_MULTIPLIER, 0);
	}
}

void LLWLWaterParamMgr::update(LLViewerCamera* cam)
{
	...
	// Sync Windlight editor if it exists
	...
	// Only do this if we are dealing with shaders
	if (gPipeline.canUseVertexShaders())
	{
		...
		LLVector3 sun_or_moon_dir = gSky.sunUp() ? gSky.getSunDirection()
												 : gSky.getMoonDirection();
		sun_or_moon_dir.normalize();
		mWaterFogKS = 1.f / llmax(sun_or_moon_dir.mV[2],
								  WATER_FOG_LIGHT_CLAMP);
