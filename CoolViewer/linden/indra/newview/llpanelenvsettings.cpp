 ...
 *
 * Copyright (c) 2018, Linden Research, Inc, (c) 2019 Henri Beauchamp.
 *
...
void LLPanelEnvSettingsSky::setSettings(const LLSettingsBase::ptr_t& settings,
										bool reset_dirty)
{
	setSky(boost::static_pointer_cast<LLSettingsSky>(settings), reset_dirty);
}
...
void LLPanelEnvSettingsWater::setSettings(const LLSettingsBase::ptr_t& settings,
										  bool reset_dirty)
{
	setWater(boost::static_pointer_cast<LLSettingsWater>(settings),
			 reset_dirty);
}
...
void LLPanelEnvSettingsDay::refresh()
{
	...
	bool env_available = LLEnvironment::isExtendedEnvironmentEnabled();
...
void LLPanelEnvSettingsDay::setSettings(const LLSettingsBase::ptr_t& settings,
										bool reset_dirty)
{
	setDay(boost::static_pointer_cast<LLSettingsDay>(settings), reset_dirty);
}
			 ...
			 it != end; ++it)
		{
			...
			if (i == LLSettingsDay::TRACK_WATER)
			{
				LLSettingsWater::ptr_t waterp =
					boost::static_pointer_cast<LLSettingsWater>(it->second);
...
void LLPanelEnvSettingsDay::updateEditEnvironment()
{
	...
	mSkyBlender = boost::make_shared<LLTrackBlenderLoopingManual>(mScratchSky,
																  mDaySettings,
																  skytrack);
	mWaterBlender =
		boost::make_shared<LLTrackBlenderLoopingManual>(mScratchWater,
														mDaySettings,
														LLSettingsDay::TRACK_WATER);
	if (LLEnvironment::isExtendedEnvironmentEnabled())
	{
										  ...
										  S32 status)
{
	...
	if (!itemp->getPermissions().allowOperationBy(PERM_TRANSFER, gAgentID) &&
	...
	else if (mCurrentTrack == LLSettingsDay::TRACK_WATER)
	{
		...
		LLSettingsDay::ptr_t dayp =
			boost::dynamic_pointer_cast<LLSettingsDay>(settings);
