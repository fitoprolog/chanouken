...
public:
	...
	// LLWorld::sendAgentPause() also controls pause/resume.
	void initMainloopTimeout(const std::string* state, F32 secs = -1.f);
	void destroyMainloopTimeout();
	void pauseMainloopTimeout();
	void resumeMainloopTimeout(const std::string* state = NULL,
							   F32 secs = -1.f);
	void pingMainloopTimeout(const std::string* state, F32 secs = -1.f);
	LL_INLINE void setMainloopTimeoutDefault(F32 t)		{ mMainloopTimeoutDefault = llmax(t, 10.f); }
