...
:	LLHUDEffect(type)
{
	...
	mVoiceSourceWorldPosition		= LLVector3(0.f, 0.f, 0.f);
	...
	mSoundSymbol.mPosition			= LLVector3(0.f, 0.f, 0.f);
	...
	for (U32 i = 0; i < NUM_VOICE_SYMBOL_WAVES; ++i)
	{
...
void LLVoiceVisualizer::render()
{
	...
	if (mCurrentlySpeaking)
	{
		...
		S32 level = 1 + (S32)(fraction * (NUM_VOICE_SYMBOL_WAVES - 2));
...
VoiceGesticulationLevel LLVoiceVisualizer::getCurrentGesticulationLevel()
{
	...
	{
		gesticulationLevel = VOICE_GESTICULATION_LEVEL_HIGH;
	}
	else if (mSpeakingAmplitude > mMinGesticulationAmplitude + range * 0.25f)
	{
		gesticulationLevel = VOICE_GESTICULATION_LEVEL_MEDIUM;
	}
	else if (mSpeakingAmplitude > mMinGesticulationAmplitude + range * 0.00000f)
	{
		gesticulationLevel = VOICE_GESTICULATION_LEVEL_LOW;
	}
...
#if 0	// TODO -- pack the relevant data for voice effects.
