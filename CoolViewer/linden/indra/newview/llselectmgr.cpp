...
:	mHideSelectedObjects(false),
	mRenderHighlightSelections(true),
...
void LLSelectMgr::initClass()
{
	...
	connectRefreshCachedSettingsSafe("RenderHighlightSelections");
	...
	connectRefreshCachedSettingsSafe("RenderHiddenSelections");
...
void LLSelectMgr::connectRefreshCachedSettingsSafe(const std::string name)
{
...
{
	// NOTE: do not bother using LLCachedControls here: this function is rarely
	...
	sRenderHiddenSelections = gSavedSettings.getBool("RenderHiddenSelections");
...
void LLSelectMgr::selectionSetImage(const LLUUID& imageid)
{
	...
	if (item &&
		!item->getPermissions().allowOperationBy(PERM_COPY, gAgentID) &&
	...
	struct f final : public LLSelectedTEFunctor
	{
		...
		f(LLViewerInventoryItem* item, const LLUUID& id)
		:	mItem(item),
			mImageID(id)
		{
		}
		...
		bool apply(LLViewerObject* objectp, S32 te) override
		{
			...
			{
				// Texture picker defaults are not inventory items
				// * Do not need to worry about permissions for them
				// * Can just apply the texture and be done with it.
				objectp->setTEImage(te,
									LLViewerTextureManager::getFetchedTexture(mImageID,
																			  FTT_DEFAULT,
																			  true,
		...
		bool apply(LLViewerObject* object, S32 te) override
		{
			if (object->permModify())
			{
				...
				if (nodep && te < (S32)nodep->mSavedTextures.size())
				{
					...
					if (id.isNull())
					{
						// This was probably a no-copy texture, leave image
						// as-is
						return false;
					}
					else
					{
						object->setTEImage(te,
			 ...
			 te_num++)
		{
			...
			if (planar == stretch)
			{
				...
				if (tep->getMaterialParams().notNull())
				{
					...
					LLMaterialPtr p = gFloaterToolsp->getPanelFace()->createDefaultMaterial(orig);
								...
								LLObjectSelectionHandle handle)
{
	...
	else if (LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		if (gSelectMgr.mSelectedObjects->mSelectType != SELECT_TYPE_HUD)
		{
			...
			effectp->setColor(LLColor4U(gAgent.getEffectColor()));
			effectp->setDuration(0.5f + (F32)objects_count / 64.f);
		}
		...
		F64 deleted = (F64)objects_count +
					  stats->getStat(LLViewerStats::ST_OBJECT_DELETE_COUNT);
		stats->setStat(LLViewerStats::ST_OBJECT_DELETE_COUNT, deleted);
	}
...
void LLSelectMgr::processObjectProperties(LLMessageSystem* msg, void**)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		node->mValid = true;
		node->mPermissions->init(creator_id, owner_id, last_owner_id,
								 group_id);
		...
		node->mAggregateTexturePerm = ag_tex_perms;
		node->mAggregateTexturePermOwner = ag_tex_perms_owner;
...
void LLSelectMgr::updateSilhouettes()
{
	...
	if (mRectSelectedObjects.size() > 0)
	{
		...
		LLVector3 camera_origin = gViewerCamera.getOrigin();
		...
		for (S32 pass = 0; pass < 2; ++pass)
		{
				 ...
				 iter != mHighlightedObjects->end(); ++iter)
			{
				...
				{
					generateSilhouette(node, camera_origin);
				...
				{
					generateSilhouette(node, LLVector3(-10000.f, 0.f, 0.f));
				}
			}
		}
	}
...
void LLSelectMgr::renderSilhouettes(bool for_hud)
{
	...
	if (mSelectedObjects->getNumNodes())
	{
			 ...
			 iter != end; ++iter)
		{
			...
			{
				bool old_hidden = LLSelectMgr::sRenderHiddenSelections;
				LLSelectMgr::sRenderHiddenSelections = false;
				node->renderOneSilhouette(sContextSilhouetteColor);
				LLSelectMgr::sRenderHiddenSelections = old_hidden;
			}
			 ...
			 iter != end; ++iter)
		{
			...
			else if (!objectp->isSelected())
			{
				const LLColor4& color =
					objectp->isRoot() ? sHighlightParentColor
									  : sHighlightChildColor;
				node->renderOneSilhouette(color);
			}
		}
	}
...
void LLSelectNode::saveTextures(const uuid_vec_t& textures)
{
	if (mObject.notNull())
	{
		...
		for (uuid_vec_t::const_iterator texture_it = textures.begin(),
			 ...
			 texture_it != end; ++texture_it)
		{
			mSavedTextures.emplace_back(*texture_it);
		}
	}
}
...
void LLSelectNode::renderOneWireframe(const LLColor4& color)
{
	...
	LLGLDisable multisample(gGLManager.mIsAMD &&
							LLPipeline::RenderFSAASamples > 0 ? GL_MULTISAMPLE_ARB
															  : 0);
	...
	if (LLSelectMgr::sRenderHiddenSelections) // && LLFloaterTools::isVisible())
	{
		...
		else
		{
			...
			F32 fog_factor =
				(F32)(gSelectMgr.getSelectionCenterGlobal() -
					  gAgent.getCameraPositionGlobal()).length() * 0.25f /
				gSelectMgr.getBBoxOfSelection().getExtentLocal().length();
			LLColor4 fog_col = color * llclamp(fog_factor, 0.f, 1.f);
			float d = (gViewerCamera.getPointOfInterest() -
					   gViewerCamera.getOrigin()).length();
			glFogf(GL_FOG_START, d);
			glFogf(GL_FOG_END,
				   d + d * gViewerCamera.getView() /
				   gViewerCamera.getDefaultFOV());
			glFogfv(GL_FOG_COLOR, fog_col.mV);
...
void LLSelectNode::renderOneSilhouette(const LLColor4 &color)
{
	...
	{
#if 0
		// Switch to "solid color" program for SH-2690 -- works around driver
		// bug causing bad triangles when rendering silhouettes
...
void LLSelectMgr::updateSelectionCenter()
{
	...
	if (gAgentID.notNull())
	{
		...
		if (mShowSelection)
		{
			...
			// tractor beam)
			LLVector3d diff = select_center_global -
							  mLastSentSelectionCenterGlobal;
