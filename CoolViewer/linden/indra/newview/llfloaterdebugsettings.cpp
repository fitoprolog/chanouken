...
void LLFloaterDebugSettings::onCommitSettings(LLUICtrl* ctrl, void* user_data)
{
	...
	LLControlVariable* controlp;
	controlp = (LLControlVariable*)self->mComboNames->getCurrentUserdata();
	if (!controlp) return;
...
//mk
	...
	switch (controlp->type())
	{
		...
		case TYPE_BOOLEAN:
//MK
			// Special case: do not allow changing WindLightUseAtmosShaders
			// neither if setenv is on
			if (gRLenabled && gRLInterface.mContainsSetenv)
			{
				if (controlp->getName() == "WindLightUseAtmosShaders")
				{
					return;
				}
			}
