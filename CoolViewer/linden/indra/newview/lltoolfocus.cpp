...
void LLToolCamera::pickCallback(const LLPickInfo& pick_info)
{
	...
	if (!toolcamera->hasMouseCapture())
	{
		return;
	}

	toolcamera->mMouseDownX = pick_info.mMousePt.mX;
	toolcamera->mMouseDownY = pick_info.mMousePt.mY;
	...
	{
		toolcamera->mValidClickPoint = false;
		return;
	}
		...
		if (!(pick_info.mKeyMask & MASK_ALT) && gAgent.cameraThirdPerson() &&
			...
			(hit_obj == gAgentAvatarp ||
			 (hit_obj && hit_obj->isAttachment() &&
			  LLVOAvatar::findAvatarFromAttachment(hit_obj)->isSelf())))
		{
			toolcamera->mMouseSteering = true;
		}
	}

	toolcamera->mValidClickPoint = true;
...
// "Let go" of the mouse, for example on mouse up or when we lose mouse
