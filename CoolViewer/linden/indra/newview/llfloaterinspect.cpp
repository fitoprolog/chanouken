 ...
 * @brief Floaters for object and avatar inspection tool
 *
...
class LLInspectObjectHandler : public LLCommandHandler
{
public:
...
void LLFloaterInspect::show(void* data)
{
	...
	// setForceSelection ensures that the pie menu does not deselect things
	// when it looses the focus (this can happen with "select own objects only"
	// enabled VWR-1471
...
{
	HBFloaterInspectAvatar* self = findInstance();
	if (!self)	// first use
	{
		self = getInstance();	// creates a new instance
	}
