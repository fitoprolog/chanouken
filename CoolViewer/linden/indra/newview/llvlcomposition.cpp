...
void LLVLComposition::setDetailTextureID(S32 terrain, const LLUUID& id)
{
	...
	// *HACK: Skip any cached texture loading, since these textures somehow get
	// corrupted in the disk cache after a while...
	static LLCachedControl<bool> skip_cache(gSavedSettings,
											"SkipCachedTerrainTextures");
	if (skip_cache)
	{
		mDetailTextures[terrain]->setSkipCache(true);
	}
}
...
bool LLVLComposition::generateTexture(F32 x, F32 y, F32 width, F32 height)
{
	...
	{
		llwarns_once << "Base texture comps != input texture comps" << llendl;
		return false;
	}
