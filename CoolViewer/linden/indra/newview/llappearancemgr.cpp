...
{
	// We now have an outfit ready to be copied to agent inventory. Do it, and
	// wear that outfit normally.
	if (mCopyItems)
	{
		...
		for (item_ref_t::iterator it = mComplete.begin(); it < end; ++it)
		{
			LLViewerInventoryItem* item =
				(LLViewerInventoryItem*)gInventory.getItem(*it);
			if (item)
			{
				if (pid.isNull() &&
					LLInventoryType::IT_GESTURE == item->getInventoryType())
				{
		...
		mCatID = gInventory.createNewCategory(pid, LLFolderType::FT_NONE,
											  name);
		...
		for (item_ref_t::iterator it = mComplete.begin(); it < end; ++it)
		{
			...
			if (item)
			{
				copy_inventory_item(item->getPermissions().getOwner(),
									item->getUUID(), mCatID, std::string(),
									cb);
			}
		}
													...
													bool append, bool replace)
{
	...
	LLWearInfo* userdata = new LLWearInfo(category->getUUID(),
										  append, replace);
...
//mk
		...
		if (gest_count > 0)
		{
			...
			catp = gInventory.getCategory(wear_info->mCategoryID);
			if (catp)
			{
									...
									const LLSD& response)
{
	...
	if (LLNotification::getSelectedOption(notification, response) == 0) // YES
	{
		...
		// For new joints first use, give a warning about viewers compatibility
		if ((attach_pt & ~ATTACHMENT_ADD) > 38)
		{
			LLFirstUse::useAttach(notification["payload"]["attachment_name"].asString());
		}
...
/*
 * Simple, no-brainer implementation of the ugly COF concept...
 */

const LLUUID LLAppearanceMgr::getCOF(bool create) const
{
...
void LLAppearanceMgr::cleanupCOF(const LLUUID& cof)
{
	...
	for (U32 i = 0, count = cats.size(); i < count; ++i)
	{
		...
		if (category)	// Paranoia
		{
			...
			llwarns << "Found (non-link) folder '" << category->getName()
	...
	for (U32 i = 0, count = items.size(); i < count; ++i)
	{
		...
		{
			llinfos << "Trashing useless folder link '" << item->getName()
					<< "' out of COF." << llendl;
		...
		else
		{
			...
			// of purging it...
			llwarns << "Found (non-link) object '" << item->getName()
					<< "' in COF: moving it to Lost And Found." << llendl;
			move_inventory_item(item_id, laf, item->getName());
		}
	}
}
...
void LLAppearanceMgr::syncAttachmentLinksInCOF()
{
	...
	for (U32 i = 0, count = obj_items.size(); i < count; ++i)
	{
		...
		{
			LLUUID item_id = item->getUUID();
			attach_id_it = attached_items_ids.find(item->getLinkedUUID());
			...
			{
				LL_DEBUGS("COF") << "Purging link '" << item->getName()
								 << "' from COF." << LL_ENDL;
				remove_inventory_item(item_id);
			}
		}
	...
	for (attach_id_it = attached_items_ids.begin(); attach_id_it != end;
		 ++attach_id_it)
	{
		item = gInventory.getItem(*attach_id_it);
		if (item)	// Paranoia
		{
			...
			LL_DEBUGS("COF") << "Creating a new link for attached item: "
							 << item->getName() << LL_ENDL;
			LLPointer<LLInventoryCallback> cb =
				new LLCreateLinkInCOFCallback(item->getUUID(), false);
			link_inventory_object(cof, LLConstPointer<LLInventoryObject>(item),
								  cb);
		}
	}
...
#if 0	// This does not work... Use the COF refetching below instead.
