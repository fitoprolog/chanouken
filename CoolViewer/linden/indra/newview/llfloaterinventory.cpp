...
void LLFloaterInventory::init(LLInventoryModel* inventory)
{
	mLastCount = 0;
...
LLFloaterInventory::~LLFloaterInventory()
{
	...
	if (!filtersFile.is_open() ||
		!LLSDSerialize::toPrettyXML(filter_root, filtersFile))
	{
...
void LLFloaterInventory::toggleFindOptions()
{
	...
	LLFloaterInventoryFilters* filters =
		new LLFloaterInventoryFilters("Inventory Finder",
									  LLRect(getRect().mLeft -
									  INV_FINDER_WIDTH,
									  getRect().mTop, getRect().mLeft,
									  getRect().mTop - INV_FINDER_HEIGHT),
									  this);
	mInvFiltersHandle = filters->getHandle();
	filters->open();
