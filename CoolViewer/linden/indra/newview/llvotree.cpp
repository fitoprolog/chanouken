...
LLVOTree::LLVOTree(const LLUUID& id, LLPCode pcode, LLViewerRegion* regionp)
...
void LLVOTree::idleUpdate(const F64& time)
{
...
void LLVOTree::updateMesh()
{
	...
	if (!mUpdateMeshBuffer->allocateBuffer(vert_count, index_count, true))
	{
		llwarns << "Failure to resize a vertex buffer with " << vert_count
				<< " vertices and " << index_count << " indices" << llendl;
		mUpdateMeshBuffer->allocateBuffer(1, 3, true);
		memset((U8*)mUpdateMeshBuffer->getMappedData(), 0,
			   mUpdateMeshBuffer->getSize());
		memset((U8*)mUpdateMeshBuffer->getMappedIndices(), 0,
				   mUpdateMeshBuffer->getIndicesSize());
		facep->setSize(1, 3);
		facep->setVertexBuffer(mUpdateMeshBuffer);
		mReferenceBuffer->flush();
		mUpdateMeshBuffer->flush();
		return;
	}
