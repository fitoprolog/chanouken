...
public:
	LLVOWLSky(const LLUUID& id, LLPCode pcode, LLViewerRegion* regionp);
	...
	void drawStars();
	void drawDome();
	// Fullscreen sky for extended environment advanced atmospherics
	void drawFsSky();
...
private:
	...
	// Helper methods used to build and prime our sky vertex buffers
	bool primeFanVerts();	// For Windlight rendering
	bool primeFsSkyVerts();	// For extended environment rendering
...
private:
	LLPointer<LLVertexBuffer>	mFanVerts;		// Windlight
	LLPointer<LLVertexBuffer>	mFsSkyVerts;	// Extended environment
