...
public:
	...
	// ensuring that the bulk uploads are chained one after the other.
	typedef void (*capability_callback_t)(const LLSD& result, void* userdata);
	LL_INLINE void setCapCallback(capability_callback_t cb, void* userdata)
	{
		mCapCallback = cb;
		mUserData = userdata;
	}
...
private:
	...
	capability_callback_t	mCapCallback;
	void*					mUserData;
...
public:
	typedef boost::function<void(LLUUID item_id,
								 LLUUID new_asset_id,
								 LLUUID new_item_id,
								 LLSD response)> invnUploadFinish_f;

	typedef boost::function<void(LLUUID item_id,
								 LLUUID task_id,
								 LLUUID new_asset_id,
								 LLSD response)> taskUploadFinish_f;
...
public:
	...
	LLScriptAssetUpload(const LLUUID& item_id, const std::string& buffer,
						TargetType_t target_type, invnUploadFinish_f finish);
