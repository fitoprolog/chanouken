...
LLVOAvatarSelf::LLVOAvatarSelf(const LLUUID& id, LLPCode pcode,
...
//virtual
void LLVOAvatarSelf::idleUpdate(const F64& time)
{
	if (isValid())
	{
...
{
	for (anim_src_map_it_t it = mAnimationSources.find(source_id),
...
void LLVOAvatarSelf::updateRegion(LLViewerRegion* regionp)
{
	...
	if (!regionp || regionp->getHandle() != mLastRegionHandle)
	{
		if (mLastRegionHandle != 0)
		{
			...
			{
				avg = stats->getStat(LLViewerStats::ST_CROSSING_AVG);
				max = stats->getStat(LLViewerStats::ST_CROSSING_MAX);
			}
			F64 delta_avg = (delta + avg * (mRegionCrossingCount - 1)) /
							mRegionCrossingCount;
			stats->setStat(LLViewerStats::ST_CROSSING_AVG, delta_avg);
			max = llmax(delta, max);
			stats->setStat(LLViewerStats::ST_CROSSING_MAX, max);
...
void LLVOAvatarSelf::idleUpdateTractorBeam()
{
	...
	else if (!mBeam || mBeam->isDead())
	{
		...
		mBeam = (LLHUDEffectSpiral*)LLHUDManager::createEffect(LLHUDObject::LL_HUD_EFFECT_BEAM);
		 ...
		 baked_iter != end; ++baked_iter)
	{
		...
		if (baked_dict)
		{
			for (wearables_vec_t::const_iterator
					type_iter = baked_dict->mWearables.begin(),
					type_end = baked_dict->mWearables.end();
...
bool LLVOAvatarSelf::isWearingAttachment(const LLUUID& inv_item_id) const
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (attachment && attachment->getAttachedObject(base_inv_id))
		{
			return true;
		}
	}
...
LLViewerObject* LLVOAvatarSelf::getWornAttachment(const LLUUID& inv_item_id)
{
		 ...
		 iter != end; ++iter)
	{
		...
		LLViewerObject* object = attachment->getAttachedObject(base_inv_id);
 		if (object)
		{
			return object;
		}
	...
	if (inv_item_id.notNull())
	{
		...
		if (base_inv_id.notNull())
		{
				 ...
				 iter != end; ++iter)
			{
				const LLViewerJointAttachment* attachment = iter->second;
				if (attachment &&
					attachment->getAttachedObject(base_inv_id))
				{
...
//virtual
const LLViewerJointAttachment* LLVOAvatarSelf::attachObject(LLViewerObject* vobj)
{
	const LLViewerJointAttachment* attachment = LLVOAvatar::attachObject(vobj);
	if (!attachment)
	{
		return NULL;
	}
...
//MK
	if (gRLenabled)
	{
		...
		gRLInterface.addAttachmentPointName(vobj);
	}
//mk
...
//virtual
bool LLVOAvatarSelf::detachObject(LLViewerObject* vobj)
{
	const LLUUID attachment_id = vobj->getAttachmentItemID();
	if (LLVOAvatar::detachObject(vobj))
	{
		// The simulator should automatically handle permission revocation
		stopMotionFromSource(attachment_id);

		LLFollowCamMgr::setCameraActive(vobj->getID(), false);
...
bool LLVOAvatarSelf::detachAttachmentIntoInventory(const LLUUID& item_id)
{
	...
	if (item && gAgent.getRegion())
	{
		...
		// This object might have been selected, so let the selection manager
		// know it is gone now
										...
										bool is_final, void* userdata)
{
	...
	LLLocalTextureObject* lto = getLocalTextureObject(index, 0);
	...
	if (success)
	{
		if (!lto->getBakedReady() && lto->getImage() &&
			lto->getID() == src_id && discard_level < lto->getDiscard())
		{
			lto->setDiscard(discard_level);
	...
	else if (is_final && !lto->getBakedReady() && lto->getImage() &&
			 lto->getImage()->getID() == src_id)
	{
		lto->setDiscard(0);
		requestLayerSetUpdate(index);
		updateMeshTextures();
...
#if 0	// Note used
bool LLVOAvatarSelf::getLocalTextureGL(ETextureIndex type,
									   LLViewerTexture** tex_pp,
									   U32 index)
{
	*tex_pp = NULL;
	...
	LLLocalTextureObject* lto = getLocalTextureObject(type, index);
	if (!lto)
	{
		return false;
	}
														  ...
														  U32 index) const
{
	...
	const LLLocalTextureObject* lto = getLocalTextureObject(type, index);
	if (!lto)
	{
		return NULL;
	}

	if (lto->getID() == IMG_DEFAULT_AVATAR)
	{
		return LLViewerTextureManager::getFetchedTexture(IMG_DEFAULT_AVATAR);
	}

	LLGLTexture* gltex = lto->getImage();
	return gltex ? gltex->asFetched() : NULL;
}
...
bool LLVOAvatarSelf::isLocalTextureDataAvailable(LLViewerTexLayerSet* layerset)
{
	for (LLAvatarAppearanceDictionary::BakedTextures::const_iterator
			baked_iter = gAvatarAppDictp->getBakedTextures().begin(),
			end = gAvatarAppDictp->getBakedTextures().end();
		 baked_iter != end; ++baked_iter)
	{
		EBakedTextureIndex baked_index = baked_iter->first;
		if (layerset == mBakedTextureDatas[baked_index].mTexLayerSet)
		{
			LLAvatarAppearanceDictionary::BakedEntry* baked_dict =
				baked_iter->second;
			texture_vec_t& textures = baked_dict->mLocalTextures;
...
// in the layerset.
bool LLVOAvatarSelf::isLocalTextureDataFinal(LLViewerTexLayerSet* layerset)
{
	for (U32 i = 0, count = mBakedTextureDatas.size(); i < count; ++i)
	{
...
bool LLVOAvatarSelf::isBakedTextureFinal(EBakedTextureIndex index)
{
	...
	LLViewerTexLayerSetBuffer* bufferp = layerset->getViewerComposite();
										 ...
										 bool upload_result)
{
	...
	LLViewerTexLayerSet* layer_set = layerset->asViewerTexLayerSet();
...
{
	LLViewerTexLayerSet* layerset = getTexLayerSet(index);
	return layerset ? layerset->getUpdatesEnabled() : false;
}
	...
	for (U32 i = 0, count = mBakedTextureDatas.size(); i < count; ++i)
	{
		...
		if (layerset)
		{
			...
			bool layer_baked = isTextureDefined(tex_index,
												gAgentWearables.getWearableCount(tex_index));
...
S32 LLVOAvatarSelf::getLocalDiscardLevel(ETextureIndex type, U32 wearable_idx)
{
	...
	LLLocalTextureObject* lto = getLocalTextureObject(type, wearable_idx);
	if (lto)
	{
		LLGLTexture* gltext = lto->getImage();
		if (gltext)
		{
			LLViewerFetchedTexture* image = gltext->asFetched();
			if (image && !image->isMissingAsset() &&
				lto->getID() != IMG_DEFAULT_AVATAR)
			{
	...
	*gl_bytes = 0;
	for (S32 type = 0; type < TEX_NUM_INDICES; ++type)
	{
		...
		for (U32 num = 0; num < max_tex; ++num)
		{
			...
			LLViewerFetchedTexture* tex = gltex->asFetched();
			if (tex && tex->hasGLTexture())
			{
				*gl_bytes += (S32)tex->getWidth() * tex->getHeight() *
							 tex->getComponents();
			}
		}
	}
									 ...
									 bool baked_version_ready, U32 index)
{
	...
	if (!lto)
	{
		...
		lto = gAgentWearables.addLocalTextureObject(wearable_type, type,
													index);
		if (!lto)
		{
		...
		if (layer_set)
		{
			layer_set->cloneTemplates(lto, type,
	...
	{
		if (tex != lto->getImage() || lto->getBakedReady())
		{
			lto->setDiscard(MAX_DISCARD_LEVEL + 1);
		}
		if (tex->getID() != IMG_DEFAULT_AVATAR)
		{
			if (lto->getDiscard() > 0)
			{
				...
				{
					tex->setLoadedCallback(onLocalTextureLoaded,
										   0, true, false,
...
void LLVOAvatarSelf::dumpLocalTextures()
{
		 ...
		 iter != end; ++iter)
	{
		...
		LLLocalTextureObject* lto = getLocalTextureObject(iter->first, 0);
		if (lto && lto->getImage())
		{
			tex = lto->getImage()->asFetched();
		}
		if (!tex)
		{
		...
		if (lto->getImage()->getID() == IMG_DEFAULT_AVATAR)
		{
		...
		llcont << " - Size: " << tex->getWidth() << "x" << tex->getHeight()
			   << " - Discard level: " << tex->getDiscardLevel()
			   << " - Priority: " << tex->getDecodePriority() << llendl;
	}
}
										  ...
										  void* userdata)
{
	LLAvatarTexData* data = (LLAvatarTexData*)userdata;
	LLVOAvatarSelf* self =
		(LLVOAvatarSelf*)gObjectList.findAvatar(data->mAvatarID);
	if (self)
	{
		self->localTextureLoaded(success, src_vi, src_raw, aux_src,
...
bool LLVOAvatarSelf::canGrabBakedTexture(EBakedTextureIndex baked_index) const
{
	...
	for (U32 t = 0, tcount = textures.size(); t < tcount; ++t)
	{
		...
		for (U32 w = 0; w < wcount; ++w)
		{
			...
			const LLLocalTextureObject* texture =
				wearable->getLocalTextureObject((S32)t_index);
			const LLUUID& texture_id = texture->getID();
			if (texture_id != IMG_DEFAULT_AVATAR &&
				texture_id != IMG_INVISIBLE &&
				!LLObjectBackup::validateTexturePerms(texture_id, true))
			{
				return false;
			}
		}
...
{
	if (covered_by_baked || !imagep || !isIndexLocalTexture(type)) return;
...
// Used by the LayerSet (layer sets don't in general know what textures depend
// on them).
		 ...
		 iter != end; ++iter)
	{
			...
			texture_id == gAgentAvatarp->getTEImage(index)->getID())
		{
			...
			if (layer_set)
			{
				...
				found = true;
				stats->incStat(LLViewerStats::ST_TEX_REBAKES);
			}
			break;
		}
	}
...
bool LLVOAvatarSelf::needsRenderBeam()
{
	...
	if (object && private_point_at &&
		(object->getPositionGlobal() - agent_pos).length() > point_at_limit)
	{
		return false;
	}

	LLTool* tool = LLToolMgr::getInstance()->getCurrentTool();
	bool is_touching_or_grabbing = (tool == LLToolGrab::getInstance() &&
...
{
	for (std::map<U32, U32*>::iterator it = sScratchTexNames.begin(),
									   end = sScratchTexNames.end();
		 it != end; ++it)
	{
		LLImageGL::deleteTextures(1, it->second);
	}
	stop_glerror();

	if (sScratchTexBytes)
	{
		LL_DEBUGS("Avatar") << "Clearing Scratch Textures "
							<< sScratchTexBytes / 1024 << "KB" << LL_ENDL;
...
{
	llinfos << "Scratch Texture GL: " << sScratchTexBytes /1024 << "KB"
			<< llendl;
}
