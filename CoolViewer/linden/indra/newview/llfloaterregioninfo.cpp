...
public:
	...
	~LLDispatchEstateUpdateInfo() override	{}
...
public:
	...
	~LLDispatchSetEstateAccess() override	{}
	bool operator()(const LLDispatcher*, const std::string& key,
					const LLUUID& invoice, const sparam_t& strings) override;
};
										   ...
										   const sparam_t& strings)
{
	...
	if (access_flags & ESTATE_ACCESS_BANNED_AGENTS)
	{
		...
		if (name_list)
		{
			...
			for (S32 i = 0; i < num_banned_agents && i < ESTATE_MAX_ACCESS_IDS;
				 ++i)
			{
				LLUUID id;
				memcpy(id.mData, strings[index++].data(), UUID_BYTES);
...
public:
	...
	~LLDispatchSetEstateExperience() override	{}
	bool operator()(const LLDispatcher*, const std::string& key,
					const LLUUID& invoice, const sparam_t& strings) override;

	LLSD getIDs(sparam_t::const_iterator it, sparam_t::const_iterator end,
				S32 count);
};
											   ...
											   const sparam_t& strings)
{
	...
	LLSD ids = LLSD::emptyMap()
		.with("blocked", getIDs(it,
			  strings.end(), num_blocked))
		.with("trusted", getIDs(it + (num_blocked),
			  strings.end(), num_trusted))
		.with("allowed", getIDs(it + (num_blocked+num_trusted),
			  strings.end(), num_allowed));
...
bool LLFloaterRegionInfo::postBuild()
{
	...
	gMessageSystemp->setHandlerFunc("EstateOwnerMessage",
											...
											void* userdata)
{
	...
	if (self && !names.empty() && !ids.empty() && ids[0].notNull())
	{
		...
		self->sendEstateOwnerMessage(gMessageSystemp, "teleporthomeuser",
									 invoice, strings);
	}
}
											   ...
											   const LLSD& response)
{
	if (LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		sendEstateOwnerMessage(gMessageSystemp, "teleporthomeallusers", invoice,
							   strings);
	}
	return false;
}
											   ...
											   const LLSD& response)
{
	...
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	sendEstateOwnerMessage(gMessageSystemp, "simulatormessage", invoice,
						   strings);
	return false;
}
...
bool LLPanelRegionGeneralInfo::sendUpdate()
{
	...
	else
	{
		...
		LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
		sendEstateOwnerMessage(gMessageSystemp, "setregioninfo", invoice,
							   strings);
	}
...
bool LLPanelRegionDebugInfo::sendUpdate()
{
	...
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	sendEstateOwnerMessage(gMessageSystemp, "setregiondebug", invoice, strings);
											...
											const LLSD& response)
{
	...
	if (target_avatar.notNull())
	{
		...
		if (return_estate_wide)
		{
			...
			LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
			sendEstateOwnerMessage(gMessageSystemp, "estateobjectreturn",
								   invoice, strings);
		}
		else
		{
...
void LLPanelRegionDebugInfo::onClickTopColliders(void* data)
{
	...
	self->sendEstateOwnerMessage(gMessageSystemp, "colliders", invoice,
								 strings);
}
...
void LLPanelRegionDebugInfo::onClickTopScripts(void* data)
{
	...
	LLFloaterTopObjects::showInstance();
	LLFloaterTopObjects::clearList();
	self->sendEstateOwnerMessage(gMessageSystemp, "scripts", invoice, strings);
}
											 ...
											 const LLSD& response)
{
	if (LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
		sendEstateOwnerMessage(gMessageSystemp, "restart", invoice, strings);
	}
	return false;
}
...
void LLPanelRegionDebugInfo::onClickCancelRestart(void* data)
{
	...
	strings.emplace_back("-1");
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	self->sendEstateOwnerMessage(gMessageSystemp, "restart", invoice, strings);
}
...
bool LLPanelRegionTextureInfo::sendUpdate()
{
	...
	sendEstateOwnerMessage(gMessageSystemp, "texturedetail", invoice, strings);
	strings.clear();
	for (S32 i = 0; i < CORNER_COUNT; ++i)
	{
		buffer = llformat("height_start_spin_%d", i);
	...
	sendEstateOwnerMessage(gMessageSystemp, "textureheights", invoice, strings);
	strings.clear();
	sendEstateOwnerMessage(gMessageSystemp, "texturecommit", invoice, strings);
	return true;
}
...
bool LLPanelRegionTerrainInfo::sendUpdate()
{
	...
	sendEstateOwnerMessage(gMessageSystemp, "setregionterrain", invoice,
						   strings);
	return true;
}
												   ...
												   void* data)
{
	...
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	self->sendEstateOwnerMessage(gMessageSystemp, "terrain", invoice, strings);
}
												   ...
												   const LLSD& response)
{
	...
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	sendEstateOwnerMessage(gMessageSystemp, "terrain", invoice, strings);
	return false;
}
...
bool LLPanelEstateInfo::refreshFromRegion(LLViewerRegion* region)
{
	...
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	strings_t strings;
	sendEstateOwnerMessage(gMessageSystemp, "getinfo", invoice, strings);
...
bool LLPanelEstateInfo::commitEstateInfoCaps()
{
	...
	LLCoros::getInstance()->launch("LLPanelEstateInfo::commitEstateInfoCaps",
										...
										const LLSD& response)
{
	if (LLNotification::getSelectedOption(notification, response) == 0)
	{
		...
		sendEstateOwnerMessage(gMessageSystemp, "kickestate",
							   LLFloaterRegionInfo::getLastInvoice(), strings);
	}
	return false;
}
										...
										const LLSD& response)
{
	...
	LLUUID invoice(LLFloaterRegionInfo::getLastInvoice());
	sendEstateOwnerMessage(gMessageSystemp, "instantmessage", invoice,
						   strings);
	return false;
}
...
bool LLPanelEstateAccess::postBuild()
{
	...
	mBannedAvatars->setMaxItemCount(ESTATE_MAX_ACCESS_IDS);
...
void LLPanelEstateAccess::updateLists()
{
	...
	else	// Use the capability
	{
		LLCoros::getInstance()->launch("LLPanelEstateAccess::requestEstateGetAccessCoro",
...
void LLPanelEstateAccess::onClickAddBannedAgent(void* user_data)
{
	...
	if (self->mBannedAvatars->getItemCount() >= ESTATE_MAX_ACCESS_IDS)
	{
		LLSD args;
														 ...
														 const LLUUID& experience_id)
{
	...
	if (panel)
	{
		panel->sendEstateOwnerMessage(gMessageSystemp, "estateexperiencedelta",
									  LLFloaterRegionInfo::getLastInvoice(),
									  str);
	}
}
