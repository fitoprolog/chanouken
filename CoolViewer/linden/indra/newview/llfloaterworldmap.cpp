...
void LLFloaterWorldMap::trackLandmark(const LLUUID& landmark_item_id)
{
	...
	{
		LLUUID asset_id = mLandmarkAssetIDList[idx];
		std::string name = mLandmarkCombo->getSimple();
		mTrackedStatus = LLTracker::TRACKING_LANDMARK;
		gTracker.trackLandmark(mLandmarkAssetIDList[idx],	// assetID
							   mLandmarkItemIDList[idx],	// itemID
							   name);						// name
...
void LLFloaterWorldMap::friendsChanged()
{
	...
	if (avatar_id.notNull())
	{
		...
		LLCtrlSelectionInterface* iface = mFriendCombo->getSelectionInterface();
		if (!buddy || !iface || gAgent.isGodlike() ||
...
void LLFloaterWorldMap::buildAvatarIDList()
{
	...
	list->setCurrentByID(gAvatarTracker.getAvatarID());
...
void LLFloaterWorldMap::onLandmarkComboPrearrange(LLUICtrl*, void* userdata)
{
	...
	if (list->getFirstSelectedIndex())	// If not "None" selected
	{
		current_choice = list->getCurrentID();
	}
...
void LLFloaterWorldMap::onLandmarkComboCommit(LLUICtrl*, void* userdata)
{
	...
	if (list->getFirstSelectedIndex() == 0)
	{
		return;
	}
