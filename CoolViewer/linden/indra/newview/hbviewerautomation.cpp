 ...
 *
 * Copyright (c) 2016-2021, Henri Beauchamp.
 *
								   ...
								   const std::string& parameter)
{
	...
	if (!LLUICtrlFactory::getInstance()->buildFloater(self, fname))
	{
		self->mInitOK = false;
		delete self;
		self = NULL;
...
bool HBLuaFloater::postBuild()
{
	...
	LLStringUtil::trimHead(title);
	LLStringUtil::toLower(title);
	if (title.compare(0, 3, "lua") != 0)
	{
...
bool HBLuaFloater::setCtrlValue(LLUICtrl* ctrl, const std::string& value)
{
	...
	if (panel)
	{
		...
		if (filter->isActive())
		{
			...
			// fetch so we better start it here.
		...
		LLOpenFolderByID opener(cat_id);
		inv->applyFunctorRecursively(opener);
...
public:
	...
	LL_INLINE void resetTimer() override
	{
		mTimer.start();
		mTimer.setTimerExpirySec(0.5f);
	}
	...
	{
		mTimer.start();
		mTimer.setTimerExpirySec(0.01f);
		mRunning = true;
		setQuitting();
	}
...
HBViewerAutomation::~HBViewerAutomation()
{
	...
	if (!sThreadsInstances.empty())
	{
		...
		mTimer.start();
		mTimer.setTimerExpirySec(0.1f);
		while (!sThreadsInstances.empty() && !mTimer.hasExpired())
		{
			for (threads_list_t::iterator it = sThreadsInstances.begin(),
										  end = sThreadsInstances.end();
				it != end; )
...
void HBViewerAutomation::resetCallbackFlags()
{
	mHasCallbacks = mHasOnSignal = mHasOnLogin = mHasOnRegionChange =
					...
					mHasOnAgentOccupationChange = mHasOnAgentPush =
					mHasOnSendChat = mHasOnReceivedChat =
					mHasOnChatTextColoring = mHasOnInstantMsg =
					mHasOnScriptDialog = mHasOnNotification =
					mHasOnFriendStatusChange = mHasOnAvatarRezzing =
					mHasOnAgentBaked = mHasOnRadar = mHasOnRadarMark =
					mHasOnRadarTrack = mHasOnLuaDialogClose =
					...
					mHasOnLuaPieMenu = mHasOnRLVHandleCommand =
...
bool HBViewerAutomation::registerCFunctions()
{
	...
	static const std::string viewer_version =
		llformat("%d.%d.%d.%d", LL_VERSION_MAJOR, LL_VERSION_MINOR,
	...
	// Set a couple useful global variables so that Lua scripts know what
	// viewer they are running within.
	lua_pushstring(mLuaState, gSecondLife.c_str());
	lua_setglobal(mLuaState, "VIEWER_NAME");
	lua_pushstring(mLuaState, viewer_version.c_str());
...
void HBViewerAutomation::onLogin()
{
	...
	// Ensure mFromObjectId is properly initialized for gAutomationp that is
	...
	if (!mLuaState || !mHasOnLogin) return;
...
void HBViewerAutomation::onRegionChange()
{
	...
	if (!mLuaState || !mHasOnRegionChange)
	{
		return;
	}
...
void HBViewerAutomation::onParcelChange()
{
	...
	if (!mLuaState || !mHasOnParcelChange)
	{
		return;
	}
										  ...
										  const LLVector3d& pos_global)
{
	...
	if (!mLuaState || !mHasOnPositionChange)
	{
		return;
	}
...
void HBViewerAutomation::onAgentOccupationChange(S32 type)
{
	...
	if (!mLuaState || !mHasOnAgentOccupationChange ||
...
void HBViewerAutomation::onAgentPush(const LLUUID& id, S32 type, F32 mag)
{
	...
	if (!mLuaState || !mHasOnAgentPush) return;
...
bool HBViewerAutomation::onSendChat(std::string& text)
{
	...
	if (!mLuaState || !mHasOnSendChat || sIgnoredCallbacks[E_ONSENDCHAT])
	{
		return false;
	}
										...
										const std::string& text)
{
	...
	if (!mLuaState || !mHasOnReceivedChat) return;
											...
											LLColor4& color)
{
	...
	if (!mLuaState || !mHasOnChatTextColoring) return false;
									  ...
									  const std::string& text)
{
	...
	if (!mLuaState || !mHasOnInstantMsg || sIgnoredCallbacks[E_ONINSTANTMSG])
	{
		return;
	}
										...
										const std::vector<std::string>& buttons)
{
	...
	if (!mLuaState || !mHasOnScriptDialog) return;
										...
										const std::string& message)
{
	...
	if (!mLuaState || !mHasOnNotification ||
											  ...
											  bool is_online)
{
	...
	if (!mLuaState || !mHasOnFriendStatusChange) return;
...
void HBViewerAutomation::onAvatarRezzing(const LLUUID& id)
{
	...
	if (!mLuaState || !mHasOnAvatarRezzing) return;
...
void HBViewerAutomation::onAgentBaked()
{
	...
	if (!mLuaState || !mHasOnAgentBaked || !isAgentAvatarValid()) return;
								 ...
								 U32 range)
{
	...
	if (!mLuaState || !mHasOnRadar) return;
	...
	lua_pushboolean(mLuaState, avlist && avlist->isAvatarMarked(id));
	resetTimer();
									 ...
									 bool marked)
{
	...
	if (!mLuaState || !mHasOnRadarMark) return;
									  ...
									  const std::string& name, bool tracked)
{
	...
	if (!mLuaState || !mHasOnRadarTrack || sIgnoredCallbacks[E_ONRADARTRACK])
	{
		return;
	}
										  ...
										  const std::string& text)
{
	...
	if (!mLuaState || !mHasOnLuaDialogClose) return;
											...
											const std::string& value)
{
	...
	if (!mLuaState || !mHasOnLuaFloaterAction) return;
										  ...
										  const std::string& parameter)
{
	...
	if (!mLuaState || !mHasOnLuaFloaterOpen) return;
										   ...
										   const std::string& parameter)
{
	...
	if (!mLuaState || !mHasOnLuaFloaterClose) return;
...
void HBViewerAutomation::onSideBarVisibilityChange(bool visible)
{
	...
	if (!mLuaState || !mHasOnSideBarVisibilityChange) return;
...
void HBViewerAutomation::onTPStateChange(S32 state, const std::string& reason)
{
	...
	if (!mLuaState || !mHasOnTPStateChange)
	{
		return;
	}
...
void HBViewerAutomation::onFailedTPSimChange(S32 agents_count)
{
	...
	if (!mLuaState || !mHasOnFailedTPSimChange ||
										   ...
										   const std::string& day_settings)
{
	...
	if (!mLuaState || !mHasOnWindlightChange ||
...
void HBViewerAutomation::onCameraModeChange(S32 mode)
{
	...
	if (!mLuaState || !mHasOnCameraModeChange ||
...
void HBViewerAutomation::onJoystickButtons(S32 old_state, S32 new_state)
{
	...
	if (!mLuaState || !mHasOnJoystickButtons)
	{
		return;
	}
											...
											const std::string& param)
{
	...
	if (!mLuaState || !mHasOnRLVHandleCommand) return;
										   ...
										   const std::string& text)
{
	...
	if (!mLuaState || !mHasOnRLVAnswerOnChat) return;
										   ...
										   const LLUUID& group_id)
{
	...
	if (!mLuaState || !mHasOnObjectInfoReply) return;
...
void HBViewerAutomation::resetTimer()
{
	...
	mTimer.start();
	mTimer.setTimerExpirySec(llclamp((F32)lua_timeout, 0.01f, 2.f));
}
...
int HBViewerAutomation::scriptDialogResponse(lua_State* state)
{
	...
	{
		LLNotifyBox* boxp =
			LLInstanceTracker<LLNotifyBox, LLUUID>::getInstance(notif_id);
		if (boxp && !boxp->isDead())
		{
...
int HBViewerAutomation::openLuaFloater(lua_State* state)
{
	...
	if (n == 2)
	{
		parameter.assign(luaL_checkstring(state, 2));
	}
	lua_pop(state, n);

	lua_pushboolean(state, HBLuaFloater::create(name, parameter) != NULL);
...
int HBViewerAutomation::sideBarButton(lua_State* state)
{
	...
	if (gLuaSideBarp)
	{
		gLuaSideBarp->setButton(number, icon, command, tooltip);
	}
		...
		if (type == LUA_TNIL || type == LUA_TSTRING)
		{
			...
			if (!control_name.empty())
			{
				...
				std::string white_list =
					"," + gSavedSettings.getString("LuaSettingsWhiteList") + ",";
					if (white_list.find("," + control_name + ",") == std::string::npos)
				{
					luaL_error(state,
							   "Setting '%s' is not white-listed for writing",
							   control_name.c_str());
				}
			}
			if (gLuaSideBarp)
...
int HBViewerAutomation::getDebugSetting(lua_State* state)
{
	...
	std::string white_list =
		"," + gSavedSettings.getString("LuaSettingsWhiteList") + "," +
		gSavedSettings.getString("LuaSettingsReadOnlyWhiteList") + ",";
	if (white_list.find(name) == std::string::npos)
	{
		lua_pushnil(state);
		return 1;
	}
	...
	switch (type)
	{
		...
		default:
			// Other setting types are unsupported for now.
			lua_pushnil(state);
	}
...
int HBViewerAutomation::setDebugSetting(lua_State* state)
{
	...
	name = "," + name + ",";
	std::string white_list = ",";
	white_list += gSavedSettings.getString("LuaSettingsWhiteList") + ",";
	if (white_list.find(name) == std::string::npos)
	{
		lua_pop(state, 2);
		lua_pushboolean(state, false);
	...
	switch (type)
	{
		case TYPE_U32:
		case TYPE_S32:
			control->setValue(LLSD::Integer(luaL_checknumber(state, 2)));
			break;

		case TYPE_F32:
			control->setValue(LLSD::Real(luaL_checknumber(state, 2)));
			break;

		case TYPE_BOOLEAN:
			control->setValue(LLSD::Boolean(lua_toboolean(state, 2)));
			break;

		case TYPE_STRING:
			control->setValue(LLSD::String(luaL_checkstring(state, 2)));
			break;
										...
										std::string* output)
{
	...
	{
		data = "base64:" + LLBase64::encode_text(data);
...
bool HBViewerAutomation::deserializeTable(lua_State* state, std::string data)
{
	...
	data = LLBase64::decode_text(data.substr(7).c_str());
...
int HBViewerAutomation::getClipBoardString(lua_State* state)
{
	...
	std::string text = wstring_to_utf8str(gClipboard.getClipBoardString());
...
int HBViewerAutomation::findInventoryObject(lua_State* state)
{
	...
	LLViewerRegion* regionp = gAgent.getRegion();
	bool export_support = regionp && regionp->isOSExportPermSupported();
	bool copy_ok = false;
	bool mod_ok = false;
	bool xfer_ok = false;
	...
	else if (obj_id.notNull())
	{
		...
		if (item)
		{
			...
			copy_ok = item->getPermissions().allowCopyBy(gAgentID);
			mod_ok = item->getPermissions().allowModifyBy(gAgentID);
			xfer_ok = item->getPermissions().allowOperationBy(PERM_TRANSFER,
															  gAgentID);
			export_ok = export_support &&
						item->getPermissions().allowExportBy(gAgentID,
															 ep_export_bit);
		}
	}
...
int HBViewerAutomation::makeInventoryLink(lua_State* state)
{
	...
	if (!is_category && item_id.notNull())
	{
		...
		else
		{
			...
			if (!is_category ||
				gInventory.isObjectDescendentOf(cat_id, trash_id) ||
				gInventory.isObjectDescendentOf(cat_id, market_id))
			{
				cat_id.setNull();
			}
		}
...
int HBViewerAutomation::deleteInventoryLink(lua_State* state)
{
	...
	if (!is_category && item_id.notNull())
	{
		...
		if (item && item->getIsLinkType() &&
			...
			!gInventory.isObjectDescendentOf(item_id, market_id))
		{
...
int HBViewerAutomation::newInventoryFolder(lua_State* state)
{
	...
	if (cat_id.notNull())
	{
		cat_id = gInventory.createNewCategory(cat_id, LLFolderType::FT_NONE,
											  folder_name);
	}
...
//mk
	...
	else if (preset == "region")
	{
		...
		{
			gSavedSettings.setBool("UseWLEstateTime", true);
		}
	}
...
//mk
	...
	if (time < 0.f)
	{
		...
		{
			// Revert to default time...
			gWLSkyParamMgr.animate(true);
		}
		return 0;
	}
	...
	if (!override_wl)
	{
		...
		gWLSkyParamMgr.mAnimator.setDayTime(time);
...
int HBViewerAutomation::getEnvironmentStatus(lua_State* state)
{
	...
	lua_pushboolean(state, gUseNewShaders);
	lua_rawset(state, -3);
											 ...
											 bool reached, bool user_cancel)
{
	...
	if (!mLuaState || !mHasOnAutoPilotFinished)
	{
		return;
	}
...
{
	if (mActiveButtons && LLStartUp::isLoggedIn())
	{
		LLPanel::draw();
	}
}
...
void HBLuaSideBar::setButton(U32 number, std::string icon, std::string command,
