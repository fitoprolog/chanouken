											  ...
											  U32 index)
{
	...
	addWearableToAgentInventory(cb, new_wearable, laf, true);
}
									 ...
									 bool rename_clothing)
{
	...
	// First, make a folder in the user-preferred folder, or the Clothing
	...
	if (wearables_to_include.size())
	{
		...
		LLViewerWearable* new_wearable;
		std::string new_name;
		for (S32 i = 0, count = wearables_to_include.size(); i < count; ++i)
		{
			const LLUUID& item_id = wearables_to_include[i];
			old_wearable = getWearableFromItemID(item_id);
			S32 index = getWearableTypeAndIndex(old_wearable, type);
			...
			if (!use_link && (no_link || can_copy))
			{
				if (can_copy)
				{
					new_wearable = wl->createCopy(old_wearable);
					if (rename_clothing)
					{
					...
					copy_inventory_item(item->getPermissions().getOwner(),
										item->getLinkedUUID(), folder_id,
										new_name, cb);
				}
				else
				{
					move_inventory_item(item->getLinkedUUID(), folder_id,
										new_name);
				}
			}
			else
			{
				link_inventory_item(item->getLinkedUUID(), folder_id,
	...
	if (attachments_to_include.size())
	{
		for (S32 i = 0, count = attachments_to_include.size(); i < count; ++i)
		{
			const LLUUID& item_id = attachments_to_include[i];
			LLViewerInventoryItem* item = gInventory.getItem(item_id);
			if (!item) continue;

			const LLUUID& inv_item_id = item->getLinkedUUID();
			if (!do_link &&
				(no_link || item->getPermissions().allowCopyBy(gAgentID)))
			{
				...
				{
					LLPointer<LLInventoryCallback> cb =
						new LLMoveAfterCopyDoneCallback(inv_item_id, folder_id,
														item_name);
					copy_inventory_item(item->getPermissions().getOwner(),
										inv_item_id, old_folder_id, item_name,
										cb);
				}
				else
				{
					move_inventory_item(inv_item_id, folder_id, item_name);
				}
			}
			else
			{
				link_inventory_item(inv_item_id, folder_id,
									item->getDescription(),
									LLAssetType::AT_LINK);
			}
		}
