...
constexpr F32 MIN_BANDWIDTH = 100.f;
constexpr F32 MAX_BANDWIDTH = 10000.f;
...
constexpr F32 DYNAMIC_UPDATE_DURATION = 5.0;	// In seconds
...
static const F32 BW_PRESET_50[TC_EOF]   = {   5,  10,   3,   3,  10,  10,   9 };
static const F32 BW_PRESET_300[TC_EOF]  = {  30,  40,   9,   9,  86,  86,  40 };
static const F32 BW_PRESET_500[TC_EOF]  = {  50,  70,  14,  14, 136, 136,  80 };
static const F32 BW_PRESET_1000[TC_EOF] = { 100, 100,  20,  20, 310, 310, 140 };
static const F32 BW_PRESET_2000[TC_EOF] = { 200, 200,  25,  25, 450, 800, 300 };
...
{
	static LLCachedControl<F32> max_bandwidth(gSavedSettings,
											  "ThrottleBandwidthKBPS");
	mMaxBandwidth = llclamp(F32(max_bandwidth), MIN_BANDWIDTH, MAX_BANDWIDTH) *
					1024;
	if (mMaxBandwidth != (F32)max_bandwidth)
	{
		gSavedSettings.setF32("ThrottleBandwidthKBPS", max_bandwidth);
	}
...
LLViewerThrottleGroup LLViewerThrottle::getThrottleGroup(F32 bandwidth_kbps)
{
...
void LLViewerThrottle::updateDynamicThrottle()
{
	...
	if (stats->mPacketsLostPercentStat.getMean() > TIGHTEN_THROTTLE_THRESHOLD)
	{
		if (mThrottleFrac <= MIN_FRACTIONAL ||
			mCurrentBandwidth / 1024.f <= MIN_BANDWIDTH)
		{
			return;
		}
	...
	else if (stats->mPacketsLostPercentStat.getMean() <= EASE_THROTTLE_THRESHOLD)
	{
