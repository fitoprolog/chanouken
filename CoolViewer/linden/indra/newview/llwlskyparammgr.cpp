...
#include "llsdutil.h"
#include "llsettingsdaycycle.h"		// For LLSettingsDay::DEFAULT_DAY*
#include "lluri.h"
...
{
	hash_vector_t::const_iterator n = mParamHashedNames.begin();
	hash_vector_t::const_iterator m = mParamHashedNames.end();
		 ...
		 i != end && n != m; ++i, ++n)
	{
		...
		// Check that our pre-hashed names are still tracking the mParamValues
		// map correctly
		llassert(param.String() == i->first);

		if (param == sStarBrightness || param == sPresetNum ||
			param == sSunAngle || param == sEastAngle ||
			param == sEnableCloudScroll || param == sCloudScrollRate ||
			param == sLightNorm)
		{
			continue;
		}
		...
		{
			LLVector4 val((F32)i->second[0].asReal() + mCloudScrollXOffset,
						  (F32)i->second[1].asReal() + mCloudScrollYOffset,
						  (F32)i->second[2].asReal(),
						  (F32)i->second[3].asReal());
			shader->uniform4fv(param, 1, val.mV);
			continue;
		}

		if (param == sCloudScale || param == sCloudShadow ||
			param == sDensityMultiplier || param == sDistanceMultiplier ||
			param == sHazeDensity|| param == sHazeHorizon ||
			param == sMaxY)
		{
			shader->uniform1f(param, (F32)i->second[0].asReal());
			continue;
		}

		LLSD::Type type = i->second.type();
		switch (type)
		{
			case LLSD::TypeArray:
			{
				if (i->second.size() == 4)
				{
					shader->uniform4fv(param, 1, LLVector4(i->second).mV);
				}
				break;
			}

			case LLSD::TypeReal:
				shader->uniform1f(param, (F32)i->second.asReal());
				break;
...
// *HACK: this method contains a collection of dirty hacks in order to deal
// with extended environment to Windlight conversion for parcel Windlight
// rendering...
// The problem is that every few seconds the sim sends us an environment update
// (which triggers a call to this method by LLEnvironment), and performing a
// getParamSet() on mCurParams causes the clouds to reset to their original
// position each time, even when cloud params did not change at all.
// So we try and detect whether we must actually call getParamSet() or not.
// Note that mCurrentParamsDirty must also be set (via setDirty()) each time
// an extended environment sky parameter is changed by one of the environment
// editor floaters (this is accounted for by LLPanelEnvSettingsSky), or as a
// result of an altitude change that would trigger a sky track change (done in
// LLEnvironment::onAgentPositionHasChanged()).
// Also do set mCurrentParamsDirty when changing any of the parcel or local
// environment debug settings/toggles (done in llviewercontrol.cpp).
// *TODO: one case is not detected by all these hacks and it is when the parcel
// environment is changed by someone else than you (via the floaters) or an
// altitude change, such as via scripts or by someone else...
void LLWLSkyParamMgr::applyToParcel(const LLSD& sky_params)
{
	// Detect fast changes in environment, which are likely to denote a
	// progressive transition in extended environment parameters and therefore
	// mandate an unconditionnal update.
	static F32 last_update = 0.f;
	if (gFrameTimeSeconds - last_update < 0.5f)
	{
		mCurrentParamsDirty = true;
	}
	last_update = gFrameTimeSeconds;

	// Detect a parcel change, which does mandate an unconditionnal update.
	static S32 last_agent_parcel_id = 0;
	S32 parcel_id = gViewerParcelMgr.getAgentParcelId();
	if (parcel_id != last_agent_parcel_id)
	{
		last_agent_parcel_id = parcel_id;
		mCurrentParamsDirty = true;
	}

	static const std::string name = "current parcel environment";
	if (mParamList.count(name))
	{
		LL_DEBUGS("Windlight") << "Updating parcel environment:\n"
							   <<  ll_pretty_print_sd(sky_params) << LL_ENDL;
		setParamSet(name, sky_params);
	}
	else
	{
		LL_DEBUGS("Windlight") << "Adding parcel environment:\n"
							   <<  ll_pretty_print_sd(sky_params) << LL_ENDL;
		addParamSet(name, sky_params);
		// Parameter added, not changed, so do update mCurParams !
		mCurrentParamsDirty = true;
	}

	if (mCurrentParamsDirty)
	{
		mCurrentParamsDirty = false;
		getParamSet(name, mCurParams);
	}
	propagateParameters();
}

void LLWLSkyParamMgr::updateShaderUniforms(LLGLSLShader* shader)
{
	...
	if (shader->mShaderGroup == LLGLSLShader::SG_DEFAULT)
	{
		...
		shader->uniform3fv(LLShaderMgr::WL_CAMPOSLOCAL, 1,
						   gViewerCamera.getOrigin().mV);
	}
...
void LLWLSkyParamMgr::propagateParameters()
{
	...
	// Bind the variables for all shaders only if we are using WindLight
	bool can_use_wl_shaders = gPipeline.canUseWindLightShaders();
	for (LLViewerShaderMgr::shader_iter it = gViewerShaderMgrp->beginShaders(),
										end = gViewerShaderMgrp->endShaders();
		 it != end; ++it)
	{
		if (it->mProgramObject &&
			(can_use_wl_shaders || it->mShaderGroup == LLGLSLShader::SG_WATER))
		{
...
void LLWLSkyParamMgr::update(LLViewerCamera* cam)
{
	LL_FAST_TIMER(FTM_UPDATE_WLPARAM);
	...
	// Sync Windlight floater if it exists
	LLFloaterWindlight* flwlp = LLFloaterWindlight::findInstance();
	if (flwlp)
	{
		flwlp->refresh();
	}

	// *TODO: as a potential optimization, this block may only need to be
	// executed some of the time. For example for water shaders only.
	LLVector3 light_norm(mLightDir);
	light_norm *= LLQuaternion(-cam->getYaw() - F_PI, LLVector3::y_axis);
	mRotatedLightDir = LLVector4(light_norm, 0.f);
