...
// Creates a vertex buffer for efficiently rendering cubes
LLVertexBuffer* ll_create_cube_vb(U32 type_mask, U32 usage)
{
	// Note: cannot use (std::nothrow) with our custom LLVertexBuffer::new()
	// allocator
	LLVertexBuffer* ret = NULL;
	try
	{
		ret = new LLVertexBuffer(type_mask, usage);
	}
	catch (std::bad_alloc&)
	{
	}
	if (!ret)
	{
		llwarns << "Cannot create a cube VB (out of memory ?)" << llendl;
		return NULL;
	}
	...
	LLStrider<U16> idx;
	if (!ret->getVertexStrider(pos) || !ret->getIndexStrider(idx))
	{
#if 0	// Ref-counted, shall auto-delete...
		delete ret;
#endif
		return NULL;
	}
...
bool LLViewerOctreeGroup::removeFromGroup(LLViewerOctreeEntry* entry)
{
	...
	if (mOctreeNode)
	{
		...
		{
			llwarns_once << "Could not remove LLVOCacheEntry from LLVOCacheOctreeGroup"
						 << llendl;
			llassert(false);
			return false;
		}
...
void LLViewerOctreeGroup::rebound()
{
		...
		mOctreeNode->getElementCount() == 0)
	{
		...
		if (!group)
		{
			llwarns_once << "NULL group found !  Cannot rebound." << llendl;
			llassert(false);
			return;
		}
	...
	else
	{
		...
		if (!group)
		{
			llwarns_once << "NULL group found !  Can't rebound." << llendl;
			llassert(false);
			return;
		}
											  ...
											  OctreeNode* child)
{
	...
	{
		llwarns_once << "LLViewerOctreeGroup redundancy detected." << llendl;
		llassert(false);
	}
									   ...
									   LLVector4a& max_out)
{
	...
	if (node->isEmpty())
	{
		...
		if (empty && mOctreeNode->getParent())
		{
			...
			llwarns_once << "Empty leaf found in octree." << llendl;
			llassert(false);
		}
		return false;
	}
...
bool LLOcclusionCullingGroup::needsUpdate()
{
	return mSpatialPartition &&
		   (S32)(LLDrawable::getCurrentFrame() %
...
bool LLOcclusionCullingGroup::isRecentlyVisible() const
{
	...
	return LLDrawable::getCurrentFrame() -
												  ...
												  OctreeNode* child)
{
	...
	{
		llwarns_once << "LLOcclusionCullingGroup redundancy detected."
					 << llendl;
		llassert(false);
	}
...
void LLOcclusionCullingGroup::checkOcclusion()
{
	...
	if (query && isOcclusionState(QUERY_PENDING))
	{
		...
		GLuint available = 0;
		glGetQueryObjectuivARB(query, GL_QUERY_RESULT_AVAILABLE_ARB,
							   &available);
		static LLCachedControl<bool> wait_for_query(gSavedSettings,
													"RenderSynchronousOcclusion");
		static LLCachedControl<U32> max_loops(gSavedSettings,
											  "RenderSyncOcclMaxLoops");
		static LLCachedControl<bool> do_useful_work(gSavedSettings,
													"RenderSyncOcclWorkWaiting");
		if (!available && wait_for_query)
		{
			U32 count = 1024;
			U32 frame = gFrameCount;
			if (max_loops > 0 && frame > 2 && !gGLManager.mIsIntel)
			{
				count = max_loops;
				frame -= 2;
			}
			if (mOcclusionIssued[LLViewerCamera::sCurCameraID] < frame)
			{
				// Query was issued last frame, wait until it is available
				F32 max_time = llmin(gFrameIntervalSeconds, 0.1f);
				LL_FAST_TIMER(FTM_OCCLUSION_WAIT);
				while (!available && count-- > 0)
				{
					// Do some useful work while we wait
					if (do_useful_work)
					{
						const F32 time_slice = max_time * 0.33333333f;
						// Un-pause the texture cache thread
						gTextureCachep->update(time_slice);
						// Un-pause the image thread
						gImageDecodeThreadp->update(time_slice);
						// Un-pause the texture fetch thread
						gTextureFetchp->update(time_slice);
					}
		...
		// If the result is not available, wait until next frame...
		if (!available)
		{
			return;
		}
		...
		{
			// Note any unoccluded water, for deciding on reflection/distortion
			// passes (if occlusion is disabled, these are set within
		...
		GLuint res = 0;
		glGetQueryObjectuivARB(query, GL_QUERY_RESULT_ARB, &res);
		if (res > 0)
		{
	...
	else if (mSpatialPartition->isOcclusionEnabled() &&
...
#if 0
	LLGLDisable stencil(gUseNewShaders ? 0 : GL_STENCIL_TEST);
#endif
	// No query pending, or previous query to be discarded
	{
...

		{
...

			{
				...
				query = getNewOcclusionQueryObjectName();
				mOcclusionQuery[LLViewerCamera::sCurCameraID] = query;
				glBeginQueryARB(mode, query);
			}

			LLGLSLShader* shader = LLGLSLShader::sCurBoundShaderPtr;
			// May be NULL while switching between WL and EE renderers: seen
			// happening on super-old (and thus super-slow) hardware. HB
			if (shader)
			{
				...
				F32 fudge_z;
				if (type == LLPipeline::RENDER_TYPE_VOIDWATER && !fast_edges)
				{
					fudge_z = 1.f;
				}
				else
			...
			if (!use_depth_clamp && type == LLPipeline::RENDER_TYPE_VOIDWATER)
			{
				...
				LLGLSquashToFarClip squash(gUseNewShaders ? 0 : 1);
				if (camera->getOrigin().isExactlyZero())
				{
...
void LLViewerOctreeCull::traverse(const OctreeNode* n)
{
	if (!n)
	{
		llwarns_once << "NULL node was passed !  Skipping..." << llendl;
		llassert(false);
		return;
	}
