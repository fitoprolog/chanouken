...
void LLSurfacePatch::setSurface(LLSurface* surfacep)
{
	...
	if (!regionp)
	{
		LL_DEBUGS("RegionTexture") << "No region associated with surface patch: "
								   << std::hex << (intptr_t)this << std::dec
								   << LL_ENDL;
	}
	else if (mVObjp.isNull())
	{
...
bool LLSurfacePatch::updateTexture()
{
	...
	// detailTexturesReady() must be called periodically
