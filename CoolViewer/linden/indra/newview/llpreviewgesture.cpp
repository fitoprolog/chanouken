...
void LLInventoryGestureAvailable::done()
{
	...
	for (item_ref_t::iterator it = mComplete.begin(), end = mComplete.end();
		 it != end; ++it)
	{
	...
	switch (cargo_type)
	{
		...
		case DAD_SOUND:
		{
			...
			if (item && gInventory.getItem(item->getUUID()))
			{
				...
				if (!(perm.getMaskBase() & PERM_ITEM_UNRESTRICTED))
				{
					*accept = ACCEPT_NO;
		...
		if (item)
		{
			...
			if (!agent_url.empty() && !task_url.empty())
			{
				...
				else if (mObjectUUID.notNull() && !task_url.empty())
				{
					...
					// NOTE: what looks like a bug in boost::make_shared
					// prevents from using it here, because of the NULL
					// boot::function pointer passed...
