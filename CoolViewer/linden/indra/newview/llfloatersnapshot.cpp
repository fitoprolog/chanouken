...
public:
	...
	LLFloaterSnapshot::ESnapshotFormat getSnapshotFormat() const
	{
	...
	LL_INLINE LLViewerTexture* getThumbnailImage() const	{ return mThumbnailImage; }
	...
	LL_INLINE void setSnapshotType(ESnapshotType type)		{ mSnapshotType = type; }

	LL_INLINE void setSnapshotFormat(LLFloaterSnapshot::ESnapshotFormat type)
	{
		mSnapshotFormat = type;
	}

	LL_INLINE void setSnapshotBufferType(LLViewerWindow::ESnapshotType type)
	{
										   ...
										   bool width_changed)
{
	...
	// profiles, etc and need to be "squashed" during upload, not cropped here
#if 0
	// If texture, ignore aspect ratio setting, round image size to power of 2.
	if (gSavedSettings.getS32("LastSnapshotType") == SNAPSHOT_TEXTURE)
	{
									...
									getMaxImageSize()))
	{
		...
		else
		{
			...
			LLFloaterSnapshot::ESnapshotFormat format =
				getSnapshotType() == SNAPSHOT_POSTCARD ?
...
bool LLFloaterSnapshot::postBuild()
{
	...
	mImageWidthSpinner->setValue(gSavedSettings.getS32(lastSnapshotWidthName().c_str()));
	mImageHeightSpinner->setValue(gSavedSettings.getS32(lastSnapshotHeightName().c_str()));
...
LLFloaterSnapshot::ESnapshotFormat LLFloaterSnapshot::getFormatIndex()
{
...
LLViewerWindow::ESnapshotType LLFloaterSnapshot::getLayerType()
{
...
void LLFloaterSnapshot::checkAspectRatio(S32 index)
{
	...
	else if (index == -1)	// Custom size
	{
		...
		if (LLSnapshotLivePreview::SNAPSHOT_TEXTURE !=
				gSavedSettings.getS32("LastSnapshotType"))
#endif
		{
...
void LLFloaterSnapshot::resetSnapshotSizeOnUI(S32 width, S32 height)
{
	...
	gSavedSettings.setS32(lastSnapshotWidthName().c_str(), width);
	gSavedSettings.setS32(lastSnapshotHeightName().c_str(), height);
}

// Sets the size combo to "custom" mode.
void LLFloaterSnapshot::comboSetCustom(LLComboBox* combo)
{
	// "custom" is always the last index
	combo->setCurrentByIndex(combo->getItemCount() - 1);

	if (combo == mPostcardSizeCombo)
	{
		gSavedSettings.setS32("SnapshotPostcardLastResolution",
							  combo->getCurrentIndex());
	}
	else if (combo == mTextureSizeCombo)
	{
		gSavedSettings.setS32("SnapshotTextureLastResolution",
							  combo->getCurrentIndex());
	}
	else if (combo == mLocalSizeCombo)
	{
...
{
	static LLCachedControl<S32> snap_type(gSavedSettings, "LastSnapshotType");
	mSnapshotTypeRadio->setSelectedIndex(snap_type);

	LLSnapshotLivePreview::ESnapshotType shot_type =
		(LLSnapshotLivePreview::ESnapshotType)getTypeIndex();
	...
	LLViewerWindow::ESnapshotType layer_type = getLayerType();
	...
	static LLCachedControl<S32> format(gSavedSettings, "SnapshotFormat");
	ESnapshotFormat shot_format = ESnapshotFormat((S32)format);
	bool is_jpeg = shot_format == LLFloaterSnapshot::SNAPSHOT_FORMAT_JPEG;
	mLocalFormatCombo->selectNthItem(format);

	mUploadButton->setVisible(is_texture);
	mSendButton->setVisible(is_postcard);
	mSaveButton->setVisible(is_local);
	mKeepAspectCheck->setEnabled(!is_texture && !sAspectRatioCheckOff);
	mLayerTypeCombo->setEnabled(is_local);

	LLViewerRegion* region = gAgent.getRegion();
	bool has_temp_upload = region ? region->getCentralBakeVersion() == 0
								  : false;
	...
	mAutoCloseCheck->setVisible(is_advance);
	mFreezeFrameCheck->setVisible(is_advance);
	mAutoSnapCheck->setVisible(is_advance);
	mImageQualitySlider->setVisible(is_advance &&
	...
	mUploadButton->setEnabled(is_texture && got_snap);
...
//mk
	...
	mLivePreview->setSnapshotFormat(shot_format);
...
void LLFloaterSnapshot::onClickKeep(void* data)
{
	...
	LLSnapshotLivePreview::ESnapshotType type =
		self->mLivePreview->getSnapshotType();
	if (type == LLSnapshotLivePreview::SNAPSHOT_POSTCARD)
	{
		LLFloaterPostcard* floater = self->mLivePreview->savePostcard();
		// If still in snapshot mode, put postcard floater in snapshot
		// floaterview and link it to snapshot floater
		if (floater && !close)
		{
			gFloaterViewp->removeChild(floater);
			gSnapshotFloaterViewp->addChild(floater);
			self->addDependentFloater(floater, false);
		}
	}
										 ...
										 bool do_update)
{
	...
	if (combobox->getCurrentIndex() >= 0)
	{
		...
		else if (width == -1 || height == -1)
		{
			...
			self->mLivePreview->setSize(gSavedSettings.getS32(lastSnapshotWidthName().c_str()),
										gSavedSettings.getS32(lastSnapshotHeightName().c_str()));
		}
		else
		{
...
void LLFloaterSnapshot::onCommitLayerTypes(LLUICtrl* ctrl, void* data)
{
	...
	LLViewerWindow::ESnapshotType type =
		(LLViewerWindow::ESnapshotType)combo->getCurrentIndex();
...
void LLFloaterSnapshot::onCommitSnapshotType(LLUICtrl* ctrl, void* data)
{
	...
	if (self)
	{
		gSavedSettings.setS32("LastSnapshotType", self->getTypeIndex());
		self->mLivePreview->updateSnapshot(true);
		self->updateControls();
	}
}
...
void LLFloaterSnapshot::onCommitSnapshotFormat(LLUICtrl* ctrl, void* data)
{
	...
	if (self)
	{
		gSavedSettings.setS32("SnapshotFormat", self->getFormatIndex());
		self->mLivePreview->updateSnapshot(true);
		self->updateControls();
	}
}
...
void LLFloaterSnapshot::onCommitCustomResolution(LLUICtrl* ctrl, void* data)
{
	...
	gSavedSettings.setS32(lastSnapshotWidthName().c_str(), w);
	gSavedSettings.setS32(lastSnapshotHeightName().c_str(), h);
...
bool LLSnapshotFloaterView::handleMouseDown(S32 x, S32 y, MASK mask)
{
	...
	{
		LLToolMgr::getInstance()->getCurrentTool()->handleMouseDown(x, y,
																	mask);
	}
	return true;
}
