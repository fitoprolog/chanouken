...
void LLSurface::create(S32 grids_per_edge, S32 grids_per_patch_edge,
					   const LLVector3d& origin_global, F32 width)
{
...
void LLSurface::createWaterTexture()
{
	...
	LLPointer<LLImageRaw> raw = new LLImageRaw(sTextureSize / 2,
											   sTextureSize / 2, 4);
	U8* default_texture = raw->getData();
	if (!default_texture)
	{
	...
	for (S32 i = 0; i < sTextureSize; i += 2)
	{
		for (S32 j = 0; j < sTextureSize; j += 2)
		{
			S32 index = i * sTextureSize + j * 2;
...
// Assumes that mGridsPerEdge, mGridsPerPatchEdge and mPatchesPerEdge have been
