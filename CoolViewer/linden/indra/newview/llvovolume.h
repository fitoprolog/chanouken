...
public:
	...
	LLVOVolume(const LLUUID& id, LLPCode pcode, LLViewerRegion* regionp);
	...
	LL_INLINE void markForUpdate(bool priority) override
	{
		LLViewerObject::markForUpdate(priority);
		mVolumeChanged = true;
	}

	LL_INLINE void faceMappingChanged()					{ mFaceMappingChanged = true; }
