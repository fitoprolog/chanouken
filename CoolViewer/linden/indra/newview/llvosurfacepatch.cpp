...
:	LLSpatialPartition(LLDrawPoolTerrain::VERTEX_DATA_MASK, false,
					   GL_DYNAMIC_DRAW_ARB, regionp)
{
													   ...
													   U32 usage)
{
	...
	LLVertexBuffer* bufferp = new LLVertexBuffer(typemask,
												 GL_DYNAMIC_DRAW_ARB);
	// Mask out coordinates 2 and 3 in the mask passed to setupVertexBuffer()
	// (this used to be done via a virtual method in a LLVertexBufferTerrain()
	// derived class). HB
...
LLVOSurfacePatch::LLVOSurfacePatch(const LLUUID& id, LLPCode pcode,
...
{
	mPool = (LLDrawPoolTerrain*)gPipeline.getPool(LLDrawPool::POOL_TERRAIN,
