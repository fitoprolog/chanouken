...
:	LLFloaterNameDesc(filename,
					  LLEconomy::getInstance()->getAnimationUploadCost()),
	mAnimPreview(NULL),
	mInWorld(false),
	mBadAnimation(false)
{
...
bool LLFloaterAnimPreview::postBuild()
{
	...
	F32 max_anim_duration = llclamp(gSavedSettings.getF32("AnimationsMaxDuration"), 30.f,
		...
		loaderp->getDuration() <= max_anim_duration)
	{
		...
		bool success = motionp && motionp->deserialize(dp, false);
		llinfos << "Done." << llendl;
...
{
	LLFloaterAnimPreview* self = (LLFloaterAnimPreview*)user_data;
...
{
	LLFloaterAnimPreview* self = (LLFloaterAnimPreview*)data;
	if (!self || !self->getEnabled())
	{
		return;
	}
...
void LLFloaterAnimPreview::onBtnOK(void* userdata)
{
	...
	if (!self || !self->getEnabled()) return;

	LLKeyframeMotion* motionp = self->getMotion();
	if (motionp)
	{
		// Make sure the animation is stopped since we sill destroy the motion
		// once uploaded
		LLVOAvatar* avatarp = self->getAvatar();
		avatarp->stopMotion(self->mMotionID, true);
		...
		if (motionp->serialize(dp))
		{
			...
			if (file.write((U8*)buffer, size))
			{
				...
				std::string desc =
					self->childGetValue("description_form").asString();
				LLResourceUploadInfo::ptr_t
					info(new LLResourceUploadInfo(self->mTransactionID,
		...
		avatarp->removeMotion(self->mMotionID);
		if (self->mInWorld)
		{
			avatarp->deactivateAllMotions();
		}
		LLKeyframeDataCache::removeKeyframeData(self->mMotionID);
	}
...
bool LLPreviewAnimation::render()
{
	...
	gViewerCamera.setPerspective(false, mOrigin.mX, mOrigin.mY, mFullWidth,
								  mFullHeight, false);
	mCameraRelPos = gViewerCamera.getOrigin() - avatarp->mHeadp->getWorldPosition();
	...
	if (avatarp->mDrawable.notNull())
	{
		...
		if (facep)	// Paranoia
		{
			...
			if (avatarPoolp)	// More paranoia !
			{
				...
				// Renders only one avatar
				avatarPoolp->renderAvatars(avatarp);
			}
		}
	}
