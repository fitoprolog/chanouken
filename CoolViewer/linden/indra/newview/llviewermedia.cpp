									   ...
									   const LLViewerMediaImpl* i2)
{
	...
	else if (i1->getUsedOnHUD() && !i2->getUsedOnHUD())
	{
		// i1 is used on a HUD, i2 is not. This makes i1 "less than" i2, so it
		// sorts earlier in our list.
		return true;
	}
	else if (i2->getUsedOnHUD() && !i1->getUsedOnHUD())
	{
	...
	{
		// The object with the larger interest value should be earlier in the
		// list, so we reverse the sense of the comparison here.
								 ...
								 const LLViewerMediaImpl* i2)
{
	...
	{
		// Both objects have the same distance. This most likely means they are
		// two faces of the same object. They may also be faces on different
		// objects with exactly the same distance (like HUD objects). We do not
		// actually care what the sort order is for this case, as long as it is
		// stable and does not change when you enable/disable media. Comparing
		// the impl pointers gives a completely arbitrary ordering, but it will
		// be stable.
		return i1 < i2;
	}
}
								...
								const std::string& token)
{
	...
	LLCoros::getInstance()->launch("LLViewerMedia::openIDSetupCoro",
								   boost::bind(&LLViewerMedia::openIDSetupCoro,
											   url, token));
}
...
void LLViewerMediaImpl::loadURI()
{
	if (mMediaSource && !mMediaURL.empty())
	{
		...
		{
			// No relevant previous media play state -- if we're loading the
...
void LLViewerMediaImpl::navigateInternal()
{
	...
	if (mNavigateRediscoverType)
	{
		...
		{
			LLCoros::getInstance()->launch("LLViewerMediaImpl::mimeDiscoveryCoro",
...
void LLViewerMediaImpl::update()
{
	...
	LLViewerMediaTexture* placeholder_img = updatePlaceholderImage();
	if (placeholder_img)
	{
		LLRect dirty_rect;

		// Since we are updating this texture, we know it's playing. Tell the
		// texture to do its replacement magic so it gets rendered.
		placeholder_img->setPlaying(true);
		...
		if (mMediaSource->getDirty(&dirty_rect))
		{
			...
			S32 width = llmin(dirty_rect.mRight,
							  placeholder_img->getWidth()) - x_pos;
			S32 height = llmin(dirty_rect.mTop,
							   placeholder_img->getHeight()) - y_pos;
			...
			if (width > 0 && height > 0)
			{
				...
				if (data)
				{
					...
					data += x_pos * mMediaSource->getTextureDepth() *
							mMediaSource->getBitsWidth() +
							y_pos * mMediaSource->getTextureDepth();

					LL_FAST_TIMER(FTM_MEDIA_SET_SUBIMAGE);
					placeholder_img->setSubImage(data,
												 mMediaSource->getBitsWidth(),
...
LLViewerMediaTexture* LLViewerMediaImpl::updatePlaceholderImage()
{
	...
	LLViewerMediaTexture* placeholder_img =
		LLViewerTextureManager::getMediaTexture(mTextureId);
	if (placeholder_img &&
		(mNeedsNewTexture || placeholder_img->getUseMipMaps() ||
		 placeholder_img->getWidth() != mMediaSource->getTextureWidth() ||
		 placeholder_img->getHeight() != mMediaSource->getTextureHeight() ||
		 ...
		 mTextureUsedHeight != mMediaSource->getHeight()))
	{
		...
		placeholder_img->destroyGLTexture();
		// MEDIAOPT: apparently just calling setUseMipMaps(false) does
		// not work ?
		placeholder_img->reinit(false);	// probably not needed
		...
		placeholder_img->setExplicitFormat(mMediaSource->getTextureFormatInternal(),
										   mMediaSource->getTextureFormatPrimary(),
										   mMediaSource->getTextureFormatType(),
										   mMediaSource->getTextureFormatSwapBytes());

		placeholder_img->createGLTexture(0, raw);	// 0 discard
