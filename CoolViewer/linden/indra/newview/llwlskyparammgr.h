...
public:
	...
	// Updates this set of shader uniforms from the parameter values.
...
// WindLight parameter manager class controlling all the Windlight shaders
...
public:
	...
	// Applies sky parameters to the "current parcel environment" preset
	void applyToParcel(const LLSD& sky_params);

	// Sets shader uniforms dirty, so they will update automatically.
	void propagateParameters();

	// Updates shader uniforms that have changed.
	...
	// Updates information camera dependent parameters
	...
	// Gets where the light is pointing
	LL_INLINE LLVector4 getRotatedLightDir() const	{ return mRotatedLightDir; }
