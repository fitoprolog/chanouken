...
static const char* mem_format_str =
	"GL tot: %d/%d MB - Bound: %d/%d MB - Raw tot: %d MB - Bias: %.2f - Cache: %.1f/%.1f MB";
static const char* tex_format_str =
	"Tex(Raw): %d(%d) Fetches: %d(%d) HTTP: %d UDP BW: %.0f Cache R/W: %d/%d LFS: %d Dec: %d Boost: %.1f";
...
constexpr S32 TEXTUREVIEW_WIDTH = 628;
...
void LLTextureBar::draw()
{
	...
	if (!fontp)
	{
		llwarns_once << "No monospace font !" << llendl;
		return;
	}
...
public:
	...
	{
		LLRect rect;
		rect.mTop = 2 * BAR_HEIGHT;	// Room for three lines of text
		return rect;
	}
...
void LLGLTexMemBar::draw()
{
	...
	S32 bound_mem =
		BYTES2MEGABYTES(LLViewerTexture::sBoundTextureMemoryInBytes);
 	S32 max_bound_mem = LLViewerTexture::sMaxBoundTextureMemInMegaBytes;
	S32 total_mem =
		BYTES2MEGABYTES(LLViewerTexture::sTotalTextureMemoryInBytes);
	S32 max_total_mem = LLViewerTexture::sMaxTotalTextureMemInMegaBytes;
	F32 discard_bias = LLViewerTexture::sDesiredDiscardBias;
	S32 line_height = (S32)(fontp->getLineHeight() + .5f);
	F32 cache_usage = ((F32)(gTextureCachep->getUsage() / 1024)) / 1024.f;
	F32 cache_max_usage = ((F32)(gTextureCachep->getMaxUsage() / 1024)) /
						  1024.f;
	...
	std::string text = llformat(mem_format_str, total_mem, max_total_mem,
								bound_mem, max_bound_mem,
								LLImageRaw::sGlobalRawMemory >> 20,
								discard_bias, cache_usage, cache_max_usage);
