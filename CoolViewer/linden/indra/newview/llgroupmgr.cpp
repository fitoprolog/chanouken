...
bool join_group_response(const LLSD& notification, const LLSD& response)
{
	...
	else if (use_cap)
	{
		...
		LLCoros::getInstance()->launch("groupInvitationResponse",
									   boost::bind(&join_group_response_coro,
												   url, group_id,
												   accept_invite));
	}
	else
									 ...
									 const uuid_vec_t& ban_list)
{
	...
	{
		case REQUEST_GET:
			LLCoros::getInstance()->launch("LLGroupMgr::getGroupBanRequestCoro",
										   boost::bind(&LLGroupMgr::getGroupBanRequestCoro,
													   cap_url, group_id));
			break;

		case REQUEST_POST:
			LLCoros::getInstance()->launch("LLGroupMgr::postGroupBanRequestCoro",
...
void LLGroupMgr::sendCapGroupMembersRequest(const LLUUID& group_id)
{
	...
	LLCoros::getInstance()->launch("LLGroupMgr::groupMembersRequestCoro",
								   boost::bind(&LLGroupMgr::groupMembersRequestCoro,
											   cap_url, group_id));
}
