	...
	mShrunk(false)
{
	...
	std::string config_string = getString("client_frame_rate_critical_fps",
										   mStringArgs);
	mClientFrameTimeCritical = 1.0f / (F32)atof(config_string.c_str());
	config_string = getString("client_frame_rate_warning_fps", mStringArgs);
	mClientFrameTimeWarning = 1.0f / (F32)atof(config_string.c_str());
	...
	mServerFrameTimeCritical = 1000.0f / (F32)atof(config_string.c_str());
	config_string = getString("server_frame_rate_warning_fps", mStringArgs);
	mServerFrameTimeWarning = 1000.0f / (F32)atof(config_string.c_str());
...
void LLFloaterLagMeter::refresh()
{
	...
	determineClient(viewerstats);
	determineNetwork(viewerstats);
	determineServer(viewerstats);
}

void LLFloaterLagMeter::determineClient(LLViewerStats* viewerstats)
{
	F32 client_frame_time = viewerstats->mFPSStat.getMeanDuration();
	...
	else if (BYTES2MEGABYTES(LLViewerTexture::sBoundTextureMemoryInBytes) >
...
void LLFloaterLagMeter::determineNetwork(LLViewerStats* viewerstats)
{
	F32 packet_loss = viewerstats->mPacketsLostPercentStat.getMean();
	F32 ping_time = viewerstats->mSimPingStat.getMean();
	...
	F32 client_frame_time_ms = 1000.0f * viewerstats->mFPSStat.getMeanDuration();
...
void LLFloaterLagMeter::determineServer(LLViewerStats* viewerstats)
{
	F32 sim_frame_time = viewerstats->mSimFrameMsec.getCurrent();
	...
	if (viewerstats->mSimSimPhysicsMsec.getCurrent() > mServerSingleProcessMaxTime)
	{
		mServerCause->setText(getString("server_physics_cause_msg", mStringArgs));
	}
	else if (viewerstats->mSimScriptMsec.getCurrent() > mServerSingleProcessMaxTime)
	{
		mServerCause->setText(getString("server_scripts_cause_msg", mStringArgs));
	}
	else if (viewerstats->mSimNetMsec.getCurrent() > mServerSingleProcessMaxTime)
	{
		mServerCause->setText(getString("server_net_cause_msg", mStringArgs));
	}
	else if (viewerstats->mSimAgentMsec.getCurrent() > mServerSingleProcessMaxTime)
	{
		mServerCause->setText(getString("server_agent_cause_msg", mStringArgs));
	}
	else if (viewerstats->mSimImagesMsec.getCurrent() > mServerSingleProcessMaxTime)
	{
