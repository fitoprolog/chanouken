...
void LLMeshRepoThread::run()
{
	...
	while (!LLApp::isExiting())
	{
		...
		// For the final three request sets, similar goal to above but slightly
		...
		if (can_req && !mSkinRequests.empty())
		{
			...
			do
			{
				...
				else if (!fetchMeshSkinInfo(req.mId))
				{
					...
					{
						// Common, harmless occurrence, so debug message only
						LL_DEBUGS("Mesh") << "Skin request failed for "
										  << req.mId << LL_ENDL;
					}
				}
				can_req = (S32)mHttpRequestSet.size() < sRequestHighWater;
...
void LLMeshRepoThread::loadMeshLOD(const LLVolumeParams& mesh_params, S32 lod)
{
	...
	mHeaderMutex.lock();
	bool exists = mMeshHeaders.count(mesh_params.getSculptID()) > 0;
	mHeaderMutex.unlock();
	...
	else
	{
		...
		pending_lod_map_t::iterator pending = mPendingLOD.find(mesh_params);
...
bool LLMeshRepoThread::fetchMeshDecomposition(const LLUUID& mesh_id)
{
	...
	if (valid && offset >= 0 && size > 0)
	{
		...
		if (!http_url.empty())
		{
			LLMeshHandlerBase::ptr_t handler(new LLMeshDecompositionHandler(mesh_id,
																			offset,
																			size));
			LLCore::HttpHandle handle =
				getByteRange(http_url, cap_version, offset, size, handler);
									   ...
									   bool can_retry)
{
	...
	if (!http_url.empty())
	{
		...
		LLMeshHandlerBase::ptr_t handler(new LLMeshHeaderHandler(mesh_params,
																 0,
																 MESH_HEADER_SIZE));
		LLCore::HttpHandle handle =
									  ...
									  U8* data, S32 data_size)
{
	...
	pending_lod_map_t::iterator iter = mPendingLOD.find(mesh_params);
										...
										S32 data_size)
{
	...
	mMutex.lock();
	mSkinInfos.emplace_back(skin, mesh_id);
	mMutex.unlock();
							   ...
							   S32 detail, S32 last_lod)
{
	...
	mesh_load_map_t::iterator iter = mLoadingMeshes[detail].find(mesh_params);
	if (iter != mLoadingMeshes[detail].end())
	{
		// Request pending for this mesh, append volume id to list
		LL_DEBUGS("MeshQueue") << "Adding object to pending requests for the associated mesh"
							   << LL_ENDL;
		iter->second.emplace(vobj->getID());
	}
	else
	{
		...
		LL_DEBUGS("MeshQueue") << "Initiating request for the associated mesh"
							   << LL_ENDL;
		mLoadingMeshes[detail][mesh_params].emplace(vobj->getID());
		...
		if (active_count < LLMeshRepoThread::sRequestLowWater)
		{
			...
			if ((S32)mPendingRequests.size() > push_count)
			{
				...
				for (U32 i = 0; i < 4; ++i)
				{
					...
					{
						F32 max_score = 0.f;
						for (uuid_list_t::iterator obj_iter = iter->second.begin(),
							 ...
							 obj_iter != end2; ++obj_iter)
						{
							...
							if (object)
							{
								...
								if (drawable)
								{
									F32 cur_score = drawable->getRadius() /
						...
						score_map.emplace(iter->first.getSculptID(),
										  max_score);
					}
				}
...
void LLMeshRepository::notifySkinInfoReceived(LLMeshSkinInfo& info)
{
	...
	for (uuid_list_t::const_iterator it = iter->second.begin(),
									 end = iter->second.end();
		 it != end; ++it)
	{
		LLVOVolume* vobj = (LLVOVolume*)gObjectList.findObject(*it);
		if (vobj)
		{
										...
										LLVolume* volume)
{
	...
	mesh_load_map_t::iterator obj_iter = mLoadingMeshes[lod].find(mesh_params);
	...
	{
		llwarns << "Mesh loading returned empty volume for mesh "
				<< mesh_params.getSculptID() << llendl;
	}
	...
	{
		llwarns << "Could not find system volume for mesh "
				<< mesh_params.getSculptID() << llendl;
	}

	// Notify waiting LLVOVolume instances that their requested mesh is
	// available
	for (uuid_list_t::iterator vobj_iter = obj_iter->second.begin(),
							   end = obj_iter->second.end();
		 vobj_iter != end; ++vobj_iter)
	{
													...
													const LLVOVolume* req_obj)
{
	...
	mLoadingSkins[mesh_id].emplace(req_obj->getID());
	mMeshMutex.unlock();
