...
void LLDrawPoolTree::render(S32 pass)
{
		 ...
		 iter != end; ++iter)
	{
		...
		buffp->setBuffer(LLDrawPoolTree::VERTEX_DATA_MASK);
		buffp->drawRange(LLRender::TRIANGLES, 0, buffp->getNumVerts() - 1,
						 buffp->getNumIndices(), 0);
		gPipeline.addTrianglesDrawn(buffp->getNumIndices());
	}
}
...
{
	LLGLState normalize(GL_NORMALIZE, GL_TRUE);
		 ...
		 iter != end; ++iter)
	{
		...
		for (U32 lod = 0; lod < MAX_NUM_TREE_LOD_LEVELS; ++lod)
		{
			...
			{
				treep->drawBranchPipeline(scale_mat, indicesp, lod, 0,
