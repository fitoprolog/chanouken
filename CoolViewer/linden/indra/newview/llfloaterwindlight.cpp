...
void LLPanelWLDayCycle::refresh()
{
	...
	// Preview
	bool no_preview = gUseNewShaders;
	mPlayBtn->setEnabled(!no_preview);
	mStopBtn->setEnabled(!no_preview);
	mNoLivePreviewText->setVisible(no_preview);
	if (no_preview)
	{
		onStopAnimSky(this);
	}
...
void LLFloaterWindlight::refresh()
{
	...
	bool enable = gPipeline.canUseVertexShaders();
	...
	// Show the "Preview frame" button when the EE render is active
	mPreviewBtn->setVisible(gUseNewShaders);
...
//mk
	...
	static bool old_use_new_shaders = false;
	if (old_use_new_shaders != gUseNewShaders)
	{
		old_use_new_shaders = gUseNewShaders;
		// Force a refresh of floater and panels for buttons status
		refresh();
	}
...
void LLFloaterWindlight::setDayTime(F32 time)
{
	...
	// If in EE rendering mode, the WL animator does not refresh our floater,
	// so we must do it here in order to get the time of day sliders synced...
	if (gUseNewShaders)
	{
...
void LLFloaterWindlight::onPreviewAsEE(void*)
{
	...
	// equivalent
	const LLWLParamSet& sky_set = gWLSkyParamMgr.mCurParams;
	LLSettingsSky::ptr_t skyp =
		LLEnvSettingsSky::buildFromLegacyPreset(sky_set.mName,
												sky_set.getAll(), msg);

	// Translate current Windlight water settings into their EE equivalent
	const LLWaterParamSet& water_set = gWLWaterParamMgr.mCurParams;
	LLSettingsWater::ptr_t waterp =
		LLEnvSettingsWater::buildFromLegacyPreset(water_set.mName,
												  water_set.getAll(), msg);
