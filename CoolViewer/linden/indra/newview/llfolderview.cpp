...
constexpr S32 LEFT_INDENTATION = 13;
...
// bug due to a race condition (see in doIdle()).
		...
		if (LLToolDragAndDrop::getInstance()->isOverThreshold(screen_x, screen_y))
		{
			...
			if (root->getCurSelectedItem())
			{
				...
				if (can_drag)
				{
					...
					// Release keyboard focus, so that if stuff is dropped into
					// the world, pressing the delete key won't blow away the
					// inventory item.
					gFocusMgr.setKeyboardFocus(NULL);

					return LLToolDragAndDrop::getInstance()->handleHover(x, y,
																		 mask);
				}
			}
		}
...
void LLFolderViewItem::draw()
{
	...
	if (!mLabel.empty())
	{
		...
		{
			sFont->render(sLoadingStr, 0, text_left, y, sSearchStatusColor,
						  LLFontGL::LEFT, LLFontGL::BOTTOM, mLabelStyle,
						  S32_MAX, S32_MAX, &right_x, false);
			text_left = right_x;
		}

		sFont->render(mWLabel, 0, text_left, y, color, LLFontGL::LEFT,
					  LLFontGL::BOTTOM, mLabelStyle, S32_MAX, S32_MAX,
					  &right_x, false);
		if (!mLabelSuffix.empty())
		{
			sFont->render(mWLabelSuffix, 0, right_x, y, sSuffixColor,
						  LLFontGL::LEFT, LLFontGL::BOTTOM, mLabelStyle,
						  S32_MAX, S32_MAX, &right_x, false);
		}
		...
		if (mStringMatchOffset != std::string::npos)
		{
				...
				combined_string_upper.find(mRoot->getFilterSubString()) == mStringMatchOffset)
			{
				S32 left = ll_round(text_left) +
						   sFont->getWidth(combined_string, 0,
										   mStringMatchOffset) - 1;
				S32 right = left +
							sFont->getWidth(combined_string, mStringMatchOffset,
				...
				F32 match_string_left = text_left +
										sFont->getWidthF32(combined_string, 0,
														   mStringMatchOffset);
				F32 y = (F32)getRect().getHeight() - sFontLineHeight -
						(F32)TEXT_PAD;
				sFont->renderUTF8(combined_string, mStringMatchOffset,
...
LLFolderViewFolder::~LLFolderViewFolder()
{
	...
	// we don't have mouse or keyboard focus
...
S32 LLFolderViewFolder::arrange(S32* width, S32* height, S32 filter_generation)
{
	...
	*height = getItemHeight();
	...
	{
		mCurHeight = lerp(mCurHeight, mTargetHeight,
...
void LLFolderViewFolder::setCompletedFilterGeneration(S32 generation,
													  bool recurse_up)
{
	mMostFilteredDescendantGeneration = llmin(mMostFilteredDescendantGeneration,
											  generation);
	mCompletedFilterGeneration = generation;
	// only aggregate up if we are a lower (older) value
	if (recurse_up &&
		mParentFolder && generation < mParentFolder->getCompletedFilterGeneration())
	{
...
void LLFolderViewFolder::filter(LLInventoryFilter& filter)
{
	...
	{
		// don't traverse children if we've already filtered them since
		 ...
		 iter != mFolders.end(); )
	{
		...
		// case we need to update it even for folders that don't need to be
		// filtered anymore
	...
	// if we didn't use all filter iterations that means we filtered all of our
	// descendants instead of exhausting the filter count for this frame
	if (filter.getFilterCount() > 0)
	{
		// flag this folder as having completed filter pass for all descendants
		setCompletedFilterGeneration(filter_generation,
									 false); // don't recurse up to root
	}
}
...
// extractItem() removes the specified item from the folder, but doesn't delete
// it.
void LLFolderViewFolder::extractItem(LLFolderViewItem* item)
{
	...
	if (it == mItems.end())
	{
		// This is an evil downcast. However, it's only doing pointer
		// comparison to find if (which it should be) the item is in the
		// container, so it's pretty safe.
...
void LLFolderViewFolder::toggleOpen()
{
	...
	{
		LLFolderViewEventListener* listener = getListener();
		if (listener)
		{
			const LLUUID& id = listener->getUUID();
			if (id.notNull())
			{
				sLastOpenId = id;
			}
														   ...
														   bool include_children)
{
	...
	if (!found_item)
	{
		// you should never call this method with an item that isn't a child so
	...
	mDragAndDropThisFrame(false)
{
	...
	LLMenuGL* menu =
		LLUICtrlFactory::getInstance()->buildMenu("menu_inventory.xml",
												  parent_panel);
	if (!menu)
	{
...
void LLFolderView::sanitizeSelection()
{
		 ...
		 ++item_iter)
	{
		...
		// Deselect item if any ancestor is closed or didn't pass filter
		// requirements.
		if (!visible)
		{
...
void LLFolderView::draw()
{
	...
	{
		EAcceptance last_accept =
			LLToolDragAndDrop::getInstance()->getLastAccept();
...
void LLFolderView::rememberMarketplaceFolders()
{
	...
	if (market_id.isNull())
	{
		// No Martketplace Listings folder: don't bother...
		return;
	}
...
bool LLFolderView::handleUnicodeCharHere(llwchar uni_char)
{
	...
	if (uni_char > 0x7f)
	{
		llwarns << "Can't handle non-ascii yet, aborting" << llendl;
		return false;
	}
...
bool LLFolderView::handleRightMouseDown(S32 x, S32 y, MASK mask)
{
	...
	if (handled && count > 0 && menu)
	{
		...
		for (selected_items_t::iterator it = mSelectedItems.begin(),
			 ...
			 it != end; ++it)
		{
			(*it)->buildContextMenu(*menu, flags);
			flags = 0x0;
		}
...
void LLFolderView::doIdle()
{
	...
	if (mNeedsAutoSelect)
	{
		...
		if (selected_itemp && sFolderViewItems.count(selected_itemp) == 0)
		{
			...
			// destructor... This code avoids the crash bug.
											  ...
											  std::string& tooltip_msg)
{
	...
	else
	{
		...
		{
			// If the last notification has expired, clear its data so to
			// re-display it again now if needed.
			LLNotificationPtr n = gNotifications.find(sLastDragTipID);
...
bool LLInventoryFilter::check(LLFolderViewItem* item)
{
	...
	if (hide_cof && !LLFolderType::getCanDeleteCOF())
	{
		...
		if (cof_id.notNull() &&
			(item_id == cof_id ||
			 gInventory.isObjectDescendentOf(item_id, cof_id)))
		{
			return false;
		}
	}
...
LLView* LLInventoryPanel::fromXML(LLXMLNodePtr node, LLView* parent,
								  LLUICtrlFactory* factory)
{
	...
	LLInventoryPanel* panel = new LLInventoryPanel(name, sort_order, rect,
												   &gInventory,
												   allow_multi_select,
...
{
	// Select the desired item (in case it wasn't loaded when the selection was
	// requested)
...
void LLInventoryPanel::buildNewViews(const LLUUID& id)
{
		...
		(objectp && objectp->getType() == LLAssetType::AT_CATEGORY))
	{
		...
		if (categories)
		{
			...
			{
				LLInventoryCategory* cat = categories->at(i);
				if (cat)	// Paranoia
				{
...
{
	ECursorType cursor = gWindowp->getCursor();
	if (LLView::handleHover(x, y, mask))
	{
		if (cursor == UI_CURSOR_ARROW &&
			LLInventoryModelFetch::getInstance()->backgroundFetchActive())
		{
			// Replace arrow cursor with arrow and hourglass cursor
			gWindowp->setCursor(UI_CURSOR_WORKING);
		}
	}
	else if (cursor != UI_CURSOR_ARROW)
	{
...
bool LLInventoryPanel::makeLastOpenCurrent()
{
	...
	mLastOpenID = LLFolderViewFolder::sLastOpenId;
	LLOpenFolderByID opener(mLastOpenID);
