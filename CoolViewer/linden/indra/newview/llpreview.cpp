	...
	mDirty(true)
{
	...
	// Do not necessarily steal focus on creation -- sometimes these guys pop
...
LLPreview::LLPreview(const std::string& name, const LLRect& rect,
					 ...
					 const LLUUID& object_uuid, bool allow_resize,
...
LLPreview::~LLPreview()
{
	...
	{
		for (preview_multimap_t::iterator
				found_it = sPreviewsBySource.find(mSourceID),
				end_it = sPreviewsBySource.end();
			 found_it != end_it; ++found_it)
		{
...
void LLPreview::setSourceID(const LLUUID& source_id)
{
	...
	mSourceID = source_id;
	sPreviewsBySource.insert(preview_multimap_t::value_type(mSourceID,
															getHandle()));
}
...
bool LLPreview::canModify(const LLUUID& taskUUID, const LLInventoryItem* item)
{
	...
	return item && gAgent.allowOperation(PERM_MODIFY, item->getPermissions(),
...
bool LLPreview::handleHover(S32 x, S32 y, MASK mask)
{
	if (hasMouseCapture())
	{
		...
		if (item && item->getPermissions().allowCopyBy(gAgentID,
													   gAgent.getGroupID()) &&
			LLToolDragAndDrop::getInstance()->isOverThreshold(screen_x,
															  screen_y))
		{
