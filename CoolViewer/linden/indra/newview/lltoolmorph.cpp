...
bool LLVisualParamHint::render()
{
	...
	LLVector3 target_joint_pos = mCamTargetJoint ? mCamTargetJoint->getWorldPosition()
												 : LLVector3(0, 0, 0);
	...
	LLVector3 camera_snapshot_offset(mVisualParam->getCameraDistance() * cosf(cam_angle_radians),
	...
	if (gAgentAvatarp->mDrawable.notNull())
	{
		...
		if (facep)	// Paranoia
		{
			...
			if (avatarPoolp)	// More paranoia !
			{
				...
				avatarPoolp->renderAvatars(gAgentAvatarp);
