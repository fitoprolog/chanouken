										 ...
										 U32 ip, U32 port)
{
	...
	LL_DEBUGS("ObjectCache") << "Local Id " << local_id
							 << " associated with UUID " << id << LL_ENDL;
	

	//llinfos << "Adding object to table, full ID " << id << ", local ID "
	//		<< local_id << ", ip " << ip << ":" << port << llendl;
}
											 ...
											 bool compressed)
{
	...
	for (S32 i = 0; i < num_objects; ++i)
	{
		...
		{
			update_cache = true;
			msg->getUUIDFast(_PREHASH_ObjectData, _PREHASH_FullID, fullid,
								 i);
			msg->getU32Fast(_PREHASH_ObjectData, _PREHASH_ID, local_id, i);
...
void LLViewerObjectList::fetchObjectCosts()
{
	...
	LLCoros::getInstance()->launch("LLViewerObjectList::fetchObjectCostsCoro",
								   boost::bind(&LLViewerObjectList::fetchObjectCostsCoro,
											   this, url));
}
...
void LLViewerObjectList::fetchPhysicsFlags()
{
	...
	LLCoros::getInstance()->launch("LLViewerObjectList::fetchPhysicsFlagsCoro",
								   boost::bind(&LLViewerObjectList::fetchPhysicsFlagsCoro,
											   this, url));
}
...
void LLViewerObjectList::cleanupReferences(LLViewerObject* objectp)
{
	...
	// Do not clean up mObject references, these will be cleaned up more
	// efficiently later !
...
// Makes sure the region is cleaned up.
void LLViewerObjectList::clearAllMapObjectsInRegion(LLViewerRegion* regionp)
{
	S32 count = mMapObjects.size();
	if (!count) return;

	std::vector<LLViewerObject*> dead_object_list;
	dead_object_list.reserve(count);	// likely too large but avoids reallocations
	std::vector<LLViewerObject*> region_object_list;
	region_object_list.reserve(count);	// likely too large but avoids reallocations
	for (S32 i = 0; i < count; )
	{
		...
		if (objectp)
		{
			...
			if (objectp->isDead())
			{
				dead_object_list.push_back(objectp);
			}
			else if (objectp->getRegion() == regionp)
			{
				region_object_list.push_back(objectp);
			}
		}
		else
		{
			// Paranoia: this really should never happen, but Murphy's laws say
			// otherwise...
			llwarns << "Found a NULL object in map objects !  Removing."
					<< llendl;
			if (i != --count)
			{
	...
	count = dead_object_list.size();
	if (count)
	{
		llwarns << "There are " << count
				<< " dead objects on the map !  Removing." << llendl;

		for (S32 i = 0; i < count; ++i)
		{
			cleanupReferences(dead_object_list[i]);
		}
	}
	count = region_object_list.size();
	if (count)
	{
		llwarns << "There are " << count
				<< " objects not removed from the deleted region !  Removing."
				<< llendl;

		for (S32 i = 0; i < count; ++i)
		{
			region_object_list[i]->markDead();
		}
	}
}
...
{
	LL_TRACY_TIMER(TRC_OBJ_PICK_LIST);
	...
	for (S32 i = 0, count = LLCharacter::sInstances.size(); i < count; ++i)
	{
		...
		if (avatarp && !avatarp->isDead())
		{
			if (avatarp->mDrawable.notNull() &&
				avatarp->mDrawable->isVisible())
			{
				mSelectPickList.insert(avatarp);
			}
		}
	}
	...
	S32 num_pickables = (S32)mSelectPickList.size() +
						LLHUDIcon::getNumInstances();
	if (num_pickables != 0)
	{
		S32 step = (0x000fffff - GL_NAME_INDEX_OFFSET) / num_pickables;

		S32 i = 0;
		for (std::set<LLViewerObject*>::iterator
				pick_it = mSelectPickList.begin(), end = mSelectPickList.end();
			 pick_it != end; )
		{
			LLViewerObject* objp = *pick_it;
			if (!objp || objp->isDead() || !objp->mCanSelect)
			{
...
LLViewerObject* LLViewerObjectList::getSelectedObject(U32 object_id)
{
	for (std::set<LLViewerObject*>::iterator
			pick_it = mSelectPickList.begin(), end = mSelectPickList.end();
		 pick_it != end; ++pick_it)
														  ...
														  U32 local_id)
{
	...
	LL_DEBUGS("ObjectCache") << "Created object " << uuid
							 << " from cache." << LL_ENDL;
