...
void LLPanelLandAccess::refresh()
{
	...
	LLCtrlSelectionInterface* passcombo = mPassCombo->getSelectionInterface();
	if (passcombo && (public_access || !use_pass))
	{
...
void LLPanelLandAccess::onCommitGroupCheck(LLUICtrl* ctrl, void* userdata)
{
	...
	LLCtrlSelectionInterface* passcombo =
		self->mPassCombo->getSelectionInterface();
	if (passcombo &&  use_access_group && use_pass_list &&
		passcombo->getSelectedValue().asString() == "group")
	{
		passcombo->selectByValue("anyone");
	}
...
void LLPanelLandAccess::onCommitAny(LLUICtrl* ctrl, void* userdata)
{
	...
	else
	{
		...
		LLCtrlSelectionInterface* passcombo;
		passcombo = self->mPassCombo->getSelectionInterface();
		if (passcombo && use_access_group && use_pass_list &&
			passcombo->getSelectedValue().asString() == "group")
		{
			use_access_group = false;
