										...
										LLMeshSkinInfo* skin)
{
	...
	// you would get a crash in avatar->getJoint(skin->mJointKeys[j])
...
#		pragma omp parallel for private(bind, world)
		// NOTE: VS2017 OpenMP requires a signed integer loop index...
									   ...
									   LLVolumeFace& vol_face)
{
	...
	static LLMatrix4a bind_shape;
	bind_shape.loadu(skin->mBindShapeMatrix);

#if !LL_OPENMP
	static LLMatrix4a inv_bind, mat;
	...
	S32 idx[4];
	assert_main_thread();
#	pragma omp parallel for private(inv_bind, mat, pos_joint_space, wght, idx)
#endif
