...
public:
	...
	// Return true if the viewer has information on all selected objects
	bool selectGetAllRootsValid();
	bool selectGetAllValid();

	// Return true if you can modify all selected objects.
	bool selectGetRootsModify();
	bool selectGetModify();

	// Return true if is all objects are non-permanent-enforced
	bool selectGetRootsNonPermanentEnforced();
	bool selectGetNonPermanentEnforced();

	// Return true if is all objects are permanent
	...
	// Return true if is all objects are not pathfinding
	bool selectGetRootsNonPathfinding();
	bool selectGetNonPathfinding();

	// Return true if is all objects are not permanent
	bool selectGetRootsNonPermanent();
	bool selectGetNonPermanent();

	// Return true if is all objects are not character
	...
	LL_INLINE LLVector3d getSelectionCenterGlobal() const
	{
...
private:
	...
	// Get the first ID that matches test and whether or not all ids are
...
public:
	...
	// do we show selection silhouettes that are occluded ?
...
private:
	...
	// diff, norm or spec, depending on editing mode:
