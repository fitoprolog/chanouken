...
void LLWorld::initClass()
{
	...
	LLViewerRegion::sVOCacheCullingEnabled =
		...
		gSavedSettings.getBool("ObjectCacheEnabled");
	...
	if (regionp == gAgent.getRegion())
	{
			 ...
			 iter != mRegionList.end(); ++iter)
		{
			...
			llwarns << "RegionDump: " << reg->getName() << " "
					<< reg->getHost() << " " << reg->getOriginGlobal()
					<< llendl;
		}

		llwarns << "Agent positions: global = " << gAgent.getPositionGlobal()
				<< " / agent = " << gAgent.getPositionAgent()
				<< " - Regions visited: " << gAgent.getRegionsVisited()
				<< " - gFrameTimeSeconds = " << gFrameTimeSeconds
				<< " - Disabling agent region: " << regionp->getName()
				<< llendl;
...
void LLWorld::updateNetStats()
{
	...
	viewerstats->mActualInKBitStat.addValue(actual_in_bits * 0.001f);
	viewerstats->mActualOutKBitStat.addValue(actual_out_bits * 0.001f);
	viewerstats->mKBitStat.addValue((F32)(bits * 0.001));
	viewerstats->mPacketsInStat.addValue(packets_in);
	viewerstats->mPacketsOutStat.addValue(packets_out);
	viewerstats->mPacketsLostStat.addValue(msg->mDroppedPackets);
	F32 packets_pct = packets_in ? (F32)(100 * packets_lost) / (F32)packets_in
								 : 0.f;
	viewerstats->mPacketsLostPercentStat.addValue(packets_pct);
...
void LLWorld::updateWaterObjects()
{
	...
	S32 add_boundary[4] = { 512 - (S32)(max_x - region_x),
...
void LLWorld::precullWaterObjects(LLCamera& camera, LLCullResult* cull)
{
...
#if 0	// Not used
void LLWorld::setSpaceTimeUSec(U64 space_time_usec)
{
...
void LLWorld::processEnableSimulator(LLMessageSystem* msg, void**)
{
	...
	if (ip_u32 != last_ip_u32 || circuit_code != last_circuit_code)
	{
		...
		llinfos << "Enabling simulator " << sim << " with code "
				<< circuit_code << llendl;
	}
...
{
	// *NOTE: pausing the mainloop timeout. Otherwise a long modal event may
	// cause the thread monitor to timeout. - Mani
