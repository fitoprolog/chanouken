...
void HBPanelWebSearch::search(const std::string& search_text)
{
	...
	if (!search_text.empty() || url.find("[QUERY]" != std::string::npos))
	{
		...
		else if (url.find("[QUERY]") == std::string::npos)
		{
			...
			llwarns_once << "Missing query field in the search URL... Using SL's OLD search engine query format..."
						 << llendl;
	...
	if (!gIsInSecondLife && url.find("[CATEGORY]") != std::string::npos)
	{
		...
		llwarns_once << "There is a category field in the search URL, but valid categories for this grid are unknown: using SL's old search engine global category..."
					 << llendl;
	}
	if (url.find("[MATURITY]") != std::string::npos)
	{
		...
		if (!gIsInSecondLife)
		{
			llwarns_once << "There is a maturity field in the search URL, but its encoding for this grid is unknown: using SL's old search engine encoding conventions..."
						 << llendl;
		}
	}
