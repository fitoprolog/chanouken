...
LLVOAvatarPuppet::LLVOAvatarPuppet(const LLUUID& id, LLPCode pcode,
...
void LLVOAvatarPuppet::markDead()
{
	...
	// destruction. In that case the control avatar may be killed first.
	if (mRootVolp)
	{
...
//virtual
void LLVOAvatarPuppet::idleUpdate(const F64& time)
{
	if (mMarkedForDeath)
	{
		if (!isDead())
