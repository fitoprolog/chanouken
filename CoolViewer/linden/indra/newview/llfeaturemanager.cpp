...
bool LLFeatureManager::loadFeatureTables()
{
	...
	mSkippedFeatures.emplace("RenderVBOEnable");
	mSkippedFeatures.emplace("RenderFogRatio");
...
public:
	...
	{
		LLCoordScreen size;
		if (gWindowp && gWindowp->getSize(&size))
		{
			// This will cause a proper screen refresh (that I sadly could not
			// achieve otherwise), probably by triggering a full redraw event
			// at the SDL level (redrawing at the viewer level is not enough;
			// I also tried swapping SDL GL buffers before and after benchmark,
			// but to no avail). HB
...
F32 LLFeatureManager::benchmarkGPU()
{
	...
	for (U32 i = 0; i < count; ++i)
	{
		...
		if (!dest[i].allocate(res, res, GL_RGBA, false, false,
							  LLTexUnit::TT_TEXTURE, true))
		{
	...
	LLPointer<LLVertexBuffer> buff =
		new LLVertexBuffer(LLVertexBuffer::MAP_VERTEX, GL_STREAM_DRAW_ARB);
...
void LLFeatureManager::loadGPUClass(bool benchmark_gpu)
{
	...
	if (benchmark_gpu)
	{
		...
		cpu_bias *= LLCPUInfo::benchmarkFactor();
	...
	if (!file.is_open())
	{
		...
		gSavedSettings.setBool("BenchmarkGPU", true);
		loadGPUClass(true);	// Retry with benchmarking
		return;
	}

	std::string raw_renderer = gGLManager.getRawGLString();
...
void LLFeatureManager::applyRecommendedSettings()
{
	...
	// because feature masks only work by downgrading values, so I needed to
	// make a true value go to false for certain cards, thus the awkward name,
	// "Disregard..."
...
void LLFeatureManager::setGraphicsLevel(S32 level, bool skip_features)
{
	...
	switch (level)
	{
		case 0:
			...
			{
				// Only use fixed function by default if GL version < 3.0 or
				// this is an intel graphics chip
...
void LLFeatureManager::applyBaseMasks()
{
	...
	if (LLMemoryInfo::getPhysicalMemoryKB() <= 256 * 1024)
	{
		maskFeatures("RAM256MB");
	}

	if (LLCPUInfo::getInstance()->getMHz() < 1100)
	{
