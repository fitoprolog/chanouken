...
bool LLPathfindingManager::isPathfindingEnabledForRegion(LLViewerRegion* region) const
{
	...
	std::string url = getRetrieveNavMeshURLForRegion(region);
	return !url.empty();
}
													  ...
													  bool get_status_only)
{
	...
	coros->launch("LLPathfindingManager::navMeshStatusRequestCoro",
											  ...
											  object_request_cb_t cb) const
{
	...
	std::string object_url = getObjectLinksetsURLForCurrentRegion();
	...
	LinksetsResponder::ptr_t responder(new LinksetsResponder(req_id, cb, true,
															 with_terrain));
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLPathfindingManager::linksetObjectsCoro",
				  boost::bind(&LLPathfindingManager::linksetObjectsCoro, this,
							  object_url, responder, LLSD()));
	if (with_terrain)
	{
		coros->launch("LLPathfindingManager::linksetTerrainCoro",
												...
												object_request_cb_t cb) const
{
	...
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLPathfindingManager::charactersCoro",
...
void LLPathfindingManager::requestGetAgentState()
{
	...
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLPathfindingManager::navAgentStateRequestCoro",
...
void LLPathfindingManager::requestRebakeNavMesh(rebake_navmesh_cb_t cb)
{
	...
	LLCoros* coros = LLCoros::getInstance();
	coros->launch("LLPathfindingManager::navMeshRebakeCoro",
...
const std::string LLPathfindingManager::getObjectLinksetsURLForCurrentRegion() const
{
	const std::string& url = gAgent.getRegionCapability("RegionObjects");
	if (!url.empty())
	{
		return url;
	}
	return getRegionCapability(gAgent.getRegion(), "ObjectNavMeshProperties");
}
