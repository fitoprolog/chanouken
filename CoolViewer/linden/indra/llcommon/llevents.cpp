...
:	mName(LLEventPumps::getInstance()->registerNew(*this, name, tweak)),
...
LLEventPump::~LLEventPump()
{
	...
	{
		LLEventPumps::getInstance()->unregister(*this);
	}
}
										 ...
										 const NameList& before)
{
	...
	if (named)
	{
		...
		ConnectionMap::const_iterator it = mConnections.find(name);
		...
		// fix it later.
		DependencyMap::node_type& new_node = mDeps.add(name, -1.f, after,
													   before);
			 ...
			 dmi != sorted_range.end(); ++dmi)
		{
			...
			if (dmi->second < previous)
			{
				...
				for (DependencyMap::sorted_iterator cdmi = sorted_range.begin(),
				...
				out << "\nnow: ";
				DependencyMap::sorted_iterator ddmi(sorted_range.begin()),
											   ddmend(sorted_range.end());
				if (ddmi != ddmend)
				{
					out << ddmi->first;
					while (++ddmi != ddmend)
...
{
	ConnectionMap::iterator it = mConnections.find(name);
	if (it != mConnections.end())
	{
...
:	mListener(boost::bind(&LLEventPump::post,
						  boost::ref(LLEventPumps::getInstance()->obtain(pumpname)),
						  _1))
{
}
...
{
	// If the original request has no value for replyKey, it is pointless to
	...
	// replyKey, it can so specify when registering the operation method.
	...
	// Send reply on LLEventPump named in request[replyKey]. Do not forget to
	// send the modified 'newreply' instead of the original 'reply'.
	return LLEventPumps::getInstance()->obtain(request[replyKey]).post(newreply);
}
#endif
