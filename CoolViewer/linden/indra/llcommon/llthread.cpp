...
static thread_local U32 tThreadID = 0;
...
{
	static U32 main_thread_id = LLThread::currentID();
	return main_thread_id;
}

LL_COMMON_API bool is_main_thread()
{
	return get_main_thread_id() == tThreadID;
}
...
LL_COMMON_API void assert_main_thread()
{
	if (!is_main_thread())
	{
		llwarns << "Illegal execution from thread id " << tThreadID
				<< " outside main thread " << get_main_thread_id()
				<< llendl;
	}
}
...
void LLThread::threadRun()
{
	...
	// When the main thread affinity is 0 (or, for now, under Darwin and
	// Windows) this call is a no-operation and no affinity is set for any
	// threads. HB
...
# endif
#else
			// Simply detach the thread so no wait will happen.
			mThreadp->detach();
			return;
#endif
		}
