...
public:
	...
	// constructor whenever possible, by explicitely marking it noexcept.
	...
	LL_INLINE LLUUID& operator=(const LLUUID& rhs) noexcept
	{
		// No need to check the case where this == &rhs. The branch is slower
		// than the write.
#if LLUUID_OPTIMIZE_64_BITS
		U64* tmp = (U64*)mData;
		U64* rhstmp = (U64*)rhs.mData;
	...
	LL_INLINE LLUUID& operator=(LLUUID&& rhs) noexcept
	{
		// No need to check the case where this == &rhs. The branch is slower
		// than the write.
#if LLUUID_OPTIMIZE_64_BITS
	...
	// result and one of the inputs can't be used to deduce the other input.
	LLUUID combine(const LLUUID& other) const;
	void combine(const LLUUID& other, LLUUID& result) const;

	friend LL_COMMON_API std::ostream& operator<<(std::ostream& s,
												  const LLUUID& uuid);

	friend LL_COMMON_API std::istream& operator>>(std::istream& s,
												  LLUUID& uuid);
...
// container type, which is so widely used in the viewer code...
typedef safe_hset<LLUUID> uuid_list_t;

// Helper structure for ordering lluuids in stl containers.
// eg: boost::unordered_map<LLUUID, LLWidget*, lluuid_less> widget_map;
struct lluuid_less
{
	LL_INLINE bool operator()(const LLUUID& lhs,
							  const LLUUID& rhs) const noexcept
	{
		return lhs < rhs;
	}
};
...
// which bucket the actual key will be stored), thus why we only care for the
// first sizeof(size_t) bytes of the UUID and return them as a "hash".
