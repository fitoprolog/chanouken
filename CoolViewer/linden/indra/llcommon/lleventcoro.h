...
class LL_COMMON_API LLEventPumpOrPumpName
{
public:
	...
	:	mPump(LLEventPumps::getInstance()->obtain(pumpname))
	{
	}
...
void LL_COMMON_API suspend();

// Yields control from a coroutine for at least the specified number of seconds
void LL_COMMON_API suspendUntilTimeout(F32 seconds);
...
LLSD LL_COMMON_API postAndSuspend(const LLSD& event,
...
LL_INLINE LLSD LL_COMMON_API suspendUntilEventOn(const LLEventPumpOrPumpName& pump)
{
...
LL_INLINE LLSD LL_COMMON_API postAndSuspendTemp(const LLSD& event,
