...
LLEventFilter::LLEventFilter(LLEventPump& source, const std::string& name,
							 bool tweak)
...
void LLEventTimeoutBase::actionAfter(F32 seconds, const Action& action)
{
	...
	mMainloop = mainloop.listen(getName(),
								boost::bind(&LLEventTimeoutBase::tick, this,
											_1));
}
...
{
public:
	ErrorAfter(const std::string& message)
	:	mMessage(message)
	{
	}
