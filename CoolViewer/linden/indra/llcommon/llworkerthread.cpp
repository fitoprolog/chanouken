...
LLWorkerClass::~LLWorkerClass()
{
	...
	if (mWorkerThread && mRequestHandle != LLWorkerThread::nullHandle())
	{
...
void LLWorkerClass::setWorkerThread(LLWorkerThread* workerthread)
{
	...
	{
		llerrs  << mWorkerClassName
			   << ": attempt to change WorkerThread with active worker !"
			   << llendl;
	}
	else
	{
...
bool LLWorkerClass::yield()
{
	...
	{
		llwarns << mWorkerClassName << " got a NULL mWorkerThread !" << llendl;
		llassert(false);
	}
	bool res;
	mMutex.lock();
...
void LLWorkerClass::addWork(S32 param, U32 priority)
{
	...
	if (mRequestHandle != LLWorkerThread::nullHandle())
	{
		llerrs << "LLWorkerClass " << mWorkerClassName
...
void LLWorkerClass::abortWork(bool autocomplete)
{
	...
	if (mRequestHandle != LLWorkerThread::nullHandle())
	{
		...
		{
			llwarns << mWorkerClassName << " got a NULL mWorkerThread !"
					<< llendl;
			llassert(false);
		}
