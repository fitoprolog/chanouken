...
public:
	...
	LL_INLINE void reset()						{ mLastClockCount = getCurrentClockCount(); mExpirationTicks = 0; }
	...
	// Returns elapsed time in seconds with reset
	...
	// Returns true when the system timer is known to be bad (only seen on
	// super old PCI hardware, and only implemented for Windows). Used only
	// in LLViewerStats. *TODO: deprecate.
...
LL_COMMON_API time_t computer_time();
...
LL_COMMON_API struct tm* utc_time_to_tm(time_t utc_time);
...
LL_COMMON_API struct tm* local_time_to_tm(time_t local_time);
...
LL_COMMON_API struct tm* utc_to_pacific_time(time_t utc_time,
											 bool pacific_daylight_time);

LL_COMMON_API void microsecondsToTimecodeString(U64 current_time,
												std::string& tcstring);
LL_COMMON_API void secondsToTimecodeString(F32 current_time,
										   std::string& tcstring);
LL_COMMON_API void timeToFormattedString(time_t time, const char* format,
										 std::string& timestr);
LL_COMMON_API void timeStructToFormattedString(struct tm* time,
