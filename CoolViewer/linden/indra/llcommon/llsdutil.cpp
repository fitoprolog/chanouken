...
// appended.
LL_INLINE static std::string colon(const std::string& pfx)
{
	return pfx.empty() ? pfx : pfx + ": ";
}
						 ...
						 const std::string& pfx)
{
	...
	if (prototype.isArray())
	{
		if (!data.isArray())
		{
			return STRINGIZE(colon(pfx) << "Array" << op
										<< sTypes.lookup(data.type()));
		}
		if (data.size() < prototype.size())
		{
			return STRINGIZE(colon(pfx) << "Array size " << prototype.size()
										<< op << "Array size " << data.size());
		}
	...
	if (prototype.isMap())
	{
		if (!data.isMap())
		{
			return STRINGIZE(colon(pfx) << "Map" << op
										<< sTypes.lookup(data.type()));
		}
