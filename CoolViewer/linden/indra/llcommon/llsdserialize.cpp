...
bool LLSDSerialize::deserialize(LLSD& sd, std::istream& str, S32 max_bytes)
{
	...
	if (!strncasecmp(LEGACY_NON_HEADER, hdr_buf, strlen(LEGACY_NON_HEADER)))
	{
...
// failure.
int deserialize_string_delim(std::istream& istr, std::string& value,
							 char delim)
{
...
// Returns number of bytes read off of the stream or PARSE_FAILURE (-1) on
// failure.
int deserialize_string_raw(std::istream& istr, std::string& value,
						   S32 max_bytes)
{
	int count = 0;
...
int deserialize_string(std::istream& istr, std::string& value, S32 max_bytes)
{
	...
	if (LLSDParser::PARSE_FAILURE == rv)
	{
		return rv;
	}
...
// Returns number of bytes read off of the stream or PARSE_FAILURE (-1) on
// failure.
int deserialize_boolean(std::istream& istr, LLSD& data,
								...
								S32 max_depth) const
{
	...
	case 'f':
		...
		if (isalpha(c))
		{
			int cnt = deserialize_boolean(istr, data, NOTATION_FALSE_SERIAL,
										  false);
	...
	case 't':
		...
		if (isalpha(c))
		{
			int cnt = deserialize_boolean(istr, data, NOTATION_TRUE_SERIAL,
										  true);
			if (PARSE_FAILURE == cnt)
			{
				parse_count = cnt;
...
bool LLSDNotationParser::parseBinary(std::istream& istr, LLSD& data) const
{
	...
	{
		// *FIX: A bit inefficient, but works for now. To make the format
		// better, I would need to add a hint into the serialization format
		// that indicated how long it was.
...
bool unzip_llsd(LLSD& data, const U8* in, S32 size)
{
	...
	{
		static const std::string deprecated_header = "<? LLSD/Binary ?>";
		static const size_t deprecated_header_len = deprecated_header.size();
		std::istringstream istr;
		// Since we are using this for meshes, data we are dealing with tend to
		// be large. So string can potentially fail to allocate, make sure this
		// would not cause problems.
		try
		{
			std::string res_str((char*)result, cur_size);
			if (res_str.substr(0, deprecated_header_len) == deprecated_header)
			{
				res_str = res_str.substr(deprecated_header_len + 1, cur_size);
			}
		...
		{
			LLMemory::allocationFailed(cur_size);
			if (result)
			{
				free(result);
			}
...
U8* unzip_llsdNavMesh(bool& valid, U32& outsize, const U8* in, S32 size)
{
	if (size <= 0)
	{
