...
LLCoros::CoroData& LLCoros::getCoroData()
{
	...
	if (!destroyed())
	{
		current = getInstance()->mCurrent.get();
	}
...
void LLCoros::printActiveCoroutines(bool at_destruction)
{
	...
	LLError::ELevel level = at_destruction ? LLError::LEVEL_WARN
										   : LLError::LEVEL_INFO;
	lllog(level, NULL, false) << "------ List of active coroutines ------\n";
	F64 now = LLTimer::getTotalSeconds();
	for (auto it = CoroData::beginInstances(), end = CoroData::endInstances();
		 it != end; ++it)
	{
		F64 life_time = now - it->mCreationTime;
		llcont << "Name: " << it->mName << " - Key: " << it->getKey()
