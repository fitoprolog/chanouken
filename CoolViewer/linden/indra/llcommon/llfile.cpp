						 ...
						 int strerror_ret)
{
	...
	{
		return STRINGIZE("strerror_r() cannot explain errno " << orig_errno
...
bool LLFile::lock(bool exclusive)
{
	...
	// the file, while all others can still read to it. We therefore can use
...
// (most of the time, we use stat to just check that a file exists).
...
:	std::ifstream(utf8str_to_utf16str(filename).c_str(), mode | ios_base::in)
{
}
