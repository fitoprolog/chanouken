...
// LLEventPump any number of times. We do not really want to have to ask the
// caller to label each such call with a distinct string; the whole point of
// suspendUntilEventOn() is to present a nice sequential interface to the
// underlying LLEventPump-with-named-listeners machinery. So we will use
// LLEventPump::inventName() to generate a distinct name for each temporary
// listener. On the other hand, because a given coroutine might call
// suspendUntilEventOn() any number of times, we do not really want to consume
// an arbitrary number of generated inventName()s: that namespace, though
// large, is nonetheless finite. So we memoize an invented name for each
// distinct coroutine instance.
std::string listenerNameForCoro()
{
	...
	// This is the first time we have been called for this coroutine instance
	name = LLEventPump::inventName("coro");
	llinfos << "Inventing coro name: " << name << llendl;
	return name;
}
...
{
	LLCoros::Promise<LLSD> promise;
	std::string listener_name = listenerNameForCoro();
