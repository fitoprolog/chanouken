...
//
// See llrefcount.h for LLThreadSafeRefCount
//----------------------------------------------------------------------------

// This is to avoid inlining a llwarns...
LL_COMMON_API LL_NO_INLINE void warnUnreferenceDidAssignment();
...
{
public:
	LL_INLINE LLConstPointer() noexcept
	:	mPointer(NULL)
	{
	}

	LL_INLINE LLConstPointer(const Type* ptr) noexcept
	:	mPointer(ptr)
	{
		ref();
	}

	LL_INLINE LLConstPointer(const LLConstPointer<Type>& ptr) noexcept
	:	mPointer(ptr.mPointer)
	{
		ref();
	}
	...
	// 3rd Ed.
	template<typename Subclass>
	LL_INLINE LLConstPointer(const LLConstPointer<Subclass>& ptr) noexcept
	:	mPointer(ptr.get())
	{
		ref();
	}

	LL_INLINE ~LLConstPointer()
	{
		unref();
	}

	LL_INLINE const Type* get() const							{ return mPointer; }
	LL_INLINE const Type* operator->() const					{ return mPointer; }
	LL_INLINE const Type& operator*() const						{ return *mPointer; }
	...
	LL_INLINE bool operator!=(const Type* ptr) const			{ return mPointer != ptr; }
	LL_INLINE bool operator==(const Type* ptr) const			{ return mPointer == ptr; }

	LL_INLINE bool operator==(const LLConstPointer<Type>& ptr) const
	{
	...
	LL_INLINE LLConstPointer<Type>& operator=(const Type* ptr)
	{
		if (mPointer != ptr)
		{
			unref();
	...
	LL_INLINE LLConstPointer<Type>& operator=(const LLConstPointer<Type>& ptr)
	{
		if (mPointer != ptr.mPointer)
		{
			unref();
	...
	// 3rd Ed.
	template<typename Subclass>
	LL_INLINE LLConstPointer<Type>& operator=(const LLConstPointer<Subclass>& ptr)
	{
		if (mPointer != ptr.get())
		{
			unref();
...
// std::hash implementation for LLPointer and LLConstPointer
namespace std
{
	...
	template <typename Subclass> struct hash<LLConstPointer<Subclass>>
	{
		LL_INLINE size_t operator()(const LLConstPointer<Subclass>& p) const noexcept
		{
			return (size_t)p.get();
		}
	};
...
template <typename Subclass>
LL_INLINE size_t hash_value(const LLConstPointer<Subclass>& p) noexcept
{
	return (size_t)p.get();
}
