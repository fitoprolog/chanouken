...
F32 LLTimer::getRemainingTimeF32() const
{
	...
	return F32((mExpirationTicks - cur_ticks) * sClockFrequencyInv);
}
...
{
#if LL_WINDOWS
	WCHAR bad_pci_list[][10] = { L"1039:0530", L"1039:0620", L"10B9:0533",
								 L"10B9:1533", L"1106:0596", L"1106:0686",
								 L"1166:004F", L"1166:0050", L"8086:7110",
								 L"\0"
	};

	HKEY hKey = NULL;
	LONG nResult = ::RegOpenKeyEx(HKEY_LOCAL_MACHINE,
								  L"SYSTEM\\CurrentControlSet\\Enum\\PCI", 0,
	...
	wcscpy(pci_id, L"0000:0000");

	while (nResult == ERROR_SUCCESS)
	{
		nResult = ::RegEnumKeyEx(hKey, key_num++, name, &name_len, NULL, NULL,
								 NULL, &scrap);

		if (nResult == ERROR_SUCCESS)
		{
			memcpy(&pci_id[0], &name[4], 4);
			memcpy(&pci_id[5], &name[13], 4);
			...
			for (S32 check = 0; bad_pci_list[check][0]; ++check)
			{
				if (!wcscmp(pci_id, bad_pci_list[check]))
				{
					...
					{
						warned = true;
						llwarns << "Unreliable PCI chipset found " << pci_id
								<< ", timer considered as bad." << llendl;
					}
					return true;
				}
			}
