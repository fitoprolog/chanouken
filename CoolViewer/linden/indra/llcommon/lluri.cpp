...
bool isDefault(const std::string& scheme, U16 port)
{
	if (scheme == "http")
	{
		return port == 80;
...
void findAuthorityParts(const std::string& authority, std::string& user,
						  ...
						  bool is_allowed_sorted)
{
	...
	{
		// If it is already sorted, or if the url is quite long, we want to
...
std::string LLURI::userName() const
{
	...
	findAuthorityParts(mEscapedAuthority, user_pass, host, port);
