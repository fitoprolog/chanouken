...
public:
	...
	LL_INLINE HASH_KEY_TYPE getHashKey() const
	{
		return mHashKey;
	}

	LL_INLINE LLSimpleHashEntry<HASH_KEY_TYPE>* getNextEntry() const
	{
		return mNextEntry;
	}

	LL_INLINE void setNextEntry(LLSimpleHashEntry<HASH_KEY_TYPE>* next)
	{
		mNextEntry = next;
	}
...
public:
	LLSimpleHash()
	{
		llassert(TABLE_SIZE);
		// %ust be a power of 2
		llassert((TABLE_SIZE ^ (TABLE_SIZE - 1)) ==
	...
	LL_INLINE virtual int getIndex(HASH_KEY_TYPE key)
	{
	...
	LL_INLINE bool erase(LLSimpleHashEntry<HASH_KEY_TYPE>* entry)
	{
	...
	// Debugging
	LL_INLINE LLSimpleHashEntry<HASH_KEY_TYPE>* get_element_at_index(S32 index) const
	{
