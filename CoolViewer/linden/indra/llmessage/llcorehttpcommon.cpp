...
{
	// Old boost (e.g. v1.59) would not find LLCORE when passed directly to
	// boost::make_shared<Details>()...
	type_enum_t type = LLCORE;
	mDetails = boost::make_shared<Details>(type, HE_SUCCESS);
}
...
{
	mDetails =
		boost::make_shared<Details>(http_status,
									http_status >= 200 &&
