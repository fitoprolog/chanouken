...
{
public:
	LLMessageThrottleEntry(const size_t hash, U64 entry_time)
	:	mHash(hash),
		mEntryTime(entry_time)
	{
	}

	LL_INLINE size_t getHash() const		{ return mHash; }
	LL_INLINE U64 getEntryTime() const		{ return mEntryTime; }
...
public:
	...
	bool addAgentAlert(const LLUUID& agent, const LLUUID& task,
