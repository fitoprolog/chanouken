...
constexpr S32 TRUST_TIME_WINDOW = 3;
								  ...
								  const LLSD& context, const LLSD& input) const
{
	...
	LL_DEBUGS("Messaging") << "Setting mLastSender "
						   << input["sender"].asString() << LL_ENDL;
	LLMessageSystem* msg = gMessageSystemp;
	msg->mLastSender = LLHost(input["sender"].asString());
	msg->mPacketsIn += 1;
	msg->mLLSDMessageReader->setMessage(namePtr, input["body"]);
	...
	if (msg->callHandler(namePtr, false, msg))
	{
...
#endif
{
	...
	{
		// This is one of the only places where we're required to get REAL
		// message system time.
...
//virtual
S32 LLMessageSystem::sendMessage(const LLHost& host, LLStoredMessagePtr msgp)
{
	return sendMessage(host, msgp->mName.c_str(), msgp->mMessage);
}
...
void LLMessageSystem::forwardReliable(U32 circuit_code)
{
	copyMessageReceivedToSend();
	sendReliable(findHost(circuit_code));
}

S32 LLMessageSystem::forwardReliable(const LLHost& host, S32 retries,
									 bool ping_based_timeout, F32 timeout,
									 void (*callback)(void**, S32),
									 void** callback_data)
{
	copyMessageReceivedToSend();
	return sendReliable(host, retries, ping_based_timeout, timeout, callback,
						callback_data);
}
...
S32 LLMessageSystem::sendMessage(const LLHost& host)
{
	...
	if (mMessageBuilder == mLLSDMessageBuilder)
	{
		...
		LLCoros::getInstance()->launch("LLMessageSystem::sendUntrustedSimulatorMessageCoro",
	...
	*((S32*)&mSendBuffer[PHL_PACKET_ID]) = htonl(cdp->getPacketOutID());
	...
	bool success = mPacketRing.sendPacket(mSocket, (char*)buf_ptr,
										  buffer_length, host);
	if (!success)
	{
	...
	if (mVerboseLog)
	{
		...
		buffer = llformat("\t%6d\t%6d\t%6d ", mSendSize, buffer_length,
						  cdp->getPacketOutID());
		str << buffer << mMessageBuilder->getMessageName()
...
#if 0	// Enable this for output of message names
	LL_DEBUGS("Messaging") << "Message name: " << msg_name
						   << " - Data: " << LLSDNotationStreamer(message)
						   << LL_ENDL;
#endif
...
#if 0
		LL_DEBUGS("Messaging") << "Ack received from " << host
							   << " for packet " << (TPACKETID)packet_id
							   << LL_ENDL;
#endif
		cdp->ackReliablePacket(packet_id);
	}
...
bool LLMessageSystem::checkAllMessages(LockMessageChecker& lmc,
									   S64 frame_count, LLPumpIO* http_pump)
#else
...
void LLMessageSystem::sendUntrustedSimulatorMessageCoro(std::string url,
														std::string message,
														LLSD body,
														UntrustedCallback_t cb)
{
	...
	llinfos << "Sending message: " << message << " - to capability: " << url
			<< llendl;
