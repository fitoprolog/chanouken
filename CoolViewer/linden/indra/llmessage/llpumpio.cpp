...
void ll_debug_poll_fd(const char* msg, const apr_pollfd_t* poll)
{
	...
	else if (poll->desc.f)
	{
		...
		{
			LL_DEBUGS("PumpIO") << "Poll -- " << (msg?msg:"") << " on fd "
								<< os_file << " at " << poll->desc.f << LL_ENDL;
		}
		else
		{
			LL_DEBUGS("PumpIO") << "Poll -- " << (msg?msg:"") << " no fd "
...
bool LLPumpIO::addChain(const chain_t& chain, F32 timeout)
{
	...
	info.mData = boost::make_shared<LLBufferArray>();
...
bool LLPumpIO::addChain(const LLPumpIO::links_t& links,
						LLIOPipe::buffer_ptr_t data, LLSD context, F32 timeout)
{
	...
	if (!data || links.empty()) return false;
...
bool LLPumpIO::setConditional(LLIOPipe* pipe, const apr_pollfd_t* poll)
{
	...
	LL_DEBUGS("PumpIO") << "Setting conditionals ("
						<< (poll ? events_2_string(poll->reqevents) :"null")
						<< ") "
#if LL_DEBUG_PIPE_TYPE_IN_PUMP
	...
	if (!poll->p)
	{
		...
		// this pool. *FIX: Should it always be this pool ?
...
bool LLPumpIO::copyCurrentLinkInfo(links_t& links) const
{
	...
	std::copy(mCurrentChain->mChainLinks.begin(),
...
#endif
								<< "' because: "
								<< events_2_string(poll->rtnevents) << llendl;
...
bool LLPumpIO::respond(const links_t& links, LLIOPipe::buffer_ptr_t data,
					   LLSD context)
{
