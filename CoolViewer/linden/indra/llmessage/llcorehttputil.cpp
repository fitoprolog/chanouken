...
{
	LLEventPump& pump = LLEventPumps::getInstance()->obtain("mainloop");
										  ...
										  LLCore::HttpHeaders::ptr_t headers)
{
	...
	HttpCoroHandler::ptr_t http_handler(new HttpCoroLLSDHandler(reply_pump));
#else
	HttpCoroHandler::ptr_t http_handler =
		boost::make_shared<HttpCoroLLSDHandler>(reply_pump);
#endif
											 ...
											 LLCore::HttpHeaders::ptr_t headers)
{
	...
	LLEventStream reply_pump(mAdapterName, true);
#if BOOST_VERSION < 107000
	HttpCoroHandler::ptr_t http_handler(new HttpCoroRawHandler(reply_pump));
#else
	HttpCoroHandler::ptr_t http_handler =
		boost::make_shared<HttpCoroRawHandler>(reply_pump);
#endif
											  ...
											  LLCore::HttpHeaders::ptr_t headers)
{
	...
	LLEventStream reply_pump(mAdapterName, true);
#if BOOST_VERSION < 107000
	HttpCoroHandler::ptr_t http_handler(new HttpCoroJSONHandler(reply_pump));
#else
	HttpCoroHandler::ptr_t http_handler =
		boost::make_shared<HttpCoroJSONHandler>(reply_pump);
#endif
...
{
	LLCoros::getInstance()->launch("HttpCoroutineAdapter::genericGetCoro",
								   boost::bind(&HttpCoroutineAdapter::trivialGetCoro,
											   url, policy_id,
											   success, failure));
}
...
void HttpCoroutineAdapter::callbackHttpPost(const std::string& url,
											...
											completionCallback_t failure)
{
	LLCoros::getInstance()->launch("HttpCoroutineAdapter::genericPostCoro",
								   boost::bind(&HttpCoroutineAdapter::trivialPostCoro,
											   url, policy_id, postdata,
											   success, failure));
}
