...
public:
	...
	virtual S32 sendMessage(const LLHost& host,
							LLStoredMessagePtr message) = 0;
};
...
public:
	...
	void forwardReliable(U32 circuit_code);
	S32 forwardReliable(const LLHost& host, S32 retries,
...
private:
	...
	void sendUntrustedSimulatorMessageCoro(std::string url,
										   std::string message,
										   LLSD body,
	...
	bool callHandler(const char* name, bool trusted_source,
					 LLMessageSystem* msg);
								  ...
								  EMsgVariableType type, size_t n)
{
	...
	case MVT_S16:
		if (n != 2)
		{
			llerrs << "Size argument passed to htonmemcpy doesn't match swizzle type size"
				   << llendl;
		}
#if LL_BIG_ENDIAN
		*(s + 1) = *(ct);
