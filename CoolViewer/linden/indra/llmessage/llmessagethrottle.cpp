...
// This used to be defined in (the now removed) llcommon/llhash.h, but since
// this source file is the only one to use llhash(), it's best defined here...
LL_INLINE size_t llhash(const char* value)
{
...
struct eq_message_throttle_entry : public std::binary_function<LLMessageThrottleEntry,
															   LLMessageThrottleEntry,
															   bool>
{
	bool operator()(const LLMessageThrottleEntry& a,
					const LLMessageThrottleEntry& b) const
	{
		return a.getHash() == b.getHash();
	}
};
#else
bool eq_message_throttle_entry(LLMessageThrottleEntry a,
							   LLMessageThrottleEntry b)
{
	return a.getHash() == b.getHash();
}
#endif

constexpr U64 SEC_TO_USEC = 1000000;
...
void LLMessageThrottle::pruneEntries()
{
	...
	for (cat = 0; cat < MTC_EOF; cat++)
	{
		...
		// If we did not find any entries young enough to keep, remove them all
		if (!found)
		{
			message_list->clear();
		}
									   ...
									   const std::string& mesg)
{
	...
	message_list_iterator_t found = std::find_if(message_list->begin(),
												 message_list->end(),
												 std::bind2nd(eq_message_throttle_entry(),
															  entry));
#else
 	message_list_iterator_t found = std::search_n(message_list->begin(),
												  message_list->end(),
 												  1, entry,
...
bool LLMessageThrottle::addAgentAlert(const LLUUID& agent, const LLUUID& task,
									  const std::string& mesg)
{
