...
// Make sure that we don't set above these values, even if the client asks to
...
bool LLThrottleGroup::throttleOverflow(S32 throttle_cat, F32 bits)
{
	...
	// What if bitsavailable goes negative ? That's OK, because it means
...
bool LLThrottleGroup::dynamicAdjust()
{
	...
	// how much weight to give to last period while determining BPS utilization
	...
	for (i = 0; i < TC_EOF; ++i)
	{
		...
		if (mBitsAvailable[i] > 0 &&
			mBitsSentHistory[i] < IDLE_PERCENT * DYNAMIC_ADJUST_TIME * mCurrentBPS[i])
		{
			channel_idle[i] = true;
		}
		else
	...
	if (channels_busy)
	{
		...
		for (i = 0; i < TC_EOF; ++i)
		{
			if (channel_idle[i] || channel_over_nominal[i])
			{
				...
				// current share, even if it's idle right now./ Make sure we
