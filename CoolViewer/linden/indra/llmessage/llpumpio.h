...
constexpr F32 SHORT_CHAIN_EXPIRY_SECS = 1.f;
constexpr F32 NEVER_CHAIN_EXPIRY_SECS = 0.f;
 ...
 * <code>pump()</code> on a thread used for IO and call <code>respond()</code>
 * on a thread that is expected to do higher level processing. You can call
 * almost any other method from any thread - see notes for each method for
 * details. In order for the threading abstraction to work, you need to call
 * <code>prime()</code> with a valid apr pool. A pump instance manages much of
 * the state for the pipe, including the list of pipes in the chain, the
 * channel for each element in the chain, the buffer, and if any pipe has
 * marked the stream or process as done. Pipes can also set file descriptor
 * based conditional statements so that calls to process do not happen until
 * data is ready to be read or written. Pipes control execution of calls to
 * process by returning a status code such as STATUS_OK or STATUS_BREAK.
 * One way to conceptualize the way IO will work is that a pump combines the
 * unit processing of pipes to behave like file pipes on the unix command line.
 */
class LLPumpIO
{
protected:
...
public:
	 ...
	 *
	 * If you fail to call this method prior to use, the pump will try to work,
	 * but will not come with any thread locking mechanisms.
	 ...
	 *
	 * This method will automatically generate a buffer and assign each link in
	 * the chain as if it were the consumer to the previous.
	 * @param chain The pipes for the chain
	 * @param timeout The number of seconds in the future to expire. Pass in
	 *				  0.0f to never expire.
	 ...
	 * @brief Add a chain to this pump and process in the next cycle.
	 *
	 * This method provides a slightly more sophisticated method for adding a
	 * chain where the caller can specify which link elements are on what
	 * channels. This method will fail if no buffer is provided since any call
	 * to generate new channels for the buffers will cause unpredictable
	 ...
	 * @return Returns true if anything was added to the pump.
	 */
	bool addChain(const links_t& links, LLIOPipe::buffer_ptr_t data,
	 ...
	 * matter.
	 * *FIX: Given the structure of the pump and pipe relationship, this should
	 * probably go through a different mechanism than the pump. I think it
	 * would be best if the pipe had some kind of controller which was passed
	 * into <code>process()</code> rather than the pump which exposed this
	 * interface.
	 ...
	 *
	 * This method iterates through the running chains, and if all pipe on a
	 *  chain are unconditionally ready or if any pipe has any conditional
	 * processiong condition then process will be called on every chain which
	 * has requested processing. That chain has a file descriptor ready,
	 * <code>process()</code> will be called for all pipes which have requested
	 * it.
	 */
	...
	/**
	 * @brief Add pipe to a special queue which will be called during the next
	 * call to <code>callback()</code> and then dropped from the queue.
	 *
	 * This call will add a single pipe, with no buffer, context, or channel
	 * information to the callback queue. It will be called once, and then
	 * dropped.
	...
	/**
	 * @brief Add a chain to a special queue which will be called during the
	 * next call to <code>callback()</code> and then dropped from the queue.
	 *
	 * It is important to remember that you should not add a data buffer or
	 * context which may still be in another chain - that will almost certainly
	 * lead to a problems. Ensure that you are done reading and writing to
	 * those parameters, have new generated, or empty pointers.
	 * @param links The pipes and io indexes for the chain
	 * @param data Shared pointer to data buffer
	 * @param context Potentially undefined context meta-data for chain.
	 * @return Returns true if anything was added to the pump.
	 */
	bool respond(const links_t& links, LLIOPipe::buffer_ptr_t data,
				 LLSD context);

	/**
	 * @brief Run through the callback queue and call <code>process()</code>.
	 *
	 * This call will process all prending responses and call process on each.
	 * This method will then drop all processed callback requests which may
	 * lead to deleting the referenced objects.
	 */
	void callback();
...
protected:
	 ...
	 *
	 * This method will potentially modify the internals of the chain. On end,
	 * the chain.mHead will equal chain.mChainLinks.end().
