...
public:
	...
	// currently used for the preprocessor #include mechanism. It ensures the
	// file name and sub-directory names are scrubbed and that the latter do
	// not allow navigating upwards in the file system.
	// In case of failure to find an adequate file, it returns an empty string.
	std::string getUserFilename(const std::string& desired_subdir,
