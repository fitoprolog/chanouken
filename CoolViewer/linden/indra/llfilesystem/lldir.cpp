...
std::string LLDir::getUserFilename(const std::string& desired_subdir,
								   const std::string& fallback_subdir,
								   const std::string& unclean_filename)
{
	// Sanitize the filename to remove forbidden characters
	std::string filename = getScrubbedFileName(unclean_filename);
	if (filename.empty())
	{
	...
	if (!desired_subdir.empty())
	{
		...
		if (!subdir.empty())
		{
			if (subdir.find("~" LL_DIR_DELIM_STR) == 0)
			{
				...
				if (subdir[subdir.length() - 1] != LL_DIR_DELIM_CHR)
				{
