...
#else
		const std::string& name = mIter->path().filename().string();
	...
	if (mGotFilter)	// Iterator with glob pattern matching
	{
		try
		{
			...
			while (mIter != end)
			{
				...
				if (not_matching)
				{
					found = !found;	// We want entries not matching the pattern
				}
				if (found)
				{
					break;
...
:	mImpl(NULL)
{
	...
	directory_iterator iter = directory_iterator(dir_path, ec);
	if (ec.failed())
	{
		llwarns << ec.message() << llendl;
		return;
	}
