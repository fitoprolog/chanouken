...
bool LLPolyMeshSharedData::loadMesh(const std::string& filename)
{
	...
	if (strncmp(header, HEADER_BINARY, strlen(HEADER_BINARY)) == 0)
	{
		...
		if (!isLOD())
		{
			...
			{
				num_read = fread(mDetailTexCoords, 2 * sizeof(float), n_vertices, fp);
				llendianswizzle(mDetailTexCoords, sizeof(float), 2*n_vertices);
				if (num_read != n_vertices)
				{
		...
		if (!isLOD())
		{
			...
			while (fread(morph_name, 1, 64, fp) == 64)
			{
				...
				LLPolyMorphData* morph_data;
				morph_data = new LLPolyMorphData(std::string(morph_name));
				if (!morph_data || !morph_data->isSuccesfullyAllocated())
				{
					llwarns << "Failure to allocate new morph data" << llendl;
					llassert(false);
					goto abortion;
				}
...
void LLPolyMesh::initializeForMorph()
{
	...
	LLVector4a::memcpyNonAliased16((F32*)mTexCoords,
								   ...
								   sizeof(LLVector2) * (num_vertices + num_vertices % 2));
...
#if 0
// Erasing but not deleting seems bad, but fortunately we don't actually use
// this...
