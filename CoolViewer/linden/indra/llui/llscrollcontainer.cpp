...
LLXMLNodePtr LLScrollableContainer::getXML(bool save_children) const
{
	...
	if (mIsOpaque)
	{
		node->createChild("color", true)->setFloatValue(4,
														mBackgroundColor.mV);
	}
...
LLView* LLScrollableContainer::fromXML(LLXMLNodePtr node, LLView* parent,
									   LLUICtrlFactory* factory)
{
	...
	LLScrollableContainer* ret;
	ret = new LLScrollableContainer(name, rect, (LLPanel*)NULL, opaque, color);
