...
bool LLResizeHandle::handleHover(S32 x, S32 y, MASK mask)
{
	...
	if (hasMouseCapture())
	{
		// Make sure the mouse in still over the application. We don't want to
		...
		if (resizing_view)
		{
			...
			switch (mCorner)
			{
				case LEFT_TOP:
					snap_view =
						resizing_view->findSnapEdge(scaled_rect.mLeft,
													mouse_dir, SNAP_LEFT,
													SNAP_PARENT_AND_SIBLINGS,
													LLUI::sSnapMargin);
					test_view =
						resizing_view->findSnapEdge(scaled_rect.mTop,
													mouse_dir, SNAP_TOP,
													SNAP_PARENT_AND_SIBLINGS,
													LLUI::sSnapMargin);
					if (!snap_view)
				...
				case LEFT_BOTTOM:
					snap_view =
						resizing_view->findSnapEdge(scaled_rect.mLeft,
													...
													LLUI::sSnapMargin);
					test_view =
						resizing_view->findSnapEdge(scaled_rect.mBottom,
				...
				case RIGHT_TOP:
					snap_view =
						resizing_view->findSnapEdge(scaled_rect.mRight,
...
bool LLResizeHandle::pointInHandle(S32 x, S32 y)
{
	if (pointInView(x, y))
	{
		...
		{
			case LEFT_TOP:
				return x <= RESIZE_BORDER_WIDTH || y >= top_border;

			case LEFT_BOTTOM:
				return x <= RESIZE_BORDER_WIDTH || y <= RESIZE_BORDER_WIDTH;

			case RIGHT_TOP:
				return x >= right_border || y >= top_border;
