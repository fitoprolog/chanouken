 ...
 */
class LLRadioGroup final : public LLUICtrl, public LLCtrlSelectionInterface
{
protected:
...
public:
	...
	LL_INLINE LLRadioCtrl* getRadioButton(const S32& index)		{ return mRadioButtons[index]; }
	...
	LL_INLINE LLCtrlSelectionInterface* getSelectionInterface() override
	{
		return (LLCtrlSelectionInterface*)this;
	}

	LL_INLINE S32 getItemCount() const override					{ return mRadioButtons.size(); }
	LL_INLINE bool getCanSelect()const override 				{ return true; }
	LL_INLINE bool selectFirstItem() override					{ return setSelectedIndex(0); }
	LL_INLINE bool selectNthItem(S32 index) override			{ return setSelectedIndex(index); }
	LL_INLINE bool selectItemRange(S32 first, S32) override		{ return setSelectedIndex(first); }
	LL_INLINE S32 getFirstSelectedIndex() const override		{ return getSelectedIndex(); }
	LL_INLINE bool setCurrentByID(const LLUUID& id) override	{ return false; }
	LL_INLINE LLUUID getCurrentID() const override				{ return LLUUID::null; }
	bool setSelectedByValue(const LLSD& value, bool selected) override;
	LL_INLINE LLSD getSelectedValue() override					{ return getValue(); }

	bool isSelected(const LLSD& value) const override;
	LL_INLINE bool operateOnSelection(EOperation) override		{ return false; }
	LL_INLINE bool operateOnAll(EOperation) override			{ return false; }
