...
LLResizeBar::LLResizeBar(const std::string& name, LLView* resizing_view,
	...
	if (hasMouseCapture())
	{
		...
		if (valid_rect.localPointInRect(screen_x, screen_y) && mResizingView)
		{
			...
			if (mSnappingEnabled)
			{
				...
				{
					case LEFT:
						snap_view = mResizingView->findSnapEdge(scaled_rect.mLeft,
																mouse_dir,
																SNAP_LEFT,
					...
					case TOP:
						snap_view = mResizingView->findSnapEdge(scaled_rect.mTop,
																mouse_dir,
																SNAP_TOP,
																SNAP_PARENT_AND_SIBLINGS,
																LLUI::sSnapMargin);
					...
					case RIGHT:
						snap_view = mResizingView->findSnapEdge(scaled_rect.mRight,
																mouse_dir,
																SNAP_RIGHT,
																SNAP_PARENT_AND_SIBLINGS,
																LLUI::sSnapMargin);
					...
					case BOTTOM:
						snap_view = mResizingView->findSnapEdge(scaled_rect.mBottom,
																mouse_dir,
																SNAP_BOTTOM,
																SNAP_PARENT_AND_SIBLINGS,
																LLUI::sSnapMargin);
