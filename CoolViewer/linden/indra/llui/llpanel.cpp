...
bool LLPanel::checkRequirements()
{
	if (!mRequirementsError.empty())
	{
		...
		gNotifications.add(LLNotification::Params("FailedRequirementsCheck").payload(args));
...
LLCtrlSelectionInterface* LLPanel::childGetSelectionInterface(const char* id) const
{
	LLUICtrl* child = getChild<LLUICtrl>(id, true);
	if (child)
	{
...
LLCtrlListInterface* LLPanel::childGetListInterface(const char* id) const
{
	LLUICtrl* child = getChild<LLUICtrl>(id, true);
	if (child)
	{
...
LLCtrlScrollInterface* LLPanel::childGetScrollInterface(const char* id) const
{
	LLUICtrl* child = getChild<LLUICtrl>(id, true);
	if (child)
	{
