...
LLFloater::~LLFloater()
{
	...
	for (itor = mFloaterControls.begin(); itor != mFloaterControls.end();
		 ++itor)
	{
		delete itor->second;
	}
...
bool LLFloater::handleMouseDown(S32 x, S32 y, MASK mask)
{
	if (mMinimized)
	{
		...
		// offerClickToButton helper method can be removed because the parent
...
LLFloater* LLFloater::getClosableFloaterFromFocus()
{
	...
	handle_map_iter_t map_end = sFloaterMap.end();
	for (iter = sFloaterMap.begin(); iter != map_end; ++iter)
	{
	...
	for (LLFloater* floater_to_close = focused_floater;
		 floater_to_close != NULL;
		 floater_to_close = gFloaterViewp->getParentFloater(floater_to_close))
	{
...
{
	LLFloater* floater_to_close = getClosableFloaterFromFocus();
	if (floater_to_close)
	{
...
void LLFloaterView::draw()
{
	...
	LLFloater* focused_floater = mFocusCycleMode ? getFocusedFloater() : NULL;
	if (focused_floater)
	{
		for (child_list_const_iter_t it = getChildList()->begin(),
									 end = getChildList()->end();
			 it != end; ++it)
		{
			if (*it != focused_floater)
			{
				drawChild(*it);
			}
		}

		drawChild(focused_floater, -TABBED_FLOATER_OFFSET,
