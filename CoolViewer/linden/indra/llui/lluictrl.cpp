...
void LLUICtrl::onFocusReceived()
{
	...
	LLUICtrl* last_focus = gFocusMgr.getLastKeyboardFocusUICtrl();
...
bool LLUICtrl::getIsChrome() const
{
	...
	while (parent_ctrl)
	{
		...
		{
			if (((LLUICtrl*)parent_ctrl)->getIsChrome())
			{
				return true;
			}
		}
...
private:
	bool compareTabOrders(const LLView::tab_order_t & a,
...
// I'd have defined it local to the one method that uses it but that broke the
// VS 05 compiler. -MG
class LLUICtrl::DefaultTabGroupFirstSorter : public LLQuerySorter,
											 public LLSingleton<DefaultTabGroupFirstSorter>
{
...
bool LLUICtrl::focusFirstItem(bool prefer_text_fields, bool focus_flash)
{
	...
	if (result.size() > 0)
	{
		LLUICtrl* ctrl = (LLUICtrl*)result.front();
		if (!ctrl->hasFocus())
		{
			ctrl->setFocus(true);
			ctrl->onTabInto();
...
bool LLUICtrl::focusLastItem(bool prefer_text_fields)
{
	...
	if (prefer_text_fields)
	{
		...
		if (result.size() > 0)
		{
			LLUICtrl* ctrl = (LLUICtrl*)result.back();
			if (!ctrl->hasFocus())
			{
				ctrl->setFocus(true);
				ctrl->onTabInto();
...
LLXMLNodePtr LLUICtrl::getXML(bool save_children) const
{
	...
	node->createChild("tab_stop", true)->setBoolValue(mTabStop);
