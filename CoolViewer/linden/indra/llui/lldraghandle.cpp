...
{
	return getTitleBox() ? getTitleBox()->getText() : LLStringUtil::null;
}
...
{
#if 0	// Disable lines. Can drag anywhere in most windows.  JC
	if (getVisible() && getEnabled() && mForeground)
	{
		constexpr S32 BORDER_PAD = 2;
		...
		constexpr S32 LINE_SPACING = (DRAG_HANDLE_HEIGHT - 2 * VPAD) / 4;
...
void LLDragHandleTop::reshapeTitleBox()
{
	...
	S32 title_width = mFont->getWidth(getTitleBox()->getText()) + TITLE_PAD;
