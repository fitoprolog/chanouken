...
LLView::~LLView()
{
	...
	for (auto it = mFloaterControls.begin(), end = mFloaterControls.end();
		 it != end; ++it)
	{
		delete it->second;
	}
...
void LLView::removeChild(LLView* child, bool delete_it)
{
	...
	{
		llerrs << "Call done with non-child" << llendl;
	}
	updateBoundingRect();
}
						   ...
						   LLRect* sticky_rect_screen)
{
	...
	// don't allow any siblings to handle this event even if we don't have a
	// tooltip
...
{
	control_map_t::iterator it = mFloaterControls.find(name);
	if (it != mFloaterControls.end())
	{
		return it->second;
	}
	if (mParentView)
...
{
	mFloaterControls[name] = new LLControlVariable(name.c_str(), TYPE_BOOLEAN,
												   initial_value, "UI");
}
...
{
	control_map_t::iterator it = mFloaterControls.find(name);
	return it != mFloaterControls.end() ? it->second : NULL;
}
