...
void HBFileSelector::init(void* user_data)
{
	...
	mCallbackDone = mMustClose = mIsDirty = mCreatingDirectory = false;
...
#if !LL_WINDOWS
		if (!mCurrentEntry.empty() && mCurrentEntry.at(0) == '.')
		{
...
void HBFileSelector::setValidExtensions()
{
	...
	if (mSavePicker)
	{
		mFileTypeDescription = LLTrans::getString("any_file") + " (*.*)";
	}
	else if (!mDirPicker)
	{
		mFileTypeDescription = LLTrans::getString("all_files") + " (*.*)";
	}
	...
	if (mSavePicker && mSaveFilter != FFSAVE_NONE && mSaveFilter != FFSAVE_ALL)
	{
		switch (mSaveFilter)
		{
			case FFSAVE_TXT:
				...
				mFileTypeDescription = LLTrans::getString("txt_file") +	
									   " (*.txt)";
			...
			case FFSAVE_XML:
				...
				mFileTypeDescription = LLTrans::getString("xml_file") +
									  " (*.xml)";
			...
			case FFSAVE_XUI:
				mValidExtensions.emplace_back("xml");
				mFileTypeDescription = LLTrans::getString("xui_file") +
									  " (*.xml)";
			...
			case FFSAVE_LSL:
				...
				mFileTypeDescription = LLTrans::getString("lsl_file") +
									   " (*.lsl)";
			...
			case FFSAVE_WAV:
				...
				mFileTypeDescription = LLTrans::getString("wav_file") +
									   " (*.wav)";
			...
			case FFSAVE_BVH:
				...
				mFileTypeDescription = LLTrans::getString("bvh_file") +
									   " (*.bvh)";
			...
			case FFSAVE_DAE:
				...
				mFileTypeDescription = LLTrans::getString("dae_file") +
									   " (*.dae)";
			...
			case FFSAVE_OBJ:
				...
				mFileTypeDescription = LLTrans::getString("obj_file") +
									   " (*.obj)";
				mContext = CONTEXT_MODEL;
				break;
			...
			case FFSAVE_RAW:
				...
				mFileTypeDescription = LLTrans::getString("raw_file") +
									   " (*.raw)";
			...
			case FFSAVE_TGA:
				...
				mFileTypeDescription = LLTrans::getString("tga_file") +
									   " (*.tga)";
			...
			case FFSAVE_PNG:
				...
				mFileTypeDescription = LLTrans::getString("png_file") +
									   " (*.png)";
				mContext = CONTEXT_IMAGE;
				break;
			...
			case FFSAVE_JPG:
				...
				mFileTypeDescription = LLTrans::getString("jpg_file") +
									   " (*.jpg;*.jpeg)";
				mContext = CONTEXT_IMAGE;
				break;
			...
			case FFSAVE_J2C:
				...
				mFileTypeDescription = LLTrans::getString("j2c_file") +
									   " (*.j2c)";
				mContext = CONTEXT_IMAGE;
				break;
			...
			case FFSAVE_BMP:
				...
				mFileTypeDescription = LLTrans::getString("bmp_file") +
									   " (*.bmp)";
				mContext = CONTEXT_IMAGE;
				break;
	...
	else if (mLoadFilter != FFLOAD_NONE && mLoadFilter != FFLOAD_ALL)
	{
		switch (mLoadFilter)
		{
			case FFLOAD_TEXT:
				mValidExtensions.emplace_back("txt");
				mFileTypeDescription = LLTrans::getString("text_files") +	
									   " (*.txt)";
				mContext = CONTEXT_TXT;
				break;
			...
			case FFLOAD_XML:
				mValidExtensions.emplace_back("xml");
				mFileTypeDescription = LLTrans::getString("xml_files") +
									  " (*.xml)";
				mContext = CONTEXT_XML;
				break;
			...
			case FFLOAD_XUI:
				mValidExtensions.emplace_back("xml");
				mFileTypeDescription = LLTrans::getString("xui_files") +
									  " (*.xml)";
				mContext = CONTEXT_XUI;
				break;
			...
			case FFLOAD_SCRIPT:
				mValidExtensions.emplace_back("lsl");
				mFileTypeDescription = LLTrans::getString("script_files") +
									   " (*.xml)";
				mContext = CONTEXT_LSL;
				break;
			...
			case FFLOAD_SOUND:
				...
				mFileTypeDescription = LLTrans::getString("sound_files") +
									   " (*.wav;*.dsf)";
				mContext = CONTEXT_SOUND;
				break;
			...
			case FFLOAD_ANIM:
				...
				mFileTypeDescription = LLTrans::getString("animation_files") +
									   " (*.bvh;*.anim)";
				mContext = CONTEXT_ANIM;
				break;
			...
			case FFLOAD_MODEL:
				mValidExtensions.emplace_back("dae");
				mFileTypeDescription = LLTrans::getString("model_files") +
									   " (*.dae)";
				mContext = CONTEXT_MODEL;
				break;
			...
			case FFLOAD_TERRAIN:
				mValidExtensions.emplace_back("raw");
				mFileTypeDescription = LLTrans::getString("raw_files") +
									   " (*.raw)";
				mContext = CONTEXT_RAW;
				break;
			...
			case FFLOAD_IMAGE:
				...
				mValidExtensions.emplace_back("bmp");
				mFileTypeDescription = LLTrans::getString("image_files") +
			...
			case FFLOAD_LUA:
				...
				mFileTypeDescription = LLTrans::getString("lua_files") +
									   " (*.lua;*.luac)";
...
void HBFileSelector::doCallback()
{
	if (!mCallbackDone)
	{
		...
		{
			if (!isFileExtensionValid(mCurrentSelection))
			{
										 ...
										 LLLineEditor* caller, void* user_data)
{
	...
	if (self && key == KEY_RETURN && mask == MASK_NONE)
	{
		...
		{
			self->setSelectionData();
			// We cannot close the floater now, as it would mean destroying
			// the object that called this method and therefore a crash...
			// Just flag for closing, which will occur in draw()
