...
const LLWString& LLClipboard::getPasteWString(LLUUID* source_id)
{
	if (gWindowp)
	{
		if (mSourceID.notNull())
		{
			...
			gWindowp->pasteTextFromClipboard(temp_string);
...
void LLClipboard::copyFromPrimarySubstring(const LLWString& src, S32 pos,
										   S32 len, const LLUUID& source_id)
{
	mSourceID = source_id;
	if (pos >= 0 && pos < (S32)src.length())
	{
...
const LLWString& LLClipboard::getPastePrimaryWString(LLUUID* source_id)
{
	if (gWindowp)
	{
		...
		{
			LLWString temp_string;
			gWindowp->pasteTextFromPrimary(temp_string);
			if (temp_string != mString)
			{
				mSourceID.setNull();
				mString = temp_string;
...
const LLWString& LLClipboard::getClipBoardString()
{
	return mString;
}
