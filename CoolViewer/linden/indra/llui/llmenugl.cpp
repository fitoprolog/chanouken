...
void LLMenuItemGL::appendAcceleratorString(std::string& st) const
{
	...
	if (mAcceleratorMask & MASK_CONTROL)
	{
		if (mAcceleratorMask & MASK_MAC_CONTROL)
		{
			static std::string symbol = LLUITrans::getString("accel-mac-control");
			st.append(symbol);
		}
		else
		{
			// Symbol would be "\xE2\x8C\x98"
			static std::string symbol = LLUITrans::getString("accel-mac-command");
			st.append(symbol);
		}
	}
	if (mAcceleratorMask & MASK_ALT)
	{
		...
		static std::string symbol = LLUITrans::getString("accel-mac-option");
		st.append(symbol);
	}
	if (mAcceleratorMask & MASK_SHIFT)
	{
		// Symbol would be "\xE2\x8C\xA7"
		static std::string symbol = LLUITrans::getString("accel-mac-shift");
		st.append(symbol);
	}
#else
	if (mAcceleratorMask & MASK_CONTROL)
	{
		static std::string symbol = LLUITrans::getString("accel-win-control");
		st.append(symbol);
	}
	if (mAcceleratorMask & MASK_ALT)
	{
		static std::string symbol = LLUITrans::getString("accel-win-alt");
		st.append(symbol);
	}
	if (mAcceleratorMask & MASK_SHIFT)
	{
		static std::string symbol = LLUITrans::getString("accel-win-shift");
		st.append(symbol);
	}
#endif
...
{
	if (getHighlight() && getMenu()->isOpen())
	{
		if (key == KEY_UP)
		{
...
public:
	...
	LL_INLINE virtual std::string getType() const	{ return "separator"; }
	...
	LL_INLINE virtual U32 getNominalHeight() const	{ return SEPARATOR_HEIGHT_PIXELS; }
};
...
bool LLMenuItemSeparatorGL::handleMouseDown(S32 x, S32 y, MASK mask)
{
	...
	{
		return parent_menu->handleMouseDown(x + getRect().mLeft,
											getRect().mTop + 1, mask);
	}
	else
	{
...
bool LLMenuItemSeparatorGL::handleMouseUp(S32 x, S32 y, MASK mask)
{
	...
	if (y > getRect().getHeight() / 2)
	{
		return parent_menu->handleMouseUp(x + getRect().mLeft,
										  getRect().mTop + 1, mask);
	}
	else
	{
...
void LLMenuItemTearOffGL::doIt()
{
	if (getMenu()->getTornOff())
	{
		LLTearOffMenu* torn_off_menu =
			dynamic_cast<LLTearOffMenu*>(getMenu()->getParent());
		if (torn_off_menu)
		{
			torn_off_menu->close();
		}
...
{
	LLMenuGL* branch = getBranch();
	if (branch && branch->getParent() == NULL)
	{
...
bool LLMenuItemBranchGL::handleKeyHere(KEY key, MASK mask)
{
	...
	if (branch && branch->getVisible() && getMenu()->getVisible() &&
		key == KEY_LEFT)
	{
		// Switch to keyboard navigation mode
		LLMenuGL::setKeyboardMode(true);
	...
	if (branch && getHighlight() && getMenu()->isOpen() && key == KEY_RIGHT &&
		!branch->getHighlightedItem())
	{
		// Switch to keyboard navigation mode
		LLMenuGL::setKeyboardMode(true);
...
void LLMenuItemBranchGL::openMenu()
{
	...
	else if (!branch->getVisible() && LLMenuGL::sMenuContainer)
	{
		// Get valid rectangle for menus
		const LLRect menu_region_rect =
			LLMenuGL::sMenuContainer->getMenuRect();
...
void LLMenuGL::arrange()
{
	...
	if (mItems.size())
	{
		const LLRect menu_region_rect = sMenuContainer ?
											sMenuContainer->getMenuRect() :
...
void LLPieMenuBranch::doIt()
{
	...
	S32 center_x;
	S32 center_y;
	parent->localPointToScreen(rect.getWidth() / 2, rect.getHeight() / 2,
							   &center_x, &center_y);
...
void LLTearOffMenu::onFocusReceived()
{
	...
	while (parent_menu_item)
	{
		if (parent_menu_item->getMenu()->getVisible())
		{
			...
			parent_menu_item =
				parent_menu_item->getMenu()->getParentMenuItem();
		}
		else
		{
			break;
