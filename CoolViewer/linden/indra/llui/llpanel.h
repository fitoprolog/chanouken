...
public:
	...
	LL_INLINE bool isPanel() const override					{ return true; }
	void draw() override;
	...
	template <class T> void requires(const char* name)
	{
		// Check for widget with matching type and name
		if (LLView::getChild<T>(name) == NULL)
		{
			mRequirementsError.append(name);
			mRequirementsError.append("\n");
		}
	}

	// Requires LLView by default
	void requires(const char* name)
	{
	...
	LLCtrlSelectionInterface* childGetSelectionInterface(const char* id) const;
	LLCtrlListInterface* childGetListInterface(const char* id) const;
	LLCtrlScrollInterface* childGetScrollInterface(const char* id) const;
