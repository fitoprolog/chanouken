 ...
 *
 * We make this noncopyable because we want to manage these through
 ...
 * pointer from a notification, we'll always get the same shared pointer.
 */
...
public:
	...
	// return response LLSD filled in with default form contents and
...
// Manages a list of notifications. Note that if this is ever copied around, we
// might find ourselves with multiple copies of a queue with notifications
// being added to different nonequivalent copies. So we make it inherit from
// boost::noncopyable, and then create a map of shared_ptr to manage it.
//
...
//
class LLNotificationChannel : boost::noncopyable,
