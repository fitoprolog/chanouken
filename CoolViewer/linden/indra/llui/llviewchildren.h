...
// Makes it easy to manipulate children of a view by id safely encapsulates
// common operations into simple, one line calls
...
public:
	...
	LL_INLINE void hide(const char* id)					{ show(id, false); }

	void enable(const char* id, bool enabled = true);
	LL_INLINE void disable(const char* id)				{ enable(id, false); }

	// LLTextBox

	void setText(const char* id, const std::string& text, bool visible = true);
	void setWrappedText(const char* id, const std::string& text,
						bool visible = true);
