...
# build-linux.sh v2.09 (c)2013-2021 Henri Beauchamp.
...
{
	for i in "python3.10" "python3.9" "python3.8" "python3.7" "python3.6" "python3.5" "python3.4" "python3.3" "python3" "python2.7" "python2.6" "python2" "python"; do
...
if has_option "h" ; then
	...
	echo "With  -v5.N         : use gcc v5.N (with N=0 to 5) if possible/found."
	echo "      -v6.N         : use gcc v6.N (with N=0 to 5) if possible/found."
	echo "      -v7.N         : use gcc v7.N (with N=0 to 5) if possible/found."
	echo "      -v8.N         : use gcc v8.N (with N=0 to 5) if possible/found."
	echo "      -v9.N         : use gcc v9.N (with N=0 to 4) if possible/found."
	echo "      -v10.N        : use gcc v10.N (with N=0 to 3) if possible/found."
	echo "      -v11.N        : use gcc v11.N (with N=0 to 2) if possible/found."
	echo "      -v12.0        : use gcc v12.N (with N=0 or 1) if possible/found."
...
BUILD_TYPE="Release"
PACKAGE_DIR="viewer-linux-x86_64-release"
echo "==============================================================================="
if has_option "d" ; then
    BUILD_TYPE="Debug"
	PACKAGE_DIR="viewer-linux-x86_64-debug"
elif has_option "s" ; then
    BUILD_TYPE="RelWithDebInfo"
	PACKAGE_DIR="viewer-linux-x86_64-relwithdebinfo"
fi
...
if has_option "v[14-9]" ; then
	if has_option "v5\.0" ; then
		find_gcc "5.0"
	elif has_option "v5\.1" ; then
		find_gcc "5.1" 
	elif has_option "v5\.2" ; then
		find_gcc "5.2" 
	elif has_option "v5\.3" ; then
		find_gcc "5.3" 
	elif has_option "v5\.4" ; then
		find_gcc "5.4"
	elif has_option "v5\.5" ; then
		find_gcc "5.5"
	elif has_option "v6\.0" ; then
		find_gcc "6.0" 
...
if [ -x "$FINAL_DIR/CoolVLViewer" ] ; then
	echo "==============================================================================="
	let delay=$now-$start
	echo "Build successful. Total time taken by the building and packaging: $delay seconds."
	RUN_DIR=`find $FINAL_DIR -type d -name "CoolVLViewer-x86_64*"`
